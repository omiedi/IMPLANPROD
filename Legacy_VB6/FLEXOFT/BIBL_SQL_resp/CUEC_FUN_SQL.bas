...
Sub RAPLIPRO(NCX%)
    ...
    Dim STATDOC%(), CODIEM%(), CODISU%(), NICLADOC%()
    RAMAX& = 0
    ATENTI% = 1
    '
...
100 Screen.MousePointer = 11
    ...
    For KU& = 1 To Len(RFF$) Step 4
      ...
      If NUDOCU% > RAMAX& Then
        RAMAX& = RAMAX& + 128
        ReDim Preserve DOCULAR$(RAMAX&), DOCUCOR$(RAMAX&), IORI#(RAMAX&), IMPO#(RAMAX&), FFDOC%(RAMAX&), FVENDOC%(RAMAX&), CODIEM%(RAMAX&), CODISU%(RAMAX&), STATDOC%(RAMAX&), NICLADOC%(RAMAX&)
      End If
