...
Begin VB.Form INGRECINT 
   ...
   Begin ACTIVESKINLibCtl.Skin Skin1 
      Left            =   2625
      OleObjectBlob   =   "INGRECINT.frx":06E0
      Top             =   0
   End
   Begin ACTIVESKINLibCtl.SkinLabel SkinLabel999 
      ...
      Index           =   0
      Left            =   3240
      OleObjectBlob   =   "INGRECINT.frx":0914
      TabIndex        =   2
      Top             =   0
      Visible         =   0   'False
      Width           =   1485
...
Private Sub Command11_Click()
    ...
    PESOX# = CTP.PesoLeidoBalanza(CI1%) - 8
...
Private Sub Command2_Click()
    ...
    Call MOVISTOK(FECHA%, CI1%, LOTE$, CMOV$, DOPRI$, DOPRI$, DOSEC$, DOSEC$, REFE$, 0, 0, 1, NC%, DEPOESLINGA%, (-1) * CANTI, , , , , , , , , , NUMEOPERACION%)   'GUARDO EL NUMERO DE OPERACION DE CADA ITEMS TRANSFERIDO
    
    DESCRIDEPO$ = ECOARCH$("TADEPOSI", Chr$(DEPOCORTE%))
    REFE$ = "Tr.A " & TRIM$(DESCRIDEPO$)
    NCOMP& = ProNroComprobante(CMOV$)
    
    Call MOVISTOK(FECHA%, CI1%, LOTE$, CMOV$, DOPRI$, DOPRI$, DOSEC$, DOSEC$, REFE$, 0, 0, 1, NC%, DEPOCORTE%, CANTI, , , , , , , , , , NUMEOPERACION%)  'GUARDO EL NUMERO DE OPERACION DE CADA ITEMS TRANSFERIDO
    ...
    Call CTP.GrabarOperacion(NORFA$, CODIEMPR%, CIFAB%, NUMEOPERACION%, CantdeCortes, LOTE$, CI1%, XVALO(TXTCORTXBSON), NUMEOPERARIO%, CORTEENMETROS, PESOKG)
...
Private Sub Command3_Click()
    ...
    Call MOVISTOK(FECHA%, CI1%, LOTE$, CMOV$, DOPRI$, DOPRI$, DOSEC$, DOSEC$, REFE$, 0, 0, 1, NC%, DEPOCORTESLINGA%, CANTI)  'GUARDO EL NUMERO DE OPERACION DE CADA ITEMS TRANSFERIDO
    Call ACSALOTE(CI1%, LOTE$, "NOMESS")
    Dim CTP As New Conteplast
...
Private Sub LBLOF_Change()
   ...
   CIFAB% = CODIFAB%(NORFA$)
   NombreOperacion$ = "ESLINGA [CM]"
     
   Dim CTP As New Conteplast
   NOPER% = CTP.NumeOperXNombre%(CIFAB%, NombreOperacion$)
   Set CTP = Nothing
   If NOPER% < 1 Then
     Call COMUNI("Atenciónm !! La Orden de Fabricación Seleccionada No Registra Operacion de Eslinga")
     Exit Sub
   End If
