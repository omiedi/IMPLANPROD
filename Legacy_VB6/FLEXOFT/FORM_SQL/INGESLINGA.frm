...
Begin VB.Form INGESLINGA 
   AutoRedraw      =   -1  'True
   BackColor       =   &H00808080&
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Informe de Ingreso de Eslingas a Depósito de Corte de Eslingas"
   ClientHeight    =   7185
   ClientLeft      =   1050
   ClientTop       =   1155
   ClientWidth     =   15525
   ...
   Begin VB.Frame Frame3 
      ...
      Begin VB.Label Label5 
         BackStyle       =   0  'Transparent
         Caption         =   "<<<<<<     Asignación Parcial  o Total de Eslingas a Orden de Fabricación     >>>>>>"
         BeginProperty Font 
            Name            =   "Arial"
            Size            =   12
            Charset         =   0
   ...
   Begin VB.TextBox Text2 
      ...
      TabIndex        =   24
      TabStop         =   0   'False
      Text            =   "INGESLINGA.frx":0000
      Top             =   6000
      Visible         =   0   'False
      Width           =   3615
   End
   Begin VB.Frame Frame2 
      Caption         =   "Materiales Transferidos  a Depósito de Eslingas (Solo Con Saldo)"
      BeginProperty Font 
         Name            =   "Arial"
         Size            =   9.75
         Charset         =   0
      ...
      Top             =   1800
      Width           =   15495
      Begin VB.TextBox TxtSaldoMetros 
   ...
   Begin ACTIVESKINLibCtl.Skin Skin1 
      Left            =   2625
      OleObjectBlob   =   "INGESLINGA.frx":06E0
      Top             =   0
   End
   Begin ACTIVESKINLibCtl.SkinLabel SkinLabel999 
      ...
      Index           =   0
      Left            =   3240
      OleObjectBlob   =   "INGESLINGA.frx":0914
      TabIndex        =   2
      Top             =   0
      Visible         =   0   'False
      Width           =   1485
   ...
   Begin VB.Menu mnuCons 
      ...
      Begin VB.Menu CintillasenOfAbiertas 
         Caption         =   "Reservas de Eslingas en O.Fab. Abiertas"
      End
      Begin VB.Menu CintillasenOfCerradas 
         Caption         =   "Reservas de Eslingas en O.Fab. Cerradas"
      End
   End
   Begin VB.Menu mnuut 
      Caption         =   "Utilidades"
...
Private Sub CintillasenOfAbiertas_Click()
20

    Campos$ = "Código/A16;Descripción/A40;Cant.Saldo/F12.1;Lote/A18;O.Fab./A14"
    Call CARGA_ENCABEZADO(GRILLAGRAL.GRID, Campos$, GRILLAGRAL, 1)
    Call CARGA_ENCABEZADO(GRILLAGRAL.MSF_2, Campos$, GRILLAGRAL, 1)
    J& = 0
    
    SQLX$ = "select M.COD_INTE,m.Cod_Exte,m.Descrip, sum(m.cantingre-m.cantsalid) AS SALDO,m.IdenLote,o.IdSubOr"
    SQLX$ = SQLX$ + " from movisto m WITH(NOLOCK) inner join ORDEFABR o on (doseclar = idsubor or  DoPriLar = idsubor)"
    SQLX$ = SQLX$ + " Where"
    SQLX$ = SQLX$ + "  o.StatuOrf = 1 and m.depomovi = 2 and m.descrip like '%eslinga%'"
    SQLX$ = SQLX$ + "  group by M.COD_INTE, m.idenlote,m.Cod_Exte,m.Descrip,o.IdSubOr"
    SQLX$ = SQLX$ + "  Having Sum(m.cantingre - m.cantsalid) > 0.05"
    ...
    Do While Not .EOF
        ...
        GRILLAGRAL.GRID.TextMatrix(J&, 3) = FORMATNUM$(SALDO, "F10.1")
   ...
   GRILLAGRAL.Caption = "Reservas de Eslingas en O.Fab. Abiertas  "
   GRILLAGRAL.Show 1
   ...
   If GRILLAGRAL.MENU1 > 0 Then
        ...
        Call CARGA_TABLA_IMPRE(MSF, Campos$, "SCRPRESU", "Eslingas en O.F. Abiertas ", "TABLA_IMPRE", RET_NAMECONS$, 1)
...
Private Sub CintillasenOfCerradas_Click()
    Campos$ = "Código/A16;Descripción/A40;Cant.Saldo/F12.1;Lote/A18;O.Fab./A14"
    Call CARGA_ENCABEZADO(GRILLAGRAL.GRID, Campos$, GRILLAGRAL, 1)
    Call CARGA_ENCABEZADO(GRILLAGRAL.MSF_2, Campos$, GRILLAGRAL, 1)
    J& = 0
    ...
    SQLX$ = SQLX$ + " from movisto m WITH(NOLOCK) inner join ORDEFABR o on (doseclar = idsubor or  DoPriLar = idsubor)"
    SQLX$ = SQLX$ + " Where"
    SQLX$ = SQLX$ + "  o.StatuOrf = 3 and m.depomovi = 2 and m.descrip like '%eslinga%'"
    SQLX$ = SQLX$ + "  group by M.COD_INTE, m.idenlote,m.Cod_Exte,m.Descrip,o.IdSubOr"
    SQLX$ = SQLX$ + "  Having Sum(m.cantingre - m.cantsalid) > 0.05"
    SQLX$ = SQLX$ + "  order by idsubor"
    Set rs = READSET(SQLX$)
   ...
   GRILLAGRAL.GRID.ZOrder 0
   GRILLAGRAL.TXTBUSCAR = ""
   GRILLAGRAL.Caption = "Reservas de Eslingas en O.Fab. Cerradas "
   GRILLAGRAL.Show 1
...
Private Sub Command11_Click()
    ...
    PESOX# = ctp.PesoLeidoBalanza(CI1%, 1) 'CAMBIE A 1 POR QUE ES PESO 0 SI NO VA A VER QUE MOSTRAR LA PREBALANZA
    If PESOX# < 0 Then PESOX# = 0          'PARA EDITAR LA TARA
...
Private Sub Command2_Click()
    ...
    DEPOESLINGA% = XVALO(Me.Label13)
    DEPOCORTE% = XVALO(Control("", "DEPODEST"))
    NUMEOPERACION% = XVALO(TXTNOP)
    If DEPOESLINGA% < 1 Then
       Call MENSERR(24, "Falta Depósito de Eslingas")
       GoTo 99
    End If
    If DEPOCORTE% < 1 Then
       Call MENSERR(24, "Falta Depósito de Corte")
       GoTo 99
    End If
    ...
    CANTI = XVALO(TXTCORTE) * CantdeCortes
    If XVALO(Me.TxtSaldoMetros) < CANTI Then
        Call MENSERR(24, "Cantidad de Cortes x Metros de Corte, supera el Saldo de Metros")
        GoTo 99
    End If

    CI1% = TXTELEMSELEC
    If Unimed(CI1%) = "M." Then METROS# = CANTI
    NCOMP& = ProNroComprobante(CMOV$)
    DOPRI$ = "TR." + TRIM$(str$(NCOMP&))
    DOSEC$ = TRIM$(LBLOF)
    NUMEOPERACION% = XVALO(TXTNOP)
    DESCRIDEPO$ = TRIM$(ECOARCH$("TADEPOSI", Chr$(DEPOESLINGA%)))
...
'    CANTI = MetrosSegunPesoSegúnLote#(LOTE$, XVALO(Me.TXTKGS))
    Call MOVISTOK(FECHA%, CI1%, LOTE$, CMOV$, DOPRI$, DOPRI$, DOSEC$, DOSEC$, REFE$, 0, 0, 1, NC%, DEPOESLINGA%, (-1) * CANTI, , , , , , , , , , NUMEOPERACION%, METROS#, PESOKG#)   'GUARDO EL NUMERO DE OPERACION DE CADA ITEMS TRANSFERIDO
    
    DESCRIDEPO$ = ECOARCH$("TADEPOSI", Chr$(DEPOCORTE%))
    REFE$ = "Tr.A " & TRIM$(DESCRIDEPO$)
    NCOMP& = ProNroComprobante(CMOV$)
    
    Call MOVISTOK(FECHA%, CI1%, LOTE$, CMOV$, DOPRI$, DOPRI$, DOSEC$, DOSEC$, REFE$, 0, 0, 1, NC%, DEPOCORTE%, CANTI, , , , , , , , , , NUMEOPERACION%, METROS#, PESOKG#)    'GUARDO EL NUMERO DE OPERACION DE CADA ITEMS TRANSFERIDO
    ...
    IdMovistoGenerado& = 0
    CMOV$ = "CF"
    REFE$ = "CONSUMO " & CODEXT$(CIFAB%)
    DOPRI$ = NORFA$
    DOSEC$ = "PF-" + TRIM$(Mid$(NORFA$, 4))
    NC = 0
    Call MOVISTOK(HOY(HO$), CI1%, TXTLOTE2, CMOV$, DOPRI$, DOPRI$, DOSEC$, DOSEC$, REFE$, 0, 0, 1, NC, DEPOCORTE%, (-1) * METROS#, , , , , , , , , , NUMEOPERACION%, METROS#, PESOKG)      'GUARDO EL NUMERO DE OPERACION DE CADA ITEMS TRANSFERIDO
    ...
    'FIN CONSUMO
    Call ctp.GrabarOperacion(NORFA$, CODIEMPR%, CIFAB%, NUMEOPERACION%, CantdeCortes, LOTE$, CI1%, XVALO(TXTCORTXBSON), NUMEOPERARIO%, CORTEENMETROS, PESOKG, RET_NEWORDEN%, Ret_NuevoIDGenerado&, , , , DESCRIOP.Caption, IdMovistoGenerado&)
    Call ctp.CargaMatTranfXOrden(MSF1, LBLOF, DEPOCORTE%, NUMEOPERACION%)
    Call ctp.TransfIngresada(MSF0, DEPOESLINGA%)
...
Private Sub Command3_Click()
    ...
    DEPOCORTESLINGA% = Me.Label13
    If DEPOCORTESLINGA% < 1 Then
      Call COMUNI("Falta Depósito de Corte de Eslingas")
      GoTo 99
    End If
    ...
    CANTI = XVALO(TXTMETROS): METROS# = CANTI
    PESOKG# = XVALO(TXTPESO)
    LOTE$ = TRIM$(TXTLOTE)
    NCOMP& = ProNroComprobante(CMOV$)
    DOPRI$ = "TR." + TRIM$(str$(NCOMP&))
    DOSEC$ = LOTE$
    ...
    REFE$ = "Tr.de " & TRIM$(DESCRIDEPO$)
    Call MOVISTOK(FECHA%, CI1%, LOTE$, CMOV$, DOPRI$, DOPRI$, DOSEC$, DOSEC$, REFE$, 0, 0, 1, NC%, DEPOALMACEN%, (-1) * CANTI, 0, 0, , , , , , , , , METROS#, PESOKG#)     'GUARDO EL NUMERO DE OPERACION DE CADA ITEMS TRANSFERIDO
    
    DESCRIDEPO$ = ECOARCH$("TADEPOSI", Chr$(DEPOCORTESLINGA%))
    REFE$ = "Tr.A " & TRIM$(DESCRIDEPO$)
    NCOMP& = ProNroComprobante(CMOV$)
    
    Call MOVISTOK(FECHA%, CI1%, LOTE$, CMOV$, DOPRI$, DOPRI$, DOSEC$, DOSEC$, REFE$, 0, 0, 1, NC%, DEPOCORTESLINGA%, CANTI, 0, , , , , , , , , , METROS#, PESOKG#)  'GUARDO EL NUMERO DE OPERACION DE CADA ITEMS TRANSFERIDO
    Call ACSALOTE(CI1%, LOTE$, "NOMESS")
    Dim ctp As New Conteplast
    Call ctp.TransfIngresada(MSF0, DEPOCORTESLINGA%)
...
Private Sub Form_Load()
    ...
    DEPOCORTESLINGA% = XVALO(Control("CONTEPLA", "DEPOESLI"))
    If DEPOCORTESLINGA% < 1 Then
       Call COMUNI("Debe Configurar Depósito de Corte de Eslingas")
       Exit Sub
    End If
    Label13 = XVALO(Control("CONTEPLA", "DEPOESLI"))
...
Private Sub LBLOF_Change()
   ...
   CIFAB% = CODIFAB%(NORFA$)
   NombreOperacion$ = "ESLINGAS [CM]"
   Dim ctp As New Conteplast
   NOPER% = ctp.NumeOperXNombre%(CIFAB%, NombreOperacion$)
   Set ctp = Nothing
   If NOPER% < 1 Then
     Call COMUNI("Atención !! La Orden de Fabricación Seleccionada No Registra Operacion de Eslinga")
     Exit Sub
   End If
   
   TXTCANPROY = CantiProyectadaOF(NORFA$)
   
   NOMBRESECTOR$ = SAFETEXT$(ctp.OperacionesDelCodigoLeidoGeneral$(XVALO(TXTELEMSELEC), NORFA$, retornoCantReg&))
   NOPERSEL% = XVALO(RESP_ZELE_VECT(8))
   If NOPERSEL% > 1 Then
     TXTNOP = NOPERSEL%
     DESCRIOP = NOMBRESECTOR$
   Else
     TXTNOP = NOPER%
     DESCRIOP = NombreOperacion$
   End If
   DEPOCORTE% = XVALO(Control("", "DEPODEST"))
   Call ctp.CargaMatTranfXOrden(Me.MSF1, NORFA$, DEPOCORTE%, NOPER%)
   Call ctp.CargaOperacionesInformadas(NORFA$, CIFAB%, MSF2, 0, "", 99)
   Set ctp = Nothing
...
Private Sub MSF0_DblClick()
    ...
    TXTELEMSELEC = CIELEM%
    Me.TxtSaldoMetros = MSF0.TextMatrix(Reg&, 5)

    If NORFA$ = "" Then Exit Sub
    CODIGOFAB% = CODIFAB%(NORFA$)
    LBLOF = "" 'PARA QUE SE REFRESQUE
