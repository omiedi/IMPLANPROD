...
Begin VB.Form INIFLEX25 
   ...
   Begin ACTIVESKINLibCtl.SkinLabel SkinLabel999 
      ...
      Index           =   0
      Left            =   3480
      OleObjectBlob   =   "INIFLEX25.frx":1D2A
      TabIndex        =   27
      Top             =   6240
      Visible         =   0   'False
      Width           =   495
   ...
   Begin ACTIVESKINLibCtl.Skin Skin1 
      Left            =   3480
      OleObjectBlob   =   "INIFLEX25.frx":1DA4
      Top             =   5520
   End
   ...
   Begin VB.PictureBox Picture10 
      ...
      Begin ACTIVESKINLibCtl.SkinLabel Label6 
         Height          =   255
         Left            =   0
         OleObjectBlob   =   "INIFLEX25.frx":1565E
         TabIndex        =   36
         Top             =   900
         Width           =   3615
      End
      Begin ACTIVESKINLibCtl.SkinLabel Label7 
         Height          =   255
         Left            =   0
         OleObjectBlob   =   "INIFLEX25.frx":156D0
         TabIndex        =   35
         Top             =   8040
         Width           =   3615
      End
   ...
   Begin ComctlLib.ImageList imlSmallIcons 
      ...
      BeginProperty Images {0713E8C2-850A-101B-AFC0-4210102A8DA7} 
         NumListImages   =   8
         BeginProperty ListImage1 {0713E8C3-850A-101B-AFC0-4210102A8DA7} 
            Picture         =   "INIFLEX25.frx":42CFF
            Key             =   "closed"
         EndProperty
         BeginProperty ListImage2 {0713E8C3-850A-101B-AFC0-4210102A8DA7} 
            Picture         =   "INIFLEX25.frx":43221
            Key             =   "cylinder"
         EndProperty
         BeginProperty ListImage3 {0713E8C3-850A-101B-AFC0-4210102A8DA7} 
            Picture         =   "INIFLEX25.frx":43743
            Key             =   "open1"
         EndProperty
         BeginProperty ListImage4 {0713E8C3-850A-101B-AFC0-4210102A8DA7} 
            Picture         =   "INIFLEX25.frx":45295
            Key             =   "leaf"
         EndProperty
         BeginProperty ListImage5 {0713E8C3-850A-101B-AFC0-4210102A8DA7} 
            Picture         =   "INIFLEX25.frx":457B7
            Key             =   "open"
         EndProperty
         BeginProperty ListImage6 {0713E8C3-850A-101B-AFC0-4210102A8DA7} 
            Picture         =   "INIFLEX25.frx":45CD9
            Key             =   "smlBook"
         EndProperty
         BeginProperty ListImage7 {0713E8C3-850A-101B-AFC0-4210102A8DA7} 
            Picture         =   "INIFLEX25.frx":4633B
            Key             =   ""
         EndProperty
         BeginProperty ListImage8 {0713E8C3-850A-101B-AFC0-4210102A8DA7} 
            Picture         =   "INIFLEX25.frx":4699D
            Key             =   "closed1"
         EndProperty
      EndProperty
   End
End
Attribute VB_Name = "INIFLEX25"
...
Private Declare Function WTSGetActiveConsoleSessionId Lib "kernel32" () As Long

Private Declare Function WTSFreeMemory Lib "Wtsapi32.dll" ( _
    ByVal pMemory As Long) As Long

Private Const WTS_CURRENT_SERVER_HANDLE As Long = 0
Private Const WTSUserName As Long = 5
...
Sub CARGAMENU()
    ...
    ' Agregar nodos al TreeView desde la base de datos
    U& = 1
    Do While U& <= Treeview1.Nodes.Count
        CLAPA$ = Treeview1.Nodes(U&).key
10      SQLX$ = "SELECT * FROM USER_MENUS WITH(NOLOCK) "
...
19                  .MoveNext
    ...
    Loop
        ...
        Call MOSTRARFLEXGRID
        Call CARGAPREFERIDOS
        Call MOSTRARFLEXGRID2
End Sub



Sub MOSTRARFLEXGRID()
    Dim nodo As ComctlLib.Node
    Dim datos() As Variant
    Dim Fila As Integer
    Dim NIVEL As Integer
    Dim i As Integer
    Dim descripcionesUnicas As Object
    
    ' Crear un diccionario para almacenar las descripciones únicas
    Set descripcionesUnicas = CreateObject("Scripting.Dictionary")
    
    ' Configurar MSFlexGrid
    With MSFlexGrid1
        .Rows = 1 ' Limpiar filas previas
        .Cols = 3 ' Descripción, Nivel, Código
        .TextMatrix(0, 0) = "Descripción"
        .TextMatrix(0, 1) = "Nivel"
        .TextMatrix(0, 2) = "Código"
        ' Ajustar ancho de la columna de Descripción a 7 cm
        .ColWidth(0) = 7 * 1440 ' Conversión de cm a Twips (1 cm = 1440 Twips)
    End With
    
    ' Crear una matriz para almacenar los datos temporalmente
    ReDim datos(1 To Treeview1.Nodes.Count, 1 To 3) ' Tres columnas: Descripción, Nivel, Código
    
    Fila = 1
    ' Leer nodos visibles y agregar datos a la matriz hasta nivel 1
    For Each nodo In Treeview1.Nodes
        If nodo.Visible Then
            GuardarDatosEnMatriz nodo, datos, Fila, 0, 1, descripcionesUnicas ' Máximo nivel es 1
        End If
    Next nodo
    
    ' Cargar los datos únicos en el MSFlexGrid
    For i = 1 To Fila - 1
        MSFlexGrid1.AddItem datos(i, 1) & vbTab & datos(i, 2) & vbTab & datos(i, 3) ' Orden: Descripción, Nivel, Código
        ' Cambiar color de la fila
'        If i Mod 2 = 0 Then
'            MSFlexGrid1.Row = i
'            MSFlexGrid1.CellBackColor = vbWhite ' Fila blanca
'        Else
'            MSFlexGrid1.Row = i
'            MSFlexGrid1.CellBackColor = vbGray ' Fila gris
'        End If
        ' Poner texto en negritas
        MSFlexGrid1.Row = i
        Call NEGRITA_SOLO_UNA_FILA(MSFlexGrid1, MSFlexGrid1.Row, 20)
    Next i
End Sub

Sub GuardarDatosEnMatriz(nodo As ComctlLib.Node, ByRef datos() As Variant, ByRef Fila As Integer, NIVEL As Integer, maxNivel As Integer, descripcionesUnicas As Object)
    Dim nodoHijo As ComctlLib.Node
    
    ' Guardar datos del nodo actual en la matriz si está dentro del nivel permitido
    If NIVEL <= maxNivel Then
        ' Solo agregar si la descripción no existe en el diccionario
        If Not descripcionesUnicas.Exists(nodo.TEXT) Then
            descripcionesUnicas.Add nodo.TEXT, True
            datos(Fila, 1) = nodo.TEXT ' Descripción
            datos(Fila, 2) = NIVEL     ' Nivel
            datos(Fila, 3) = nodo.key  ' Código
            Fila = Fila + 1
        End If
    End If
    
    ' Recorrer hijos solo si el nivel actual está dentro del rango
    If NIVEL < maxNivel Then
        Set nodoHijo = nodo.Child
        Do While Not nodoHijo Is Nothing
            GuardarDatosEnMatriz nodoHijo, datos, Fila, NIVEL + 1, maxNivel, descripcionesUnicas
            Set nodoHijo = nodoHijo.Next
        Loop
    End If
End Sub
Sub OrdenarMatrizPorDescripcion(ByRef datos() As Variant, filas As Integer)
    Dim i As Integer, J As Integer
    Dim temp As Variant
    ' Implementar una burbuja para ordenar por la columna "Descripción" (columna 1)
    For i = 1 To filas - 1
        For J = i + 1 To filas
            If datos(i, 1) > datos(J, 1) Then
                ' Intercambiar filas
                temp = datos(i, 1): datos(i, 1) = datos(J, 1): datos(J, 1) = temp
                temp = datos(i, 2): datos(i, 2) = datos(J, 2): datos(J, 2) = temp
                temp = datos(i, 3): datos(i, 3) = datos(J, 3): datos(J, 3) = temp
            End If
        Next J
    Next i
End Sub
...
Private Sub MSFlexGrid2_Click()
    Dim CLANODO As String
    Dim EXECONN As String
    Dim TTXX As String
    Dim PPXX As Integer
    ...
    With MSFlexGrid2
        If .Row > 0 Then
            ...
            CLANODO = TRIM$(.TextMatrix(.Row, 2)) ' Código (columna 1)
            TTXX = .TextMatrix(.Row, 0)          ' Descripción (columna 0)
            
            ' Procesar el código para extraer el ejecutable
            PPXX = InStr(CLANODO, "/")
            If PPXX < 1 Then PPXX = 1 + Len(CLANODO)
            EXECONN = Left$(CLANODO, PPXX - 1)
            
            ' Verificar el tipo de acción según el código
            If TRIM$(Left$(CLANODO, 1)) = "?" Or TRIM$(Left$(CLANODO, 1)) = "*" Or EXISTE%(EXECONN + ".EXE") > 0 Then
...
Private Sub Form_Resize()
   ...
   Picture3.Height = Height - Picture3.Top
      ...
      MSFlexGrid2.Top = Frame2.Top + Frame2.Height + Label7.Height - 50
      On Error Resume Next
      TreeView2.Height = Height - TreeView2.Top - 450
      MSFlexGrid2.Height = Height - MSFlexGrid2.Top - 450
...
Sub ACOMODATODO()
    ...
    MSFlexGrid1.Width = Frame1.Left + 50
    TreeView2.Width = Frame1.Left + 50
    MSFlexGrid2.Width = Frame1.Left + 50
    ...
    MSFlexGrid1.Top = Label6.Top + Label6.Height - 10
    Treeview1.Height = TreeView2.Top - Label7.Height - Treeview1.Top
    MSFlexGrid1.Height = TreeView2.Top - Label7.Height - MSFlexGrid1.Top
    Picture7.Refresh
    Picture8.Refresh
    Picture10.Refresh
    '
    ...
    TreeView2.Top = Frame2.Top + Frame2.Height + Label7.Height - 50
    MSFlexGrid2.Height = Height - TreeView2.Top - 450
    TreeView2.Height = Height - TreeView2.Top - 450
    MSFlexGrid2.Height = Height - TreeView2.Top - 450
    Treeview1.Height = TreeView2.Top - Label7.Height - Treeview1.Top
    MSFlexGrid1.Height = TreeView2.Top - Label7.Height - MSFlexGrid1.Top
...
Private Sub MSFlexGrid1_Click()
    ...
    With MSFlexGrid1
        If .Row > 0 Then
            ' Obtener los valores de la fila seleccionada
            CLANODO = TRIM$(Left$(.TextMatrix(.Row, 2), 64)) ' Código (columna 2)
            TTXX = .TextMatrix(.Row, 0) ' Descripción (columna 0)
...
Private Sub MSFlexGrid1_MouseDown(Button As Integer, Shift As Integer, X As Single, Y As Single)
    ...
    ' Determinar la fila y columna bajo el cursor
    With MSFlexGrid1
        Fila = .MouseRow
        COLUMNA = .MouseCol
        
        ' Asegurarse de que no sea la fila de encabezados o un área inválida
        If Fila > 0 And COLUMNA >= 0 Then
            ' Cambiar el color de fondo para simular un efecto de botón presionado
            .Row = Fila
            .COL = COLUMNA
            .CellBackColor = vbButtonFace ' Color de fondo similar a un botón
            .CellFontBold = True ' Texto en negritas
        End If
    End With
End Sub
...
Private Sub MSFlexGrid2_MouseUp(Button As Integer, Shift As Integer, X As Single, Y As Single)
    ...
    ' Restaurar el color de fondo después de soltar el botón del mouse
    With MSFlexGrid2
        Fila = .MouseRow
        COLUMNA = .MouseCol
        ...
        If Fila > 0 And COLUMNA >= 0 Then
            .Row = Fila
            .COL = COLUMNA
            .CellBackColor = vbWhite ' Restaurar el color original (puedes ajustarlo según prefieras)
            .CellFontBold = False ' Restaurar el estilo del texto
        End If
    End With
End Sub
...
Sub MOSTRARFLEXGRID2()
    Dim nodo As ComctlLib.Node
    Dim Fila As Integer
    
    ' Configurar MSFlexGrid2
    With MSFlexGrid2
        .Rows = 1 ' Limpiar filas previos
        .Cols = 3 ' Descripción, Código, Cantidad de Accesos
        .TextMatrix(0, 0) = "Descripción"
        .TextMatrix(0, 1) = "Código"
        .TextMatrix(0, 2) = "Cantidad de Accesos"
        ' Ajustar ancho de la columna de Descripción
        .ColWidth(0) = 7 * 1440 ' 7 cm en Twips
    End With
    
    Fila = 1
    ' Recorrer TODOS los nodos del TreeView2, sin filtrar por visibilidad
    For Each nodo In TreeView2.Nodes
        MSFlexGrid2.AddItem nodo.TEXT & vbTab & nodo.key & vbTab & ExtractAccessCount(nodo.TEXT)
        ' Cambiar color de la fila
        If Fila Mod 2 = 0 Then
            MSFlexGrid2.Row = Fila
            MSFlexGrid2.CellBackColor = vbWhite ' Fila blanca
        Else
            MSFlexGrid2.Row = Fila
            MSFlexGrid2.CellBackColor = vbGray ' Fila gris
        End If
        ' Poner texto en negritas
        MSFlexGrid2.Row = Fila
    ...
    Next nodo
End Sub
Function ExtractAccessCount(description As String) As Integer
    ' Extrae el número de accesos desde la descripción, asumiendo el formato "(N)"
    Dim openParen As Integer, closeParen As Integer
    openParen = InStr(description, "(")
    closeParen = InStr(description, ")")
    If openParen > 0 And closeParen > openParen Then
        ExtractAccessCount = Val(Mid(description, openParen + 1, closeParen - openParen - 1))
    Else
        ExtractAccessCount = 0
    End If
End Function
...
Function GetCurrentUser() As String
    ...
    result = WTSQuerySessionInformation(WTS_CURRENT_SERVER_HANDLE, sessionId, WTSUserName, buffer, bytesReturned)
    
    If result <> 0 Then
        GetCurrentUser = Left(buffer, bytesReturned)
        WTSFreeMemory StrPtr(buffer)
    Else
        GetCurrentUser = "Error al obtener el usuario"
    End If
End Function
