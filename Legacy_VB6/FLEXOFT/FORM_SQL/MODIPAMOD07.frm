...
Begin VB.Form MODIPAMOD07 
   ...
   Begin ACTIVESKINLibCtl.Skin Skin1 
      Left            =   420
      OleObjectBlob   =   "MODIPAMOD07.frx":33E2
      Top             =   0
   End
   Begin ACTIVESKINLibCtl.SkinLabel SkinLabel999 
      ...
      Index           =   0
      Left            =   0
      OleObjectBlob   =   "MODIPAMOD07.frx":3616
      TabIndex        =   66
      Top             =   315
      Visible         =   0   'False
      Width           =   1590
...
8       PPXX% = InStr(TCELU$, "/")
   ...
   If TRIM$(Label4) <> "" Then
     ...
     ORFANUM$ = TRIM$(Label4)
     SQLX$ = "SELECT * FROM OrdeFabr " ' CORREGIDO FALTABA IDSUBOR
     SQLX$ = SQLX$ + " WHERE  IDSUBOR = '" & ORFANUM$ & "'"
...
19 NOPXX$ = Text3
   ...
   With INFOFABTMP
     ...
     If Err < 1 Then
       On Error GoTo 0
       Do While Not .EOF
         If XVALO(!NuOrInf) <> CAINFANT% Then
'           If IsNull(!RETRABA) Or !RETRABA < 1 Then
   ...
   'TEXTO DE CELULA  / LE QUITO LA ULTIMA ';'  PARA GUARDAR EN LA BASE
   ...
   '
   'MODIFIQUE POR QUE FALTABA LA SELECT Y TABLA
   ABUS$ = "SELECT * FROM INFOFAB WHERE IdSubOr = '" & NROOF$ & "' "
   ...
   'With InfoFab
     ...
     INFOFFFAB.Open FILTSQL$(ABUS$), FLEXCONN, adOpenDynamic, adLockPessimistic, adCmdText
     With INFOFFFAB
        ...
        If Err < 1 Then
            ...
            !TiemStan = TIESTAN(CIXXI%, NOPE%, CAINF#, MODO%, ALTEX%)  ' 1 o mas es para que no considere preparacion
               ...
               RENDI = 999.9: If XVALO(!TiemStan) > 0 Then RENDI = 100 * XVALO(!TiemEjec) / XVALO(!TiemStan)
   ...
   With INFOFABTMP1
     ...
     If Err < 1 Then
       ...
       Do While Not .EOF
         'If IsNull(!ReTraba) Or !ReTraba < 1 Then
'         If !RETRABA Is Null Or !RETRABA < 1 Then
          If XVALO(!RETRABA) < 1 Then
   ...
   With OPERFFFAB
     ...
     If Err < 1 Then
       ...
       !TiemPend = TIEMAQUI(CIXXI%, NOPE%, CANTPE#, MODO%)  ' 1 es para que no considere preparacion
           If XVALO(!TiemPend) < 0 Or STATUOP% = 3 Or STATUOP% = 0 Then !TiemPend = 0
       !StatOper = STATUOP%
       .Update
     End If
   End With
...
19   NOPXX$ = Text3
     ...
     If TRIM$(NROOF$) <> "" Then
        ...
        SQLX$ = "SELECT * FROM OrdeFabr "
        SQLX$ = SQLX$ + " WHERE "
        SQLX$ = SQLX$ + " IdSubOr = '" & NROOF$ & "' "
        SQLX$ = SQLX$ + " AND Cod_Inte = " & CIXXI% & " "
        'OrdeFabr.FindFirst ABUS$
        Dim ORDEFFFABR1 As ADODB.Recordset
        Set ORDEFFFABR1 = FLEXCONN.Execute(FILTSQL$(SQLX$))
        With ORDEFFFABR1
          On Error Resume Next
          .MoveFirst
          'If OrdeFabr.NoMatch = False Then
          If Err < 1 Then
            On Error GoTo 0
            If !StatuOrf < 3 Then                       ' no marca CERRADAS Y cumplidos"
            GoTo 20
            End If
          End If
          On Error GoTo 0
     ...
     End If
     '
20   ABUS$ = "SELECT * FROM infofab "
     ABUS$ = ABUS$ + " WHERE IdSubOr = '" & NROOF$ & "' "
     ABUS$ = ABUS$ + " AND Cod_Inte = " & CIXXI% & " "
     ABUS$ = ABUS$ + " AND NumeOper = " & NOPE% & " "
     ABUS$ = ABUS$ + " AND NuOrInf = " & CAINFANT% & " "
     'Call ABREORFAB
     'With InfoFab
       '.FindFirst ABUS$
     Dim ABREORRRFAB As ADODB.Recordset
     Set ABREORRRFAB = New ADODB.Recordset
     ABREORRRFAB.Open FILTSQL$(ABUS$), FLEXCONN, adOpenDynamic, adLockPessimistic, adCmdText
     With ABREORRRFAB
      'If .NoMatch = False Then
       On Error Resume Next
       .MoveFirst
     ...
     Dim INFOFABTMP As ADODB.Recordset
     Set INFOFABTMP = New ADODB.Recordset
     'INFOFABTMP.Open SQLX$, FLEXCONN, adOpenKeyset, adLockReadOnly, adCmdText
     Dim INFOFABTMP1 As ADODB.Recordset
     Set INFOFABTMP1 = New ADODB.Recordset
     INFOFABTMP1.Open FILTSQL$(SQLX$), FLEXCONN, adOpenKeyset, adLockReadOnly, adCmdText
...
Private Sub List1_Click()
   ...
   'DATOS DE NRO DE INFORME DE NO CONFORMIDAD Y REFERENCIA DE INFORME
   Condi$ = "IDSUBOR = '" & TRIM$(Label4) & "' AND COD_INTE = " & CODINT(Text10) & " AND NUORINF = " & NORI% & " AND NUMEOPER = " & XVALO(Text3) & ""
   Text12 = VALOBADA("infofab", "NROINFNC", Condi$)
   Text13 = VALOBADA("infofab", "REFEINF", Condi$, "NOMESS")
   '
   If XVALO(LG) < 1 Then
...
Private Sub Text10_Change()
   ...
   If CIXXI% > 0 Then
      ...
      With ORFASEL
        .MoveFirst
        Do While (.EOF = False)
          If HAYOPINF%(SAFETEXT$(!IdSubOr)) > 0 Then
             If Label4 = "" Then Label4 = SAFETEXT$(!IdSubOr)
             Y$ = REGBLAN("ECORDEP")
             Call REPLA(Y$, SAFETEXT$(!IdSubOr), 1, 12)
             Call REPLA(Y$, ECOARCH$("ECORDEF", SAFETEXT$(!IdSubOr)), 13, 52)
             '
             JJ& = JJ& + 1
...
'\\\OT-06-0309-MH-28/08/06///
          ...
          '
          If ALTIN% = ALTEX% Then
             NORI% = XVALO(!NuOrInf)      'numero de INFORME
             'AGREGADAS POR MI
             FEC% = CVINT(!FeHorLib)
             HDES% = XVALO(!Hora_Des)
             HHAS% = XVALO(!Hora_Has)
             ...
             HFRI% = XVALO(!Hora_Fri)
             On Error GoTo 0
             MAQ$ = SAFETEXT$(!MAQUINA)
             CAN# = XVALO(!CANTREAL)
             CANOCO# = 0
             On Error Resume Next
             CANOCO# = XVALO(!CaNoConf)
             On Error GoTo 0
             LEGAX$ = SAFETEXT$(!LegaOps)
             If Len(LEGAX$) > 2 Then
                  LEG% = 0
                  DES$ = "CÉLULA"
                Else
             ...
             MARETRA$ = " ": ATEMUL$ = " ": OBSX$ = ""
             On Error Resume Next
             MARETRA$ = TRIM$(Str$(SAFETEXT$(!RETRABA)))
             ATEMUL$ = TRIM$(Str$(SAFETEXT$(!AtMulti)))
             OBSX$ = SAFETEXT$(!Referen)
             On Error GoTo 0
             'Label5.Caption = ECOARCH$("PADRON", MKI$(XVALO(Text1.TEXT)))
             YY$ = Space$(256)
     ...
     If Err < 1 Then
       ...
       With SECOPTEMP
         Do While Not .EOF
           ...
           NOPX$ = Left$(NOPX$ + "      ", 6) + "- "
           DOPE$ = SAFETEXT$(!DESCOPER)      'Descripcion de la operacion
           MAQUI$ = SAFETEXT$(!CodMaq0)      'Máquina
           '
           JJ& = JJ& + 1: JJI% = JJ&
           YY$ = MKI$(JJI%) + Space$(58) + String(4, 0)
