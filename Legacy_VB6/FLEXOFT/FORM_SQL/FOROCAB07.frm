...
10     Caption = Caption + "  -  " + EPAC$
    ...
    If CONTROL$("OCOMPRA", "REPLAORI") = "SI" Then
      Nuorig.Enabled = True
      'Nuorig1.Enabled = True           'SE COMENTA PORQUE NO EXISTE LA OPCION MENU --> CONFIGURACION
    End If
    '
    Show
    '
...
'   Call ABRECOMPRAS
'   '
'   SQLX$ = "SELECT Nume_Ocom FROM EncoCabi "
...
'
   ...
   SQLX$ = "Select max(Nume_ocom) as maximo from EncOcabi "
   Dim OEncaTemp As ADODB.Recordset
   Set OEncaTemp = FLEXCONN.Execute(FILTSQL$(SQLX$))
    ...
    For J& = 1 To ULTREG&("!OCOMDETA")
      ...
      With DETOKABI
        ...
        On Error GoTo 0
         ...
         !FVig_Ocom = CVDAT(FECHANUM(Text14.TEXT))
      ...
      For I% = 1 To CAIT%
        ...
        If TRIM$(CODEXT$(CII%)) <> "" Then
          If CANTI# > 0.05 Then
            Call ABRESTOCKS
            'With MaConsig
            Dim MAKONSIG As ADODB.Recordset
            Set MAKONSIG = New ADODB.Recordset
            SQLX$ = "SELECT * FROM MaConsig"
            MAKONSIG.Open FILTSQL$(SQLX$), FLEXCONN, adOpenDynamic, adLockPessimistic, adCmdText
            With MAKONSIG
              .AddNew   ' YA CONVERTIDO
              On Error Resume Next
              !CodiEmpr = CodiEmp%
...
'                   .Edit
                Dim MASTTTER2 As ADODB.Recordset
                Set MASTTTER2 = New ADODB.Recordset
                SQLX$ = "SELECT * FROM MASTER"
                SQLX$ = SQLX$ + "WHERE CODINT = " & CI0% & ""
                MASTTTER2.Open FILTSQL$(SQLX$), FLEXCONN, adOpenDynamic, adLockPessimistic, adCmdText
                With MASTTTER2
                  On Error Resume Next
                  .MoveFirst
                  If Err < 1 Then
...
9  Next KKI%
   '
   FOSEMA07.LIMATSEL.Clear
   For UKI% = 1 To Len(PCI$) Step 2
     CI0% = CVI(Mid$(PCI$, UKI%, 2))
     TTXX$ = AJUSTI$(CODEXT$(CI0%), 16) + DESCRIT0$(CI0%)
     FOSEMA07.LIMATSEL.AddItem TTXX$
   Next UKI%
   FOSEMA07.Caption = "Materiales a Comprar - " + Text2
   FOSEMA07.Label2 = FECHATEX$(UFECHA)
   For U& = 1 To FOSEMA07.LIMATSEL.ListCount
     FOSEMA07.LIMATSEL.Selected(U& - 1) = True
   Next U&
   '
   Screen.MousePointer = 1
   On Error Resume Next
   FOSEMA07.BOTREC.SetFocus
   FOSEMA07.LIMATSEL.TopIndex = 0
   On Error GoTo 0
   FOSEMA07.Show 1
   '
   DoEvents
   If TRIM$(FOSEMA07.Label2 = "") Then Exit Sub
   '
   Screen.MousePointer = 11
   PCI$ = "":
   UFECHAI% = FECHANUM(FOSEMA07.Label2)
   For U& = 1 To FOSEMA07.LIMATSEL.ListCount
     If FOSEMA07.LIMATSEL.Selected(U& - 1) = True Then
       PCI$ = PCI$ + MKI$(CODINT%(TRIM$(Left$(FOSEMA07.LIMATSEL.List(U& - 1), 15))))
     End If
   Next U&
   '
   RFF$ = RECFILT$("PLACOMP", 8, MKI$(NPRO%))
...
Sub CARLISCO()
   ...
   For U& = 1 To FIN&
     ...
     Call REPLA(TTXX$, DESCRIT0$(CI%), 17, 65)
       If COLTELIB% > 0 Then
         Call REPLA(TTXX$, RTrim$(REGLEIDO$(ARCHILIB$, U&)), 17, 65)
       End If
...
Sub PRICOMPRA()
    ...
    If TRIM$(FORIPRE$) <> "" Then
      ...
      NUMEDOC$ = NUMEDOC$ + "-" + "0"
      '\\\OT-06-0082-WAR-FIN///
      TIPODOC$ = "Nota de Pedido - Abierta"
      '
      CODPARAM$(1) = "COND-PAG"
      CODPARAM$(2) = "NUBULTOS"
      CODPARAM$(3) = "IMP-NETO"
