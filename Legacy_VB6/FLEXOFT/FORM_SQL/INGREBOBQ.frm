...
Begin VB.Form INGREBOBQ 
   ...
   Begin ACTIVESKINLibCtl.Skin Skin1 
      Left            =   2625
      OleObjectBlob   =   "INGREBOBQ.frx":061E
      Top             =   0
   End
   Begin ACTIVESKINLibCtl.SkinLabel SkinLabel999 
      ...
      Index           =   0
      Left            =   3240
      OleObjectBlob   =   "INGREBOBQ.frx":0852
      TabIndex        =   2
      Top             =   0
      Visible         =   0   'False
      Width           =   1485
   ...
   Begin VB.Label Label1 
      BackStyle       =   0  'Transparent
      Caption         =   "Materiales Transferidos para La Orden Activa (Devoluciones Descontadas), sin Considerar Consumos"
      BeginProperty Font 
         Name            =   "Arial"
         Size            =   9
         Charset         =   0
...
Private Sub lbldepogeneral_DblClick()
    Call SELECHO("ECODEP")
    DPX1% = Val(VALACT1$("ECODEP"))
    If DPX1% > 0 Then
        DP1% = DPX1%
        lbldepogeneral = TRIM$(str$(DP1%))
    End If
...
'    End If
    ...
    'DEPOCORTE% = XVALO(Control("", "DEPODEST"))
    DEPOCORTE% = XVALO(Control("CONTEPLA", "DEBOAIMP")) 'Dep.Bobinas Para Impresión (Quilmes)(FRM ConfigDepo)
    Label13 = DEPOCORTE% 'REFRESCO POR SI CAMBIO POR EJ. CASO DE CINTILLAS
    Dim CTP As New Conteplast
    Call CTP.CargaMatTranfXOrden(MSF1, TRIM$(TXTOF), DEPOCONSUM%)
    Set CTP = Nothing
...
'      Label13 = Control("", "DEPODEST")
      Label13 = XVALO(Control("CONTEPLA", "DEBOAIMP")) 'Dep.Bobinas Para Impresión (Quilmes)(FRM ConfigDepo)
End Sub
...
'   PesoPorMetro# = PESMETRO(CI1%)
    ...
    OPX% = COMALTER%("Atenciòn !!! Situaciòn de Devoluciòn: \Total Cortes en Metros (Teòricos) = " & FORMATNUM$(MetroInforma#, "F9.0") & "\Saldo a Regresar a Depòsito: " & FORMATNUM$(SALDOENMETROS#, "F9.0") & "\\Cancelar\Generar Devoluciòn")
    If OPX% < 2 Then
       GoTo 99
    End If
'    SaldoBobinaEnKilos# = SALDOENMETROS# * PesoPorMetro#
    ...
    If Button = 2 Then
      ...
      If MSF1.Rows > 1 Then
        If Reg& >= 1 Then
            ...
            OPX% = COMALTER("Realmente Desea Retornar La Bobina Seleccionada\\Cancelar\Retornar a Depósito Original (Pesada en Balanza Recomendado)\Retornar a Depósito Original (Sin Pesar (Cálculo X Informe de Cortes))")
            DepoRetorno% = XVALO(lbldepogeneral)
            If OPX% = 2 Then
              DEPOBAJA% = XVALO(Control("CONTEPLA", "DEPOBAJA"))
              SaldoEnOrigen = XVALO(MSF1.TextMatrix(Reg&, 7))
              If DEPOBAJA% > 0 Then 'SI ESTA CONFIGURADO DEPOSITO DE BAJA FRM CONFIGDEPO 30/03/21
                DESCRIDEPOBAJA$ = TRIM$(ECOARCH$("TADEPOSI", Chr$(DEPOBAJA%)))
                OPX2% = COMALTER%("Seleccione Depósito de Ingreso del Material que se Esta Retornando\\Cancelar\Depósito: " & lbldepogeneral & " - " & Label3 & "\Depósito de Baja: " & DEPOBAJA% & " - " & DESCRIDEPOBAJA$)
                If OPX2% = 1 Then Exit Sub 'CANCELO
                'si la opcion es 2 por defecto tiene el deposito de retorno el general(1)
                If OPX2% = 3 Then 'DEPOSITO DE BAJA
                  DepoRetorno% = DEPOBAJA%
                End If
              End If
              ...
              CIELEM% = XVALO(MSF1.TextMatrix(Reg&, 5))
              If (XVALO(Mid$(TXTOF, 4, 6)) >= 2341 Or XVALO(Mid$(TXTOF, 4, 6)) = 2325) And InStr(CODFAMI$, "BB") > 0 And InStr(EPAC$, "CONTEPLAST") > 0 Then  'ESTA ES A PARTIR DE LA CUAL SE VA CONSIDRERAR DIFERENTE LA DEVOLUCION DE LA BOBINA
                 Call CTP.NuevaDevoluciondeBobina(CIELEM%, TXTOF, LOTE$, XVALO(Label13), DepoRetorno%, NOP%)
              Else
                 Call CTP.DevoluciondeBobina(MSF1, Reg&, TXTOF, XVALO(Label13), DepoRetorno%, SaldoEnOrigen)
              End If
...
Private Sub TXTOF_Change()
    ...
    If InStr(EPAC$, "QUILMES") > 0 Then
        lbldepogeneral = Control$("", "DEPOCEN")
        'Label13 = Control("", "DEPODEST")
        Label13 = XVALO(Control("CONTEPLA", "DEBOAIMP"))
    End If
    NORFA$ = TRIM$(TXTOF)
    CI1% = CODIFAB%(NORFA$)
    TXTCODINT = CI1%
    ...
    Dim CTP As New Conteplast
    DEPOCORTE% = XVALO(Label13)
    CODFAMI$ = TRIM$(CODFAMIL$(CI1%)) 'CODIGO DEL PRODUCTO DE LA OF

    If InStr(EPAC$, "QUILMES") > 0 Then
'       If Left$(CODFAMI$, 2) = "BO" Or Left$(CODFAMI$, 2) = "TI" Then
        If Left$(CODFAMI$, 2) = "TI" Then
