...
Sub Command10_Click()
    ...
    If List1.ListIndex >= 0 Then
       FIKARDEX07.Text1 = Left$(List1.TEXT, 16)
    End If
    FIKARDEX07.Show 1
...
'             SQLX$ = SQLX$ + " AND Cod_Inte = " & CI2% & " "
    ...
    MARESER.OPEN FILTSQL$ (SQLX$), FLEXCONN, adOpenDynamic, adLockPessimistic, adCmdText
    '
    With MARESER
       On Error Resume Next
       .MoveFirst
...
24  SQLX$ = " SELECT * FROM RESERVA "
    ...
    Set MARESANT = New ADODB.Recordset
    MARESANT.OPEN FILTSQL$ (SQLX$), FLEXCONN, adOpenDynamic, adLockPessimistic, adCmdText
    '
    With MARESANT
       On Error Resume Next
25     .MoveFirst
...
Private Sub Command16_Click()
   ...
   '
   OPX% = COMALTER%("Opciones de Trabajo Disponibles:\\Registro de Trazabilidad\Lista de Picking")
   If OPX% < 1 Or OPX% > 2 Then GoTo 99
   '
   If OPX% = 2 Then
      Call FINAL("?LISTAPICK(" + NORFA$ + ")")
      GoTo 99
   End If
   '
...
Sub AUTOTRA()
    ...
    For KKUU& = 1 To ULTREG&("!LILOTSE")
       ...
       If VDEVU$ <> "" Then
          ...
          If CAMAXI < 0 Then CAMAXI = 0
          '
          If XVALO(Mid$(VDEV2$, 19, 10)) < CAMAXI Then
             CAMAXI = XVALO(Mid$(VDEV2$, 33, 12))
          End If
          '
          If CAMAXI >= 0.05 Then
