...
Begin VB.Form ENGANNUMSERIE 
   ...
   Begin ACTIVESKINLibCtl.Skin Skin1 
      Left            =   2160
      OleObjectBlob   =   "ENGANNUMSERIE.frx":0000
      Top             =   4920
   End
   ...
   End
End
Attribute VB_Name = "ENGANNUMSERIE"
...
Private Sub Command3_Click()
       ...
       If (MSFSELECCIONADOS.Rows - 1 + Me.MSFSERIEVINCULADOS.Rows - 1) > XVALO(Label4) Then
          ...
          GoTo 99
       End If
       CI1% = XVALO(lblcodvinculado)
       
'       ESEXCEPCION_SERIE% = ES_EXCEPCION_GENERAR_SERIE%(Label6) ' SI ESTA EN LA LISTA (DATASQL) DEVUELVE EL CODIGO INTERNO
       On Error GoTo ERROR_GUARDAR
       FLEXCONN.BeginTrans
       '''''''''''*************************
       'ESTE ENJUAGUE ES POR EL TEMA QUE PIDIERON QUE EL CODIGO SC0019 QUE EN REALIDAD SIGUE SIENDO TRAZABLE = 2
       'Y ES EL QUE GENERA EL NUMERO DE SERIE ORIGINAL Y SE ENGANCHA EN LA TABLA TRAZAXSERIE APAREZCA COMO UN ITEM PARA
       'ASOCIAR PERO SIN NUMERO DE SERIE, CUANDO LO ASIGNA AL SIGUIENTE HACIA ARRIBA SE GENERA UN NUMERO DE SERIE NUEVO
       'Y SE ACTUALIZA SI EL NUEVO EXISTE YA PARA EL SC0019 TAMBIEN SE CAMBIA POR EL ORIGINAL
       'EJEMPLO: SI EL USUARIO ASOCIA EL N.SERIE ORIGINAL ABC005 AL SIGUIENTE DE LA ESTRUCTURA Y AHORA SE GENERA EL NUEVO
       'N.SERIE ABC001 CAMBIA EL ABC005 POR EL ABC001 Y SI EL ABC001 EXISTIA ANTERIORMENTE LE APLICA LO CAMBIA POR ABC005
       
       ESEXCEPCION_SERIE% = ES_EXCEPCION_GENERAR_SERIE%(Label6)
       If ESEXCEPCION_SERIE% > 0 Then 'CASO DEL CODIGO SC0019 QUE ES TRAZABLE 2 PERO SE UTILIZA SOLO A FIN DE METERLO EN EL CIRCUITO DE TRAZABILIDAD
            For i& = 1 To Me.MSFSELECCIONADOS.Rows - 1
               ID& = MSFSELECCIONADOS.TextMatrix(i&, 4)
               NSERIEORIGINAL$ = MSFSELECCIONADOS.TextMatrix(i&, 3)
               NUMSERINUEVO$ = NUEVONUMEROSERIE_EYM$(CI1%, RET_PARTEFIJA$, 0, ESEXCEPCION_SERIE%)
               'actualizar todo el trazaxserie
               ID_ACTUALIZADO = REEMPLAZO_NS_TANUMSE(NSERIEORIGINAL$, NUMSERINUEVO$, RET_PARTEFIJA$, XVALO(LBLCI1DODIORFA), Label44, Label3, Label13)
               Call ACTUALIZO_TODO_TRAZAXSERIE(NSERIEORIGINAL$, NUMSERINUEVO$, ID_ACTUALIZADO, XVALO(lblcodvinculado))
               MSFSELECCIONADOS.TextMatrix(i&, 3) = NUMSERINUEVO$
            Next i&
       End If
       'VER QUE ASOCIE CON EL TEMA DE LOS COMPONENTES.
       
       'Y TAMBIEN QUE SE INGRESE UN NUMERO DE ORDEN CON LA OF
       '''''''''''*************************
...
60     rs.Close
       ...
       For i& = 1 To Me.MSFSELECCIONADOS.Rows - 1
          NS$ = MSFSELECCIONADOS.TextMatrix(i&, 3)
          Call GENERATRAZAXSERIEM1(Label13)
...
Private Sub Command6_Click()
        ...
        NORFA$ = Label13
        CodigoExtConj$ = TRIM$(Label44) '
        CICONJ% = XVALO(LBLCI1DODIORFA)
        FAMI$ = CODFAMIL$(CICONJ%)
        If FAMI$ <> "10" Then
            With MSFSERIEVINCULADOS
            FORIMPRE$ = Control("", "STIKAPRO")
             For i& = 1 To .Rows - 1
                MARCA$ = .TextMatrix(i&, 0)
                If MARCA$ = "*" Then
                    NSERIE$ = .TextMatrix(i&, 3)
                    FE% = CVINT(.TextMatrix(i&, 1))
                    TEXTOQR$ = Label44 & " " & NSERIE$
                    ANOX$ = Right(FETEXCOR1$(FE%), 4)
                    FILTX$ = NSERIE$ & ";" & ANOX$ & ";" & NORFA$
                    Call IMPRIMIR_ETIQUETAQR(FORIMPRE$, FILTX$, TEXTOQR$, "IMAGNSERIE_QR.JPG", 1, "QR_NSERIE", PIMPRE$, ANCHOX%, ALTOX%)
                End If
             Next i&
            End With
        ElseIf FAMI$ = "10" Then  ' las familias 10 imprimen con la etiqueta c15
           FORIMPRE$ = Control("", "STIKAC15")
            With MSFSERIEVINCULADOS
              For i& = 1 To .Rows - 1
                MARCA$ = .TextMatrix(i&, 0)
                If MARCA$ = "*" Then
                    NSERIE$ = TRIM$(.TextMatrix(i&, 3))
                    FE% = CVINT(.TextMatrix(i&, 1))
                    TEXTOQR$ = TRIM$(Label44) & " " & NSERIE$
                    ANOX$ = TRIM$(Right(FETEXCOR1$(FE%), 4))
                    CODIGOX$ = TRIM$(Label44)
                    FILTX$ = CODIGOX$ & ";" & NSERIE$ & ";" & ANOX$
                    Call IMPRIMIR_ETIQUETAQR(FORIMPRE$, FILTX$, TEXTOQR$, "IMAGNSERIE_QR.JPG", 1, "QR_NSERIE", PIMPRE$, ANCHOX%, ALTOX%)
                End If
               Next i&
             End With
...
Private Sub Label13_Change()
   ...
   If CI1% > 0 Then
     ...
     '
     If KODIELEM < 1 Then
        Call COMUNI("Imposible Trazar un  N.Serie Para Esta O.Fab\La Misma No Posee Dentro de Su Estructura Item Con Trazabilidad x Serie")
        Exit Sub
     End If
     ...
     SQLX$ = SQLX$ + " Where COD_INTE = " & KODIELEM & " AND (OF_ARRASTRA IS NULL OR OF_ARRASTRA = '') ORDER BY IDSUBOR,NUMEROSERIE "
     Set rs = READSET(SQLX$)
     With rs
        Do While Not .EOF
           i& = i& + 1
...
'     'CARGA DE RESERVAS
'     '============================
'     CAMPOS$ = "Cod_Exte/A12; Descrip/A20; Cant.Res./F8.2;Cant.Cons./F8.2;IdenLote/A16; OrdConsum/F8"
'     CONDI$ = CONDI$ + " AND MA.TRAZABLE > 0  ORDER BY RE.COD_INTE" 'AND MA.TRAZABLE > 0
'     Call CARGA_ENCABEZADO(MSFRESERVA, CAMPOS$, Me)
'     Set RS1 = obj.RS_RESERVAS(CONDI$)
'     Call Carga_MSF_Recordset(RS1, CAMPOS$, MSFRESERVA)
'     Set obj = Nothing
'     RS1.Close
'     Set RS1 = Nothing
'     '
'     If ESTRAZABLE(CI1%) = 2 Then
'        'CARGA DE NUMEROS DE SERIE
'        '============================
'        CONDI$ = " IDSUBOR = '" & NORFA$ & "' AND COD_INTE = " & CI1%
'        CONDI$ = CONDI$ + "  ORDER BY NUMEROSERIE"
'        CAMPOS$ = "N.SERIE/A20;FECHA ALTA"
'        Call CARGA_ENCABEZADO(MSFSERIE, CAMPOS$, Me)
'        Set RS1 = obj.RS_NSERIE(CONDI$)
'        Call Carga_MSF_Recordset(RS1, CAMPOS$, Me.MSFSERIE)
'        Set obj = Nothing
'        RS1.Close
'        Set RS1 = Nothing
'        '============================
''ACA VER DE TRAER LOS NUMEROS DE SERIE QUE SE HAN ARRASTRADO.
'
'     ElseIf ESTRAZABLE(CI1%, 1) = -1 Then
'        CONDI$ = " IDSUBOR = '" & NORFA$ & "' AND COD_INTE = " & CI1%
'        CONDI$ = CONDI$ + "  ORDER BY NUMEROSERIE"
'        CAMPOS$ = "N.SERIE/A20;FECHA ALTA"
'        Call CARGA_ENCABEZADO(MSFSERIE, CAMPOS$, Me)
...
'     End If
'
'     If MSFCONSU.Rows + MSFRESERVA.Rows < 2 Then Call MENSERR(24, "Este Producto no Registra Componentes Trazables.")
'
   End If
...
Private Sub mnuquitar_Click()
       ...
       NORFA$ = Label13
       NORFA_SELECCIONADA$ = MSFGLOBALFRM.TextMatrix(GLOBALFRM_FILA&, 2) 'LA QUE SELECCIONO EN LA GRILLA A DESVINCULAR
'LO QUITO POR QUE INDICA QEU ESTAN CERRADOS PERO NO ES EL CASO CON LA VALIDACION DE ABAJO NO DEBERIA HABER PROBLEMAS
'       Condi$ = " TRAZAXSERIE_ID = " & IDTRAX
'       Condi$ = Condi$ + " AND STATUORF = 3 "
'       If CantRegistros("TRAZAXSERIE", Condi$, "NOMESS") > 0 Then
'          Call COMUNI("Imposible Desvincular !!!\Orden de Fabricación Cerrada\Para Este Número de Serie")
'          Exit Sub
'       End If
       
       CONDI$ = "TRAZABLE = 1 AND IDSUBOR = '" & NORFA$ & "' AND NUMEROSERIE ='" & NSERIE$ & "' AND CONSUMIDO > 0"
       If CantRegistros("TRAZAXSERIE", CONDI$, "NOMESS") > 0 Then
          Call COMUNI("Imposible Desvincular !!!\Ya Existen Consumos De Los Componentes\Para Esta Orden de Fabricación y  Número de Serie")
          Exit Sub
       End If
...
Private Sub MSFSERIEVINCULADOS_Click()
   ...
   Call COLOREAR_GRILLA_SOLAFILA(Me.MSFSERIEVINCULADOS, vbYellow, REG&, 0, 0, 0)
...
'      hay que revisar bien para activarla
     ...
     REG& = MSFGLOBALFRM.MouseRow
     col% = MSFGLOBALFRM.MouseCol
'     Call SELECCIONARFILA(Me.MSFSERIEVINCULADOS, REG&)
