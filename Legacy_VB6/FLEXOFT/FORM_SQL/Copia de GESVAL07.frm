...
Sub CONCILIAEXTRACTO(CODICONCI$)
    ...
    SQLX$ = SQLX$ + " AND COMPROBANTE <> 'GASTOS'"
...
Sub GASTOEXTRACTO(CODICONCI$)
    ...
    XX$ = REGLEIDO$("GASEXTRA")
    REFERE$ = TRIM$(Mid$(XX$, 13, 32))
    Contable% = CUEINT%(Left$(XX$, 12))
    '
    SQLX$ = "SELECT * FROM CONCILIA WITH(NOLOCK)"
    SQLX$ = SQLX$ + " WHERE CODICONCI = '" & CODICONCI$ & "'"
    SQLX$ = SQLX$ + " AND COMPROBANTE = ''"
    SQLX$ = SQLX$ + " AND CONCEPTO LIKE '" & REFERE$ & "'"
    SQLX$ = SQLX$ + " ORDER BY FECHA "
    '
25  On Error Resume Next
    RSTTMP.Open FILTSQL$(strsql), FLEXCONN, adOpenKeyset, adLockPessimistic, adCmdText
...
End Sub
   ...
   Call CREACAMPO("", "CONCILIA", "Concepto", 10, 32)
   Call CREACAMPO("", "CONCILIA", "Debe", 7, 0)
   Call CREACAMPO("", "CONCILIA", "Haber", 7, 0)
   Call CREACAMPO("", "CONCILIA", "Comprobante", 10, 24)
   Call CREACAMPO("", "CONCILIA", "FechaComprobante", 8, 0) 'fecha que esta registrada en el comprobante flexoft (contable)
   Call CREACAMPO("", "CONCILIA", "Status", 10, 32)
   Call CREACAMPO("", "CONCILIA", "CueContaI", 3, 0, 1)
   Call CREACAMPO("", "CONCILIA", "ID_REFERENCIA", 10, 24)
   '
End Sub
...
'        SOBREESCRIBE$ = VALIDA_CONCILIACION$(CUENTA_SELECCIONADA$, COL_FECHA%, FILA_INICIO%, COL_INICIO%, COL_CONCEPTO%, COL_IDENT%, COL_DEBE%, COL_HABER%)
        ...
        SQLX$ = "SELECT * FROM CONCILIA CODICONCI = 'A' " 'CONDICION INVALIDA
25      On Error Resume Next
