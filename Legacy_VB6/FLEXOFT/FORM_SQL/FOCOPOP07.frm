...
Sub GRAOPERA()
  ...
  Dim GRAOPERATMP As ADODB.Recordset
  Set GRAOPERATMP = New ADODB.Recordset
  GRAOPERATMP.Open FILTSQL$(SQLX$), FLEXCONN, adOpenKeyset, adLockReadOnly, adCmdText
...
5   Do While .EOF = False
      'With TmusOper
      SQLX$ = "SELECT * FROM TmusOper "
      Dim TmusOperTemp As ADODB.Recordset
      Set TmusOperTemp = New ADODB.Recordset
      TmusOperTemp.Open FILTSQL$(SQLX$), FLEXCONN, adOpenDynamic, adLockPessimistic, adCmdText
      With TmusOperTemp
        ...
        On Error GoTo 0
          ...
          COTMU$ = !Cod_TMU
          CANTI1 = !Cantidad
          MSEG1# = AMASTOK07.MSTMU#(COTMU$) 'ME DEVUELVE LOS MILISEGUNDOS DEL TMU
...
'        .Edit
       ...
       SQLX$ = SQLX$ + " WHERE CODICONJ =" & CID1% & " "
       SQLX$ = SQLX$ + " and NumeOper = " & OPD1% & ""
...
Private Sub Command5_Click()
  ...
  Descripcion$ = VALACT2$("ECOMAST")
  If CODI1$ <> "" Then
    Text3.TEXT = CODI1$
    Label8.Caption = Descripcion$
  End If
  '
End Sub
