...
Private Sub Command2_Click()
    ...
    End If
    '
    If MSF2.TextMatrix(REG2&, 4) <> "GASTOS" Then
        'VARIABLE CODICONCI DE ALCANCE PUBLICO A NIVEL DE FRM
        ESGASTO% = VALIDA_SI_ES_GASTO%(CODICONCI$, MSF2.TextMatrix(REG2&, 2))
        If ESGASTO% < 1 Then
    ...
    With MSF2
           ...
           MSF1.TextMatrix(J&, 2) = .TextMatrix(REG2&, 2)
           MSF1.TextMatrix(J&, 3) = .TextMatrix(REG2&, 3)
           ...
           MSF1.TextMatrix(J&, 10) = .TextMatrix(REG2&, 10)
...
Private Sub Command3_Click()
   ...
   With MSF1
        ...
        MSF2.TextMatrix(J&, 2) = .TextMatrix(REG1&, 2)
        ...
        MSF3.TextMatrix(J&, 4) = .TextMatrix(REG1&, 4)
        MSF3.TextMatrix(J&, 5) = .TextMatrix(REG1&, 5)
        MSF3.TextMatrix(J&, 6) = .TextMatrix(REG1&, 6)
        MSF3.TextMatrix(J&, 7) = "" 'NUNERO DE ORDEN VACIO
           CONDI$ = " CODICOM_LAR = '" & MSF1.TextMatrix(REG1&, 4) & "'"
           CODICORTO = VALOBADA("CAJABANC", "CODICOM_LAR", CONDI$, "NOMESS")
...
Private Sub Command4_Click()
  ...
  'EJEMPLO DE COMO SUBIR MULTISELECCION DE UNA GRILLA A OTRA
  grid1.Cols = 10
  grid1.Rows = 1
  GRID.Rows = GRID.Rows + 1
  For I& = 1 To MSF1.Rows - 1
    If MSF1.TextMatrix(I& - 1, 0) = "*" Then
       J& = J& + 1
       MSF11.Rows = J& + 1
       MSF11.TextMatrix(J&, 1) = MSF1.TextMatrix(I& - 1, 0)
       MSF11.TextMatrix(J&, 2) = MSF1.TextMatrix(I& - 1, 1)
       MSF11.TextMatrix(J&, 3) = MSF1.TextMatrix(I& - 1, 2)
       MSF11.TextMatrix(J&, 4) = MSF1.TextMatrix(I& - 1, 3)
        MSF11.TextMatrix(J&, 5) = MSF1.TextMatrix(I& - 1, 4)
       MSF11.TextMatrix(J&, 6) = MSF1.TextMatrix(I& - 1, 5)
       '
    End If
  Next I&
  
  Do While Not K& <= MSF1.Rows
     K& = K& + 1
     If MSF1.TextMatrix(K& - 1, 0) = "*" Then
        Call borrafila(MSF1, K& - 1)
     End If
  Loop
...
Private Sub Command5_Click()
    ...
    With MSF2
           ...
           MSF1.TextMatrix(J&, 4) = MSF3.TextMatrix(REG3&, 4) 'REFERENCIA DEL PENDIENTE DEL SISTEMA
           MSF1.TextMatrix(J&, 5) = .TextMatrix(REG2&, 5)
           MSF1.TextMatrix(J&, 6) = .TextMatrix(REG2&, 6)
           MSF1.TextMatrix(J&, 7) = .TextMatrix(REG2&, 7)
           MSF1.TextMatrix(J&, 8) = .TextMatrix(REG2&, 8)
...
Private Sub Command6_Click()
    ...
    For REG1& = 1 To FIN&
      ...
      With MSF1
        ...
        ORIMO$ = .TextMatrix(REG1&, 8)
        
        If ORIMO$ = "CABAN" Then
            SQLX$ = "SELECT * FROM CAJABANC "
            SQLX$ = SQLX$ + "WHERE FechConta = '" & FECONTA1 & "' "
            SQLX$ = SQLX$ + "AND CodiCom_LAR = '" & IDASIE$ & "' "
...
29  Next REG1&
    ...
    ENCTMP.Open FILTSQL$(SQLX$), FLEXCONN, adOpenKeyset, adLockPessimistic, adCmdText
        With ENCTMP
          ...
          On Error GoTo 0
          !NUIDASIEN = NUASI$
          !Refeasien = "CA-GASTOS" + Left$(REFEASI$, 64)
...
Sub COLOREAR_GRILLASQL_CON_MARCA(MSF As MSFlexGrid, COLOR As Variant, REG&, Optional MARCA$, Optional UNICASELECCION%, Optional REGISTRO_a_BLANQUEAR&)
   ...
   With MSF
        ...
        If UNICASELECCION% > 0 Then
          ...
          If REGISTRO_a_BLANQUEAR& > 0 Then
...
Function VALIDA_SI_ES_GASTO%(CODICONCI$, CONCEPTOMSF2$)
    ...
    For U& = 1 To Len(RFF$) Step 4
        ...
        CUENTACONT% = CUEINT%(TRIM$(Mid$(XX$, 45, 12)))
        If UCase$(TRIM$(CONCEPTOMSF2$)) Like UCase$(TRIM$(CONCEP$)) Then
      ...
      Do While Not .EOF
         ...
         With CONCIMAN07.MSF2
            ...
            .TextMatrix(REG&, 8) = DETABLA$
                TX$ = FILTERGETRECORD$("GASEXTRA", 2, AJUSTI$(CONCEP$, 32))
                CODICUENTA$ = TRIM$(Mid$(TX$, 45, 12))
