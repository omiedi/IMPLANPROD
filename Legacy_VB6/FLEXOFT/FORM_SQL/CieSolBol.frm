...
Begin VB.Form CieSolBol 
   BackColor       =   &H00808080&
   Caption         =   "Informar Cierres de Telas Impresas"
   ClientHeight    =   3240
   ClientLeft      =   60
   ClientTop       =   645
   ClientWidth     =   13710
   ...
   Begin ACTIVESKINLibCtl.Skin Skin1 
      Left            =   420
      OleObjectBlob   =   "CieSolBol.frx":0614
      Top             =   0
   End
   Begin ACTIVESKINLibCtl.SkinLabel SkinLabel999 
      ...
      Index           =   0
      Left            =   0
      OleObjectBlob   =   "CieSolBol.frx":0848
      TabIndex        =   9
      Top             =   315
      Visible         =   0   'False
      Width           =   1590
   ...
   Begin VB.Menu mnuVer 
      ...
      End
      Begin VB.Menu ReImpreEtiquetaTi 
         Caption         =   "Re-Impresión de Etiqueta de Cierres de Telas Impresas"
      End
   End
...
Private Sub Command6_Click()
   ...
   NORFA$ = TRIM$(Label4)
    ...
    'TIENEN QUE ESTAR ASOCIADAS A LA OF ACTIVA PARA MOSTRAR SUS LOTES.
    SQLX$ = "SELECT  IDENLOTE, SUM(CANTINGRE - CANTSALID)  AS RESULTADO"
    SQLX$ = SQLX$ + " FROM MOVISTO WHERE COD_EXTE = '" & TRIM$(TXTCODIGOBOBINABASE) & "' AND DEPOMOVI = " & XVALO(LBLDEPOCONS)
    SQLX$ = SQLX$ + " AND (DOSECLAR = '" & NORFA$ & "' or  DOPRICOR =  '" & NORFA$ & "')"
...
Private Sub Command1_Click()
    ...
    '
    If TRIM$(TXTCODIGOBOBINABASE) = "" Then
       Call COMUNI("Falta Ingresar Codigo Bobina Base")
       Call PINTATEXT(TXTCODIGOBOBINABASE)
       GoTo 99
    End If
...
50        TXTPENDIENTE = CANT_OF_PENDIENTEDECIERRE(CIFAB%, NORFA$) 'actualizo por las dudas por si se realizo un cierre desde otra terminal
    ...
    If InStr(EPAC$, "CONTEPLAST") > 0 Then
        If Left(TRIM$(Text10), 3) = "CTP" Then 'VALIDACION PARA BOLSAS
          If ValidacionOcultaDeBolsasCTP(NORFA$, CANTI) < o Then
             OPX% = COMALTER%("Atención !!!! Al Cerrar ésta Orden, Se Registrará en la Auditoría del Sistema\Las Alertas Presentadas\Responsabilizando al Usuario Activo\\Cancelar Cierre\Generar Cierre Sin Respetar Alertas")
             If OPX% < 2 Then GoTo 99
          End If
        End If
    ...
    FORIPRE$ = TRIM$(Control("ETIQPRTE", "ETPROTER"))
     ...
     End If
     '
     TieneSaldoEnElDeposito = SaldoLote(CODINT%(TXTCODIGOBOBINABASE), TRIM$(TXTLECTURA), XVALO(Me.LBLDEPOCONS))
     If TieneSaldoEnElDeposito < 1 Then
        Call COMUNI("La Bobina Base del Lote Seleccionado, No Posee Stock En el Depósito de Consumo Actual")
        GoTo 99
     End If
    ...
    'esto estaba dentro del if instr(codfami$,"bo") < 1 then
    'no le ponen peso a las bolsas en el maestro de articulos
    ...
    If metrosReal <= 0 Then
       ...
       GoTo 99
    End If
    'hasta aqui esto estaba dentro del if instr(codfami$,"bo") < 1 then
    If TRIM$(TXTCODIGOBOBINABASE) <> "" And UCase$(Unimed$(CIFAB%)) = "U." Then
       If XVALO(TXTCANTIDADACERRAR) > 1 Then
         Call COMUNI("Cantidad a Cerrar Debe ser 1, Cuando se Asigna Bobinas Bases ")
         GoTo 99
       Else
         TXTCANTIDADACERRAR = 1 'si tiene 1 le asigna 1 si esta vacio  le asigna 1
       End If
       
    End If
    '___________________________________________________________________________
    'SI SON TELAS IMPRESAS
    codielem% = CODINT%(TXTCODIGOBOBINABASE)

    CFAMILIA$ = TRIM$(UCase$(CODFAMIL$(codielem%)))

    'If CFAMILIA$ = "TI" Then QUE SIEMPRE ENTRE A VALIDAR
       LOTE$ = UCase$(TRIM$(Me.TXTLECTURA)) 'lote pertenece a la bobina base
       If LOTE$ = "" Then
       ...
       If TRIM$(TXTCODIGOBOBINABASE) = "" Then
        ...
        GoTo 99
       End If
       'valdio que sea componente en la estructura
       ...
       If ExisteEnReserva%(NORFA$, codielem%) < 1 Then 'valido que exista el codigo en la reserva por que en ORFCIE
       'cosume el/los reservados, no el que se elige, entonceas debe coincidir
            Call MENSERR(24, " No Existen Reservas de este código\Regenere la reserva.")
            GoTo 99
       End If
       ...
       'ESTE CALCULO VA A TENER SENTIDO SI SE DESEA CALCULAR EL PESO DE LA TINTA (DIF, ENTRE BASE E IMPRESA
...
'    End If
    ...
    If InStr(EPAC$, "QUILMES") > 0 Then
      'If InStr(CODFAMI$, "TP") > 0 Or InStr(CODFAMI$, "TT") > 0 Then
      If InStr(CODFAMI$, "BO") < 1 Then 'CAMBIO REALIZADO EL 14 02 24
       'CAMBIO SOLICITADO X JUAN CONSTANCI QUE SOLO VALIDE EN ESTOS 4 CASOS
       If InStr(CODFAMI$, "TP") > 0 Or InStr(CODFAMI$, "TT") > 0 Or InStr(CODFAMI$, "PL") > 0 Or InStr(CODFAMI$, "TL") > 0 Then
        On Error Resume Next
        coeficienteSegunMaestro = PesoDelMaestro / MetrosDelMaestro
        coeficienteSegunDatosPantalla = pesoReal / metrosReal
        resultadoDiferenciaAbs = Abs(coeficienteSegunMaestro - coeficienteSegunDatosPantalla)
    ...
    Do While Not rs.EOF       'LOS QUE PERTENECEN A LA FAMILIA .....
      ...
      CFAMILIA$ = TRIM$(UCase$(CODFAMIL$(CI1%)))
          ...
          'ACA ABAJO ES PARA LAS TELAS IMPRESAS
      If CFAMILIA$ = "TP" Or CFAMILIA$ = "TT" Or CFAMILIA$ = "PA" Or CFAMILIA$ = "PL" Or CFAMILIA$ = "TL" Then 'agregado tl caso CA TTL 47 72 BL no entraba y consumia sin lote
          ...
          'ESTE PARRAFO ES X Q A VECES LE PONEN CANTIDAD DE CONSUMO MENOR A 1 Y SON BOBINAS UNIDADES, SI ES  U. Y EL CONSUMO ES MENOR A 1 ASIGNA 1, SI NO MANTIENE EL QUE ESTA.
          CANTICONSUMO = XVALO(VALOBADA("MOVISTO", "CANTSALID", "MOVISTO_ID = " & idMov&, "NOMESS"))
          If Unimed$(CI1%) = "U." Then
            If CANTICONSUMO < 1 Then CANTICONSUMO = 1
          End If
 
          SQLX$ = "UPDATE MOVISTO SET IDENLOTE =  '" & LOTE$ & "'"
          SQLX$ = SQLX$ + " ,PESOKG ='" & FORMATNUM$(XVALO(TXTKBB), "F16.5") & "'"
          SQLX$ = SQLX$ + " ,METROS ='" & FORMATNUM$(XVALO(TXTMBB), "F16.5") & "'"
          SQLX$ = SQLX$ + " ,CANTSALID ='" & FORMATNUM$(XVALO(CANTICONSUMO), "F10.2") & "'"
    ...
    End If
    Call PRINTETIQORFA_QUILMES(NORFA$, LoteNorfa$, pesoReal, metrosReal, 1)
...
End Sub
 ...
 Sub PRINTETIQORFA_QUILMES(NORFA$, LoteNorfa$, pesoReal, metrosReal, CANCOPIAS&)
...
'        PESOENKG# = XVALO(VALOBADA("MOVISTO", "PESOKG", CONDI$, "NOMESS"))
        ...
        TEXTOQRModificado$ = REPLACAR(TEXTOQR$, "|", "_")  'le cambio los | x _ para que no afecte al archivo que guarda la info
'        FILTX$ = LOTE$ & ";" & Codigo$ & ";" & Descripcion$ & ";" & RazonSocialProveedor$ & ";" & TRIM$(FORMATNUM$(METROSREALES$, "F12.1")) & ";" & PESOENKG$ & ";" & NORFA$ & ";" & USERNAME$ & ";" & FECHATEX$(FECHAGENERACION%)
        FILTX$ = LoteNorfa$ & ";" & Codigo$ & ";" & Descripcion$ & ";" & RazonSocialProveedor$ & ";" & METROSREALES$ & ";" & PESOENKG$ & ";" & TEXTOQRModificado$
...
Private Sub Form_Load()
   ...
   DEPOCORTE% = XVALO(Control("CONTEPLA", "DEBOAIMP")) 'DEPOSITO CONFIGURADO PARA BOBINAS BASE
...
'   MSF1.Rows = 1
   ...
   If InStr(EPAC$, "QUILMES") > 0 Then
     ...
     ElseIf FAM$ = "TI" Then
       Me.LBLDEPOCONS = XVALO(Control("CONTEPLA", "DEBOAIMP"))
       'Me.LBLDEPOENT = XVALO(Control("CONTEPLA", "DEPOCINT")) 'el resto cierra en deposito 1 (FRM configdepo)
       Me.LBLDEPOENT = XVALO(Control("CONTEPLA", "DEBOAIMP")) ' SI ES TELA IMPRESA CIERRA EN 3 DEPOSITO EN PROCESO
     Else
...
Private Sub ReImpreEtiquetaTi_Click()
10  Dim obj As New RECORXET
    Campos$ = "CI/A0;Còdigo/A16;Descripción./A48"
    Call CARGA_ENCABEZADO(FRMZELECHO.MSF2, Campos$, FRMZELECHO)
    Call CARGA_ENCABEZADO(FRMZELECHO.MSF1, Campos$, FRMZELECHO)


    Set rs1 = obj.RS_ArtClasificado("TI") 'TELAS IMPRESAS
    Call Carga_MSF_Recordset(rs1, Campos$, FRMZELECHO.MSF2, 1)
    Set obj = Nothing
    rs1.Close
    ...
    FRMZELECHO.Show 1
    Set rs1 = Nothing
    CIXI% = XVALO(RESP_ZELE_VECT(1))
    If CIXI% < 1 Then Exit Sub
    
    TituloLista$ = "Seleccionar Cierre de O.F."
    
    Campos$ = "Orden Fab./A12;Fecha/d8;Cantidad/F10.0;Lote/A20;Identificador/F12;Fecha y Hora/A16,Kgs./F10.1;Metros/F10.1"
    Call CARGA_ENCABEZADO(FRMZELECHO.MSF2, Campos$, FRMZELECHO)
    Call CARGA_ENCABEZADO(FRMZELECHO.MSF1, Campos$, FRMZELECHO)
    FRMZELECHO.Caption = TituloLista$
   
    Call APERBASDAT("STOCKS")
    SQLX$ = " SELECT DoPriCor, FECHMOV, CANTINGRE, FeReMovi,IDENLOTE,MOVISTO_ID,PESOKG,METROS "
    SQLX$ = SQLX$ + " FROM MOVISTO WITH(NOLOCK)"
    ...
    With MOVISEL
      ...
      Do While .EOF = False
         ...
         FRMZELECHO.MSF2.TextMatrix(J&, 7) = FORMATNUM$(PESOKG#, "F9.1")
         FRMZELECHO.MSF2.TextMatrix(J&, 8) = FORMATNUM$(METROS#, "F9.1")
   ...
   Screen.MousePointer = 1
   '
   If J& = 0 Then Call MENSERR(24, "No se Detectaron Cierres\para el Código Elegido."): GoTo 10
   ...
   PESOKG# = XVALO(RESP_ZELE_VECT(7))
   METROS# = XVALO(RESP_ZELE_VECT(8))
   '
   Call PRINTETIQORFA_QUILMES(NOFASE$, LOTE$, PESOKG#, METROS#, 1)
...
Private Sub TXTLECTURA_Change()
    CIBobinaBase% = CodigoIntSegunLote(TRIM$(TXTLECTURA))
