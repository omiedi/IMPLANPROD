...
Private Sub Command1_Click()
   ...
   '
   If PRONUORPA& <> NORPAG& Then
       Call COMUNI("Se va a Actualizar el Número de Orden de Pago, Acepte y Vuelva a Grabar")
       TXTNROOPAGO = PRONUORPA&
       GoTo 99
   End If
   '
   SELUSUARIO.lblNUSER = USNBR% Mod 100
   SELUSUARIO.Show 1
   USURESP% = XVALO(SELUSUARIO.lblNUSER)
   If USURESP% < 1 Then
     Call COMUNI("No se Ha Seleccionado Un Usuario Responsable, Se Registrará Usuario Generador.")
     USURESP% = USNBR% Mod 100
   End If
   '
...
'       GoTo 99
'    End If
    IMPO_OPAGO# = XVALO(Text15)
    REFERENCIA_LISTA$ = Left$(Text5, 64)
    Screen.MousePointer = 11
    
    CUENTACONTABLE% = CUEINT(Control$("AMACLI", "ECUECON"))
    'RECORRE LA GRILLA Y A LOS SELECCIONADOS  MARCA CON STATUS 2 Y EL N DE LISTA A LOS MOVIMIENTOS GENERADOS EN EL RECIBO DE COBRANZA
    For I& = 1 To MSF1.Rows - 1
    ...
    For I& = 1 To MSF1.Rows - 1
      If TRIM$(MSF1.TextMatrix(I&, 0)) = "*" Then
        ...
        CODIMPU% = MSF1.TextMatrix(I&, 5)
        IDENTIF$ = MSF1.TextMatrix(I&, 6)
        VALOR_REAL# = XVALO(MSF1.TextMatrix(I&, 7))
        TICKET$ = TRIM$(MSF1.TextMatrix(I&, 9))
        STAT% = 2: If TICKET$ <> "" Then STAT% = 4
        COLAR_IND$ = COLAR$ & "-" & TRIM$(Str$(j&))
65      ABUS& = RANDSTRING$(-8)
        If CantRegistros("CAJABANC", "NUMECOMP = " & ABUS&, "NOMESS") > 0 Then GoTo 65
        Call CreaRegistro("CAJABANC", "NUMECOMP", ABUS&, "NOMESS")
        '
        CONDI$ = "NUMECOMP =   " & ABUS&
        Call ACTUREGISTRO("CAJABANC", "IHABECOMP ", CONDI$, FORMATNUM$(VALOR_REAL#, "F16.5"), REGAF&, "NOMESS")
        Call ACTUREGISTRO("CAJABANC", "IDEBECOMP", CONDI$, 0, REGAF&, "NOMESS")
        Call ACTUREGISTRO("CAJABANC", "CUECONTAI", CONDI$, CUENTACONTABLE%, REGAF&, "NOMESS")
        Call ACTUREGISTRO("CAJABANC", "TIPOMOVIM", CONDI$, "CHENTR", REGAF&, "NOMESS")
        '
        Call ACTUREGISTRO("CAJABANC", "CODICOM_COR", CONDI$, COCOR$, REGAF&, "NOMESS")
        Call ACTUREGISTRO("CAJABANC", "CODICOM_LAR", CONDI$, COLAR$, REGAF&, "NOMESS")
        Call ACTUREGISTRO("CAJABANC", "NUMEIMPU", CONDI$, CODIMPU%, REGAF&, "NOMESS")
        Call ACTUREGISTRO("CAJABANC", "Status", CONDI$, STAT%, REGAF&, "NOMESS")
        Call ACTUREGISTRO("CAJABANC", "ID", CONDI$, IDENTIF$, REGAF&, "NOMESS")
        Call ACTUREGISTRO("CAJABANC", "NroCai", CONDI$, COLAR_IND$, REGAF&, "NOMESS")
        Call ACTUREGISTRO("CAJABANC", "NUMLISTA", CONDI$, NUMLI&, REGAF&, "NOMESS")
        Call ACTUREGISTRO("CAJABANC", "NUMEUSER", CONDI$, NUSER1%, REGAF&, "NOMESS")
        Call ACTUREGISTRO("CAJABANC", "FECHEMISI", CONDI$, CVDAT(HOY(HO$)), REGAF&, "NOMESS")
        Call ACTUREGISTRO("CAJABANC", "REFERENCIA_LISTA", CONDI$, REFERENCIA_LISTA$, REGAF&, "NOMESS")
        Call ACTUREGISTRO("CAJABANC", "NUCLIEN", CONDI$, NC%, REGAF&, "NOMESS")
        Call ACTUREGISTRO("CAJABANC", "DESCRIMOV", CONDI$, CONCEPTO$, REGAF&, "NOMESS")
        Call ACTUREGISTRO("CAJABANC", "NUMECOMP", CONDI$, NORPAG&, REGAF&, "NOMESS")
        Call ACTUREGISTRO("CAJABANC", "PEDIDOS", CONDI$, TICKET$, REGAF&, "NOMESS") 'GRABA LA INFORMACION DEL TICKETES
        Call ACTUREGISTRO("CAJABANC", "VendeComis", CONDI$, USURESP%, REGAF&, "NOMESS") ' GRABA EL USUARIO RESPONSABLE DE PAGO
      End If
    Next I&       '

75 ABUS& = RANDSTRING$(-8)
   If CantRegistros("CAJABANC", "NUMECOMP = " & ABUS&, "NOMESS") > 0 Then GoTo 75
   Call CreaRegistro("CAJABANC", "NUMECOMP", ABUS&, "NOMESS")
   '
   CONDI$ = "NUMECOMP =   " & ABUS&
   CUENTACONTABLE% = CUEINT(Control$("", "CUENCAJA"))
   ...
   Call ACTUREGISTRO("CAJABANC", "REFERENCIA_LISTA", CONDI$, REFERENCIA_LISTA$, REGAF&, "NOMESS")
   Call ACTUREGISTRO("CAJABANC", "NUMECOMP", CONDI$, NORPAG&, REGAF&, "NOMESS")
   Call ACTUREGISTRO("CAJABANC", "VendeComis", CONDI$, USURESP%, REGAF&, "NOMESS")
...
Private Sub Command4_Click()
   ...
   With RS
    Do While Not .EOF
          ...
          IMPOX# = XVALO(!IHABECOMP)
          CONCEPTO$ = SAFETEXT$(!DESCRIMOV)
          CODIMPU% = SAFETEXT$(!NUMEIMPU)
          IDENTIF$ = SAFETEXT$(!ID)
          IDENTIF_PAGO$ = SAFETEXT$(!NROCAI)
          TICKET$ = TRIM$(SAFETEXT$(!PEDIDOS))
          '
          ...
          MSF1.TextMatrix(I&, 7) = FORMATNUM$(IMPOX#, "F12.2")
          MSF1.TextMatrix(I&, 8) = IDENTIF_PAGO$
          MSF1.TextMatrix(I&, 9) = TICKET$
          ...
          If VUELTA% = 0 Then
            VUELTA% = 1
              ...
              Text6(5) = IMPUESTO$
              USUARIO_RESP% = XVALO(!VendeComis)
              TXTRESP = USERNAM$(USUARIO_RESP%)
          End If
          '
          .MoveNext
     Loop
...
Private Sub Command8_Click()
   ...
   lblNUSER = USUARIO_RESP% ' VARIABLE PUBLICA ' Se lee con el boton listar
   SELUSUARIO.Show 1
   USURESP% = XVALO(SELUSUARIO.lblNUSER)
   If USURESP% < 1 Then
     Call COMUNI("No se Ha Seleccionado Un Usuario Responsable, Se Registrará Usuario Actual.")
     USURESP% = USNBR% Mod 100
   End If
...
'    End If
    ...
    REFERENCIA_LISTA$ = Left$(Text5, 64)
    '
    'ACA RECORRE EL MSF Y ACTUALIZA EL ESTADO Y EL NUMERO DE LISTA DE LOS
    'INGRESOS DE COBRANZA DE IMPUESTO EN FRM RECIM14
    
    For I& = 1 To MSF1.Rows - 1
        NC% = XVALO(MSF1.TextMatrix(I&, 1))
        CONCEPTO$ = MSF1.TextMatrix(I&, 3)
        IMPOX# = XVALO(MSF1.TextMatrix(I&, 7))
        CODIMPU% = MSF1.TextMatrix(I&, 5)
        IDENTIF$ = MSF1.TextMatrix(I&, 6)
        TICKET$ = TRIM$(MSF1.TextMatrix(I&, 9))
        STAT% = 2: If TICKET$ <> "" Then STAT% = 4
        ...
        CONDI$ = CONDI$ + " And NUCLIEN = " & NC%
      If MSF1.TextMatrix(I&, 0) = "*" Then
        CONDI1$ = CONDI$ + "  And TIPOMOVIM = 'CHRCIM'"
        'SI ESTA SELECCIONADO LO REGISTRA NUEVAMENTE CON STATUS = 2 O 4 SEGUN CORRESPONDA Y LE GRABA EL NUMERO DE LISTA
        Call ACTUREGISTRO("CAJABANC", "STATUS", CONDI1$, STAT%, REGAF&, "NOMESS")
        Call ACTUREGISTRO("CAJABANC", "NUMLISTA", CONDI1$, NUMLI&, REGAF&, "NOMESS")
        '
        CONDI2$ = CONDI$ + "  And TIPOMOVIM = 'CHENTR'"
        If TICKET$ <> "" Then STAT% = 4
        Call ACTUREGISTRO("CAJABANC", "IHABECOMP", CONDI2$, FORMATNUM$(VALOR_REAL#, "F16.5"), REGAF&, "NOMESS")
        Call ACTUREGISTRO("CAJABANC", "REFERENCIA_LISTA", CONDI2$, REFERENCIA_LISTA$, REGAF&, "NOMESS")
        Call ACTUREGISTRO("CAJABANC", "PEDIDOS", CONDI2$, TICKET$, REGAF&, "NOMESS")
        Call ACTUREGISTRO("CAJABANC", "VendeComis", CONDI2$, USURESP%, REGAF&, "NOMESS")
        Call ACTUREGISTRO("CAJABANC", "STATUS", CONDI2$, STAT%, REGAF&, "NOMESS")
        
      Else
        'CUANDO NO ESTA SELECCIONADO LE CAMBIA EL STATUS = 0 Y NUMERO DE LISTA A 0
         COND2$ = CONDI$ + " And TIPOMOVIM = 'CHENTR'"
        Call BORRAREGISTROS("CAJABANC", COND2$, REGAF&, "NOMESS")
      End If
    Next I&
    '
    j& = 0
...
'    Next I&       '
    ...
    Call ACTUREGISTRO("CAJABANC", "", CONDI$, FORMATNUM$(IMPO_OPAGO#, "F16.5"), REGAF&, "NOMESS")
...
Private Sub Form_Load()
    ...
    Call CARGA_ENCABEZADO(LISTAP14.MSF1, "Cta/F6;Nombre/A24;Concepto/A32;Importe/F12.2;Cod_Impuesto/A0;IDENTIFICADOR/A0;Imp.Real/F12.2;IDENT_DETALLE_PAGO/A0;Ticket/A24", LISTAP14)
    '
    TXTNROOPAGO = ""
    TXTNROOPAGO = PRONUORPA&
    MSF1.Height = 4275
...
End Sub
Sub ACTUTOTALES()
   Text18 = SUMA_COLUMNA_SELECCION(LISTAP14.MSF1, 7) 'TOTAL IMPORTES TODOS
   Text15 = SUMA_COLUMNA_SELECCION(LISTAP14.MSF1, 7, "*") 'TOTAL SELECCIONADO
   'Call ACTUTOTALLISTA
...
Private Sub MSF1_MouseDown(Button As Integer, Shift As Integer, x As Single, y As Single)
          '
'1, "Cta/F6;Nombre/A32;Concepto/A32;Importe/F12.2;Cod_Impuesto/A0;IDENTIFICADOR/A0;ValorRealPagado/F12.2", LISTAP14)
 ...
 If Button = 2 Then
    ...
    CTA = XVALO(MSF1.TextMatrix(I&, 1))
    If CTA = 0 Then
        Exit Sub
    Else
        EDITLPAG.TXTCUENTA = CTA
        EDITLPAG.TXTDESCRIPCION = MSF1.TextMatrix(I&, 3) 'DESC
        EDITLPAG.TXTIMPORTE = MSF1.TextMatrix(I&, 4)  'IMPORTE
        EDITLPAG.TXTCODIGOIMP = MSF1.TextMatrix(I&, 5) 'CODIGO IMPUESTO
        EDITLPAG.TXTID = MSF1.TextMatrix(I&, 6) 'IDENTIFICADOR
        EDITLPAG.TXTVALORREAL = MSF1.TextMatrix(I&, 7) 'VALOR REAL
        EDITLPAG.TXTTICKET = MSF1.TextMatrix(I&, 9) 'NUMERO DE TICKET
        EDITLPAG.Show 1
    End If
    If EDITLPAG.MODO1 = "ACTUALIZAR" Then
        MSF1.TextMatrix(I&, 7) = EDITLPAG.TXTVALORREAL
        MSF1.TextMatrix(I&, 9) = EDITLPAG.TXTTICKET
    End If
    Call ACTUTOTALES
  End If
End Sub
