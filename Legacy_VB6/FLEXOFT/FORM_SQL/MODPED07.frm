...
Begin VB.Form MODPED07 
   ...
   Begin ACTIVESKINLibCtl.Skin Skin1 
      Left            =   0
      OleObjectBlob   =   "MODPED07.frx":31A6
      Top             =   1800
   End
   Begin ACTIVESKINLibCtl.SkinLabel SkinLabel999 
      ...
      Index           =   0
      Left            =   300
      OleObjectBlob   =   "MODPED07.frx":33DA
      TabIndex        =   116
      Top             =   0
      Visible         =   0   'False
      Width           =   1485
...
Private Sub Check3_Click()
    ...
    CheckAhpRetiraDep.Visible = Conmutar(estadoVisible)
End Sub
...
'   End If
   ...
   If InStr(EPAC$, "AHP") > 0 Then
     If Check3.Value > 0 And RetiraDomicilioAHP% < 1 Then
        If InStr(EPAC$, "AHP") > 0 Then
            ...
            CONDI$ = "Nume_Cli =" & XVALO(Text1) 'SI NO TIENE SUCURSALES NO ENTRA
   ...
   If InStr(EPAC$, "AHP") > 0 Then
    ...
    If Entrega.Label1.Caption = "OK" And OKX% > 0 Then
         Call Entrega.grabarDatosEntregaAhp(NUPED&, XVALO(Text1))
         Unload Entrega
    End If
    '
   ...
   If OKX% > 0 Then
     ...
     If InStr(EPAC$, "ARCON") > 0 Then
       CONDI$ = "NPED_GENERADO_EN_AVELLANEDA = " & NUPED&
       If CantRegistros("OC_SUCURSAL", CONDI$, "NOMESS") > 0 Then 'VEO SI ES PEDIDO DE NEUQUEN
         NUOC& = XVALO(VALOBADA("OC_SUCURSAL", "O_COMPRA_SUC", CONDI$, "NOMESS")) 'ES EL PEDIDO DE REPOSICION DE NEUQUEN
'         Call OCOMSUCU.GENERAR_ARCHIVO_GENERO_PEDIDO(NC, NUPED&, XVALO(Text10), "MODIPEDI")ESTA FILA ESTABA ASI
          'OBTENGO EL NUOC& QUE SE GRABO EN LA TABLA CUANDO SE GENERO EL PÉDIDO ORIGINAL, POR SI LO CAMBIA EN PANTALLA
          'APARENTEMENTE LO MODIFICABAN Y SE DESBORDABA, EN LA MODIFICACION DEL PEDIDO VA A QUEDAR EL DE PANTALLA
         Call OCOMSUCU.GENERAR_ARCHIVO_GENERO_PEDIDO(NC, NUPED&, NUOC&, "MODIPEDI")
       End If
     End If
...
Private Sub Command30_Click()
   ...
   OBSERVAC.Show 1
   '
   '*** ATENCION SI SON LOS ITEMS ORIGINALES SE VA A GRABAR AL APROBAR LA VENTANA DE OBSERRVACIONES
   '*** SI ES UN ITEM NUEVO LO VA A GRABAR EN EL MOMENTO DE LA GRABACION DE LA MODIFICACION DEL PEDIDO
   '*** ESTO ES POR QUE LA INFO LA LEE DEL PEDDETA CUANDO LA TRAE NO LA PASA AL CARDETPEDI
   
   If OBSERVAC.Label1 = "OK" Then
        CI1% = CODINT%(Codigo$)
        NROPED& = XVALO(Me.Text11)
        CONDI$ = "CODIINT = " & CI1%
        CONDI$ = CONDI$ + " AND NROPED = " & NROPED&
        CONDI$ = CONDI$ + " AND NUORITEM = " & FilaActiva
        'SI ES UN DATO EXISTENTE LO GRABA CON LA CONDI DEL NRO DE ORDEN
        Call ACTUREGISTRO("PEDDETA", "OBSERVAITEM", CONDI$, Left$(OBSERVAC.Text28, 256), REGAF&, "NOMESS")
        
     Text22 = OBSERVAC.Text28
     MSF.TextMatrix(FilaActiva, 9) = Text22
   End If
      ...
      If rasocli$(NCLIEN) <> "" Then
       ...
       '
        ...
        Else
            ...
            Call FILTERFILE("SUCENTRE", "", 5, MKS$(NCLIEN))
        End If
      End If
    End If
    Screen.MousePointer = 1
...
'                 NPROV& = XVALO(VALOBADA("PRESUVEN", "Num_Prov", CONDI$, "NOMESS"))
            ...
            End If
            '
            'leo las 4 descripciones posibles
...
109 Next U&
    ...
    CODPARAM$(24) = "LOCA-TRA": If RetiraDomicilioAHP% < 1 Then DOCPARAM$(24) = LENTREGA(1).TEXT Else DOCPARAM$(24) = ""
    ...
    CODPARAM$(30) = "F-PLANO": DOCPARAM$(30) = FECHATEX$(HOY(HO$))
...
Sub GRAPEDIDO(OKX%, Optional FUERZAPROBACION$)
   ...
   End If
   '
   If InStr(1, EPAC$, "CONTEPLAST") > 0 Then 'USO ESTE CAMPO PARA GUARDAR EL VENDEDOR QUE SE VA A PASAR A ENVAPLAST
...
25 On Error Resume Next
   ...
   With PedEnca
     ...
     !Revision = XVALO(Label34)
     If InStr(1, EPAC$, "CONTEPLAST") > 0 Then !AtributoComercial = idAtributo%
...
Sub CARDETPEDI(MODO%)
   ...
   End If
   '
   If InStr(1, EMPREAC$, "TER S.R.L.") > 0 Then
        ATRI$ = ATRI$ + "/POSEDIT(1)=2"
        ATRI$ = ATRI$ + "/POSEDIT(3)=2"
        ATRI$ = ATRI$ + "/POSEDIT(5)=2"
        ATRI$ = ATRI$ + "/POSEDIT(6)=2"
   ...
   If InStr(1, EMPREAC$, "CONTEPLAST") > 0 Then
        STATUS_PED% = XVALO(VALOBADA("PEDENCA", "STATUS", "NROPED= " & NUPEDIDO1&, "NOMESS"))
        EDITAPRECIO% = 1 'POR DEFECTO PUEDE MODIFICAR PRECIO
        If STATUS_PED% >= 0 Then EDITAPRECIO% = 2 'SI ESTA APROBADO NO PUEDE MODIFICAR
        ATRI$ = ATRI$ + "/POSEDIT(1)=2"
        ATRI$ = ATRI$ + "/POSEDIT(3)=" & EDITAPRECIO%
'        ATRI$ = ATRI$ + "/POSEDIT(4)=2"
        ATRI$ = ATRI$ + "/POSEDIT(5)=1"
        ATRI$ = ATRI$ + "/POSEDIT(6)=2"
        ATRI$ = ATRI$ + "/POINDEL=NO" ' DESHABILITA EL INSERTAR Y EL DEL
   End If
     ...
     If PRECARGA < VAMINI1 Or PRECARGA > 9999999999.99 Then
       If MODO% < 2 Then
         If ECOPRECIO% >= 0 Then
           ...
           MODO% = 2
           If InStr(1, EMPREAC$, "CONTEPLAST") > 0 Then
             'NO PERMITE AGREGAR SI MODIFICAR CANTIDADES, PERO COMO EN LA VENTANA PUEDE AGREGAR CODIGO Y NO PRECIO
             'SI NO TIENE PRECIO LO VUELVE HACIA ATRAS
             Call COMUNI("Artículo '" + TRIM$(CODEXT$(Ci%)) + "' sin Precio.\(Si Agregó Códigos Salga y Vuelva a Ingresar) \Solo Puede Cambiar Cantidad de los Items Originales")
             MODO% = 1 'le vuelvo el modo a 1 para que siempre ingrese aqui
             GoTo 10
           Else
...
Sub CARLISPE()
   List1.Clear
   UTILIZAOBSERVACIONITEM$ = Control("ADMIPED", "OBSXITEM")
   FIN& = ULTREG&("!CARDETPE")
   ...
   For U& = 1 To FIN&
     ...
     If UTILIZAOBSERVACIONITEM$ = "SI" Then
        CI1% = CODINT%(MSF.TextMatrix(U&, 2))
        NROPED& = XVALO(Me.Text11)
        CONDI$ = "CODIINT = " & CI1%
        CONDI$ = CONDI$ + " AND NROPED = " & NROPED&
        ObservaItemx$ = VALOBADA("PEDDETA", "OBSERVAITEM", CONDI$, "NOMESS")
        MSF.TextMatrix(U&, 9) = ObservaItemx$
     End If
