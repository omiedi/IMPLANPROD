...
Begin VB.Form ANACOSI09 
   ...
   Begin VB.Frame Frame3 
      ...
      Begin VB.Label Label16 
         ...
         Left            =   4200
         TabIndex        =   91
         ToolTipText     =   "Doble Click para Asignacion de Lista de Precios al Artículo Activo"
         Top             =   705
         Width           =   450
      End
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VB.Label Label30 
         ...
         Left            =   3675
         TabIndex        =   120
         ToolTipText     =   "Precio Minorista en Pesos"
         Top             =   960
         Width           =   1395
      End
      Begin VB.Line Line3 
      ...
      Begin VB.Label Label26 
         ...
         Left            =   4290
         TabIndex        =   118
         ToolTipText     =   "Doble Clic para modificar porcentaje solo para el item activo."
         Top             =   1620
         Width           =   780
      End
      Begin VB.Label Label22 
         ...
         Left            =   3675
         TabIndex        =   117
         ToolTipText     =   "Precio Minorista en Dolares"
         Top             =   1290
         Width           =   1395
      End
      Begin VB.Label Label10 
   ...
   Begin MSFlexGridLib.MSFlexGrid MSF 
      ...
      EndProperty
   End
   Begin VB.Label LBLCantidadItems 
      Alignment       =   1  'Right Justify
      BackColor       =   &H00E0E0E0&
      BorderStyle     =   1  'Fixed Single
      Height          =   270
   ...
   Begin VB.Menu Archiv 
      ...
      Begin VB.Menu plh2 
         Caption         =   "-"
      End
      Begin VB.Menu mnuComprasPedidos 
         Caption         =   "Compras para Pedidos"
      End
      Begin VB.Menu php7a 
         Caption         =   "-"
      End
...
Sub ActualizaFechaEnListas(MON%)
      ...
      Do While Not .EOF
          If CI% > 0 Then
            ...
            If REG& > 0 Then
              ...
              Call ACLISPRE07.Actualiza_DAT_X_Codigo(NUMLIS%, CI%)
              OK% = 1
            End If
          End If
          .MoveNext
...
Sub ACTUALIZALISTA(MON%, Optional OK%)
    ...
    If MON% = 1 Then            'MONEDA PESOS
      PREPES# = XVALO(Label1)
      FEXI% = FECHANUM(Text6)
      NUMEROLISTA_LABEL% = XVALO(Label15) 'LISTA EN PANTALLA
    ElseIf MON% = 2 Then        'MONEDA DOLARES
      PREPES# = XVALO(Label6)
      FEXI% = FECHANUM(Text7)
      NUMEROLISTA_LABEL% = XVALO(Label16) 'LISTA EN PANTALLA
    End If
    '
    'INICIALIZO BANDERA PARA MARCAR SI YA EL CODIGO ESTA EXISTENTE EN ALGUNA LISTA
    SINLIST% = 0
    ...
    With LISTtmp
      Do While Not .EOF
          If CI% > 0 Then
            ...
            If REG& > 0 Then
              SINLIST% = 1 ' EXISTE EN LISTA
              'Call ACTUALIZADATLISTA(NUMLIS%) 'ANTES ACTUALIZABA EL CODIGO EN TODAS LAS LISTAS DE LA MISMA MONEDA ESE CODIGO
              Call ACLISPRE07.Actualiza_DAT_X_Codigo(NUMLIS%, CI%) 'EN ESTA ACTUALIZA SOLO EN LA LISTA DONDE SE MODIFICA PARA ESE CODIGO
              OK% = 1
            End If
          End If
          .MoveNext
...
Sub CalcuPesosyDolar()
     '17-03-22
    lblMPpesos = ""
    lblMPdolar = ""
    ACUMULACOSTOPAR = 0: ACUCOSTOPARPESOS = 0
    COTIDOLAR = FORMATNUM$(XVALO(Label11), "F10.4")
    If XVALO(COTIDOLAR) <= 0 Then Exit Sub
    
    For i& = 0 To List1.ListCount - 1
       TX$ = List1.List(i&)
       If TRIM$(Mid(TX$, 65, 2)) = "$" Then
          COSTOPARPESOS = ROUND(XVALO(Mid(TX$, 76, 12)), 4)
          ACUCOSTOPARPESOS = ACUCOSTOPARPESOS + COSTOPARPESOS
       Else
         COSTOPARDOLAR = ROUND(XVALO(Mid(TX$, 76, 12)), 4)
         ACUMULACOSTOPAR = ACUMULACOSTOPAR + COSTOPARDOLAR
       End If
    Next i&
    lblMPpesos = TRIM$(FORMATNUM$(ACUCOSTOPARPESOS, "F12.4"))
    lblMPdolar = TRIM$(FORMATNUM$(ACUMULACOSTOPAR / XVALO(Label11), "F11.4"))
    
End Sub

Sub cargarEnGrillaGral(TXT$)

    PERIOSEL$ = (Mid$(TXT$, 1, 16))
    'Call REPLA(TXT$, (Des%), 17, 2)
    FECHA% = CVI(Mid$(TXT$, 17, 2))
    MINMANOBRA# = CVS(Mid$(TXT$, 19, 4))
    PESMANOBRA# = CVD(Mid$(TXT$, 23, 8))
    TOTMATPRIM# = CVD(Mid$(TXT$, 31, 8))
    GASTOSGRAL# = CVD(Mid$(TXT$, 39, 8))
    SCRAPMOBRA# = CVD(Mid$(TXT$, 47, 8))
    UTILIDAD# = CVD(Mid$(TXT$, 55, 8))
    IMPUESTOS# = CVD(Mid$(TXT$, 63, 8))
    COMIVENDED# = CVD(Mid$(TXT$, 71, 8))
    
    COSTOTOTAL# = CVD(Mid$(TXT$, 111, 8))
    IMPNET# = CVD(Mid$(TXT$, 119, 8))
    fech$ = HOY(HOS$)
    MINMANOBRA_REAL# = CVS(Mid$(TXT$, 129, 4))
    
    i& = GRILLAGRAL.GRID.Rows
    GRILLAGRAL.GRID.Rows = i& + 1
    GRILLAGRAL.GRID.TextMatrix(i&, 1) = PERIOSEL$
    GRILLAGRAL.GRID.TextMatrix(i&, 2) = FORMATNUM$(MINMANOBRA#, "F12.2")
    GRILLAGRAL.GRID.TextMatrix(i&, 3) = FORMATNUM$(PESMANOBRA#, "F12.2")
    GRILLAGRAL.GRID.TextMatrix(i&, 4) = FORMATNUM$(TOTMATPRIM#, "F12.2")
    GRILLAGRAL.GRID.TextMatrix(i&, 5) = FORMATNUM$(GASTOSGRAL#, "F12.2")
    GRILLAGRAL.GRID.TextMatrix(i&, 6) = FORMATNUM$(SCRAPMOBRA#, "F12.2")
    GRILLAGRAL.GRID.TextMatrix(i&, 7) = FORMATNUM$(UTILIDAD#, "F12.2")
    GRILLAGRAL.GRID.TextMatrix(i&, 8) = FORMATNUM$(IMPUESTOS#, "F12.2")
    GRILLAGRAL.GRID.TextMatrix(i&, 9) = FORMATNUM$(COMIVENDED#, "F12.2")
    GRILLAGRAL.GRID.TextMatrix(i&, 10) = FORMATNUM$(COSTOTOTAL#, "F12.2")
    GRILLAGRAL.GRID.TextMatrix(i&, 11) = FORMATNUM$(IMPNET#, "F12.2")
    GRILLAGRAL.GRID.TextMatrix(i&, 12) = FORMATNUM$(MINMANOBRA_REAL#, "F12.2")

End Sub

Sub EDITARGRUPOCONTABLE()
...
Sub GRAUSUARIOYFECHA()
   ...
   Call ACTUREGISTRO("Master", "FechApFor", CONDI$, CVDAT(HOY(HO$)), REG&)
   '
End Sub

Sub LIPOCLI(PERIO$)
'    Call DESHASFECHA(DES%, HAS%, PERIO$)
    Call COPYSTRU("SEGUIDET", "SEGUIDEF")
'    FIN& = ULTREG&("SEGUIDET")
'    j& = 0
'    For JJ& = 1 To FIN&
'        TXT$ = REGLEIDO$("SEGUIDET", JJ&)
'        x% = CVI(Mid$(TXT$, 19, 2))
'        If x% >= DES% And x% <= HAS% Then
'            j& = j& + 1
'            Call GRAREG("SEGUIDEF", TXT$, j&)
'        End If
'    Next JJ&
'    Call SETULTREG("SEGUIDEF", j&)
    Call CIERRARCH("SEGUIDEF")
    Call HEADERS("SEGUIDEF", "")
    Call HEADERS("SEGUIDEF", PERIO$)
    Call CONECRUN("*SEGUIDET", "Seguimiento de Items Recaudados")
...
'    Next U&
   '
   'CAMBIO SOLICITADO PARA QUE SE ASIGNE UNA LISTA DE PRECIOS AL ARTICULO
   'SI TIENE ASIGNADA UNA LISTA AL ARTICULO TRAE EL PRECIO DE LA LISTA CORRESPONDIENTE
   'OJO LA ACTUALIZACION SI SE CAMBIA EL PRECIO LO HACE PARA TODAS LAS LISTAS
   'ESO NO CAMBIA HABLADO CON LEANDRO.
   'SI QUIEREN CAMBIAR INDIVIDUAL DEBEN HACERLO DE LA LISTA DE PRECIOS.
   CI1% = XVALO(Label28)
   If CI1% < 1 Then Exit Sub
   CONDI$ = "CODINT = " & CI1%
   LISTA_ASIGNADA_PESOS% = XVALO(VALOBADA("MASTER", "Rel_Lista_Ancos", CONDI$, "NOMESS"))
   LISTA_ASIGNADA_DOLAR% = XVALO(VALOBADA("MASTER", "Rel_Lista_AN_DOL", CONDI$, "NOMESS"))
   
   SQLX$ = "SELECT * FROM LISTAPRE with(NOLOCK) "
   SQLX$ = SQLX$ + " WHERE moneda = 1 "
   If LISTA_ASIGNADA_PESOS% > 0 Then SQLX$ = SQLX$ + " AND Cod_Lista = " & LISTA_ASIGNADA_PESOS%
   SQLX$ = SQLX$ + " ORDER BY COD_LISTA"
...
'   Set LISTtmp = FLEXCONN.Execute(FILTSQL$(SQLX$))
   ...
   With LISTtmp
     ...
     Do While Not .EOF
          ...
          PREPES# = PRELISTA#(APREPES1$, CI%)
          If PREPES# >= 0.001 Then
             Label1 = FORMATNUM$(PREPES#, "F9." & TRIM$(Str(DECILISTA%(NUM%))))
             
             Label7 = FORMATNUM$(PREPES# * (1 + ALIVA / 100), "F9." & TRIM$(Str(DECILISTA%(NUM%))))
...
20 SQLX$ = "SELECT * FROM LISTAPRE with(NOLOCK) "
   ...
   If LISTA_ASIGNADA_DOLAR% > 0 Then SQLX$ = SQLX$ + " AND Cod_Lista = " & LISTA_ASIGNADA_DOLAR%

   SQLX$ = SQLX$ + " ORDER BY COD_LISTA"
'   Dim LISTtmp1 As New ADODB.Recordset
'   Set LISTtmp1 = FLEXCONN.Execute(FILTSQL$(SQLX$))
   Set LISTtmp1 = READSET(SQLX$)
   With LISTtmp1
     ...
     Do While Not .EOF
         ...
         If PREDOL# >= 0.005 Then
             ...
             Label6 = FORMATNUM$(PREDOL#, "F10." & TRIM$(Str(DECILISTA%(XVALO(Label16)))))
             Label8 = FORMATNUM$(PREDOL# * (1 + ALIVA / 100), "F10." & TRIM$(Str(DECILISTA%(XVALO(Label16)))))
             CONDI$ = "COD_LISTA = " & NUM% & " And COD_PIEZA = " & CI%
...
25  On Error Resume Next
    ...
    With INFO_GRILLA07
          For i& = 1 To .MSF.Rows - 1
             ...
             OBSERV$ = Left$(.MSF.TextMatrix(i&, 7), 512)
             '
             With Rst
                .AddNew
                !PORCENTAJE = PJE
...
Private Sub Command1_Click()
   ...
   Call GRATIEMOD
   If MODIFIC%(4) = 1 Then
    Call ActualizaFechaEnListas(1)
    MODIFIC%(4) = 0
   End If
   If MODIFIC%(5) = 1 Then
    Call ActualizaFechaEnListas(2)
    MODIFIC%(5) = 0
   End If
   '
...
Private Sub Command10_Click()
   ...
   '
    ...
    End If
    '
    Label1.Caption = FORMATNUM$(XVALO(Label103.Caption) + XVALO(Label110.Caption), "F9." & TRIM$(Str(DECILISTA%(XVALO(Label15)))))
    If XVALO(Label11) > 0 Then
       Label6.Caption = FORMATNUM$((XVALO(Label103.Caption) + XVALO(Label110.Caption)) / XVALO(Label11.Caption), "F9." & TRIM$(Str(DECILISTA%(XVALO(Label16)))))
    End If
    '
    Label7.Caption = FORMATNUM$(((XVALO(Label1.Caption) * ALIVA) / 100) + XVALO(Label1.Caption), "F9." & TRIM$(Str(DECILISTA%(XVALO(Label15)))))
    Label8.Caption = FORMATNUM$(((XVALO(Label6.Caption) * ALIVA) / 100) + XVALO(Label6.Caption), "F9." & TRIM$(Str(DECILISTA%(XVALO(Label16)))))
    Text6 = HOS$: Text7 = HOS$
    '
...
'       End If
       ...
       Alias1$ = VALOBADA("MASTER", "ALIAS", CONDI$, "NOMESS")
       ...
       CONDI2$ = "CODINT = " & CINUE%
       Call ACTUREGISTRO("MASTER", "ALIAS", CONDI2$, Alias1$, REG&, "NOMESS")
...
Private Sub Command6_Click()
    '
10  opt% = ALTERNA%("Actualización de Costos por Proveedor\Recálculo de Items Recaudados\ - \Acceso a Archivos Maestros\Grupos Contables de Venta\Listas de Precios de Venta\Acceso a Presupuestos\Estadísticas de Cambios de Precios\Generar Mail de Cotización\Recálculo de Compras y Pedidos \Proyección de Mano de Obra\-\Otros Accesos Directos")
    If opt% = 1 Then
...
10  OBX$ = OBJPLA$("DESHASFECHA", VDEF$)
    ...
    If Des% > Has% Then
        GoTo 10
    End If
    diaslaborables = DiasLaborales(Des%, Has%)
    
    CANTIOP = XVALO(Control("ANACOSJM", "CANOPPMO"))
    AUSENTISMO% = XVALO(Control("ANACOSJM", "AUSENTIS"))
    STOCX% = XVALO(Control("ANACOSJM", "STOCKJYM"))

    Call REPLA(TX$, MKI$(diaslaborables), 1, 2)
    Call REPLA(TX$, MKI$(CANTIOP), 3, 2)
    Call REPLA(TX$, MKI$(AUSENTISMO%), 5, 2)
    Call REPLA(TX$, MKI$(STOCX%), 7, 2)
    
    VDEF$ = TX$
20  OBX$ = OBJPLA$("PRMOBJYM", VDEF$)
    If OBX$ = "" Then
        Exit Sub
    End If
    '
    diaslaborables = CVI(Left$(OBX$, 2))
    CANTIOP = CVI(Mid$(OBX$, 3, 2))
    AUSENTISMO% = CVI(Mid$(OBX$, 5, 2))
    STOCX% = CVI(Mid$(OBX$, 7, 2))
    NOPYI% = 10 'POR DEFECTO

    ACUMUMOBRAREAL = 0: ACUMUMOBRA = 0: AUSENT$ = "": STOC$ = "": MINU = 0: ACUMUCANTENTR = 0: ACUMUCANTPED = 0
    Campos$ = "Cliente/A24;Código/A12;F.Ent./D8;Pedida/F9.1;M.O./F9.1;M.O.R./F9.1;Entregada/F9.1"
    Call CARGA_ENCABEZADO(GRILLAGRAL.GRID, Campos$, GRILLAGRAL, 1)
    Dim obj As New RECORXET
    Set rs1 = obj.RS_PEDIDOS_TODOS_X_FECHA(Des%, Has%)
    With rs1
      Do While Not .EOF
         CI1% = XVALO(!CodiInt)
         NC = XVALO(!NroClie)
         rs$ = SAFETEXT$(!RasoClie)
         CODI$ = SAFETEXT$(!CODIEXT)
         FENT% = CVINT(!FeSolEnt)
         FEMIS% = CVINT(!FECHEMIS)
         CANTPED = XVALO(!CanPed)
         CONDI$ = "CODICONJ =" & CI1% & " and NumeOper = " & NOPYI%
         MINUMOBRA# = XVALO(VALOBADA("SECOPER", "MINUSTAN", CONDI$, "NOMESS"))
         MINUMOBRA# = MINUMOBRA# * CANTPED
         MOBRA = MINUMOBRA#
         CANTENTR = XVALO(!CANTENT)
         
         
         ACUMUMOBRA = ACUMUMOBRA + MINUMOBRA#
         CONDI$ = "CODINT = " & CI1%
         MOBRAREAL = XVALO(VALOBADA("MASTER", "MANO_OBRA_REAL", CONDI$, "NOMESS"))
         MOBRAREAL = MOBRAREAL * CANTPED
         ACUMUMOBRAREAL = ACUMUMOBRAREAL + MOBRAREAL
         ACUMUCANTENTR = ACUMUCANTENTR + CANTENTR
         ACUMUCANTPED = ACUMUCANTPED + CANTPED
         '
         i& = i& + 1
         GRILLAGRAL.GRID.Rows = i& + 1
         GRILLAGRAL.GRID.TextMatrix(i&, 1) = rs$
         GRILLAGRAL.GRID.TextMatrix(i&, 2) = CODI$
         GRILLAGRAL.GRID.TextMatrix(i&, 3) = FECHATEX$(FENT%)
         GRILLAGRAL.GRID.TextMatrix(i&, 4) = FORMATNUM$(CANTPED, "F10.1")
         GRILLAGRAL.GRID.TextMatrix(i&, 5) = FORMATNUM$(MOBRA, "F10.1")
         GRILLAGRAL.GRID.TextMatrix(i&, 6) = FORMATNUM$(MOBRAREAL, "F10.1")
         GRILLAGRAL.GRID.TextMatrix(i&, 7) = FORMATNUM$(CANTENTR, "F10.1")
         .MoveNext
      Loop
    End With
    Set obj = Nothing
    ...
    Set rs1 = Nothing
    '
    GRILLAGRAL.Caption = "Proyección de Mano de Obra (Rango: " & FECHATEX$(Des%) & " al " & FECHATEX$(Has%) & ")"
...
'    DES% = FECHANUM(DESS$)
'    HAS% = FECHANUM(HASS$)
    RANGOX$ = FECHATEX$(Des%) & " al " & FECHATEX$(Has%)
    diaslaborables = CVI(Left$(OBX$, 2))
    CANTIOP = CVI(Mid$(OBX$, 3, 2))
    AUSENTISMO% = CVI(Mid$(OBX$, 5, 2))
    STOCX% = CVI(Mid$(OBX$, 7, 2))
    NOPYI% = 10 'POR DEFECTO
    MINU = diaslaborables * CANTIOP * 8 * 60
    MINUTOX$ = FORMATNUM$(MINU, "f12.2")
    AUSENT$ = FORMATNUM$((MINU * AUSENTISMO%) / 100, "F12.2")
    STOC$ = FORMATNUM$((MINU * STOCX%) / 100, "F12.2")
    MINUTOSDI = FORMATNUM$(MINU - ACUMUMOBRA, "F12.2")
    FILTX$ = TRIM$(RANGOX$) & ";" & SAFETEXT$(diaslaborables) & ";" & SAFETEXT$(CANTIOP) & ";" & MINUTOX$ & ";"
    FILTX$ = FILTX$ & TRIM$(AUSENT$) & ";" & TRIM$(STOC$) & ";" & TRIM$(FORMATNUM$(MINU - (XVALO(AUSENT$) + XVALO(STOC$)), "F12.2")) & ";" & TRIM$(FORMATNUM$(ACUMUMOBRA, "F12.2")) & ";"
    FILTX$ = FILTX$ & TRIM$(FORMATNUM$(ACUMUMOBRAREAL, "F12.2")) & ";" & TRIM$(FORMATNUM$(ACUMUCANTPED, "F12.2")) & ";" & TRIM$(FORMATNUM$(ACUMUCANTENTR, "F12.2"))
...
10  OBX$ = OBJPLA$("DESHASFECHCLIN", VDEF$)
    If OBX$ = "" Then
        Des% = 0: Has% = 0: PERIO$ = ""
        Exit Sub
    End If
    ...
    'PERIOSEL$ = MESANO$("Selección: ", DES%, HAS%)
    PERIOSEL$ = FECHATEX$(Des%) & "-" & FECHATEX$(Has%)
    DESDAT = FETEXCOR1$(Des%)
    HASDAT = FETEXCOR1$(Has%)
    NC1% = CVS(Mid$(OBX$, 5, 4))
    '
    ...
    Dim CANTISALI()
    ReDim CANTISALI(4, 4)
    Dim CANTIPORCODIGO(32767)
    Dim IMPORTEXCODIGO(32767)
    ...
    For U& = 0 To 32767
      ...
      TOTCOSTO#(U&) = 0
    Next U&
    Erase CANTIPORCODIGO, IMPORTEXCODIGO
    ...
    If NC1% > 0 Then CONDI$ = CONDI$ + " AND NUCLIEN = " & NC1%
        ...
        If Abs(TOTFAC#) >= 0.005 Then
          ...
          With STOTEMP1
            ...
            Do While Not .EOF
              ...
              ReDim Preserve CANTISALI(4, JN&)
              CANTISALI(1, JN&) = NCLIE%
              CANTISALI(2, JN&) = nart%
              CANTISALI(3, JN&) = CANTI#
              AAA = CANTISALI(1, 1)
              '\\\OT-06-0494-RG-FIN///
              NETO# = (XVALO(!CANTSALID) - XVALO(!CANTINGRE)) * XVALO(!VALOUNIT) * COEFIMA#
              COSTONETO# = ROUND((XVALO(!CANTSALID) - XVALO(!CANTINGRE)) * XVALO(!CostoRep), 2)
              If XVALO(!MoneVal) > 1 Then
...
30    .MoveNext
    ...
    'DESDE AQUI NUEVO POR DETALLE
    'RECALCULO DE LAS CANTIDADES E IMPORTES VENDIDAS POR CADA CLIENTE Y POR CADA ARTICULO
    For i% = 1 To JN& 'RECORRO EL VECTOR COMPLETO
       NC1% = CANTISALI(1, i%) 'numero de cliente
       If NC1% > 0 Then
            For JL% = 1 To JN& 'vuelvo a recorrer y comparo con el mismo numero de cliente LEIDO del primer for
               If NC1% = CANTISALI(1, JL%) Then
                 CI1% = CANTISALI(2, JL%)  ' si es el mismo cliente
                 CANTIPORCODIGO(CI1%) = CANTIPORCODIGO(CI1%) + CANTISALI(3, JL%) 'ACUMULO en un vector identificado por el codigo la cantidad
                 IMPORTEXCODIGO(CI1%) = IMPORTEXCODIGO(CI1%) + CANTISALI(4, JL%) 'ACUMULO en un vector identificado por el codigo la el importe
                 CANTISALI(1, JL%) = 0 ' LO PONGO EN 0 PARA QUE NO LO VUELVA A LEER EN EL CASO QEU SE REPITA EL CLIENTE
               End If
               If JL% >= 32767 Then Exit For
            Next JL%
            '
            For CI1% = 1 To ARTMAX% 'RECORRO PARA PASAR LAS CANTIDADES POR CADA CODIGO Y EL IMPORTE
               If CANTIPORCODIGO(CI1%) > 0 Then
                  CANTIK = CANTIPORCODIGO(CI1%)
                  IMPOK# = IMPORTEXCODIGO(CI1%)
                  GoSub Re_Calcular
                  CANTIPORCODIGO(CI1%) = 0
                  IMPORTEXCODIGO(CI1%) = 0
               End If
               If CI1% >= 32767 Then Exit For
            Next CI1%
            GoSub GRABAR_DETALLE
        End If
        If i% >= 32767 Then Exit For
    Next i%

 Call LIPOCLI(PERIOSEL$)
...
GRABAR_DETALLE:
       Text1.TEXT = ""
       TXT$ = REGBLAN$("SEGUIDET")
       Call REPLA(TXT$, MKI(NUCLIEN), 1, 2)
       Call REPLA(TXT$, "Rango/Fecha", 3, 16)
       Call REPLA(TXT$, MKI(Des%), 19, 2)
       Call REPLA(TXT$, MKS(MINMANOBRA#), 21, 4)
       Call REPLA(TXT$, MKD(PESMANOBRA#), 25, 8)
       Call REPLA(TXT$, MKD(TOTMATPRIM#), 33, 8)
       Call REPLA(TXT$, MKD(GASTOSGRAL#), 41, 8)
       Call REPLA(TXT$, MKD(SCRAPMOBRA#), 49, 8)
       Call REPLA(TXT$, MKD(UTILIDAD#), 57, 8)
       Call REPLA(TXT$, MKD(IMPUESTOS#), 65, 8)
       Call REPLA(TXT$, MKD(COMIVENDED#), 73, 8)
       Call REPLA(TXT$, MKD(COSTOTOTAL#), 113, 8)
       Call REPLA(TXT$, MKD(IMPNET#), 121, 8)
       HOII% = HOY(HOS$)
       Call REPLA(TXT$, MKI(HOII%), 129, 2)
       Call REPLA(TXT$, MKS(MINMANOBRA_REAL#), 131, 4)
       Call REGAPP("SEGUIDET", TXT$)
       MINMANOBRA# = 0
       MINMANOBRA_REAL# = 0
       PESMANOBRA# = 0
       TOTMATPRIM# = 0
...
10  OBX$ = OBJPLA$("DESHASFECHA", VDEF$)
    ...
    '
    OPFECHA% = COMALTER%("Fecha a Considerar\\Fecha de Entrega Pactada\Fecha de Emisión")
    FEXI$ = " (F.Entrega) "
    If OPFECHA% = 2 Then FEXI$ = " (Emisión) "
    RECALPEND09.Label2.Caption = PERIO$ & FEXI$
    '
    ...
    If PEDIDO% = 1 Then
        ...
        If OPFECHA% = 2 Then
          SQLX$ = SQLX$ + " AND PEDDETA.Fechemis >=  '" & Desde & "'"
          SQLX$ = SQLX$ + " AND PEDDETA.Fechemis <=  '" & Hasta & "'"
        Else
...
'            Call COLOREAR_GRILLA_SOLO_UNA_AFILA(RECALPEND09.MSF1, COLOR1, j&)
    ...
    ElseIf compra% = 1 Then
        ...
        If OPFECHA% = 2 Then
          SQLX$ = SQLX$ + " AND OCOMDETA.FEMI_OCOM >=  '" & Desde & "'"
          SQLX$ = SQLX$ + " AND OCOMDETA.FEMI_OCOM <=  '" & Hasta & "'"
        Else
   ...
   With rs
    Do While Not .EOF
      ...
      Call TRACE(24, REG&, FIN&)
        OBSER$ = SAFETEXT$(!Observacion)
        INFO_GRILLA07.Label7 = OBSER$
        ALTO& = INFO_GRILLA07.Label7.Height: If ALTO& < 350 Then ALTO& = 350
        INFO_GRILLA07.MSF.RowHeight(REG&) = ALTO&
        INFO_GRILLA07.Label7.Width = 5100
      '
      INFO_GRILLA07.MSF.TextMatrix(REG&, 1) = FECHATEX$(CVINT(!Fech_Mod))
      P_ANT# = XVALO(!Prec_Ant): COTIZA1 = XVALO(!COTIZA_DOLAR): If COTIZA1 <= 0 Then COTIZA1 = 1
      P_NEW# = XVALO(!Prec_New): COTIZA2 = XVALO(!COTIZA_DOLAR): If COTIZA2 <= 0 Then COTIZA2 = 1
      ...
      INFO_GRILLA07.MSF.TextMatrix(REG&, 5) = FORMATNUM$(TIPOMONE%, "F4.1")
       
      INFO_GRILLA07.MSF.TextMatrix(REG&, 6) = TRIM$(ECOARCH$("FLEXUSR", MKI$(XVALO(!Nume_Usuar))))

      INFO_GRILLA07.MSF.TextMatrix(REG&, 7) = INFO_GRILLA07.Label7
      
      INFO_GRILLA07.MSF.TextMatrix(REG&, 8) = FORMATNUM$(P_ANT# / COTIZA1, "F12.2")
      INFO_GRILLA07.MSF.TextMatrix(REG&, 9) = FORMATNUM$(P_NEW# / COTIZA2, "F12.2")
...
End Sub
   ...
   Sub CARGA_ENCABEZADO_X_ART()
     ...
     With INFO_GRILLA07
        ...
        NUCOLU% = 1: .TEPRUEBA = "90": .MSF.ColWidth(NUCOLU% - 1) = .TEPRUEBA.Width: .MSF.ColAlignment(NUCOLU% - 1) = 6: .MSF.TextMatrix(0, NUCOLU% - 1) = ""
        '
        NUCOLU% = 2: .TEPRUEBA = "123456789012": .MSF.ColWidth(NUCOLU% - 1) = .TEPRUEBA.Width: .MSF.ColAlignment(NUCOLU% - 1) = 1: .MSF.TextMatrix(0, NUCOLU% - 1) = "Fecha"
        ...
        NUCOLU% = 9: .TEPRUEBA = "123456789": .MSF.ColWidth(NUCOLU% - 1) = .TEPRUEBA.Width: .MSF.ColAlignment(NUCOLU% - 1) = 6: .MSF.TextMatrix(0, NUCOLU% - 1) = "U$S.Ant"
        NUCOLU% = 10: .TEPRUEBA = "123456789": .MSF.ColWidth(NUCOLU% - 1) = .TEPRUEBA.Width: .MSF.ColAlignment(NUCOLU% - 1) = 6: .MSF.TextMatrix(0, NUCOLU% - 1) = "U$S.Nuevo"
...
'   Exit Sub
   ...
   Call ABRECONEXION
   '
   Call CREACAMPO("ARTICULOS", "Master", "Rel_Lista_Ancos", 3, 0, 1)
   Call CREACAMPO("ARTICULOS", "Master", "Rel_Lista_AN_DOL", 3, 0, 1)
   Call PoneEnCeroNull("Master", "Rel_Lista_AN_DOL", REGAF&, "NOMESS")
   Call PoneEnCeroNull("Master", "Rel_Lista_AN_DOL", REGAF&, "NOMESS")
   Call CREACAMPO("BADACLIEN", "Deudor12", "CateMobra", 10, 6)
   '
End Sub
...
Private Sub Label1_DblClick_VIE()
    ...
    '
    Label1.Caption = FORMATNUM$(XVALO(NUPRECIO$), "F9." & TRIM$(Str(DECILISTA%(XVALO(Label15)))))
    Label7.Caption = FORMATNUM$(((XVALO(Label1.Caption) * ALIVA) / 100) + XVALO(Label1.Caption), "F9." & TRIM$(Str(DECILISTA%(XVALO(Label15)))))
    Text6 = HOS$ ': Text7 = HOS$
    '
    '\\\OT-08-0095-FL-14/03/08///
...
Private Sub label1_Change()
  ...
  CI1% = XVALO(Label28)
  If CI1% < 1 Then Exit Sub
  PorcMinorista = PorcentajeMinorista(CI1%) 'esta funcion le devuelve el de la tabla, por si lo cambio manualmente
  Label26 = PorcMinorista                   'si no esta en la tabla devuelve el configurado en el setup de anacos
  Label30 = FORMATNUM$(XVALO(Label1) * (1 + (PorcMinorista / 100)), "F10.2")
...
30  NUPRECIO$ = InputBox$("Nuevo Precio", "Registrar Nuevo Precio en Pesos al " + HOS$, Label1)
    ...
    If Abs(PREC_ACTUAL# - PREC_NVO#) > 0.00005 Then 'SI CAMBIO EL PRECIO
        ...
        Label1.Caption = FORMATNUM$(PREC_NVO#, "F9." & TRIM$(Str(DECILISTA%(XVALO(Label15)))))
        Label7.Caption = FORMATNUM$(((XVALO(Label1.Caption) * ALIVA) / 100) + XVALO(Label1.Caption), "F9." & TRIM$(Str(DECILISTA%(XVALO(Label15)))))
        Text6 = HOS$
...
10 Call SELECHO("LISTAS")
   NUMLIS% = XVALO(VALACT1$("LISTAS")) '
   If NUMLIS% < 1 Then Exit Sub
   CI1% = XVALO(Label28)
   If CI1% < 1 Then Exit Sub
   CONDI$ = "Cod_Lista = " & NUMLIS%
   MON% = XVALO(VALOBADA("LISTAPRE", "MONEDA", CONDI$, "NOMESS"))
   If MON% > 1 Then
      Call COMUNI("Lista de Precios No Coincide Con la Moneda")
      GoTo 10
   End If
   
   CONDI$ = "CODINT = " & CI1%

   Call ACTUREGISTRO("MASTER", "Rel_Lista_Ancos", CONDI$, NUMLIS%, REGAF&, "NOMESS")
   COD$ = Text1
   Text1 = ""
   Text1 = COD$
End Sub
...
10 Call SELECHO("LISTAS")
   ...
   CONDI$ = "CODINT = " & CI1%
   Call ACTUREGISTRO("MASTER", "Rel_Lista_AN_DOL", CONDI$, NUMLIS%, REGAF&, "NOMESS")
   COD$ = Text1
   Text1 = ""
   Text1 = COD$
...
Private Sub Label24_DblClick()
    NUPRECIO$ = InputBox$("Cotizaciòn Para Repuestos", "Registrar Valor Dolar para Repuestos")
'    If TRIM$(NUPRECIO$) = "" Then Exit Sub
'    If XVALO(NUPRECIO$) < 0.00005 Then Exit Sub
    '
    Label24.Caption = FORMATNUM$(XVALO(NUPRECIO$), "F10.4")
    If XVALO(Label24) <= 0.5 Then
       Label11_DblClick ' PARA REFRESCAR EL CALCULO
       Exit Sub
    End If
    '
    ...
    For U& = 1 To ULTREG&("!BOLREDET")
       ...
       Call REPLA(X$, FORMATNUM$(COSUNI(CICOMPO%), "F9.4"), 67, 9)
       If MONECO% <= 1 Then
         COSTOPAR = COSPESOS(CICOMPO%) * CANTI# 'si es = 1 trae el costo en pesos sin calcuar cotizacion
       Else
          'si el tipo de moneda del codigo esta en dolares calcula segun la cotizacion del
          COSTOPAR = COSUNI(CICOMPO%) * ROUND(XVALO(Label24), 4) * (1 + PORNACIO(CI1%) / 100)
          COSTOPAR = COSTOPAR * CANTI#
       End If
       Call REPLA(X$, FORMATNUM$(COSTOPAR, "F12.4"), 76, 12)
       List1.AddItem X$
    Next U&
...
Private Sub Label26_DblClick()
    ...
    If DERACCE%("MODPJEMI", "MODIFICA PORCENTAJE MINORISTA") < 2 Then
        GoTo 99
    End If
    '
    HOII% = HOY(HOS$)
    '
    'INGRESO DE NUEVO PRECIO Y VALIDACION
30  NUPORCENTAJE = InputBox$("Nuevo Porcentaje", "Registrar Nuevo Porcentaje")
    If TRIM$(NUPORCENTAJE) = "" Then GoTo 99
    textox$ = "Modificar Porcentaje Actual (" & Label26 & ") Por Este Porcentaje: (" & TRIM$(NUPORCENTAJE) & ")"
    OPX% = COMALTER%("Confirmación:\\Cancelar\" & textox$)
    If OPX% > 1 Then
       Call GrabarPorcentajeMinorista(CI1%, XVALO(NUPORCENTAJE))
       Label26 = TRIM$(FORMATNUM$(XVALO(NUPORCENTAJE), "F7.2"))
       Label30 = FORMATNUM$(XVALO(Label1) * (1 + (PorcentajeMinorista(CI1%) / 100)), "F10.2")
       Label22 = FORMATNUM$(XVALO(Label6) * (1 + (PorcentajeMinorista(CI1%) / 100)), "F10.2")
       'LA FUNCION PorcentajeMinorista() DEVUELVE EL VALOR GRABADO EN LA TABLA (SI SE MODIFICO MANUALMENTE) Y SI
       'DEVUELVE 0 APLICA EL PORCENTAJE CONFIGURADO EN EL SETUP DE ANACOS
    End If
99
End Sub
...
Private Sub Label28_DblClick()
   If XVALO(Label28) > 0 Then
      CANTIDADINGRE = XVALO(InputBox$("Cantidad de Cálculo", "Ingrese Cantidad a Cálcular", Mid$(CANTIDADINGRE, 2)))
      If CANTIDADINGRE < 1 Then Exit Sub
      PROPUCOM.Label5 = CANTIDADINGRE
      Call CARGARENCABEZADOPROPUCOM(RetCampos1$, RetCampos2$)
      PROPUCOM.Label8 = Text1.TEXT
      '
      Dim MSF As msFlexGrid
      If PROPUCOM.MENU1 > 0 Then
            Set MSF = PROPUCOM.MSF1
            TX$ = "PROPUESTA DE COMPRA"
            Call CARGA_TABLA_IMPRE(MSF, RetCampos1$, "PROPCOMP", TX$, "TABLA_IMPRE", RET_NAMECONS$, 1)
...
Sub CARGARENCABEZADOPROPUCOM(RetCampos1$, RetCampos2$)
 'retorno los campos para usar en la llamada al tema de la impresion
 Campos$ = "CI/A0"                                   '  1
 ...
 Call CARGA_ENCABEZADO(PROPUCOM.MSF1, Campos$, Me)
 RetCampos1$ = Campos$
 Campos$ = "Cta/f5"                                   '  1
 ...
 Campos$ = Campos$ + ";Imp.Seg.Compra/F24.4"         '  6
 
 Call CARGA_ENCABEZADO(PROPUCOM.msf2, Campos$, Me)
 RetCampos2$ = Campos$
End Sub
...
Private Sub Label6_Change()
  CI1% = XVALO(Label28)
  If CI1% < 1 Then Exit Sub
  PorcMinorista = PorcentajeMinorista(CI1%)
  Label26 = PorcMinorista
  Label22 = FORMATNUM$(XVALO(Label6) * (1 + (PorcMinorista / 100)), "F10.2")
End Sub
...
Sub Label6_DblClick()
    ...
    '
    'INGRESO DE NUEVO PRECIO Y VALIDACION
30  NUPRECIO$ = InputBox$("Nuevo Precio", "Registrar Nuevo Precio en Dólares al " + HOS$, Label6)
    If TRIM$(NUPRECIO$) = "" Then GoTo 99
    PREC_NVO# = XVALO(NUPRECIO$)
    If PREC_NVO# < 0.00005 Then GoTo 99
    '
    ...
    If Abs(PREC_ACTUAL# - PREC_NVO#) > 0.005 Then 'SI CAMBIO EL PRECIO
        ...
        '
        'ASIGNACION DE PRECIO EN PANTALLA
         Label6.Caption = FORMATNUM$(PREC_NVO#, "F9." & TRIM$(Str(DECILISTA%(XVALO(Label16)))))
         Label8.Caption = FORMATNUM$(((XVALO(Label6.Caption) * ALIVA) / 100) + XVALO(Label6.Caption), "F9." & TRIM$(Str(DECILISTA%(XVALO(Label16)))))
         Text7 = HOS$
...
Private Sub mnuComprasPedidos_Click()
   PROPUCOMAS.Show 1
End Sub
...
Private Sub Text1_Change()
   ...
   If CIXII% > 0 Then
      ...
      MODIFIC%(4) = 0
      MODIFIC%(5) = 0
      LBLCantidadItems = List1.ListCount
      Timer2.Enabled = True 'ESTE TIMER VALIDA SI EL CODIGO SELECCIONADO POSEE ARCHIVOS POETS ASOCIADOS
   End If                   'SI TIENE MENOS DE 2 PRESENTA UN MENSAJE SOLICITADO POR JORGE 31/05/21
   
   '05/03/07 (FIN)
   '
End Sub
Function CantPoets&(CI1%)
   CONDI$ = "COD_INTE = " & CI1%
   CantPoets& = CantRegistros&("DOCUREL", CONDI$, "NOMESS")
...
Sub CALCUCOS()
   ...
   'If TOTACUM# > 0 Then ... SE SUPRIME PARA QUE HAGA EL CÁLCULO AUNQUE HAYA SOLO M.O.
      ...
      '
      Label12.Caption = FORMATNUM$(XVALO(Label103.Caption) + XVALO(Label110.Caption), "F12.4")
      If XVALO(Label11) > 0 Then
          Label3.Caption = FORMATNUM$((XVALO(Label103.Caption) + XVALO(Label110.Caption)) / XVALO(Label11.Caption), "F12.4")
      End If
      '
      On Error Resume Next
...
Sub PRIANACOS()
   '
        ...
        If DERACCE%("FORMUSUP/NOMESS", "Impresión de Formulario con Suplemento de Costos Calculados") > 1 Then
          OPX% = COMALTER%("Opciones de Impresión:\\Con Suplemento\Sin Suplemento\Exportar Formula a Excel")
          If OPX% = 1 Then
            FORIPRE$ = "ANC1-JYM"
            If EXISTE%(LUDAT$ + FORIPRE$ + ".FRM") < 1 Then
              Call COMUNI("Falta Formulario de Impresión: " & FORIPRE$ & ".FRM")
        ...
        U& = 1
        If EXPEXCEL% > 0 Then
          FIN& = UBound(TETABU1$, 2)
          Campos$ = "Código/C16;Descripción/A32;U.M./A4;Cantidad/F10.1;Prec.Unit./F12.2;Importe/F12.2;M/A2 "
          Call CARGA_ENCABEZADO(Me.MSF, Campos$, ANACOSI09)
             MSF.Rows = U& + 1
             MSF.TextMatrix(U&, 1) = "Código"
             MSF.TextMatrix(U&, 2) = "Descripción"
             MSF.TextMatrix(U&, 3) = "U.M."
             MSF.TextMatrix(U&, 4) = "Cantidad"
             MSF.TextMatrix(U&, 5) = "Prec.Unit"
             MSF.TextMatrix(U&, 6) = "Importe"
             MSF.TextMatrix(U&, 7) = "M."

          For U& = 0 To FIN&
             MSF.Rows = U& + 2
             MSF.TextMatrix(U& + 1, 1) = TETABU1$(1, U&)
             MSF.TextMatrix(U& + 1, 2) = TETABU1$(2, U&)
             MSF.TextMatrix(U& + 1, 3) = TETABU1$(3, U&)
             MSF.TextMatrix(U& + 1, 4) = TETABU1$(4, U&)
             MSF.TextMatrix(U& + 1, 5) = TETABU1$(5, U&)
             MSF.TextMatrix(U& + 1, 6) = TETABU1$(6, U&)
             MSF.TextMatrix(U& + 1, 7) = TETABU1$(7, U&)
             'FIN& DEVUELVE
             If TRIM$(TETABU1$(1, U&)) = "" Then
                VACIO% = VACIO% + 1
             Else
                VACIO% = 0
             End If
             If VACIO% > 3 Then Exit For
          Next U&
          ARCHIX$ = SELECCARCHIVOEXCEL$
          If TRIM$(ARCHIX$) = "" Then GoTo 99
          Call GeneraExcelDeGrilla(ARCHIX$, Me.MSF, " Estructura de Código: " & Text1 & " " & Text3)
          GoTo 99
...
Sub GRUPCONT()
    '
10  OPX% = COMALTER%("Opciones de Trabajo\\Cancelar\Nuevo Grupo Contable\GrupoS Contable con Filtro")
    If OPX% <= 1 Then Exit Sub
    If OPX% = 3 Then
      Call EDITARGRUPOCONTABLE
      GoTo 10
    End If
...
Sub SEGUITRE()
10  OPX% = COMALTER%("Opciones de Procesamiento:\\Recálculo Mensual\Listado por Fechas\Listado por Fechas y Cliente")
    If OPX% = 1 Then
    ...
    ElseIf OPX% = 2 Then
        ...
        GoTo 10
    ElseIf OPX% = 3 Then
        If DERACCE%("LIPOCLI", "Listados Resumen de Costos por Fecha y Cliente") <> 0 Then
            Call CALCU_CATEGORIA
        End If
        GoTo 10
    Else
       Exit Sub
...
30    .MoveNext
    ...
    For JX& = 1 To FIN&
        ...
        Openforms = DoEvents
        MINMANOBRA# = MINMANOBRA# + XVALO(Text2.TEXT) * CANITEM# 'MANO DE OBRA
        MINMANOBRA_REAL# = MINMANOBRA_REAL# + XVALO(Text4.TEXT) * CANITEM# 'MANO DE OBRA REAL
        PESMANOBRA# = PESMANOBRA# + XVALO(Label4.Caption) * CANITEM# '$ VALOR
        TOTMATPRIM# = TOTMATPRIM# + XVALO(Label5.Caption) * CANITEM# 'Total Materia Prima:
        GASTOSGRAL# = GASTOSGRAL# + XVALO(Label68.Caption) * CANITEM# ' GASTOS GENERALES SEGUN PORCENTAJE CONFIGURADO
        SCRAPMOBRA# = SCRAPMOBRA# + XVALO(Label93.Caption) * CANITEM# 'SCRAP SEGUN  PORCENTAJE CONFIGURADO
        UTILIDAD# = UTILIDAD# + XVALO(Label107.Caption) * CANITEM# 'UTILIDAD
        IMPUESTOS# = IMPUESTOS# + XVALO(Label104.Caption) * CANITEM#
        COMIVENDED# = COMIVENDED# + XVALO(Label110.Caption) * CANITEM#
        COSTOTOTAL# = COSTOTOTAL# + XVALO(Label12.Caption) * CANITEM#
...
Sub LIPORFE()
    ...
    J& = 0
    'nuevo
20  Campos$ = "Período/A16;Min.MOD/F12.2;Costo MOD/F12.2;Mat.Prima/F12.2;G.Grales/F12.2;Scrap/F12.2;Utilidad./F12.2;Impuestos/F12.2;Comision./F12.2;Costo Total/F12.2;Imp.Neto/F12.2;Min.M.O.R./F12.2;"
    Call CARGA_ENCABEZADO(GRILLAGRAL.GRID, Campos$, GRILLAGRAL, 1)
    Call CARGA_ENCABEZADO(GRILLAGRAL.MSF_2, Campos$, GRILLAGRAL, 1)
    TITU$ = "Seguimiento de Items Recaudados (Emitido: " & FECHATEX$(HOY(HO$)) & ")   Período:    " & PERIO$
...
'  LIMPIO LAS VARIABLES ACUMULDORAS
    ACU_MINMANOBRA# = 0
    ACU_PESMANOBRA# = 0
    ACU_TOTMATPRIM# = 0
    ACU_GASTOSGRAL# = 0
    ...
    ACU_COSTOTOTAL# = 0
    ACU_IMPNET# = 0
    ACU_MINMANOBRA_REAL# = 0
    '
    'RECORRO LA GRILLA Y SUMO PARA TOTALIZAR EN LA ULTIMA FILA
    For i& = 1 To GRILLAGRAL.GRID.Rows - 1
       ACU_MINMANOBRA# = ACU_MINMANOBRA# + XVALO(GRILLAGRAL.GRID.TextMatrix(i&, 2))
       ACU_PESMANOBRA# = ACU_PESMANOBRA# + XVALO(GRILLAGRAL.GRID.TextMatrix(i&, 3))
       ACU_TOTMATPRIM# = ACU_TOTMATPRIM# + XVALO(GRILLAGRAL.GRID.TextMatrix(i&, 4))
       ACU_GASTOSGRAL# = ACU_GASTOSGRAL# + XVALO(GRILLAGRAL.GRID.TextMatrix(i&, 5))
       ACU_SCRAPMOBRA# = ACU_SCRAPMOBRA# + XVALO(GRILLAGRAL.GRID.TextMatrix(i&, 6))
       ACU_UTILIDAD# = ACU_UTILIDAD# + XVALO(GRILLAGRAL.GRID.TextMatrix(i&, 7))
       ACU_IMPUESTOS# = ACU_IMPUESTOS# + XVALO(GRILLAGRAL.GRID.TextMatrix(i&, 8))
       ACU_COMIVENDED# = ACU_COMIVENDED# + XVALO(GRILLAGRAL.GRID.TextMatrix(i&, 9))
       ACU_COSTOTOTAL# = ACU_COSTOTOTAL# + XVALO(GRILLAGRAL.GRID.TextMatrix(i&, 10))
       ACU_IMPNET# = ACU_IMPNET# + XVALO(GRILLAGRAL.GRID.TextMatrix(i&, 11))
       ACU_MINMANOBRA_REAL# = ACU_MINMANOBRA_REAL# + XVALO(GRILLAGRAL.GRID.TextMatrix(i&, 12))
    Next i&
    '
    'CARGO LOS TOTALES
    i& = GRILLAGRAL.GRID.Rows + 1
    GRILLAGRAL.GRID.Rows = i& + 1
    GRILLAGRAL.GRID.TextMatrix(i&, 1) = "Total Gral."
    GRILLAGRAL.GRID.TextMatrix(i&, 2) = FORMATNUM$(ACU_MINMANOBRA#, "F12.2")
    GRILLAGRAL.GRID.TextMatrix(i&, 3) = FORMATNUM$(ACU_PESMANOBRA#, "F12.2")
    GRILLAGRAL.GRID.TextMatrix(i&, 4) = FORMATNUM$(ACU_TOTMATPRIM#, "F12.2")
    GRILLAGRAL.GRID.TextMatrix(i&, 5) = FORMATNUM$(ACU_GASTOSGRAL#, "F12.2")
    GRILLAGRAL.GRID.TextMatrix(i&, 6) = FORMATNUM$(ACU_SCRAPMOBRA#, "F12.2")
    GRILLAGRAL.GRID.TextMatrix(i&, 7) = FORMATNUM$(ACU_UTILIDAD#, "F12.2")
    GRILLAGRAL.GRID.TextMatrix(i&, 8) = FORMATNUM$(ACU_IMPUESTOS#, "F12.2")
    GRILLAGRAL.GRID.TextMatrix(i&, 9) = FORMATNUM$(ACU_COMIVENDED#, "F12.2")
    GRILLAGRAL.GRID.TextMatrix(i&, 10) = FORMATNUM$(ACU_COSTOTOTAL#, "F12.2")
    GRILLAGRAL.GRID.TextMatrix(i&, 11) = FORMATNUM$(ACU_IMPNET#, "F12.2")
    GRILLAGRAL.GRID.TextMatrix(i&, 12) = FORMATNUM$(ACU_MINMANOBRA_REAL#, "F12.2")
    '
    GRILLAGRAL.mnuMenu1.Caption = "Imprimir"
    GRILLAGRAL.mnuMenu1.Visible = True
    ...
    If GRILLAGRAL.MENU1 > 0 Then
        ...
        Call CARGA_TABLA_IMPRE(MSF, Campos$, "SEGUITRF", TITU$, "TABLA_IMPRE", RET_NAMECONS$, 1)
        Call FINAL("?" & RET_NAMECONS$)
        GRILLAGRAL.MENU1 = 0
        Screen.MousePointer = 1
    End If
...
Private Sub Timer2_Timer()
      CIXII% = CODINT%(Text1.TEXT)
      If CIXII% > 0 Then
        CantPoet& = CantPoets&(CIXII%)
        If CantPoet& < 2 Then
           OPX% = COMALTER%("Atención El Código Activo Posee: " & CantPoet& & " Archivos Asociados\\Continuar\Asignar Archivo")
           If OPX% = 2 Then
