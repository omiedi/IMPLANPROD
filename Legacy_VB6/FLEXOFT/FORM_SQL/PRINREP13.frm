...
Begin VB.Form PRINREP13 
   ...
   Begin VB.PictureBox Picture1 
      ...
      Begin VB.Image Image2 
         Height          =   390
         Left            =   -315
         Picture         =   "PRINREP13.frx":0454
         Top             =   3885
         Width           =   1515
      End
   End
End
Attribute VB_Name = "PRINREP13"
...
Sub LISTABU()
    ...
    '
    ATOT% = 0
    For j& = 1 To SHOWREP13.GRID.Cols
       ACOLUM%(j&) = XVALO(SHOWREP13.GRID.Rows - 1)
       If SHOWREP13.GRID.ColWidth(j& - 1) < 1 Then
          ACOLUM%(j&) = 0
       End If
    ...
    End If
    '
    FIN& = XVALO(SHOWREP13.GRID.Rows - 1)
    HAYTOTA% = 0
    For KI& = 1 To SHOWREP13.Label4.Count
      If SHOWREP13.Label4(KI& - 1).Visible = True Then TOTALI% = 1
    Next KI&
    '
    For U& = 1 To FIN&
       ...
       ALTUREMAX = 0
       For j& = 1 To SHOWREP13.GRID.Cols
          SHOWREP13.GRID.Row = U&
          SHOWREP13.GRID.Col = j& - 1
          VLXX$ = Left$(SHOWREP13.GRID.TEXT + Space$(ACOLUM%(j&)), ACOLUM%(j&))
          SUMACOL#(j&) = SUMACOL#(j&) + XVALO(VLXX$)
          '
          If TRIM$(VLXX$) = "DDDD78" Then
            A = B
          End If
          ...
          TTXX$ = TTXX$ + VLXX$
          If TRIM$(VLXX$) <> "" Then
             If SHOWREP13.GRID.CellFontUnderline = True Then
                FINSUBTO% = Len(TTXX$)
             End If
          End If
          '
'          If Left$(FORCOLUM$(j&), 3) = "JPG" Then
'               Call SHOWREP13.IMPRIMELAIMAGEN(SHOWREP13.GRID, U&, j& - 1)
'               FINSUBTO% = ATOT%
'             ElseIf Left$(FORCOLUM$(j&), 1) = "X" Then
'               Call SHOWREP13.IMPRIMEMULIN(SHOWREP13.GRID, U&, j& - 1)
'               FINSUBTO% = ATOT%
'             Else
...
Sub LITRANSPO()
    ...
    If TOTALI% > 0 Then
        '
        TTXX$ = ""
        For K2% = 1 To SHOWREP13.GRID.Cols
           If SHOWREP13.Label4(K2% - 1).Visible = True Then
...
Sub LITOTA()
    ...
    If TOTALI% > 0 Then
        ...
        For K2% = 1 To SHOWREP13.GRID.Cols
           If SHOWREP13.Label4(K2% - 1).Visible = True Then
                VLXX$ = AJUSTD$(SHOWREP13.Label4(K2% - 1), ACOLUM%(K2%))
             Else
                VLXX$ = Space$(ACOLUM%(K2%))
           End If
           If TTXX$ <> "" Then TTXX$ = TTXX$ + " "
...
19      Next IPRIN&
      ...
      End If
        ...
        If SHOWPLA07.Visible = True Then
           ...
           Else
             For j& = 1 To SHOWREP13.GRID.Cols
'               ATOT% = ATOT% + XVALO(SHOWREP13.GRID.ColWidth(j&))
               ATOT% = ATOT% + XVALO(SHOWREP13.Label8(j&).Caption) + 1
             Next j&
        End If
        '
100     TEPRUEBA$ = String$(ATOT%, "*")
        ...
        For I% = 1 To CAFUFIJ%
            ...
            While TALEBA > FUENTEMIN
               ...
               If Printer.FontSize > TALEBA Then
               GoTo 105
               End If
               '
               On Error Resume Next
...
Sub ENCAHOJA()
    ...
    '
'    For RI% = 1 To SHOWREP13.Combo1.Count
'      If TRIM$(SHOWREP13.Combo1(RI% - 1).TEXT) <> "" Then
'        LEYEN$ = TRIM$(SHOWREP13.Label1(RI% - 1)) + ": "
'        LEYEN$ = LEYEN$ + SHOWREP13.Combo1(RI% - 1).TEXT
'        On Error Resume Next
'          ACLA$ = TRIM$(SHOWREP13.FiltEco1(RI% - 1).Caption)
'        On Error GoTo 0
'        If ACLA$ <> "" Then
'          LEYEN$ = LEYEN$ + " - " + ACLA$
'        End If
...
Sub ARMAENCA()
    ...
    If SHOWPLA07.Visible = True Then
       ...
       Else
          For j& = 1 To SHOWREP13.GRID.Cols
             SHOWREP13.GRID.Row = 0
             SHOWREP13.GRID.Col = j& - 1
             ALI = SHOWREP13.GRID.ColAlignment(j& - 1)
             '
             VLXY$ = SHOWREP13.GRID.TEXT
             If ALI >= 6 Then
                  VLXX$ = AJUSTD$(VLXY$, ACOLUM%(j&))
                Else
                  VLXX$ = AJUSTI$(VLXY$, ACOLUM%(j&))
