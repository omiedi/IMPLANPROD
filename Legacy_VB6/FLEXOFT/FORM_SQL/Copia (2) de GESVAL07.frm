...
Sub CONCILIAEXTRACTO(CODICONCI$)
    ...
    With RSTTMP
      Do While Not .EOF
         ...
         If IMPODEBEX# > 0 Then
           CONDI$ = "IHABECOMP = IMPODEBE_PTO AND FECHEMIS <= '" & FECH1 & "'"
           CONDI$ = CONDI$ + "  AND FECHEMIS >= '" & FECHD & "'"
         ...
         ElseIf IMPOHABEX# > 0 Then
            CONDI$ = "IDEBECOMP = IMPOHABE_PTO AND FECHEMIS <= '" & FECH1 & "'"
            CONDI$ = CONDI$ + "  AND FECHEMIS >= '" & FECHD & "'"
            CONDI$ = CONDI$ + "  AND TIPOMOVIM = 'CHRECI' "
            CONDI$ = CONDI$ + " CueContaI = " & CUEII%
         End If
         '
         COINCIDE% = CantRegistros&("CAJABANC", CONDI$, "NOMESS")
         
         If COINCIDE% > 1 Then
            CONDI$ = CONDI$ + " AND SUBSTRING(ID_REFERENCIA,19,14)  Like '" & IDREFE$ & "'"
            If CantRegistros("CAJABANC", CONDI$, "NOMESS") = 1 Then
              COMPROBTE$ = VALOBADA("CAJABANC", "CODICOM_LAR", CONDI$, "NOMESS")
              FECONTA = FETEXCOR1$(CVINT(VALOBADA("DETAASIEN", "FECHCONTA", CONDI$, "NOMESS")))
              Call ACTUREGISTRO("CONCILIA", "COMPROBANTE", CONDI$, COMPROBTE$, REGAF&, "NOMESS")
              Call ACTUREGISTRO("CONCILIA", "FECHACOMPROBANTE", CONDI$, FECONTA, REGAF&, "NOMESS")
         ElseIf COINCIDE% = 1 Then
             'AGREGA A LA LISTA DE 1
              COMPROBTE$ = VALOBADA("CAJABANC", "CODICOM_LAR", CONDI$, "NOMESS")
              FECONTA = FETEXCOR1$(CVINT(VALOBADA("CODICOM_LAR", "FECHCONTA", CONDI$, "NOMESS")))
              Call ACTUREGISTRO("CONCILIA", "COMPROBANTE", CONDI$, COMPROBTE$, REGAF&, "NOMESS")
              Call ACTUREGISTRO("CONCILIA", "FECHACOMPROBANTE", CONDI$, FECONTA, REGAF&, "NOMESS")
         End If
            
         CONDI$ = "DETAASIEN.FECHASIEN <= '" & FECH1 & "'"
         CONDI$ = CONDI$ + " AND DETAASIEN.FECHASIEN >= '" & FECHD & "'"
         CONDI$ = CONDI$ + " AND DETAASIEN.CUECOINT = " & CUEII%
         CONDI$ = CONDI$ + " AND DETAASIEN.statpase > 0"
         CONDI$ = CONDI$ + " AND DETAASIEN.statpase > 0"
         CONDI$ = CONDI$ + " AND (DETAASIEN.MarConsi = '' or DETAASIEN.MarConsi IS NULL ) "
         CONDI$ = CONDI$ + " AND ENCABEASIEN.TIPOASIEN = 0 "
         IMPODEBE_ASIEN# = XVALO(VALOBADA("ENCABEASIEN INNER JOIN DETAASIEN ON ENCABEASIEN.NUIDASIEN = DETAASIEN.NUIDASIEN ", "IDEBEPASE", CONDI$, "NOMESS"))
         IMPOHABE_ASIEN# = XVALO(VALOBADA("ENCABEASIEN INNER JOIN DETAASIEN ON ENCABEASIEN.NUIDASIEN = DETAASIEN.NUIDASIEN ", "IHABEPASE", CONDI$, "NOMESS"))
         '
         IMPODEBE_PTO = 0: IMPOHABE_PTO = 0
         If IMPODEBEX# <> 0 Then IMPODEBE_PTO = TRIM$(FORMATNUM$(IMPODEBEX#, "F16.5"))
         If IMPOHABEX# <> 0 Then IMPOHABE_PTO = TRIM$(FORMATNUM$(IMPOHABEX#, "F16.5"))
         ...
         If IMPODEBEX# > 0 Then
            CONDI$ = CONDI$ + " IMPODEBEX# = IHABEPASE "
         ElseIf IMPOHABEX# > 0 Then
            CONDI$ = CONDI$ + " IMPOHABEX# = IDEBEPASE "
            End If
         End If
         '
         COINCIDE% = CantRegistros&("ENCABEASIEN INNER JOIN DETAASIEN ON ENCABEASIEN.NUIDASIEN = DETAASIEN.NUIDASIEN", CONDI$, "NOMESS")
         If COINCIDE% = 1 Then
            'AGREGA A LA LISTA DE 1
            IDASIEN$ = VALOBADA("DETAASIEN", "NUIDASIEN", CONDI$, "NOMESS")
            FEASIEN = FETEXCOR1$(CVINT(VALOBADA("DETAASIEN", "FEHCASIEN", CONDI$, "NOMESS")))
            Call ACTUREGISTRO("CONCILIA", "COMPROBANTE", CONDI$, IDASIEN$, REGAF&, "NOMESS")
            Call ACTUREGISTRO("CONCILIA", "FECHACOMPROBANTE", CONDI$, FEASIEN, REGAF&, "NOMESS")
         End If
         '
80      .MoveNext
      Loop
...
Sub PRESENTA_CONCILIA(CODICONCI$)
    '
    I& = 0
    '
    MSF1.Cols = 7
    ...
    SQLX$ = "SELECT * FROM CONCILIA WITH(NOLOCK)"
    SQLX$ = SQLX$ + " WHERE CODICONCI = '" & CODICONCI$ & "'"
    SQLX$ = SQLX$ + " AND (COMPROBANTE <> '' OR COMPROBANTE IS NOT NULL"
    SQLX$ = SQLX$ + " AND status < 1"
    SQLX$ = SQLX$ + " ORDER BY FECHA"
    '
    ...
    With RSTTMP
      Do While Not .EOF
         ...
         CUEII% = XVALO(!CueContaI)
         IDREFE$ = SAFETEXT$(ID_REFERENCIA)
         FE% = CVINT(!FECHACOMPROBANTE)
         FECH1 = FETEXCOR1$(FE%)
         CONCEP$ = SAFETEXT$(!CONCEPTO)
         REFECON$ = SAFETEXT$(!REFECONCI)
         COMPRBTE$ = SAFETEXT$(!COMPROBANTE)
         CONDI$ = "CODICOM_LAR = '" & COMPRBTE$ & "'"
         CONDI$ = CONDI$ + " AND FECHCONTA = '" & FECH1 & "'"
         CONCI$ = TRIM$(VALOBADA("CAJABANC", "MarConsi", CONDI$, "NOMESS"))
         With CONCIMAN07.MSF1
            I& = I& + 1
            .Rows = I&
            .TextMatrix(REG&, 1) = FECHATEX$(FECHANUM(FECH1))
            .TextMatrix(REG&, 2) = CONCEP$
            .TextMatrix(REG&, 3) = TRIM$(FORMATNUM$(IMPODEBEX#, "F12.2"))
            .TextMatrix(REG&, 4) = TRIM$(FORMATNUM$(IMPOHABEX#, "F12.2"))
            .TextMatrix(REG&, 5) = CONCI$
            .TextMatrix(REG&, 6) = REFECON$
         End With
    End With
    ...
    Set RSTTMP2 = Nothing
    '
    'CARGO LA GRILLA DE LOS PENDIENTES EN EL SISTEMA
    ...
    With CONCIMAN07.MSF3
        ...
        For U& = 1 To ULTREG("!LIMAYOA1")
         ...
         Z$ = Space$(128)
           FEX = CVI(Left$(x$, 2))
           FEXA$ = FECHATEX$(FEX)
         .TextMatrix(REG&, 1) = FEXA$
           COCOM$ = TRIM$(Mid$(x$, 3, 12))
         .TextMatrix(REG&, 2) = COCOM$
           DEPASE$ = TRIM$(Mid$(x$, 17, 32))
         .TextMatrix(REG&, 3) = DEPASE$
           IDEBE# = CVD(Mid$(x$, 49, 8))
           IHABE# = CVD(Mid$(x$, 57, 8))
         If IDEBE# > 0 Then
             .TextMatrix(REG&, 4) = FORMATNUM$(IDEBE#, "F14.2")
           Else
             .TextMatrix(REG&, 5) = FORMATNUM$(IHABE#, "F14.2")
         End If
...
Sub GASTOEXTRACTO(CODICONCI$)
    ...
    For U& = 1 To Len(RFF$) Step 4
        ...
        CONCEP$ = TRIM$(Mid$(XX$, 13, 32))
        CONDI$ = "COMPROBANTE = '' AND CONCEPTO LIKE '" & CONCEP$ & "' AND STATUS < 1 AND CODICONCI = '" & CODICONCI$ & "'"
...
Function CUENTA_BCO_CONCILIA%(CODICONCI$)
   CUENTA_BCO_CONCILIA% = XVALO(VALOBADA("CONCILIA", "CUENTACONTBANCO", "CODICONCI= '" & CODICONCI$ & "'", "NOMESS"))
End Function
...
10      Call SELECHO("CUEBANC/BANCOS")
        ...
        '
        DEBE_Y_HABER% = 0
        If VALIDA_POS% = 2 Then DEBE_Y_HABER% = 1
