...
Sub AnalizaCorrelativaDeComprobantes(MODO As Integer, Optional MostrarResultado As Boolean = True)
    ...
    FECHAFIN% = DIANTE(HOY(HO$), 75)
    Fechaini% = DIANTE(FECHAFIN%, 180)
...
Function BusquedaHuecos(TipoComprobante As String, LETRA As String, Talonario As Integer, Inicio As Long, FIN As Long, Optional CantidadDiasDeBusqueda As Integer) As String
    
    CantidadDias% = 75
    If CantidadDiasDeBusqueda > 0 Then CantidadDias% = CantidadDiasDeBusqueda
    
    Dim RstFC As ADODB.Recordset
    
    On Error GoTo ControlError
    Set RstFC = FLEXCONN.Execute("exec HUECOS_EN_FACTURACION '" & TipoComprobante & "','" & LETRA$ & "'," & CStr(Talonario) & "," & Inicio & "," & FIN & "," & CStr(CantidadDias%))
    Resultado$ = "OK"
...
'    End If
'
'    BusquedaHuecos_LimiteInferior = LIMITEINFERIOR& + 1
...
Sub BusquedaHuecos_StoreProcedure()
    ...
    & " @inicio int," & vbCrLf _
    & " @fin int," & vbCrLf _
    & " @cantidaddiasbusqueda int" & vbCrLf _
    & " as" & vbCrLf _
    & " begin" & vbCrLf _
    & " declare @resultado nvarchar(max)" & vbCrLf _
    & " declare @fechamin datetime" & vbCrLf _
    & " set @resultado = '' " & vbCrLf _
    & " set @fechamin = dateadd(dd,datediff(dd,0,getdate()),0)-@cantidaddiasbusqueda" & vbCrLf
    ...
    & "  end" & vbCrLf _
    & " " _
    & "  select * from (select t1.numero AS Comprobantes FROM HUECOS_EN_FACTURA as t1 LEFT OUTER JOIN (select numecomp from CAJABANC with(nolock) where opcimovim=@comprobante and varimovim=@letra and numetalo=@talonario and fechconta>@fechamin) as t2 ON (t1.numero=t2.NumeComp) WHERE t2.NumeComp IS NULL) t3" & vbCrLf _
    & " " _
    & " end" & vbCrLf _
    & " else" & vbCrLf _
    & " begin" & vbCrLf _
...
'                      End If
                      ...
                      If Abs(INOGRAD1#) >= 1 And FECHCO% >= FECHANUM("01/07/17") Then
                        Call REPLA(ZZ$, MKD$(BASECALC# * SIG%), 137, 8)
                      End If
                  End If
