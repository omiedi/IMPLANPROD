...
'EN EL SETUP CONFIGURAR EL CONTROL DEL FORMULARIO DE IMPRESION
  ...
  If TRIM$(FORIPRE$) <> "" Then
    ...
    Dim AETMP As ADODB.Recordset
    Set AETMP = New ADODB.Recordset
    AETMP.OPEN FILTSQL$ (SQLX$), FLEXCONN, adOpenKeyset, adLockReadOnly, adCmdText
    With AETMP
      On Error Resume Next
      .MoveFirst
      If Err Then
...
Private Sub Label14_Change()
   ...
   If NUOCO& > 0 Then
     ...
     Set COMPLETEMP = New ADODB.Recordset
     COMPLETEMP.OPEN FILTSQL$ (SQLX$), FLEXCONN, adOpenKeyset, adLockReadOnly
     '
     With COMPLETEMP
       On Error Resume Next
       .MoveFirst
...
Private Sub LICOPEN_DblClick()
  ...
  Set MARCTEMP = New ADODB.Recordset
  MARCTEMP.OPEN FILTSQL$ (SQLX$), FLEXCONN, adOpenKeyset, adLockReadOnly, adCmdText
  '
  Y$ = REGBLAN$("AUTOENT")
  With MARCTEMP
    .MoveFirst
  ...
  Dim AUTOTEMP As ADODB.Recordset
  Set AUTOTEMP = New ADODB.Recordset
  AUTOTEMP.OPEN FILTSQL$ (SQLX$), FLEXCONN, adOpenKeyset, adLockReadOnly, adCmdText
  '
  Y$ = REGBLAN$("AUTOENT")
  With AUTOTEMP
    JJJ& = 0
...
'      SQLX$ = SQLX$ + " AND Sald_Aten > 0.005 "
      ...
      '
      SQLX$ = "Update AutoEnt SET Stat_Aten = 9 "
      SQLX$ = SQLX$ + " WHERE Nume_Ocom = " & NUOCO& & ""
      SQLX$ = SQLX$ + " AND Stat_Aten < 9 "
      SQLX$ = SQLX$ + " AND Cod_Inte= " & CI% & ""
      SQLX$ = SQLX$ + " AND Sald_Aten > 0.005 "
      ...
      For J& = 1 To ULTREG&("!AUTOENT")
         ...
         Call ABRECOMPRAS
         SQLX$ = "SELECT * FROM AUTOENT"
         Dim AUTOTEMP1 As ADODB.Recordset
         Set AUTOTEMP1 = New ADODB.Recordset
         AUTOTEMP1.OPEN FILTSQL$ (SQLX$), FLEXCONN, adOpenKeyset, adLockPessimistic, adCmdText

         'Set AUTOTEMP1 = FLEXCONN.Execute(SQLX$, 4)
         'With AutoEnt
         With AUTOTEMP1
...
Function PRONUAE&()
   ...
   Dim AutoEnTemp As ADODB.Recordset
   Set AutoEnTemp = New ADODB.Recordset
   AutoEnTemp.OPEN FILTSQL$ (SQLX$), FLEXCONN, adOpenKeyset, adLockReadOnly, adCmdText
   On Error Resume Next
   With AutoEnTemp
     .MoveFirst
     If Err < 1 Then
