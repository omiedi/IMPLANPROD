VERSION 5.00
Begin VB.Form FRALOT09 
   BackColor       =   &H00C0E0FF&
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Fracionamiento de Lotes"
   ClientHeight    =   7965
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   9345
   ...
   Begin VB.Frame Frame2 
      ...
      Begin VB.ListBox List1 
         ...
         EndProperty
         Height          =   960
         ItemData        =   "FRALOT09.frx":030A
         Left            =   600
         List            =   "FRALOT09.frx":030C
...
Private Sub Command13_Click()
   ...
   If XVALO(CIXI%) > 0 Then
      ...
      Call CommSel("!TAUBICA", Command13, Text3)
   End If
...
Private Sub Command2_Click()
   ...
   If TRIM$(Label39) = "" Then
      Call COMUNI("ATENCION: Falta Ubicación del Lote Original.")
   End If
   '
   If CIXI% < 1 Then
   ...
   If XVALO(Text1.TEXT) <= 0 Then ' Verifica que la cantidad de kilos sea valida
      Call MENSERR(24, "Kilos no Valido en Lote 1.")
      GoTo 99
   End If
   '
   If XVALO(Label32) <= 0 Then ' Verifica que la cantidad de kilos sea valida
      Call MENSERR(24, "Kilos no Valido en Lote 2.")
      GoTo 99
   End If
   '
   If TRIM$(Text3) = "" Then
   ...
   If CantRegistros("BALAREQMO", CONDI$, "NOMESS") > 0 Then
      Call COMUNI("Imposible Realizar Fraccionamiento de Este Lote\Deberá Primero Desasignar ")
      GoTo 99
   End If
   
   
   '
   UBICAIN$ = Mid$(TRIM$(UCase(Text3)), 1, 16)
   If UBICAIN$ <> "" Then
   ...
   End If
   '
   If XVALO(Text1) < 0.05 Then ' Verifica que la cantidad de kilos sea valida
      Call MENSERR(24, "Cantidad no Valida en Lote '" + Label6 + "'.")
      GoTo 99
   End If
   '
   If XVALO(Label32) < 0 Then ' Verifica que la cantidad de kilos sea valida
      Call MENSERR(24, "Cantidad no Valida en Lote Remanente.")
      GoTo 99
   End If
   '
   ...
   End If
   '
   DPRI$ = TRIM$(Str$(1 + XVALO(Mid$(VALOBADA("MOVISTO", "MAX(DOPRILAR)", "CodiMovi = 'FL'"), 4, 6))))
   While Len(DPRI$) < 6: DPRI$ = "0" + DPRI$: Wend
   DPRI$ = "FL-" + DPRI$
   HOII% = HOY(HOS$)
   DSEC$ = "FL-" + Mid$(HOS$, 7, 2) + Mid$(HOS$, 4, 2) + Mid$(HOS$, 1, 2)
   For KKI% = 0 To 99
      KKX$ = TRIM$(Str$(KKI%))
...
25 SQLX$ = "Select * from LOTINGRE WITH(NOLOCK) where IdenLote = '" & IDLOTX$ & "'" 'Selecciona el registro correspondiente al LOTE
   ...
   With LOTINGTMP
      If Not (.BOF And .EOF) Then
         ...
         DESTIN$ = ""     ' EL FRACCIONADO QUEDA SIN DESTINO - epb 2/4/2010    ' SAFETEXT$(!Cod_Desti)
         DEPX% = XVALO(!DEPOUBIC)
         GoTo 35
      End If
...
35 LOTINGTMP.Close
   ...
   Call MOVISTOK(FECHA%, CIXI%, TRIM$(Label27), "FL", DPRI$, DPRI$, DSEC$, DSEC$, "Fracc.Lote " + Label6, 0, 0, 0, 0, DEPX%, (-1) * XVALO(Text1))
   Call MOVISTOK(FECHA%, CIXI%, TRIM$(Label6), "FL", DPRI$, DPRI$, DSEC$, DSEC$, "Fracc.Lote " + Label27, 0, 0, 0, 0, DEPX%, XVALO(Text1))
   '
   Call CREALOTIN(FECHALTT%, FECHFABR%, FECHAVEN%, CIXI%, Label6, _
           MARCAX$, LOTEINGR$, XVALO(Text1), , , DEPX%, Text3, ORIGENIN$, _
           REMIPROX$, ENVASX$, STALIB$, XVALO(Label20), DESTIN$)
           '
   '
   ...
   Call ACSALOTE(CIXI%, TRIM$(Label6), "NOMESS")
...
Private Sub Label28_Change()
    ...
    Label43 = DEUNIMED$(CIXI%)
    Label12 = DEUNIMED$(CIXI%) + ":"
    Label5 = DEUNIMED$(CIXI%) + ":"
    Label23 = DEUNIMED$(CIXI%) + ":"
End Sub
...
20 Label6 = NCR1$                   'CARGA EL NUEVO SUBLOTE
   ...
   Label33 = Label40.Caption        'CARGA LA CANTIDAD DE SALDO
...
Private Sub Text1_Change()
   '
   CIXI% = CODINT(Label28)
   Label32.Caption = FORMATNUM$(XVALO(Label41.Caption) - XVALO(Text1.TEXT), "F13." & CNTDC$(CIXI%))  'RESTA AL LOTE ORIGINALLOS KILOS DEL NUEVO LOTE
   COEFI = XVALO(Label35)
   Label20 = FORMATNUM$(XVALO(Text1) * COEFI, "F13." & CNTDC$(CIXI%))
   Label33 = FORMATNUM$(XVALO(Label32) * COEFI, "F13." & CNTDC$(CIXI%))
...
'
Text1.TEXT = FORMATNUM$(Text1.TEXT, "F13." & CNTDC$(CIXI%)) ' FORMATEA EL TEXT1 "KILOS"
'
End Sub

Private Sub Text2_Change()
'
Label33.Caption = FORMATNUM$(XVALO(Label40.Caption) - XVALO(Text2.TEXT), "F13." & CNTDC$(CIXI%))  'RESTA AL LOTE ORIGINALLOS KILOS DEL NUEVO LOTE
If XVALO(Label33.Caption) < 0 Then
    Call MENSERR(24, "Cantidad Supera la del Lote Original")
End If
'
End Sub
...
'
Text2.TEXT = FORMATNUM$(Text2.TEXT, "F13." & CNTDC$(CIXI%)) ' FORMATEA EL TEXT2 "CANTIDAD"
'
End Sub
