...
Begin VB.Form FINFACTUR07 
   ...
   Begin VB.Frame Frame6 
      BackColor       =   &H00E2D3C0&
      Caption         =   "OBSERVACIONES EN FACTURA, NOTA DE DÉBITO Y NOTA DE CRÉDITO"
      BeginProperty Font 
         Name            =   "Arial"
         Size            =   9
         Charset         =   0
   ...
   Begin VB.Frame Frame2 
      ...
      Begin VB.CheckBox Check1 
         Appearance      =   0  'Flat
         BackColor       =   &H00E2D3C0&
         Caption         =   "Envíar Factura por  Mail "
         BeginProperty Font 
            Name            =   "Arial"
            Size            =   8.25
            Charset         =   0
      ...
      Begin VB.Label Label1 
         ...
         Left            =   2580
         TabIndex        =   74
         ToolTipText     =   "Doble Clic para Cambiar Dirección de Mail"
         Top             =   735
         Width           =   2925
      End
   ...
   Begin ACTIVESKINLibCtl.Skin Skin1 
      Left            =   0
      OleObjectBlob   =   "FINFACTUR07.frx":2B92
      Top             =   1680
   End
   Begin ACTIVESKINLibCtl.SkinLabel SkinLabel999 
      ...
      Index           =   0
      Left            =   60
      OleObjectBlob   =   "FINFACTUR07.frx":2DC6
      TabIndex        =   72
      Top             =   0
      Visible         =   0   'False
      Width           =   1485
    ...
    Do While Not EOF(NARCHI1%) ' RECORRE
       ...
       TX$ = Mid$(XX$, 1, Pos%)
          ...
          Select Case CONT%
           ...
           Case 6: DATO$ = TRIM$(Str(XVALO(FORFASQL.Label12))) ' Punto de Venta
           Case 7: DATO$ = TRIM$(FORFASQL.Text11)            ' Numero de comprobante inicial
           Case 8: DATO$ = TRIM$(FORFASQL.Total(6))          ' Importe Total
           ...
           Case 20:           '1)Fecha Comprobante,2)Fecha de comienzo de Servicio, 3)Fecha Fin de Servicio, Fecha de Vencimento
                    FECHA$ = FECHATEXLAR$(FECHANUM(FORFASQL.Text9))
...
Sub SOLICITARCAE_WSFEX()
    ...
    DEST_COMP$ = Left$(TRIM$(DATFAEX07.Text2.TEXT), 3)
    CUIT_PAIS$ = Left$(TRIM$(DATFAEX07.Text3.TEXT), 11)
    ...
    If FORFASQL.Label7.Caption = "N.Débito" Then S_N$ = ""
    If FORFASQL.Label7.Caption = "N.Crédito:" Then S_N$ = ""
    ...
    Do While Not EOF(NARCHI1%) ' RECORRE
       ...
       TX$ = Mid$(XX$, 1, Pos%)
          ...
          Select Case CONT%
           ...
           Case 4: DATO$ = TRIM$(Str(XVALO(FORFASQL.Label12))) ' Punto de Venta
           Case 5: DATO$ = TRIM$(FORFASQL.Text11)            ' Numero de comprobante inicial
           ...
           Case 18: DATO$ = TRIM$(FORFASQL.Total(6)) ' Importe Total
           ...
           Case 21:
              If COMP_ASOCIAD% > 0 Then
                ...
                Next J%
              End If
           Case 22: DATO$ = TRIM$(Left$(FORFASQL.Eco21(1), 50)) ' FORMA DE PAGO
           Case 23: DATO$ = INCOTS$ ' TIPO DE VENTA FOB,CIF,ETC
           FORFASQL.Text18 = INCOTS$ ' ACTUALIZO EL FRM
...
Private Sub Command1_Click()
  '\\\OT-07-0023-RG-24/01/07///
   ...
   DEP% = XVALO(Text7)
   '
   'VALIDACION DE DEPOSITO
   If VALIDEPOSI%(DEP%) < 1 Then
   ...
   If TRIM$(Control$("STOCKS", "CONEGATI")) = "SI" And FORFASQL.Label6.Caption <> "(T.Libre)" And FORFASQL.CONSICLI% < 1 And FORFASQL.TIFADOC% <= 3 Then
     If VALIDASTOCK(DEP%) < 1 Then
        Exit Sub
     End If
   End If
   '
   ...
   End If
   '
   'VALIDACION DE CANTIDAD DE COPIAS CUANDO SE TRATA DE UNA FACTURACION DE CONSIGNACION
   If FORFASQL.CONSICLI > 0 And XVALO(Text2(1)) > 0 Then
      Call MENSERR(24, "Imposible Modificar Cantidad de Copias de Remito\Cuando se Factura Consignaciones.\Si Desea Re-imprimir un Remito\Ingrese al Modulo de Remitos, Botón 'More', Botón 'Consulta y Re-Impresion'")
      Exit Sub
   End If
   '
   If Check1.Value = 1 Then
     ...
     If InStr(MAIX$, "@") < 1 Then
       Call COMUNI("Mail de Factura Inexistente o No Válido.\Si Desea Cancelar el Envio por Mail\Desactive la Casilla 'Enviar Factura x Mail'.")
       Exit Sub
     End If
   End If
   '
...
Private Sub Form_Load()
   ...
   '
'    Check1.Value = 0
'    If Control("FACTURA", "ENVFAMAIL") = "SI" Then
'      Check1.Visible = True
'      Check1.Value = 1
'       CONDI$ = "Nume_Cli =" & XVALO(FORFASQL.Text1)
'       MAIX$ = TRIM$(VALOBADA("DEUDOR12", "MAILFACTURA_COB", CONDI$, "NOMESS"))
'       If MAIX$ <> "" Then
'          Label1(14) = MAIX$
'       Else
'          Label1(14) = "Falta Correo Electrónico"
'          Label1(14).ToolTipText = "Configurar Correo de Mail de Cobranzas desde Maestros de Clientes"
'       End If
'    End If
   '
   'If InStr(EMPREAC$, "DOSIVAC") > 0 Then Check1.Visible = True
   '
   UTILIZA_SKIN% = 1
...
Private Sub Label1_DblClick(Index As Integer)
    ...
    If Index = 14 Then
         MAI_COB$ = Label1(14)
         MAILCOBRANZA$ = TRIM$(InputBox("Ingrese Mail de Recepción de Factura", "Mail", MAI_COB$))
         If MAILCOBRANZA$ = "" Then
              Check1.Value = 0
              Exit Sub
         End If
         '
         If InStr(MAILCOBRANZA$, "@") < 1 Then
         ...
         Check1.Value = 1
         Label1(14) = MAILCOBRANZA$
         Label1(14).ToolTipText = ""
         '
         Condi$ = "Nume_Cli =" & XVALO(FORFASQL.Text1)
         MailGuardado$ = TRIM$(VALOBADA("DEUDOR12", "MAILFACTURA_COB", Condi$, "NOMESS"))
         If UCase$(MailGuardado$) <> UCase$(MAILCOBRANZA$) Then
            OPX% = COMALTER("Se Ha Cambiado El Mail Configurado para el Envio de Comprobantes\\Utilizar Este por Defecto\Mantener el Pre-configurado")
            If OPX% = 1 Then
               Call ACTUREGISTRO("DEUDOR12", "MAILFACTURA_COB", Condi$, MAILCOBRANZA$, REGAF&, "NOMESS")
            End If
        End If
    End If
...
Private Sub Timer2_Timer()
   ...
   If KAE$ <> "" Then
      ...
      Text4 = "CAE: " + KAE$ + "................Id.Tran: " + IDX$ + vbCrLf + Text4
...
Function LEECAE$(FVENCIM$)
    ...
    YY$ = CODIBARRA$(XX$) ' SE REEMPLAZA POR ESTA FUNCION
...
Sub SOLICITARCAE()
    ...
    Do While Not EOF(NARCHI1%) ' RECORRE
       ...
       TX$ = Mid$(XX$, 1, Pos%)
          ...
          Select Case CONT%
           'Case 4: DATO$ = TRIM$(FORFASQL.Label18)          ' Cantidad de registros del detalle de la factura
           ...
           Case 12: DATO$ = TRIM$(Str(XVALO(FORFASQL.Label12))) ' Punto de Venta
           Case 13: DATO$ = TRIM$(FORFASQL.Text11)            ' Numero de comprobante inicial
           Case 14: DATO$ = TRIM$(FORFASQL.Text11)            ' Numero de comprobante hasta
           Case 15: DATO$ = TRIM$(FORFASQL.Total(6))          ' Importe Total
...
Function TIPO_DOC$()
    ' TIPO DE DOCUMENTO
    NCLI = XVALO(FORFASQL.Text1)
    POSDEIVA% = PIVACLI%(NCLI)
...
Function TIPO_CBTE$()
    ...
    TIPOCOMP$ = UCase$(FORFASQL.Label7 + FORFASQL.Label13)
    
    Select Case TIPOCOMP$
        Case "FACTURA:A": valor$ = "1": If FORFASQL.chkModoCredito.Value = 1 Then valor$ = "201"
        Case "N.DÉBITO:A": valor$ = "2": If FORFASQL.chkModoCredito.Value = 1 Then valor$ = "202"
        Case "N.CRÉDITO:A": valor$ = "3": If FORFASQL.chkModoCredito.Value = 1 Then valor$ = "203"
        Case "FACTURA:B": valor$ = "6": If FORFASQL.chkModoCredito.Value = 1 Then valor$ = "206"
        Case "N.DÉBITO:B": valor$ = "7": If FORFASQL.chkModoCredito.Value = 1 Then valor$ = "207"
        Case "N.CRÉDITO:B": valor$ = "8": If FORFASQL.chkModoCredito.Value = 1 Then valor$ = "208"
    End Select
    '
    TIPO_CBTE$ = valor$
    '
...
Function NRO_DOC$()
    NRO_DOC$ = ""
    POXIVA% = PIVACLI%(TRIM$(FORFASQL.Text1))
    '
    NUMERO_CUIT_DOC1$ = CuitCli$(TRIM$(FORFASQL.Text1))
    NUMERO_CUIT_DOC$ = REPLACAR(NUMERO_CUIT_DOC1$, "-", "")
    '
    If POXIVA% < 1 And TRIM$(NUMERO_CUIT_DOC$) = "" Then
        POS1% = InStr(FORFASQL.Text12, " - CUIT ")
        If POS1% > 0 Then
            POS1% = POS1% + 8
            CUITCP$ = TRIM$(Mid$(FORFASQL.Text12, POS1%))
        End If
        NUMERO_CUIT_DOC$ = CUITCP$
        'NUMERO_CUIT_DOC$ = "0"
    End If
    '
...
Function IMPTO_LIQ$()
  IMPTO_LIQ$ = ""
  IMPTOLIQUID$ = FORMATNUM$(XVALO(FORFASQL.Total(3)) + XVALO(FORFASQL.Total(4)) + XVALO(FORFASQL.Total(5)), "F12.2")
  IMPTO_LIQ$ = TRIM$(IMPTOLIQUID$)
  'ESTO SE AGREGA PARA QUE EN CASO QUE NO HAYA IVA AL .NET LE ENVIE UN CERO Y NO COMILLAS
  If IMPTO_LIQ$ = "" Then IMPTO_LIQ$ = "0"
End Function

Function IMP_TOT_CONC$()

   IMP_TOT_CONC$ = "" ' SUMA DE IVA NO GRAVADO + PERCEPCION DE IVA + PERCEPCION DE INGRESOS BRUTOS
   'IMPTOTACONC$ = FORMATNUM$(XVALO(FORFASQL.TLABEL28) + XVALO(FORFASQL.Text17) + XVALO(FORFASQL.Text15) + XVALO(FORFASQL.Text24), "F12.2")
   IMPTOTACONC$ = FORMATNUM$(XVALO(FORFASQL.TLABEL28), "F12.2")
...
Function IMP_IIBB$()
   ...
   IMP_IIBB$ = "" ' SUMA DE IVA NO GRAVADO + PERCEPCION DE IVA + PERCEPCION DE INGRESOS BRUTOS
   IMPTOTACONC$ = FORMATNUM$(XVALO(FORFASQL.Text26) + XVALO(FORFASQL.Text25) + XVALO(FORFASQL.Text24) + XVALO(FORFASQL.Text15), "F12.2")
   If TRIM$(IMPTOTACONC$) = "" Then IMPTOTACONC$ = "0"
   IMP_IIBB$ = TRIM$(IMPTOTACONC$)
   
End Function

Function IMP_PERC$()

   IMP_PERC$ = "" ' SUMA DE IVA NO GRAVADO + PERCEPCION DE IVA + PERCEPCION DE INGRESOS BRUTOS
   IMPTOTACONC$ = FORMATNUM$(XVALO(FORFASQL.Text17), "F12.2")
   If TRIM$(IMPTOTACONC$) = "" Then IMPTOTACONC$ = "0"
   IMP_PERC$ = TRIM$(IMPTOTACONC$)
...
Function IMP_NETO$()
   IMP_NETO$ = ""
   IMPORNETO$ = FORMATNUM$(XVALO(FORFASQL.Total(2)) - XVALO(FORFASQL.TLABEL28), "F12.2")
...
'!!! ESTA RUTINA FUE QUITADA DEL MODULO MOVISTO07_SQL
Function VALIDASTOCK(Optional DepOrig%)
   '
   If DepOrig% < 1 Then DepOrig% = Control$("", "DEPOEXPE")
   '
   CAITMO% = 0
   For i& = 1 To ULTREG&("!CARDEFAC")
       XX$ = REGLEIDO$("!CARDEFAC", i&)
       CI% = CVI(Mid$(XX$, 5, 2))
       CAN# = CVS(Mid$(XX$, 1, 4))
       If CAN# > 0.005 And ESTRAZABLE%(CI%) <> 2 Then
         CAITMO% = CAITMO% + 1
         CIMOVIM%(CAITMO%) = CI%
   ...
   Next i&
   '
'   DepOrig% = XVALO(FORFINFAC.Text7) 'Control$("", "DEPOEXPE")
   MODO$ = "" ' CONTROL DE VERIFICACIONES DE RESERVAS.
   If Control$("FACTURA", "VERIRESE") = "SI" Then MODO$ = "CONSIDERA_RESERVAS"
   AA = PUEDEMOVI%(DepOrig%, MODO$)
   '
   'VALIDA PARA LOS TRAZABLES POR N/SERIE SI EXISTE EL EQUIPO EN EL DEPOSITO
   For i& = 1 To ULTREG&("!CARDEFAC")
       XX$ = REGLEIDO$("!CARDEFAC", i&)
       CI% = CVI(Mid$(XX$, 5, 2))
       If ESTRAZABLE%(CI%) = 2 Then
         NUSERIE$ = TRIM$(Mid$(XX$, 139, 64))
         Condi$ = "COD_INTE = " & CI% & " AND DEPOMOVI = " & DepOrig% & " AND IDENLOTE = '" & NUSERIE$ & "' AND CANTINGRE-CANTSALID > 0"
         EXISTE_NUMEROSERIE& = CantRegistros&("MOVISTO", Condi$, "NOMESS")
         If EXISTE_NUMEROSERIE& < 1 Then
            Call COMUNI("Imposible Facturar. El Código : " & CODEXT$(CI%) & " N/Serie: " & NUSERIE$ & "\No Se encuentra en el Depósito: " & DepOrig%)
            AA = -1: GoTo 99
         End If
       End If
   Next i&
