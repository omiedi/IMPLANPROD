...
Begin VB.Form ETIQAHP 
   BackColor       =   &H00FFFFFF&
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Impresiòn de Etiquetas de items de Pedidos"
   ClientHeight    =   8850
   ClientLeft      =   -3210
   ClientTop       =   465
   ClientWidth     =   15135
   ...
   Begin VB.Frame Frame3 
      ...
      Begin VB.TextBox Text5 
         ...
         TabIndex        =   16
         TabStop         =   0   'False
         Text            =   "Detalle Pedido Seleccionado"
         Top             =   600
         Width           =   14860
      End
      ...
      Begin VB.CommandButton Command3 
         ...
         Left            =   13200
         TabIndex        =   8
         ToolTipText     =   "Imprmie las Etiquetas que tengan Cantidadad"
         Top             =   3840
         Width           =   1815
      End
      Begin VB.TextBox Text5 
         ...
         TabIndex        =   4
         TabStop         =   0   'False
         Text            =   "Pedidos del Cliente Activo"
         Top             =   4440
         Width           =   14880
      End
      Begin MSFlexGridLib.MSFlexGrid MSF1 
         ...
         Left            =   120
         TabIndex        =   5
         ToolTipText     =   "Doble Clic Para Completar Etiquetas a Imprimir"
         Top             =   960
         Width           =   14865
         _ExtentX        =   26220
         _ExtentY        =   4895
...
Sub Carga_Sit_General()
     ...
     SQLX$ = SQLX$ + "   PEDDETA.NuOrItem "
     SQLX$ = SQLX$ + " FROM PEDDETA WITH(NOLOCK)  "
     SQLX$ = SQLX$ + " WHERE  "
     SQLX$ = SQLX$ + "  NROCLIE = " & XVALO(TXTNUMCLI)
     SQLX$ = SQLX$ + "  ORDER BY PEDDETA.NROPED desc"
...
Private Sub Command2_Click()
    ...
    Dim obj As New RECORXET
    Set RS1 = obj.RS_PedidoxCte(XVALO(Me.TXTNUMCLI))
...
Private Sub Command3_Click()
 ...
 OCOM$ = TRIM$(TXTLABEL(2)): If OCOM$ = "" Then OCOM$ = "s/n"
 razonSocial$ = "AMERICAN HEAVY PARTS"
 NCLIE = XVALO(ClienteSegunNroPedido(XVALO(TXTNROPED)))
 NroTipoCliente% = XVALO(GRUCOCLI%(NCLIE)) 'grupo contable aqui definene el tipo de cliente
 If NroTipoCliente% = 5 Then
'    razonSocial$ = TRIM$(rasocli$(NCLIE))
     razonSocial$ = TRIM$(TEL2CLI$(NCLIE))
 End If
 If Len(razonSocial$) > 20 Then razonSocial$ = Left(razonSocial$, 20)
' razonSocial$ = CenterString(razonSocial$, 20, "-")  'CENTRA EL TEXTO AGREGANDOLE UN GUION PRINCIPIO Y FIN PARA QUE NO LO AFECTE EL TRIM
'Campos$ = "Código/a16;Descripción/A32;Q.Pedida/f9;Q.Entreg./F9;Orden/F7;Cant.Etiq./F10;Unidad/F10"
      For i& = 1 To MSF1.Rows - 1
        CANCOPIAS& = XVALO(MSF1.TextMatrix(i&, 6))
        CantUnid& = XVALO(MSF1.TextMatrix(i&, 7))
        If CantUnid& > 0 Then
          RegistrosValidos% = 1
          If CANCOPIAS& < 1 Then CANCOPIAS& = 1
          CodigoParte$ = TRIM(MSF1.TextMatrix(i&, 1))
          Descripcion = TRIM(MSF1.TextMatrix(i&, 2))
          Call PrintEtiqAhp(CodigoParte$, Descripcion, CantUnid&, XVALO(TXTNROPED), OCOM$, CANCOPIAS&, razonSocial$)
        End If
      Next i&
      If RegistrosValidos% < 1 Then
         Call COMUNI("No se Detectaron Items a Imprimir")
         GoTo 99
      End If
99
...
Sub PrintEtiqAhp(PARTE_CODIGO$, Descripcion, CANTI, NROPED&, NUOCO$, CANCOPIAS&, razonSocial$)
        Call SETPRINPORT("RESTORE")
        PRINTERPORT$ = ""
        PIMPRE$ = CONTROL$(IDENTER$, "PUERETIP")
        If PIMPRE$ = "" Then PIMPRE$ = CONTROL$("*", "PUERETIP") ' en SETUP DE PEDIDO
...
'        FILTX$ = LOTE$ & ";" & Codigo$ & ";" & Descripcion$ & ";" & RazonSocialProveedor$ & ";" & TRIM$(FORMATNUM$(METROSREALES$, "F12.1")) & ";" & PESOENKG$ & ";" & NORFA$ & ";" & USERNAME$ & ";" & FECHATEX$(FECHAGENERACION%)
        FILTX$ = PARTE_CODIGO$ & ";" & Descripcion & ";" & TRIM$(FORMATNUM$(CANTI, "F10.0")) & ";" & TRIM$(FORMATNUM$(NROPED&, "F10.0")) & ";" & NUOCO$ & ";" & razonSocial$
        FORIPRE$ = "ETPE-AHP"
...
Private Sub Form_Load()
   Campos$ = "N.Ped./F8;Código/a16;Descripción/A24;Q.Ped/F9;Q.Entr./F9;F.Sol./D8"
   Call CARGA_ENCABEZADO(MSF2, Campos$, Me)
   Campos$ = "Código/a16;Descripción/A32;Q.Pedida/f9;Q.Entreg./F9;Orden/A0;Cant.Etiq./F10;Cant.Unid./F10"
...
Private Sub TXTNROPED_Change()
     ...
     Campos$ = "Código/a16;Descripción/A32;Q.Pedida/f9;Q.Entreg./F9;Orden/A0;Cant.Etiq./F10;Cant.Unid./F10"
     Call CARGA_ENCABEZADO(MSF1, Campos$, Me)
     TXTLABEL(2) = OrdenCompraPed$(XVALO(TXTNROPED))
...
Sub CARGAGRILLADETALLE(NROPED&)
   ...
   Dim obj As New RECORXET
   Set RS1 = obj.RS_DetaPedidoAhp(NROPED&)
   Call Carga_MSF_Recordset(RS1, Campos$, MSF1, 1)
   Set obj = Nothing
   RS1.Close
   Set RS1 = Nothing
   
   For i& = 1 To MSF1.Rows - 1
      MSF1.TextMatrix(i&, 6) = 1
      MSF1.TextMatrix(i&, 7) = MSF1.TextMatrix(i&, 3)
   Next
   
   Call numerargrilla(MSF1, 0)
   MSF1.ColWidth(0) = 600
