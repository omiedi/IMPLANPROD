...
Begin VB.Form ANURECIBO1 
   Caption         =   "Anula Recibos"
   ClientHeight    =   4995
   ClientLeft      =   60
   ClientTop       =   450
   ClientWidth     =   9945
   ...
   Begin ACTIVESKINLibCtl.Skin Skin1 
      Left            =   0
      OleObjectBlob   =   "ANURECIBO1.frx":0000
      Top             =   0
   End
...
Private Sub Form_Load()
    Call APLICAR_SKIN1(Me, App.Path & "\SKINS\SteelBlue.skn")
...
Private Sub MSF_DblClick()
   ...
   SUPAGO$ = TRIM$(FORMATNUM$(XVALO(MSF.TextMatrix(MSF.Row, 9)), "F12.2"))
   '
   TX$ = "CONFIRMA ANULAR RECIBO: " & DOCU$ & vbCrLf
   TX$ = TX$ + "DEL ALUMNO: " & TEXT2 & vbCrLf
   TX$ = TX$ + "POR EL IMPORTE ORIGINAL DE : $ " & TRIM$(FORMATNUM$(VALOR_CUOTA_ORIGINAL#, "F12.2"))
   If MsgBox(TX$, vbYesNo, "Confirmación de Anulación") <> vbYes Then Exit Sub
   '
   CONDI$ = "CODICOM_LAR = '" & DOCU$ & "' AND LEGAJO = " & LGJO&
'   CONDI$ = CONDI$ + " AND VARIMOVI = '" & VARMOV$ & "'"
   ...
   If REGAF& > 0 Then
      ...
      TX$ = "ANULACION DE RECIBO: " & DOCU$ & vbCrLf
      TX$ = TX$ + "POR EL IMPORTE ORIGINAL DE : $ " & TRIM$(FORMATNUM$(VALOR_CUOTA_ORIGINAL#, "F12.2")) & vbCrLf
      TX$ = TX$ + "CON UN PAGO DE : $ " & SUPAGO$
...
Sub CARGA_RECIBOS()
   ...
   CONDI$ = "LEGAJO = " & LEGAJO& & " AND (VARIMOVI = 'RCOB' OR VARIMOVI = 'COBMATRI')"
   ...
   MSF.Redraw = False
   '
   SQLX$ = "SELECT * FROM CAJABANC_COL WITH(NOLOCK)"
...
Sub ENCABEZA_GRILLA()
    ...
    NUCOLU% = 2: TEPRUEBA = "10/10/201200": MSF.ColWidth(NUCOLU% - 1) = TEPRUEBA.Width: MSF.ColAlignment(NUCOLU% - 1) = 1: MSF.TextMatrix(0, NUCOLU% - 1) = "Fecha Emis."
    NUCOLU% = 3: TEPRUEBA = "1234567890000000": MSF.ColWidth(NUCOLU% - 1) = TEPRUEBA.Width: MSF.ColAlignment(NUCOLU% - 1) = 1: MSF.TextMatrix(0, NUCOLU% - 1) = "Documento"
    ...
    NUCOLU% = 6: TEPRUEBA = "1234567890123456": MSF.ColWidth(NUCOLU% - 1) = TEPRUEBA.Width: MSF.ColAlignment(NUCOLU% - 1) = 1: MSF.TextMatrix(0, NUCOLU% - 1) = "Concepto"
