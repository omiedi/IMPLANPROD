...
Private Sub Text1_DblClick()
    ...
    Text1 = TRIM$(NUSEX$)
    If NUSEX$ = "" Then
    If VALIDANUMEROSERIE%(NUSEX$) > 0 Then
    ...
    Else
       ...
       If OPX% = 1 Then
         Call GRABAR_NSEREIE(NORFA$, NUSEX$)
       End If
End Sub
         ...
         With RS
            Do While Not .EOF
             ...
             Else
                ...
                End If
                '
                If LOTE$ = LOTE2$ Then GoTo 80 'POR QUE SI ES EL MISMO LOTE DUPLICA
                '                               'INFORMACION
                SQLX2$ = "SELECT * FROM CERTIFICADOS WITH(NOLOCK)" '**TABLA CERTIFICADOS***QUE SE GRABA EN ABMCERTIFICADOS
                SQLX2$ = SQLX2$ + " WHERE " & CONDI2$
                Set RST = READSET(SQLX2$)
