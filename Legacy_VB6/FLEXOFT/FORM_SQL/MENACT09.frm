...
Begin VB.Form MENACT09 
   BackColor       =   &H00C0C0C0&
   Caption         =   "FLEXOFT - Sistemas de Activos Fijos"
   ClientHeight    =   8445
   ClientLeft      =   60
   ClientTop       =   315
   ClientWidth     =   11880
   Icon            =   "MENACT09.frx":0000
   LinkTopic       =   "Form1"
   ScaleHeight     =   8445
   ScaleWidth      =   11880
   ...
   Begin ComctlLib.ImageList imlSmallIcons 
      ...
      BeginProperty Images {0713E8C2-850A-101B-AFC0-4210102A8DA7} 
         NumListImages   =   6
         BeginProperty ListImage1 {0713E8C3-850A-101B-AFC0-4210102A8DA7} 
            Picture         =   "MENACT09.frx":F1802
            Key             =   "closed"
         EndProperty
         BeginProperty ListImage2 {0713E8C3-850A-101B-AFC0-4210102A8DA7} 
            Picture         =   "MENACT09.frx":F1D24
            Key             =   "cylinder"
         EndProperty
         BeginProperty ListImage3 {0713E8C3-850A-101B-AFC0-4210102A8DA7} 
            Picture         =   "MENACT09.frx":F2246
            Key             =   "leaf"
         EndProperty
         BeginProperty ListImage4 {0713E8C3-850A-101B-AFC0-4210102A8DA7} 
            Picture         =   "MENACT09.frx":F2768
            Key             =   "open"
         EndProperty
         BeginProperty ListImage5 {0713E8C3-850A-101B-AFC0-4210102A8DA7} 
            Picture         =   "MENACT09.frx":F2C8A
            Key             =   "smlBook"
         EndProperty
         BeginProperty ListImage6 {0713E8C3-850A-101B-AFC0-4210102A8DA7} 
            Picture         =   "MENACT09.frx":F32EC
            Key             =   ""
         EndProperty
      EndProperty
   End
End
Attribute VB_Name = "MENACT09"
...
Sub ABRETABLATIPACT()
    '
    Call TRALOCAL("TIPOACTI", "")
    VTB% = VENTABU%("TIPOACTI")
    If VTB% < 0 Then Exit Sub
    '
    J& = 0
    For U& = 1 To ULTREG&("!TIPOACTI")
        X$ = REGLEIDO$("!TIPOACTI", U&)
        CODTIP% = CVI(Left$(X$, 2))
        If CODTIP% > 0 Then
            J& = J& + 1
            Call GRAREG("!TIPOACTI", X$, J&)
        End If
    Next U&
    '
    Call TRACENTRAL("TIPOACTI", "")
    Call SETULTREG("TIPOACTI", J&)
    Call CIERRARCH("TIPOACTI")
    Call CIERRARCH("!TIPOACTI")
...
Sub RECALCUTOT()
    '
    'CALCULO TOTAL DE TODOS LOS ITEMS SEGUN M
10  OBX$ = OBJPLA$("ENTRAFECHA1", VDEF$)
    If OBX$ = "" Then Exit Sub
    '
    Screen.MousePointer = 11
    fe% = CVI(Mid$(OBX$, 1, 2))
    DESDE = FETEXCOR1$(FECHANUM("01/01/80"))
    HASTA = FETEXCOR1$(fe%)
    '
    SQLX$ = "SELECT CODIGO_EQU FROM EQUIPOS WITH(NOLOCK)"
    SQLX$ = SQLX$ + " WHERE VALORIGPESOS_EQU > 0 "
    SQLX$ = SQLX$ + " AND FECHAACTIV_EQU >= '" & DESDE & "'"
'    SQLX$ = SQLX$ + " AND FECHAACTIV_EQU <= '" & HASTA & "'"
    Dim EQUITMP As New ADODB.Recordset
    Set EQUITMP = FLEXCONN.Execute(FILTSQL$(SQLX$))
    With EQUITMP
      Do While Not .EOF
       CODIMAQ$ = SAFETEXT$(!CODIGO_EQU)
       Call CALCUVALRES(CODIMAQ$, fe%)
      .MoveNext
      Loop
    End With
    EQUITMP.Close
    ...
    '
    Call COMUNI("Proceso de Recalculo Terminado")
    '
End Sub
...
Private Sub Form_Load()
   ...
   Call ABRECONEXION
   Call ABREMAQUI
   Call CREACAMPO("MAQUINARIAS", "EQUIPOS", "ValOrigPesos_Equ", 7, 0)
   Call CREACAMPO("MAQUINARIAS", "EQUIPOS", "FechaActiv_Equ", 8, 0)
   Call CREACAMPO("MAQUINARIAS", "EQUIPOS", "PlazoAmort_Equ", 3, 0)
   Call CREACAMPO("MAQUINARIAS", "EQUIPOS", "ValorResid_Equ", 7, 0)
   Call CREACAMPO("MAQUINARIAS", "EQUIPOS", "AniosTransc_EQU", 3, 0)
   Call CREACAMPO("MAQUINARIAS", "EQUIPOS", "Moneda_Equ", 3, 0)
   Call CREACAMPO("MAQUINARIAS", "EQUIPOS", "DeprecAnual_Equ", 6, 0)
   Call CREACAMPO("MAQUINARIAS", "EQUIPOS", "DeprecAnualFisc_Equ", 6, 0)
   Call CREACAMPO("MAQUINARIAS", "EQUIPOS", "TipoBien_Equ", 3, 0)
   Call CREACAMPO("MAQUINARIAS", "EQUIPOS", "ValOrig_Equ", 7, 0)
   Call CREACAMPO("MAQUINARIAS", "EQUIPOS", "FechCalc_Equ", 8, 0)
   Call CREACAMPO("MAQUINARIAS", "EQUIPOS", "ValResCalc_Equ", 7, 0)
   Call CREACAMPO("MAQUINARIAS", "EQUIPOS", "PlazoAmortFisc_Equ", 3, 0)
   Call CREACAMPO("MAQUINARIAS", "EQUIPOS", "ValorResidFisc_Equ", 7, 0)
   Call CREACAMPO("MAQUINARIAS", "EQUIPOS", "ValResCalcFisc_Equ", 7, 0)
   Call CREACAMPO("MAQUINARIAS", "EQUIPOS", "FechaVta_Equ", 8, 0)
   Call CREACAMPO("MAQUINARIAS", "EQUIPOS", "ImpVta_Equ", 7, 0)
   '
   ...
   Call CARGAMENU
   '
   Call COPYSTRU("INDOPORT", "INCARPIM")
   '
   Visible = True
   Screen.MousePointer = 1
   '
...
Sub AGREGANODO(CODPA$, CODIJ$, DESIJ$)
    '
    On Error Resume Next
    If CODIJ$ = "ROOT" Then
    ...
    End If
    If CODPA$ = "ROOT" Or CODIJ$ = "ROOT" Then
       nodX.EnsureVisible       ' Muestra SOLO LOS NODOS RAIZ
    End If
    On Error GoTo 0
End Sub
'
Sub CARGAMENU()
    ...
    '
    Call AGREGANODO("", "ROOT", "Sistema FLEXOFT de Activo Fijo")
        '
        Call AGREGANODO("ROOT", "ARCHIVOS", "Archivos Maestros")
            Call AGREGANODO("ARCHIVOS", "EQUPRO09", "Equipos de Producción")
            Call AGREGANODO("ARCHIVOS", "AMASTO07", "Maestro de Items de Stock")
            Call AGREGANODO("ARCHIVOS", "AMAPRO", "Maestro de Proveedores")
        '
        Call AGREGANODO("ROOT", "CALCUTOTAL", "ReCalculo General")
        Call AGREGANODO("ROOT", "VTAEQUI", "Venta de Equipos")
        Call AGREGANODO("ROOT", "REVTAEQU", "Revertir Venta de Equipos")
        '
        Call AGREGANODO("ROOT", "LISTADOS", "Reportes, Consultas y Listados")
           Call AGREGANODO("LISTADOS", "LISTTCAL", "Todos Agrupados por Grupo ")
           Call AGREGANODO("LISTADOS", "LISTXACAL", "Por Tipo ")
           Call AGREGANODO("LISTADOS", "LISTAPCAL", "Por Planta ")
           Call AGREGANODO("LISTADOS", "LISTACAL", "Por Seccion ")
           Call AGREGANODO("LISTADOS", "BIENVTA", "Bienes Vendidos ")
           '
        '
        Call AGREGANODO("ROOT", "CONFIGUR", "Herramientas de Configuracion")
            Call AGREGANODO("CONFIGUR", "TABLAMON", "Tabla de Monedas")
            Call AGREGANODO("CONFIGUR", "TIPOACT", "Tabla de Tipo de Activos")
        '
End Sub
...
Private Sub Form_Resize()
   ...
   Command6.Top = Picture4.Top - Command6.Height
   Command6.Visible = True
   Picture4.Visible = True
   If Picture4.Top < Command6.Top + Command6.Height Then Command6.Top = Picture4.Top - Command6.Height
...
Private Sub Treeview1_NodeClick(ByVal Node As ComctlLib.Node)
   ...
   '
   If execonn$ = "AMAPRO" Then
         Call CONECRUN("ARCHIG07/AMAPRO", "Padron Maestro de Proveedores")
       ElseIf execonn$ = "AMACLI" Then
         Call CONECRUN("ARCHIG07/AMACLI", "Padron Maestro de Clientes")
       ElseIf execonn$ = "AMASTO07" Then
         Call CONECRUN("ARCHIG07/AMASTO", "Padron Maestro de Artículos")
       ElseIf execonn$ = "TABLAMON" Then
         Call ABRETABLAMON
       ElseIf execonn$ = "TIPOACT" Then
         Call ABRETABLATIPACT
       ElseIf execonn$ = "LISTTCAL" Then ' "Valor Calculado por Grupo
         Call REPCALGRU
       ElseIf execonn$ = "LISTXACAL" Then 'Valor Calculado Por Tipo
         Call REPCALTODOS
       ElseIf execonn$ = "LISTAPCAL" Then 'Valor Calculado Por Planta
         Call REPCALPLAN
       ElseIf execonn$ = "LISTACAL" Then 'Valor Calculado Por Seccion
         Call REPCAPLAN
       ElseIf execonn$ = "CALCUTOTAL" Then
         Call RECALCUTOT
       ElseIf execonn$ = "VTAEQUI" Then
         Call VENTAEQUIPO
       ElseIf execonn$ = "REVTAEQU" Then
         Call REVERTIR
       ElseIf execonn$ = "BIENVTA" Then
         Call BINESVENDIDOS
...
Sub NUEVACARPIMP()
    ...
    NUOPO$ = PROXICARP$
    If NUOPO$ <> "" Then
       CONDI$ = "CODCARPIMP = '" & NUOPO$ & "'"
       If CantRegistros("CARPEIMP", CONDI$) < 1 Then
          Call CreaRegistro("CARPEIMP", "CODCARPIMP", NUOPO$, "NOMESS")
          HOII% = HOY(HOS$): FEALTA = FETEXCOR1$(HOII%)
          Call ACTUREGISTRO("CARPEIMP", "FECHALTA", CONDI$, FEALTA, RAFE&, "NOMESS")
          Call ACTUREGISTRO("CARPEIMP", "STATUS", CONDI$, 0, RAFE&, "NOMESS")
          Call STDFORM("CARPIMPO", NUOPO$)
       End If
    End If
    '
End Sub
...
Sub ABRECARPIMP()
    '
    CONDI$ = "CodcarpImp <>'' AND STATUS < 9 ORDER BY CodCarpImp"
    Call CARGALISEL("INCARPIM", "CARPEIMP", "CodcarpImp/A12;FechAlta/d8;RefeCarpImp/A48", CONDI$, "NOMESS")
    Call SELECHO("INCARPIM/INDICE DE CARPETAS DE IMPORTACION")
    
    CODIOPO$ = TRIM$(VALACT1$("INCARPIM"))
    If CODIOPO$ <> "" Then
       Call STDFORM("CARPIMPO", CODIOPO$)
    End If
    '
End Sub
...
Sub ANULCARP()
    '
    OPX% = COMALTER%("Opciones de Trabajo:\\Anular Carpetas\Revertir Anulacion")
    If OPX% = 1 Then
        CONDI$ = "CodcarpImp <>'' AND STATUS < 9 ORDER BY CodCarpImp"
        Call CARGALISEL("INCARPIM", "CARPEIMP", "CodcarpImp/A12;FechAlta/d8;RefeCarpImp/A48", CONDI$, "NOMESS")
        Call SELECHO("INCARPIM/INDICE DE CARPETAS DE IMPORTACION")
        ...
        CODIOPO$ = TRIM$(VALACT1$("INCARPIM"))
        If CODIOPO$ <> "" Then
           CONDI$ = "CODCARPIMP = '" & CODIOPO$ & "'"
           DESOPO$ = TRIM$(VALOBADA("CARPEIMP", "REFECARPIMP", CONDI$, "NOMESS"))
           TTXX$ = "Está Absolutamente Seguro de Anular\la Carpeta de Importación\'" + CODIOPO$ + " - " + DESOPO$ + "' ?"
           If COMALTER%(TTXX$ + "\\No, Conservar\Si, Anularla") = 2 Then
              Call ACTUREGISTRO("CARPEIMP", "STATUS", CONDI$, 9, RAFE&, "NOMESS")
           End If
        End If
     ElseIf OPX% = 2 Then
        CONDI$ = "CodcarpImp <>'' AND STATUS >= 9 ORDER BY CodCarpImp"
...
10    COTERMI$ = CONTROL$(ITERMI$, "COTERMI")
   ...
   '
   OTX$ = "IX-______%"
   CAMPOSE$ = "MAX(CAST(SUBSTRING(CodCarpImp,4,6) AS Integer))"
   CONDI$ = "CODCARPIMP LIKE '" & OTX$ & "'"
   NUCONSU& = 1 + XVALO(VALOBADA("CARPEIMP", CAMPOSE$, CONDI$))
   '
20 NEXTNU$ = TRIM$(Str$(NUCONSU&))
   While Len(NEXTNU$) < 6: NEXTNU$ = "0" + NEXTNU$: Wend
   PROXICARP$ = "IX-" + NEXTNU$ + "-" + COTERMI$
   '
End Function
     
Sub REVERTIR()
  '
   If DERACCE%("REVERTIR", "Revertir Venta de Bienes") < 2 Then
    Exit Sub
   End If
  '
   Call ABREMAQUI
   Call PoneEnCeroNull("EQUIPOS", "ImpVta_Equ", REGAF&, "NOMESS")
   Call PoneEnCeroNull("EQUIPOS", "FechaVta_Equ", REGAF&, "NOMESS")
   
   CONDI$ = "ImpVta_Equ > 0"
   Call CARGALISEL("INDIEQU1", "EQUIPOS", "CODIGO_EQU/A6;DESCRI_EQU/A40;CodLar_Equ/A16", "CODIGO_EQU <> '' ORDER BY CODIGO_EQU ASC")

   Call CARGALISEL("INDIEQU1", "EQUIPOS", "CODIGO_EQU/A6;DESCRI_EQU/A40;CodLar_Equ/A16", CONDI$, "NOMESS")
10 Call SELECHO("INDIEQU1")
   CODIMAQ$ = TRIM$(VALACT1$("INDIEQU1"))
   '
   If CODIMAQ$ <> "" Then
        CONDI$ = "CODIGO_EQU =  '" & CODIMAQ$ & "'"
        Call ACTUREGISTRO("EQUIPOS", "FechaVta_Equ", CONDI$, CVDAT(0), REGAF&, "NOMESS")
        Call ACTUREGISTRO("EQUIPOS", "ImpVta_Equ", CONDI$, 0, REGAF&, "NOMESS")
        Call COMUNI("Proceso Completado")
   End If
End Sub
Sub VENTAEQUIPO()

   If DERACCE%("VENTAEQUI", "Venta de Bienes") < 2 Then
    Exit Sub
   End If
   '
   Call ABREMAQUI
   ...
   Call PoneEnCeroNull("EQUIPOS", "FechaVta_Equ", REGAF&, "NOMESS")
   '
   CONDI$ = "(ImpVta_Equ <= 0 OR ImpVta_Equ IS NULL) ORDER BY CODIGO_EQU ASC"
   Call CARGALISEL("INDIEQU1", "EQUIPOS", "CODIGO_EQU/A6;DESCRI_EQU/A40;CodLar_Equ/A16", CONDI$)
   
   Call SELECHO("INDIEQU1")
   '
   CODIMAQ$ = VALACT1$("INDIEQU1")
   If CODIMAQ$ <> "" Then
     Call STDFORM("VTAACTJO", CODIMAQ$)
   End If
   '
End Sub
