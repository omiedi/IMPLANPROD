...
102 RESELE.Close
   ...
   '
'   If ESTRAZABLE%(CIXI%) = 2 Then 'Nº de Serie Inexistente
'      If NSERIE$ <> "" Then
...
Private Sub Form_Load()
    ...
    End If
    Call DETECTAORDENREPARA
    '
End Sub
Sub DETECTAORDENREPARA()
    'RUTINA PARA EVITAR REDUNDANCIA, YA QUE SE UTILIZA EN VARIAS OCACIONES
    '
    Screen.MousePointer = 11
    FIN& = ULTREG&("PRESREPA")
    For UI& = 1 To FIN&
      ...
      If CVS(Mid$(XX$, 125, 4)) < 0 Then
        ...
        NPX& = CVS(Left$(XX$, 4))
        ORDEREPA07.Label18 = TRIM$(Str$(NPX&))
        ORDEREPA07.Text2 = TRIM$(Str$(Abs(CVS(Mid$(XX$, 125, 4)))))
        ORDEREPA07.Text2.Enabled = False
        Screen.MousePointer = 1
        ORDEREPA07.Show
        ORDEREPA07.Text2.Enabled = True
        Screen.MousePointer = 11
      End If
   Next UI&
   Screen.MousePointer = 1
...
Private Sub MARGENREP()
  '
  If DERACCE%("MARGEREP", "Margen de Reparaciones") < 2 Then Exit Sub
  '
  Call DESHASFECHA(DES%, HAS%, PERIO$)
  '
  If TRIM$(PERIO$) = "" Then Exit Sub 'SI CANCELO
  ...
  FIN& = ULTREG&("PRESREPA")
  JJ& = 0
  COEF# = XVALO(CONTROL("REPARA", "COEFPREA"))
  For U& = 1 To FIN&
    Call TRACE(20, U&, FIN&)
    XX$ = REGLEIDO$("PRESREPA", U&)
    FEIN1% = CVI(Mid$(XX$, 65, 2))   ' FECHA DE RECEPCION
    NUOVEN1 = CVS(Mid$(XX$, 125, 4)) ' NUMERO DE PEDIDO
    '
    If NUOVEN1 < 1 Then GoTo 10
    '
    If FEIN1% >= DES% Then
      If FEIN1% <= HAS% Then
        JJ& = JJ& + 1
        NUREP1& = CVS(Mid$(XX$, 1, 4))
        REFE$ = Mid$(XX$, 5, 60)
        INSUMOS# = CVS(Mid$(XX$, 113, 4))
        HORASMOBRA# = CVS(Mid$(XX$, 101, 4))
        MAOBRA# = CVS(Mid$(XX$, 109, 4))
        COSTOPRE# = INSUMOS# + MAOBRA#
        '
        ' COSTO SEGUN COSTO DE REPOSICION
        TOTALCOS# = 0
        RFF$ = RECFILT$("REPUREPA", 1, MKS(NUREP1&))
        For II& = 1 To Len(RFF$) Step 4
            REG& = CVS(Mid$(RFF$, II&, 4))
            YY$ = REGLEIDO$("REPUREPA", REG&)
            CIXI% = CVI(Mid$(YY$, 7, 2))
            '
            'CALCULA EL COSTO A PARTIR DEL PRECIO UNITARIO MENOS EL COEFICIENTE
            COSTOCIXI# = CVS(Mid$(YY$, 13, 4))
            If COEF# > 0.05 Then COSTOCIXI# = COSTOCIXI# / COEF#
            TOTALCOS# = TOTALCOS# + COSTOCIXI#
        Next II&
        '
        MARGEN# = COSTOPRE# - TOTALCOS#
        '
        YY$ = REGBLAN$("MARGEREP")
        '
        Call REPLA(YY$, MKS$(NUREP1&), 1, 4)
        Call REPLA(YY$, REFE$, 5, 60)
        Call REPLA(YY$, MKD$(INSUMOS#), 65, 8)
        Call REPLA(YY$, MKD$(MAOBRA#), 73, 8)
        Call REPLA(YY$, MKS$(HORASMOBRA#), 81, 4)
        Call REPLA(YY$, MKD$(COSTOPRE#), 85, 8)
        Call REPLA(YY$, MKD$(TOTALCOS#), 93, 8)
        Call REPLA(YY$, MKD$(MARGEN#), 101, 8)
        '
        Call GRAREG("MARGEREP", YY$, JJ&)
      End If
    End If
10 Next U&
  '
  Call SETULTREG("MARGEREP", JJ&)
  Call CIERRARCH("MARGEREP")
  Screen.MousePointer = 1
  '
  If JJ& > 0 Then
    Call HEADERS("MARGEREP", "")
    Call HEADERS("MARGEREP", "Período: " & PERIO$)
    Call FINAL("*MARGEREP")
  End If
  '
End Sub
