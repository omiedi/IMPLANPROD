...
Private Sub Form_Load()
   ...
   Text5.TEXT = COTERMI$
   Call CARGARCOMBOUSERTEL(ITARFE17.Combo1)
   RESPO$ = COTERMI$
...
Sub CARLISTAPEN()
   ...
   For i% = 1 To Line1.Count - 1
        ...
        Unload Label8(NUPIC%)
        Unload Label9(NUPIC%)
        Unload Label10(NUPIC%)
        Unload Label11(NUPIC%)
        Unload Label13(NUPIC%)
        Unload Label15(NUPIC%)
        Unload Label17(NUPIC%)
   Next i%
   VScroll1.Max = (Picture4.Height - Picture3.Height) / 10 + 1
   On Error GoTo 0
...
Private Sub Label15_Change(Index As Integer)
    Label6(Index) = ""
    If TRIM$(Label15(Index)) = "" Then Exit Sub
    ASAS = TRIM$(VALOBADA("CORTRAB", "Descripcion", "Nume_Ot = '" & TRIM$(Label15(Index)) & "'", "NOMESS"))
...
Private Sub Label6_Change(Index As Integer)
   AAAA = Label6(Index)
End Sub
