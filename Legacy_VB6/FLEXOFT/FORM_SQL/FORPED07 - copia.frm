...
Begin VB.Form FORPED07 
   ...
   Begin ACTIVESKINLibCtl.Skin Skin1 
      Left            =   0
      OleObjectBlob   =   "FORPED07.frx":8904
      Top             =   1800
   End
   Begin ACTIVESKINLibCtl.SkinLabel SkinLabel999 
      ...
      Index           =   0
      Left            =   300
      OleObjectBlob   =   "FORPED07.frx":8B38
      TabIndex        =   126
      Top             =   0
      Visible         =   0   'False
      Width           =   1485
   ...
   Begin VB.Menu OC 
      ...
      Begin VB.Menu GENESOLICITUD 
         Caption         =   "Generar Solicitud de Compra del Pedido Activo"
      End
      Begin VB.Menu mnuctoc 
         Caption         =   "Control de Generación de O.Compra a Partir de un Pedido"
      End
...
Private Sub GENESOLICITUD_Click()
        NROPEDIDO& = XVALO(Text11)
        If NROPEDIDO& < 1 Then GoTo 99
        '
        NCLI = XVALO(Text1)
        If NCLI < 1 Then
          Call COMUNI("Falta N.Cliente")
          Exit Sub
        End If
        
        Call SELECHO("PROVED1")
        NPRO% = XVALO(TRIM$(VALACT1$("PROVED1/Selección de Proveedor")))
        If NPRO% < 1 Then Exit Sub
        
        '
        Call GENERA_TABLA_COMPRAS_DE_PEDIDOS
        '
        'ORDENO POR LA FILA DE PROVEEDOR
         ...
         msf2.Sort = 7
         '
         OC_GENERADA% = VALIDAR_OCOMP_GENERADA%(NROPEDIDO&)
         If OC_GENERADA% > 0 Then
            GoTo 99
         End If
         '
          NORD% = 0: NPROV_ANT& = 0: VUELTA% = 0
          SOLCOMPRA& = ProxNumSolCompraPedido&
          SQLX$ = "SELECT * FROM PEDICOMPRA "
          SQLX$ = SQLX$ + " Where NuClien = " & NCLIE
          SQLX$ = SQLX$ + " AND NROPED =   " & NROPEDIDO&
          JJ& = 0
          Dim RS As ADODB.Recordset
          Set RS = New ADODB.Recordset
25        On Error Resume Next
          RS.Open FILTSQL$(SQLX$), FLEXCONN, adOpenKeyset, adLockReadOnly, adCmdText
          If Err Then  'VALIDACION DE ERROR POR SI GENERA ERROR POR TIEMPO DE ESPERA
             On Error GoTo 0
             Call CapturaErrorOpen
             GoTo 25
...
Private Sub mnuctoc_Click()
    Call GENERA_TABLA_COMPRAS_DE_PEDIDOS
    CONDI$ = "NROPED > 0   ORDER BY NROPED DESC"
10  Call CARGALISEL("INDICEP2", "PEDENCA", "NROPED/F6.0;NroClie/F6.0;RasoClie/A24", CONDI$, "DISTINCT")
    Call SELECHO("INDICEP2/INDICE DE PEDIDOS")
    NROPEDIDO& = XVALO(VALACT1("INDICEP2"))
    If NROPEDIDO& < 1 Then Exit Sub
    '
    '
...
50  Call CARGA_ENCABEZADO(FRMZELECHO.MSF1, "Cta.Prov/F9;R.Social/A40;O.Compra/F12;N.Solic./F8.0", FRMZELECHO, , , ANTOT)
    ...
    If ANTOT < 5500 Then ANTOT = 5500
    FRMZELECHO.Width = ANTOT + 1100
    FRMZELECHO.Caption = "Generación de Ordenes de Compra - Pedido N°: " & NROPEDIDO&
    '
    'PRESENTO TODAS LAS SOLICITUD DE Compras RELACIONADAS CON EL PRESUPUESTO
  I& = 0
    SQLX$ = "SELECT DISTINCT(Num_Prov),NumSolCompra FROM PEDICOMPRA WITH(NOLOCK)"
    SQLX$ = SQLX$ + " WHERE Nroped = " & NROPEDIDO&
    ...
    If I& < 1 Then
       Call COMUNI("No se Detectaron Solicitud de Compras Para El Pedido Seleccionado")
       GoTo 10
    End If
    FRMZELECHO.Show 1
    '
    ...
    '
    If NPROV& > 0 Then
       APLI$ = "CPRE" & NPROV& & "_" & NROPEDIDO&
       Call CONECRUN("ORCOMP07/" + APLI$, "Generacion de Ordenes de Compra x Pedido")
    Else
       GoTo 10
    End If
    '
...
Private Sub mnuvidetasol_Click()
    ...
    CONDI$ = "NroPEd > 0   ORDER BY NroPEd DESC"
10  Call CARGALISEL("INDICEP2", "PEDENCA", "NroPEd/F6.0;NroClie/F6.0;RasoClie/A24", CONDI$, "DISTINCT")
    Call SELECHO("INDICEP2/INDICE DE PRESUPUESTOS")
    NROPEDIDO& = XVALO(VALACT1("INDICEP2"))
    If NROPEDIDO& < 1 Then Exit Sub
    '
    Call GENERA_TABLA_COMPRAS_DE_PEDIDOS
    ...
    If ANTOT < 5500 Then ANTOT = 5500
    FRMZELECHO.Width = ANTOT + 1100
    FRMZELECHO.Caption = "Generación de Ordenes de Compra - Pedido N°: " & NPRES&
    '
    'PRESENTO TODAS LAS SOLICITUD DE Compras RELACIONADAS CON EL PRESUPUESTO
    I& = 0
    SQLX$ = "SELECT DISTINCT(Num_Prov),NumSolCompra FROM PEDICOMPRA WITH(NOLOCK)"
...
10  CONDI$ = "NROPED > 0  ORDER BY NROPED DESC"
    Call CARGALISEL("INDICEP2", "PEDENCA", "NROPED/F6.0;NroClie/F6.0;RasoClie/A24", CONDI$, "DISTINCT")
    Call SELECHO("INDICEP2/INDICE DE PRESUPUESTOS")
    NROPED& = XVALO(VALACT1("INDICEP2"))
    ...
    If ANTOT < 5500 Then ANTOT = 5500
    FRMZELECHO.Width = ANTOT + 1100
    FRMZELECHO.Caption = "Anulación de Solicitud de Compras - Pedido N°: " & NPRES&
    '
    'PRESENTO TODAS LAS SOLICITUD DE Compras RELACIONADAS CON EL PRESUPUESTO
30  I& = 0
...
50  FRMZELECHO.Show 1
    ...
    If NSOLIC& > 0 Then
        ...
        If OPX% = 2 Then
           ...
           Call BORRAREGISTROS("PEDICOMPRA", CONDI$, REGAF&, "NOMESS")
