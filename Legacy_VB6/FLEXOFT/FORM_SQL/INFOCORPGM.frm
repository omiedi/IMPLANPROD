...
Begin VB.Form INFOCORPGM 
   ...
   Begin VB.Frame Frame5 
      BackColor       =   &H00E2D3C0&
      Caption         =   "Items Terminados a Consumir para este Código"
      BeginProperty Font 
         Name            =   "Arial"
         Size            =   7.5
         Charset         =   0
      ...
      Begin VB.TextBox Text17 
         ...
         Width           =   540
      End
      Begin MSFlexGridLib.MSFlexGrid MSFPT 
         Height          =   1635
         Left            =   120
         TabIndex        =   147
         Top             =   720
         ...
         EndProperty
      End
      Begin MSFlexGridLib.MSFlexGrid MSFCF 
         Height          =   1635
         Left            =   7920
         TabIndex        =   151
         Top             =   720
   ...
   Begin VB.Frame Frame3 
      BackColor       =   &H00E2D3C0&
      Caption         =   "Informes de Retiros de Corte de Orden de Fabricación Activa"
      BeginProperty Font 
         Name            =   "Arial"
         Size            =   7.5
         Charset         =   0
   ...
   Begin ACTIVESKINLibCtl.Skin Skin1 
      Left            =   420
      OleObjectBlob   =   "INFOCORPGM.frx":04D1
      Top             =   0
   End
   Begin ACTIVESKINLibCtl.SkinLabel SkinLabel999 
      ...
      Index           =   0
      Left            =   0
      OleObjectBlob   =   "INFOCORPGM.frx":0705
      TabIndex        =   41
      Top             =   315
      Visible         =   0   'False
      Width           =   1590
   ...
   End
End
Attribute VB_Name = "INFOCORPGM"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False

Sub AgregarModificarUso(Reg&, codiconjunto$, CodiConjYDescripcion$, LOTE$, NOPE%, CantCortes, CI1%, Descrip$, Codigo$)
...
Sub CargaConsumosDeProdTerminados(CI1%, NORFA$)
      'Call CARGA_ENCABEZADO(MSFCF, "CI/a0;IDMOV/A0;Fecha/D8;Código/A12;Descripción/A20;Lote/A10;Cantidad/F6", Me)
  MSFCF.Rows = 1: Reg& = 0
  SQLX$ = "select COD_INTE,COD_EXTE,DESCRIP,MOVISTO_ID, FECHMOV, IDENLOTE, CANTSALID from MOVISTO  WITH(NOLOCK) where DOPRILAR = '" & NORFA$ & "' "
  SQLX$ = SQLX$ + " and CODIMOVI = 'CF' AND ID_OPERACION = 9999"
  Set rs = READSET(SQLX$)
  ...
  With rs
    Do While Not .EOF
       Reg& = Reg& + 1
       MSFCF.Rows = Reg& + 1
       CIELEM% = XVALO(!cod_inte)
       MSFCF.TextMatrix(Reg&, 1) = CIELEM%
       MSFCF.TextMatrix(Reg&, 2) = XVALO(!MOVISTO_ID)
       MSFCF.TextMatrix(Reg&, 3) = FECHATEX$(CVINT(!FechMov))
       MSFCF.TextMatrix(Reg&, 4) = SAFETEXT$(!Cod_Exte)
       MSFCF.TextMatrix(Reg&, 5) = SAFETEXT$(!Descrip)
       MSFCF.TextMatrix(Reg&, 6) = SAFETEXT$(!idenlote)
       MSFCF.TextMatrix(Reg&, 7) = XVALO(!CANTSALID)
       .MoveNext
    Loop
  End With
...
'        MSF1.Row = i  ' Selecciona la fila
'        If XVALO(TXTRESTAN.TEXT) > 0 Then
'            OPX% = COMALTER%("Faltan\\No, No Imprimir\Sí, Imprimir")
'            If OPX% < 2 Then GoTo 99
'
'            Exit For ' Detiene el recorrido si encuentra que aún falta
'        End If
'    Next i&
   NORFA$ = TRIM$(Label4)
   If NORFA$ = "" Then MSF1.Rows = 1: Exit Sub
   CI1% = CODIFAB%(NORFA$)
   CantiPendiente = CANT_OF_PENDIENTEDECIERRE(CI1%, NORFA$)
   CantiaCerrar = XVALO(InputBox("Ingrese Cantidad de Cierre", CantiPendiente))
   If CantiaCerrar > 0.05 Then
      OPX% = COMALTER%("Confirmar Cierre de Orden Fabricación\\Cancelar\Sí, Cerrar")
      If OPX% < 2 Then GoTo 99
   End If
   If OPX% > 1 Then
      Call ENRAPRODUCTO_TERMINADO(CI1%, NORFA$, CantiaCerrar, HOY(HO$), DEPOENTRA%, "Fabricación Propia", "")
      OPX% = COMALTER%("ATENCION: Desea Cerrar la Orden de Fabricación?.\\Mantener Abierta\Cerrar")
      If OPX% = 2 Then
              CONDI$ = "IdSubOr = '" & NORFA$ & "'"
              FECHAHOY = Now
...
Private Sub Command2_Click()
    ...
    '
     ...
     '
     DEPOCORTE% = XVALO(Control("", "DEPODEST"))
     If CantiEnMetros# > SaldoLote(CIELEM%, TRIM$(TXTLOTE), DEPOCORTE%) Then
        Call MENSERR%(24, "Cantidad de metros informados supera el saldo Transferido de este lote al Depósito de Corte.")
        GoTo 99
     End If
...
'     End If
    ...
    IdMovistoGenerado = 0
'    If (InStr(CODFAMI$, "BB") > 0 Or InStr(CODFAMI$, "EE") Or InStr(CODFAMI$, "CC") Or InStr(CODFAMI$, "VC")) And InStr(EPAC$, "PERGAMINO") > 0 Then  'ESTA ES A PARTIR DE LA CUAL SE VA CONSIDRERAR DIFERENTE LA DEVOLUCION DE LA BOBINA
    If InStr(EPAC$, "PERGAMINO") > 0 Then   'cambiado para que todo informe el consumo
        CMOV$ = "CF"
...
Private Sub Form_Load()
    ...
    Call CARGA_ENCABEZADO(MSF1, "Código/A16;Descripción/A28;Lote/A16;Cons.Mts./a0;CI/a0;ID_OPERACION/A7;Desc.Oper/A12;Corte/F7.1;Cant.Cortes x U./F7", Me)
'    Call CARGA_ENCABEZADO(MSF2, "Código/A16;Descripción/A28;Lote/A8;Cant.Cortes/F10.1;CI/a0;N°Op./F5;Descrip/A12;Hora/A0;Material/A16;Ubicación/A12;NroInf/a0;IDUbic/a0;N°Operario/F9;Nombre/A16;Peso/f8.2;IdReg/a0", Me)
    ...
    Call CARGA_ENCABEZADO(MSFPT, "CI/a0;Código/A12;Descripción/A20;Uso/F6;Necesidad/F6;Informado/F6;Restan/F6", Me)
    Call CARGA_ENCABEZADO(MSFCF, "CI/a0;IDMOV/A0;Fecha/D8;Código/A12;Descripción/A20;Lote/A10;Cantidad/F6", Me)
    Text4 = FECHATEX$(HOY(HO$))
    Call CREATABLA_UBILOTE_ENVAP
    Text17 = XVALO(Control("PERGAMIN", "DEPCONPTE"))
End Sub
  ...
  With rs
    Do While Not .EOF
       ...
       MSF1.TextMatrix(Reg&, 6) = XVALO(!ID_OPERACION)
        CONDI$ = " CodIConj = " & XVALO(!cod_inte) & " AND NumeOper =" & XVALO(!ID_OPERACION)
        Descrioper$ = VALOBADA("SECOPER", "descoper", CONDI$, "NOMESS")
       MSF1.TextMatrix(Reg&, 7) = Descrioper$
       MSF1.TextMatrix(Reg&, 8) = XVALO(TXTCORTE)
       MSF1.TextMatrix(Reg&, 9) = TXTCANTICORTES
       .MoveNext
    Loop
 End With
 rs.Close
...
'   Me.Height = 7380
   ...
   Call CargaProductosTerminadosSinOperacion(CI1%, NORFA$) ' SOLO PERGAMINO
   Call CargaConsumosDeProdTerminados(CI1%, NORFA$)
...
End Sub
Sub CargaProductosTerminadosSinOperacion(CI1%, NORFA$)
  MSFPT.Rows = 1: Reg& = 0
  DEPOCORTE% = XVALO(Control("", "DEPODEST"))
  SQLX$ = "select * from coti1  WITH(NOLOCK) where CODINTCONJ =  " & CI1%
  SQLX$ = SQLX$ + " and componente = '' and corte < 1 and codcomponente < 1"
  Set rs = READSET(SQLX$)
  ...
  With rs
    Do While Not .EOF
       Reg& = Reg& + 1
       MSFPT.Rows = Reg& + 1
       CIELEM% = XVALO(!CODINTELEM)
       MSFPT.TextMatrix(Reg&, 1) = CIELEM%

       MSFPT.TextMatrix(Reg&, 2) = CODEXT$(CIELEM%)
       MSFPT.TextMatrix(Reg&, 3) = DESCRIT$(CIELEM%)
       MSFPT.TextMatrix(Reg&, 4) = XVALO(!moduxbb)
       MSFPT.TextMatrix(Reg&, 5) = XVALO(!moduxbb) * XVALO(Text14)  ' USO POR CANTIDAD (NECESIDAD TOTAL
       MSFPT.TextMatrix(Reg&, 6) = XVALO(VALOBADA("MOVISTO", "SUM(CANTSALID)", "CODIMOVI='CF' AND DOPRILAR = '" & NORFA$ & "' AND COD_INTE =" & CIELEM%))
       RESUL = XVALO(MSFPT.TextMatrix(Reg&, 5)) - XVALO(MSFPT.TextMatrix(Reg&, 6)): If RESUL < 0 Then RESUL = 0
       MSFPT.TextMatrix(Reg&, 7) = FORMATNUM$(RESUL, "F8")
       .MoveNext
    Loop
  End With
...
Private Sub MSF1_DblClick()
   ...
   CodiConjYDescripcion$ = Label2
   CODICONJ% = XVALO(TXTCODINT)
   codiconjunto$ = CODEXT$(CODICONJ%)
   Codigo$ = TXTCODELEM
   CI1% = CODINT(Codigo$)
...
'    ;55 CI/a0;N°Op./F5;66 Descrip/A12;77 Hora/A12;88 Material/A16;99 Ubicación/A10", Me)
    ...
    OPX% = COMALTER%("Puede Informar Una Ubicación en Sector \\Cancelar\Ubicación en Sector\Imprmir Etiqueta Para Pallet")
    If OPX% < 2 Then Exit Sub
    If OPX% = 2 Then
        NORFA$ = Label4 'o.f
        NOP% = XVALO(MSF2.TextMatrix(Reg&, 6)) 'numeoper
...
Private Sub MSFCF_MouseDown(Button As Integer, Shift As Integer, X As Single, Y As Single)
    Reg& = MSFCF.MouseRow
    COL& = MSFCF.MouseCol
    If Reg& = 0 Or Reg& >= MSFCF.Rows Then Exit Sub
    If Button = 2 Then
     Call SELECCIONARFILA(MSFCF, Reg&)
    
        
    
        OPX% = COMALTER%("Eliminar de modo permanente el consumo\\Cancelar\Eliminar")
        If OPX% < 2 Then Exit Sub
        If OPX% = 2 Then
           ID = MSFCF.TextMatrix(Reg&, 2)
           If ID > 0 Then
              CONDI$ = "MOVISTO_ID = " & ID
...
Private Sub MSFPT_DblClick()
    ...
    If Reg& = 0 Then
     ...
     Else
        ...
        MODODEORDEN$ = "DESC"
     End If
     Call ORDENAR_MSF(INFOCORPGM.MSFPT, COL%, MODODEORDEN$)
     Exit Sub
    End If
    
    
    If Reg& = 0 Or Reg& >= MSFPT.Rows Then Exit Sub
'    If COL% > 0 Then Exit Sub
    
    CI1% = 130 'MSFPT.TextMatrix(REG&, 1)
    CI1% = MSFPT.TextMatrix(Reg&, 1)
    If CI1% = 0 Then
      Call COMUNI("No se Detecto el Código")
      GoTo 99
    End If
   '
    DEPOCONSUMOSELEC% = XVALO(Text17)
    If DEPOCONSUMOSELEC% < 1 Then 'DEPOSITO SELECCIONADO
      Call COMUNI("Debe Seleccionar un Depósito de Consumo")
    End If
   
    TituloLista$ = "Lotes Disponibles en Dep.: " & TRIM$(Label32)
    ...
    SQLX$ = SQLX$ + " FROM MOVISTO WHERE COD_INTE = " & CI1%
    SQLX$ = SQLX$ + " AND DEPOMOVI = " & XVALO(Text17) 'DEPOSITO SELECCIONADO
    ...
    CantiDisponibleEnDeposito = XVALO(RESP_ZELE_VECT(2))
    
    CantidadAConsumir# = XVALO(InputBox("CANTIDAD A CONSUMIR", CantiDisponibleEnDeposito))
    If CantidadAConsumir# > 0 Then
       If CantidadAConsumir# > CantiDisponibleEnDeposito Then
          Call COMUNI("Cantidad Ingresada, supera la cantidad disponible en depósito: " & TRIM$(Label32))
          GoTo 99
       End If
       'si pasa consumo
        CMOV$ = "CF"
        REFE$ = "CONSUMO " & TRIM$(Text10)
        DOPRI$ = TRIM$(Label4)
        DOSEC$ = "PF-" + TRIM$(Mid$(DOPRI$, 4))
        NC = XVALO(Text6)
        Call MOVISTOK(HOY(HO$), CI1%, LOTE$, CMOV$, DOPRI$, DOPRI$, DOSEC$, DOSEC$, REFE$, 0, 0, 1, NC, DEPOCONSUMOSELEC%, (-1) * CantidadAConsumir#, XVALO(Text12), , , , , , , , , 9999, 0, 0)
        SQLX$ = "SELECT TOP 1 MOVISTO_ID FROM MOVISTO WITH(NOLOCK) ORDER BY MOVISTO_ID DESC"
        Set rs = READSET(SQLX$)
'        If Not rs.EOF Then
'          IdMovistoGenerado = XVALO(rs!MOVISTO_ID)
...
Private Sub Text17_Change()
   If Val(Text17) < 256 Then
     If Val(Text17) < 1 Or ECOARCH$("TADEPOSI", Chr$(Val(Text17))) = "" Then
         Label32 = "Todos los Depósitos"
       Else
         Label32 = ECOARCH$("TADEPOSI", Chr$(Val(Text17)))
         Call GRACONTROL("PERGAMIN", "DEPCONPTE", SAFETEXT$(Text17))
     End If
   End If
End Sub
