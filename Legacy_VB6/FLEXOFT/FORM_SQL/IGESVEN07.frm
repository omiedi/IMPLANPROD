...
Public Function CODIMES0%()
    CODIMES0% = CODIMES%
End Function
Public Function GRCV0%(ID As Integer)
    GRCV0% = GRCV%(ID)
End Function
...
'    Call CIERRARCH("DEUDOR2")
    ...
    RST.Open SQLX$, FLEXCONN, adOpenForwardOnly, adLockReadOnly, adCmdText
    With RST
        Do While Not .EOF
...
'    Call CIERRARCH("MASTER")
    ...
    RST1.Open SQLX$, FLEXCONN, adOpenForwardOnly, adLockReadOnly, adCmdText
    With RST1
        Do While Not .EOF
            CII% = XVALO(!CODINT)
...
15 MONECO% = 1
   ...
   For UI% = 1 To CAGRU%
     ...
     If TOVEN(NUGRU%(UI%)) > 1 Then
       ...
       Call REPLA(TTXX$, FORMATNUM$(100 * (TOVEN(NUGRU%(UI%)) - COSVEN(NUGRU%(UI%))) / TOVEN(NUGRU%(UI%)), "F8.1"), 85, 8)
     End If
     GRATABLA.List1(0).AddItem TTXX$ & Space(32) & CStr(NUGRU%(UI%))
     '
     TTXX$ = Space$(96)
     Call REPLA(TTXX$, DEGRU$(UI%), 1, 20)
     Call REPLA(TTXX$, FORMATNUM$(ACCAN(NUGRU%(UI%)), "F6.0"), 21, 6)
     ...
     If ACVEN(NUGRU%(UI%)) > 1 Then
       ...
       Call REPLA(TTXX$, FORMATNUM$(100 * (ACVEN(NUGRU%(UI%)) - ACCOS(NUGRU%(UI%))) / ACVEN(NUGRU%(UI%)), "F8.1"), 85, 8)
     End If
     GRATABLA.List1(1).AddItem TTXX$ & Space(32) & CStr(NUGRU%(UI%))
   Next UI%
   '
   TTXX$ = Space$(96)
   Call REPLA(TTXX$, "Total", 1, 20)
   ...
   If MODO% = 1 Then
     ATENTIA% = ATENTI%
     GRATABLA.Command4.Visible = True
     GRATABLA.Command1.Visible = True
     GRATABLA.Label5.Visible = True
     GRATABLA.Show 1
     If ATENTI% <> ATENTIA% Then
       ATENTI% = ATENTIA%
       Call FRESHALL
...
19          TTXX$ = "Precio Unitario (en PESOS) para " + TRIM$(CDD$) + " en " + TRIM$(REMIFAC$)
         ...
         COU = 0
         If CII% > 0 Then
            If UCase(TRIM$(MESAN)) = "DICIEMBRE 13" And (CII% = 3902 Or CII% = 3946 Or CII% = 3952 Or CII% = 3997 Or CII% = 3954) Then
                'A = CODEXT$(CII%)
                If MODOCALCOS% = 2 Then
            ...
            If COREGIS#(CII%) > 0.00005 Then
               ...
               Else
                 ...
                 End If
            End If
44          If REG& < 0 Then COU = 0          ' REPARACIONES
         End If
         '
...
28  Next JII%
    ...
    Screen.MousePointer = 1
    '
    Call COMUNI("Proceso de Cálculo Finalizado")
    '
Exit Sub
    '
' ***********************************************************
...
Sub GENERAFACTUR(DES%, HAS%)
    ...
    Call COPYSTRU("FACTUR", "FACTUR1")
    Call SETULTREG("FACTUR1", 0)
    ...
    On Error GoTo 0
    '
    Call SETULTREG("FACTUR1", REG&)
    Call CIERRARCH("FACTUR1")
    '
End Sub

Sub GENERAMOVISTO(DES%, HAS%)

    Call COPYSTRU("MOVISTO", "MOVISTO1")
    Call SETULTREG("MOVISTO1", 0)
    
    DES1% = DIASANTES(DES%, 30)
    Desde = FETEXCOR1$(DES1%)
    Hasta = FETEXCOR1$(HAS%)
    REG& = 0
    
    SQLX$ = "SELECT * FROM MOVISTO WITH(NOLOCK) WHERE (FECHMOV >= '" & Desde & "' AND FECHMOV <='" & Hasta & "')"
    ...
    On Error GoTo 0
    '
    Call SETULTREG("MOVISTO1", REG&)
    Call CIERRARCH("MOVISTO1")
    '
End Sub
Sub GENERESU()
    '
    ...
    For U& = 1 To FIN&
      ...
      If CII% > 0 Then  ' excluye debitos y creditos libres y facturas de texto libre
        ABUSQUE$ = Mid$(Z$, 5, 2)
...
10  FEX = HOY(HO$)
    ...
    J& = 0
    For XI% = 1 To NUMAS
        If STATUIT%(XI%) > 0 Then
          If Abs(TOTCAN(XI%)) > 0.005 Then
            RELIB$ = REGBLAN$("REVEPRO")
            Call REPLA(RELIB$, MKI$(XI%), 1, 2)
            Call REPLA(RELIB$, MKS$(TOTVENTA#(XI%) / TOTCAN(XI%)), 3, 4)
            Call REPLA(RELIB$, MKS$(TOTCAN(XI%)), 7, 4)
            Call REPLA(RELIB$, MKD$(TOTVENTA#(XI%)), 11, 8)
            J& = J& + 1
