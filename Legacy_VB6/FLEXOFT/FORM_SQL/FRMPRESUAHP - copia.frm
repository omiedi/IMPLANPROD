...
Sub Grabar_Presupuesto(OKX%, Optional UPDATE_AUTOMAT%)
   ...
   If UPDATE_AUTOMAT% > 0 Then MODIFICA% = 1
   If MODIFICA% < 1 Then
    NPP& = NUPRESUP&
...
5     TEXTO_TITULO$ = TRIM$(InputBox("Título", "Ingreso Título de Texto de Observaciones", TITUTEXT$))
      If TEXTO_TITULO$ = "" Then Exit Sub
      If CantRegistros&("TAB_TEXTO", "TITU_OBS  ='" & TEXTO_TITULO$ & "'", "NOMESS") > 0 Then
         Call COMUNI("Título Existente")
         GoTo 5
      End If
      '
      Call SETULTREG("!TEXTOBS2", 0)
8     VTB% = VENTABU%("TEXTOBS2/TITUPAN=" + TEXTO_TITULO$)
      If VTB% >= 0 Then
        EPAC$ = TRIM$(EMPREAC$)
        If InStr(UCase$(EPAC$), "AHP") > 0 Then
            ...
            If CODFOR$ = "PRRS-AHP" Then
                ...
                End If
                TEXTX$ = ""
                For U& = 1 To ULTREG&("!TEXTOBS2")
                  XX$ = REGLEIDO$("!TEXTOBS2", U&)
                  XX1$ = Mid$(XX$, 3, 96)
...
10      RNDX$ = RANDSTRING$(10)
    ...
    ElseIf OPX% = 2 Then
      ...
      For j% = 1 To UBound(CADENATEX$)
         ...
         Call REPLA(TXX1$, TROZO$, 3, 96)
         Call REGAPP("!TEXTOBS2", TXX1$)
      Next j%
      VTB% = VENTABU%("TEXTOBS2/TITUPAN=" + VALACT2$("LISTTITU"))
      TXXT$ = ""
      If VTB% >= 0 Then
        For I& = 1 To ULTREG&("!TEXTOBS2")
          XX$ = REGLEIDO$("!TEXTOBS2", I&)
          TXX1$ = TRIM$(Mid$(XX$, 3, 96))
...
'
    ...
    Call CARGA_ENCABEZADO(FRMPRESUAHP.MSF1, "Código/A22;Cod.Cte/A22;Descripción/A22;Q/F4.A;Prov/AO;Proveedor/A12;Costo (U$S)/F10.2;Pr.Venta/F8.2;Importe/F10.2;Via Apr./A10;LOTES/A0;CANTI/F0;NPROV/F0;KILOS/f12.2;DEMORA/F7", FRMPRESUAHP, , , , 5)
    '1.CODIGO
    '2.CODIGO CLIENTE
    '3.DESCRIPCION
    '4.CANTIDAD
...
'           MSF1.TextMatrix(I&, 7) = TRIM$(FORMATNUM$(XVALO(!COSTO_UNIT) / COTIMONEY, "F12.2"))
           ...
           MSF1.TextMatrix(I&, 14) = PESUNI(CI1%) * CANTI
...
'       End If
    ...
    Call RUTINACOMMAND13(1) 'GRABA EL PRESUPUESTO
    '
    On Error Resume Next
