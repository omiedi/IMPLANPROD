...
1410 NROOF$ = AJUSTI$(VALACT1$("ECORDEP"), 12)
     ...
     '
     Screen.MousePointer = 11
     ' SELECCIONA LAS OPERACIONES DE LA ORDEN ELEGIDA
     Call ABREORFAB
     SQLX$ = "SELECT * FROM OPERFAB WHERE IDSUBOR='" + TRIM$(NROOF$) + "' AND Observa='' ORDER BY NUMEOPER ASC"
     ...
     With OPERFABTMP
       ...
       On Error GoTo 0
       '
       ' ESTE ORDENAMIENTO SE REALIZA EN LA CONSULTA SQL
'       J% = 0
     ...
     '
        ...
        Do While .EOF = False
           ...
           If SALDO# > 0.05 Then
               ...
               CI% = !Cod_Inte
               NOPE% = !NUMEOPER
               DOPE$ = AJUSTI$(SAFETEXT(!DESCOPER), 38)
               CAINF# = SALDO#
               MAQUI$ = !MAQUINA
               FEOP% = CVINT(!COMIEPRO)
               'HOOP% = CVI(Mid$(XX$, 141, 2))
               MALIB$ = "S"
               TITOT = XVALO(!TIEMTOTA)
               '
               J% = J% + 1
...
1415 Call SELECHO("!OPERINF/Operaciones Programadas para O/F " + NROOF$)
     ...
     End If
     '
     OPERACION$ = VALACT2$("!OPERINF")
     NUMEOPE% = XVALO(Mid$(OPERACION$, 1, 3))
     SQLX$ = "SELECT * FROM OPERFAB WHERE IDSUBOR='" + TRIM$(NROOF$) + "' AND NumeOper=" + TRIM(FORMATNUM(NUMEOPE%, "I5"))
     Set OPERFABTMP =  FLEXCONN.EXECUTE (FILTSQL$ (SQLX$))
     With OPERFABTMP
       .MoveFirst
       CI% = !Cod_Inte
       ...
       Call REPLA(ZZ$, DOPE$, 17, 48)
         EQUIPO$ = SAFETEXT$(!MAQUINA)
       Call REPLA(ZZ$, MAQUINA$, 73, 6)
       Call REPLA(ZZ$, MKD$(CAPROD#), 79, 8)
       Call REPLA(ZZ$, MKD$(CAPROD#), 95, 8)
         MINPREP = XVALO(!TIEMPREP)
       Call REPLA(ZZ$, MKS$(MINPREP), 103, 4)
         MINFIJO = XVALO(!TIEMFIJO)
       Call REPLA(ZZ$, MKS$(MINFIJO), 107, 4)
         MINVARI = XVALO(!TIEMVARI)
       Call REPLA(ZZ$, MKS$(MINVARI), 111, 4)
         MINDEMO = XVALO(!TIEMDEMO)
       Call REPLA(ZZ$, MKS$(MINDEMO), 115, 4)
         MINLIMP = XVALO(!TIEMLIMP)
       ...
       Call REPLA(ZZ$, String$(16, 0), 135, 16)
         OBSERVA1$ = SAFETEXT$(!OBSERVA)
       Call REPLA(ZZ$, OBSERVA1$, 177, 6)
     End With
     '
     ' CODIGO ANTERIOR
...
' Marca como "Cumplida" Operación en OPERFAB.DAT
     ...
     '
     REALIZADO$ = TRIM(FORMATNUM(CAREAL#, "F10.1"))
     PENDIENTE$ = TRIM(FORMATNUM(SALREL#, "F10.1"))
     If PENDIENTE$ = "" Then PENDIENTE$ = "0"
     SQLX$ = "UPDATE OPERFAB SET CantReal=" + REALIZADO$ + ",CantPend=" + PENDIENTE$ + " WHERE IDSUBOR='" + TRIM(NROOF$) + "' AND NUMEOPER=" + Str(NOPE%)
     MsgBox SQLX$
...
' Genera la correspondiente Orden de Compra
     ...
     strsql = "Select * from OcomEnca where Nume_Ocom=" & Str(NUOCONUE&)
     ...
     strsql = "Select * from Ocomdeta where Nume_ocom=" & NUOCONUE&
