...
Sub SolicitudManualFacturaElectronicaComun(PuntoVenta As Integer)
        ...
        Select Case TipoCbte$
            ...
            Case "LQA": RecuperacionTipoComprobante%(0) = 63 'LQA
            Case "LQB": RecuperacionTipoComprobante%(0) = 64 'LQB
        End Select
...
Private Sub mnuFacturaElectronicaComun_Click()
    ...
    If PuntoVenta% > 0 Then
        ...
        RecuperacionPuntoVenta% = PuntoVenta%
        ReDim RecuperacionTipoComprobante%(8)
        ...
        RecuperacionTipoComprobante%(6) = 63 'LQA
        RecuperacionTipoComprobante%(7) = 64 'LQB
        RecuperacionModoCredito = False
        ReDim RecuperacionUltimoNumeroPorTipo&(8)
...
TXTRAZA$ = TXTRAZA$ & "PUNTO 69 = " & Now & vbCrLf
   ...
   '
'MsgBox "Enviar Captura de Pantalla: " + vbCrLf + CodigoQRAfip$
...
Sub PRINFACTU(NUFACTU$, NCLI, LETR$, MODO$)
    ...
    If Mid$(NUFACTU$, 1, 2) = "ND" Or Mid$(NUFACTU$, 1, 2) = "LQ" Then TIFADOC% = 3
    '
...
TXTRAZA$ = TXTRAZA$ & "PUNTO 59 = " & Now & vbCrLf
    ...
    If EXISTE%(ARCHIV$) > 0 Then
        On Error GoTo ControlEliminaConsultaCae
        Kill ARCHIV$
        On Error GoTo 0
    End If
    NARCHI2% = FILEOPEN%(ARCHIV$, 2, 128) ' ABRE EL ARCHIVO DE ESCRITURA
...
ControlEliminaConsultaCae:
    Call MENSERR(24, "Factura Electrónica Bs.Capital:\ \Problemas al Eliminar Archivo\CONSULTA_CAE_" & IDENCAE$)
    Exit Sub
...
ControlEliminaConsultaCae:
    Call MENSERR(24, "Factura Electrónica Común:\ \Problemas al Eliminar Archivo\CONSULTA_CAE_" & IDENCAE$)
    Exit Sub
...
10  If TOTACOPIAS% < 1 Then TOTACOPIAS% = XVALO(FINFACTUR07.Text2(0))
    ...
    NUFACTURA$ = Mid$(FILTX$, 1, 18)
    Select Case Mid$(NUFACTURA$, 1, 2)
        Case "FC": TipoComprobante$ = "Factura"
        Case "ND": TipoComprobante$ = "Nota de Débito"
        Case "NC": TipoComprobante$ = "Nota de Crédito"
        Case "LQ": TipoComprobante$ = "Liquidación"
        Case Else: TipoComprobante$ = "Comprobante"
    End Select
    Call STDFORM(ARCSQR$, FILTX$ & ";" & ORCOP$(TOTACOPIAS% - (CANTCOPIAS%)) & ";" & "QR_FACTURA" & ";" & TipoComprobante$, "PRINT", , ENVIAMAILFACTURA%)
...
Private Sub tmrRecuperaComprobanteDesdeAfip_Timer()
    ...
    If Inicializa = 0 Then
        ...
        If ModoSolicitudManual = True Then Campos$ = "Fecha/D10;Comprobante/A18;Cuit/A13;Cuenta/I6;Cliente/A36;Importe/G14.2;Cae/A20;Vto/A10;Manual/A6"
...
'                    & " and numetalo=" & XVALO(Mid$(REPLACAR$(CbteRecuperacionManual$, "X", "0"), 4, 4))
                    CondicionSql$ = "numecomp<0 and tipomovim='fven' " _
                    & " and codicom_lar='" & TRIM$(CbteRecuperacionManual$) & "'"
...
10          FRMZELECHO.Show 1
            ...
            If Cae$ <> "" Then
                ...
                If ModoSolicitudManual = True Then
                    If TRIM$(RESP_ZELE_VECT(9)) = "" Then
                        Call COMUNI("El Comprobante Elegido\No Coincide con el que se Intenta Recuperar.")
                        GoTo 10
                    End If
                End If
        ...
        If XVALO(Cae$) > 0 Then
            ...
            FRMZELECHO.MSF2.TextMatrix(J&, 3) = CuitCliente$
                ...
                If Len(CuitCliente$) < 11 Then
                    Cuit$ = TRIM$(CuitCliente$)
                End If
                CondicionSql$ = "(cuit_cli like '%" & Cuit$ & "%' or cuit_cli like '%" & CuitCliente$ & "%') and stat_cli>0 and gru_div=0"
            ...
            If ModoSolicitudManual = True Then
                If Abs(XVALO(ImporteTotalCbteRecuperacionManual$) - XVALO(ImporteTotal$)) < 0.005 Then
'                If FechaCbteRecuperacionManual$ = FechaFormateada$ And _
'                    CuitCbteRecuperacionManual$ = CuitCliente$ And _
'                    XVALO(ImporteTotalCbteRecuperacionManual$) = XVALO(ImporteTotal$)
                    
                    FRMZELECHO.MSF2.TextMatrix(J&, 9) = "*"
...
'         Dim BC_doc As New MSXML2.DOMDocument30
...
'         Dim EXPO_doc As New MSXML2.DOMDocument30
...
Function BuscaComprobanteAfipEnSistema(ComprobanteARecuperar$, FechaEmision$, CuitCliente$, ImporteTotal#, Cae$, VencimientoCae$, NroCliente&) As Boolean
    ...
    If Len(CuitCliente$) < 11 Then
        ...
        Cuit$ = TRIM$(CuitCliente$)
    End If
    ConsultaSql$ = "select nume_cli, raso_cli from deudor12 with(nolock) where (cuit_cli like '%" & Cuit$ & "%' or cuit_cli like '%" & CuitCliente$ & "%') and stat_cli>0"
...
Sub BuscaHuecosDeComprobantesEnSistema()
    ...
    Do While TiposProcesados% < UBound(RecuperacionTipoComprobante%)
            ...
            Select Case RecuperacionTipoComprobante%(TiposProcesados%)
                ...
                Case 63: Tipo$ = "LQ": LETRA$ = "A"
                Case 64: Tipo$ = "LQ": LETRA$ = "B"
            ...
            DESDE% = DIANTE(HOY(HO$), 100)
            n = 1
15:         FechaInicioBusqueda% = DIANTE(DESDE%, 180 * n)
16:         ConsultaSql$ = "select max(numecomp) as iniciobusqueda from cajabanc with(nolock) where " _
            & " nuclien>0 and tipomovim='fven' and opcimovim='" & Tipo$ & "' and varimovim='" & LETRA$ & "'" _
            & " and numetalo=" & CStr(RecuperacionPuntoVenta%) & " and fechconta>='" & FETEXCOR1$(FechaInicioBusqueda%) & "' and fechconta<='" & FETEXCOR1$(DESDE%) & "'"
...
'            Set RstInicio = READSET(ConsultaSql$)
            ...
            'MsgBox ConsultaSql$
            'MsgBox CStr(XVALO(RstInicio!iniciobusqueda))
            If XVALO(RstInicio!iniciobusqueda) > 0 Then
                INI = XVALO(RstInicio!iniciobusqueda)
            Else
                If n > 2 Then
                    If n = 3 Then
                        FechaInicioBusqueda% = FECHANUM("01/01/80")
                        n = n + 1
                        GoTo 16
                    Else
                        INI = 1
            ...
            On Error GoTo ErrorBusquedaStore
            Resultado$ = REMEVE07.BusquedaHuecos(Tipo$, LETRA$, CStr(RecuperacionPuntoVenta%), CLng(INI), CLng(FIN), 100)
            On Error GoTo 0
'            For i = INI To FIN
...
50:         TiposProcesados% = TiposProcesados% + 1
    Loop
    ...
    Else
        If ModoSolicitudManual = True Then
            RecuperaFactura% = COMALTER%("No se Pudo Recuperar Comprobante '" & Text6 & "'.\¿Desea Solicitar Nuevamente Autorización a AFIP?\\No, Continuar\Si, Solicitar")
            If RecuperaFactura% = 2 Then
                Text3 = ""
                If CantRegistros("cajabanc", "codicom_lar='" & Text6 & "' and tipomovim='fven'") > 0 Then
                    Call SOLICITACAE
                End If
...
ErrorBusquedaStore:
    ...
    Call MENSERR(24, "Error en la Busqueda de Huecos 'Store'")
    Exit Sub
