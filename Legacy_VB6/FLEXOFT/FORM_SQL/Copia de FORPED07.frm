    ...
    If FIN& > 0 Then
      For U& = 1 To FIN&
        ...
        If CI% > 0 And CI% <= NUMAS% Then
            ...
            DESCUXX$ = DESCOMBINA$(NCX%, CI%, MODODESC%): MODO% = MODODESC%
    ' MsgBox DESCUXX$ & "   " & MODO%
            If MODO% > 0 Then
...
Private Sub Command14_Click()
   ...
   If OKX% > 0 Then
     ...
     If CONPRIPE.Label1.Caption = "OK" Then
        ...
        End If
        '
        'SE COMENTO EL SIGUIENTE CODIGO POR QUE EN EL FORMULARIO CONPRIPE FALTA LA OPCION REPLICA POR MAIL
        If CONPRIPE.Check1.Value = 1 Then ' VERIFICA SI TIENE QUE REPLICAR LA ORDEN DE DESPACHO POR MAIL
           NCI% = XVALO(Text1)
           COCLI$ = VALOBADA("DEUDOR12", "CODI_CLI", "NUME_CLI = " & NCI%, "NOMESS")
           'PERMITE ENVIAR A PARTIR DE LA LISTA DE MAILS DE CONTACTOS
           Call CONTACTO_MAIL(COCLI$, NCI%) 'AGREGA A SENDBYMAIL LOS MAIL DE LOS CONTACTOS SELECCIONADOS
           If SENDBYMAIL$ = "" Then
              SENDBYMAIL$ = TRIM$(EMAILCLI$(NCI%))
           End If
...
'           SENDBYMAIL$ = EMAI$
           For Each PRX1 In Printers ' VERIFICA SI EXISTE LA IMPRESORA PDFWRITER
              If InStr(UCase$(PRX1.DeviceName), "PDFWRITER") > 0 Or InStr(UCase$(PRX1.DeviceName), "PDF4U") > 0 Then GoTo 25
           Next
...
Sub PRINCOMPO(CI%, CANTI)
        ...
        For KKJ% = 1 To CAIT%
          ...
          CANTX$ = FORMATNUM$(CACICO * CANTI, "F16.6")
          '
...
Private Sub Form_Load()
    ...
    If InStr(EPAC$, "DOSIVA") > 0 Or InStr(EPAC$, "MEDITEA") > 0 Or InStr(EPAC$, "GABAL") > 0 Then
...
Sub CARGALISTBARRA()
   ...
   For U& = 1 To FIN&
     ...
     Call REPLA(TTXX$, FECHATEX$(FENTRESOL%), 70, 8)
     '
...
Sub CARDACLI()
   ...
   If NC% > 0 Then
     ...
     If RASOCLI$(NC%) <> "" Then
       ...
       Label26 = CODICLI$(NC%) 'Carga el numero de cliente/prospect
       '
       '27/03/07 (OT-07-0133)
...
109 Next U&
    ...
    NTRA% = XVALO(Text19)
    If Text19.TEXT = "" Then
       If TRIM(NTRANCLI$(NC%)) = TRIM(LENTREGA(2)) Then NTRA% = NUTRANCLI%(NC%)
    End If
    '
    ...
    '
     If FORIPRE$ <> "" Then
      'Call PRINTDOC("FORPECLI")   ' PEDIDO DE CLIENTE
       If PUERDESPA$ <> "" Then Call SETPRINPORT(PUERDESPA$)
...
35 On Error Resume Next
   ...
   With PedEnca
     ...
     If CODCARPE$ <> "" Then ' Anotador de Carpeta de Exportaciones
        ...
        TTXX$ = Space(76)
        Call REPLA(TTXX$, "Cliente: " + Left$(RASOCLI$(NCLIE%), 64), 11, 56)
        Call AGREGA_ANOTACARPI(CODCARPE$, TTXX$)
...
26     On Error Resume Next
       ...
       If NUPRESU& < 0 Then
         ...
         If REPRESU& <= UREPREP& Then
           ...
           If CVI(Mid$(YY$, 67, 2)) = NCLIE% Then    ' Coincide cliente
...
'  With Pedenca
   ...
   End If
   '
   If EXEDELIMCC%(NCLIE%, TPED#, DIFER) > 0 Then
       LIMCCTE$ = TRIM$(FORMATNUM$(LCred_CCte(NCLIE%), "f12.2")): If LIMCCTE$ = "" Then LIMCCTE$ = "0.00"
...
20 If ULTREG&("!CARDETPE") < 1 Then
   ...
   For U& = 1 To FIN&
     ...
     If VALICODIENCLIENTE% > 0 Then
        NC% = XVALO(Text1)
        CONDI$ = "NROCLIE = " & NC% & " AND CODIINT = " & CI%
...
Sub GENORFA()
   ...
   For U& = 1 To ULTREG&("!CARDETPE")
     ...
     NC% = XVALO(Text1)     'toma numero del cliente
          ...
          If CAN > CANSTO + 0.05 Then
             ...
             If CAIT% > 0 Then
               ...
               If COMALTER%(TTXX$ + "\\Si, Generar O/F\No, Continuar") = 1 Then
                  ACONEC$ = "ORFABR07/ORFNUE" + Chr$(124) + CODE$ + Chr$(124) + TRIM$(Str$(CAN)) + Chr$(124) + TRIM$(Str$(NC%)) + Chr$(124) + TRIM$(Str$(NUMPEDI%)) + Chr$(124) + ORCOCLI$
...
Sub PEDIPEND()
    '
    NCLIE% = XVALO(Text1.TEXT)
    CONDICION$ = " NroClie =  " & NCLIE% & " AND Status < 8 AND CantEnt < (CanPed - 0.005) AND  Status <> 3 ORDER BY NroPed ASC"
    '
