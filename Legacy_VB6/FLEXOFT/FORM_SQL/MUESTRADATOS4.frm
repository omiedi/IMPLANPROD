...
Begin VB.Form MUESTRADATOS4 
   ...
   Begin VB.CommandButton Command2 
      ...
      Height          =   400
      Left            =   7920
      Picture         =   "MUESTRADATOS4.frx":0000
      TabIndex        =   7
   ...
   Begin ACTIVESKINLibCtl.Skin Skin1 
      Left            =   2625
      OleObjectBlob   =   "MUESTRADATOS4.frx":030A
      Top             =   0
   End
   Begin ACTIVESKINLibCtl.SkinLabel SkinLabel999 
      ...
      Index           =   0
      Left            =   120
      OleObjectBlob   =   "MUESTRADATOS4.frx":053E
      TabIndex        =   1
      Top             =   240
      Visible         =   0   'False
      Width           =   1485
   ...
   End
End
Attribute VB_Name = "MUESTRADATOS4"
...
Private Sub Command3_Click()
        ...
        If Command3.Caption = "Cargar Cuit" Then
             'VALIDACION
5            CUITX$ = TRIM$(InputBox$("Carga de Cuit a Comprobantes Seleccionados", "Ingrese Cuit", CUITX$))
             If CUITX$ = "" Then GoTo 99
             CuitFormatoNumerico = XVALO(SOLONUM$(CUITX$))
             If CuitFormatoNumerico < 1 And VALICUIT%(CUITX$) < 1 Then
                Call COMUNI("Dato Ingresado No Válido (" & CUITX$ & ")")
                GoTo 5
             End If
             CUITX$ = Mid$(CuitFormatoNumerico, 1, 2) & "-" & Mid$(CuitFormatoNumerico, 3, 8) & "-" & Mid$(CuitFormatoNumerico, 11, 2)
             
             For i& = 1 To MSF.Rows - 1
               If MSF.TextMatrix(i&, 0) = "*" Then
                 Comprobante$ = MSF.TextMatrix(i&, 1)
                 CONDI$ = "CODICOM_LAR = '" & Comprobante$ & "'  AND NUPROVE >  0"
                 Call ACTUREGISTRO("CAJABANC", "CUIT", CONDI$, CStr(CuitFormatoNumerico), REGAF&, "NOMESS")
                 Call COLOREAR_GRILLA_SOLAFILA(GRID, vbYellow, MSF.Row, 0, 0, 0) ' BLANQUEO LA FILA
                 MSF.TextMatrix(i&, 5) = CuitFormatoNumerico
               End If
             Next i&
        ElseIf Command3.Caption = "Cargar Desp." Then
10           Despa$ = TRIM$(InputBox$("Carga de Despacho de Exportación a Comprobantes Seleccionados", "Ingrese Despacho de Exportación", Despa$))

             For i& = 1 To MSF.Rows - 1
               If MSF.TextMatrix(i&, 0) = "*" Then
                 Comprobante$ = MSF.TextMatrix(i&, 1)
                 CONDI$ = "CODICOM_LAR = '" & Comprobante$ & "'  AND NUPROVE >  0"
                 Call ACTUREGISTRO("CAJABANC", "DespaImpo", CONDI$, Despa$, REGAF&, "NOMESS")
                 Call COLOREAR_GRILLA_SOLAFILA(GRID, vbYellow, MSF.Row, 0, 0, 0) ' BLANQUEO LA FILA
                 MSF.TextMatrix(i&, 5) = Despa$
               End If
             Next i&
        End If
        APROBO% = 1
