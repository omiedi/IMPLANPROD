...
Begin VB.Form IGESVEN 
   ...
   Caption         =   "Informe Ventas vs Costos"
   ClientHeight    =   8055
   ClientLeft      =   390
   ClientTop       =   375
   ClientWidth     =   7005
...
'    Call CIERRARCH("MASTER")
    ...
    On Error GoTo 0
    '
'    CONDI$ = "doprilar='RT-X-0001-00023486'"
'    Call ACTUREGISTRO("MOVISTO", "doseclar", CONDI$, "FC-0004-00003030-A", REGAF&)
'    Call ACTUREGISTRO("MOVISTO", "doseccor", CONDI$, "FC-003030-A", REGAF&)
'    '
'    CONDI$ = "doprilar='RT-X-0001-00023404'"
'    Call ACTUREGISTRO("MOVISTO", "doseclar", CONDI$, "FC-0004-00002956-A", REGAF&)
'    Call ACTUREGISTRO("MOVISTO", "doseccor", CONDI$, "FC-002956-A", REGAF&)
    '
    Screen.MousePointer = 1
    '
End Sub
...
99 GRABARRA.MSChart1.RowCount = CAGRU%
   ...
   For UI% = 1 To CAGRU%
     ...
     GRABARRA.MSChart1.Column = 1
     GRABARRA.MSChart1.Data = TOVEN(NUGRU%(UI%))
     'GRABARRA.MSChart1.ChartData = CStr(TOVEN(NUGRU%(UI%)))
     GRABARRA.MSChart1.Column = 2
     GRABARRA.MSChart1.Data = COSVEN(NUGRU%(UI%))
     '
     GRABARRA.MSChart2.Row = UI%
...
Sub CALCURESU()
    ...
    Next KKI%
    '
    Call COPYSTRU("MOVISTO", "MOVISVI2")
    Call COPYSTRU("DVECLART", "DVECLAR2")
    Call COPYSTRU("IRESVENT", "IRESVEN2")
    Call COPYSTRU("PVECLART", "PVECLAR2")
    '
    If TRIM$(Label3) <> "" Then
...
19          TTXX$ = "Precio Unitario (en PESOS) para " + TRIM$(CDD$) + " en " + TRIM$(REMIFAC$)
            ...
            If ARMO% = 1 Then
                Call GRAREG("MOVISTO2", x$, REG&)
              ElseIf ARMO% = 2 Then
                Call GRAREG("MOVISVI2", x$, REG&)
              ElseIf ARMO% = 3 Then
         ...
         '
         J1& = J1& + 1
         Call GRAREG("!DVECLAR2", Z$, J1&)
         '
         VALOMO# = CANTU# * VUNI * SIFAC%(JII%)
         IFAC#(JII%) = IFAC#(JII%) - VALOMO#
         '
         A$ = CStr(NRO1&)
         If A$ = "233" Or A$ = "123" Or A$ = "124" Or A$ = "125" Or A$ = "126" Then
         A = B
         End If
         If CII% = (-2) Then
...
28  Next JII%
    '
    Call SETULTREG("!DVECLAR2", J1&)
    Call CIERRARCH("!DVECLAR2")
    '
    Call GENERESU
    Call GUARDARESU
    '
    ...
    '
    REIRE& = 0
    RFF$ = RECFILT$("IRESVEN2", 1, MKI$(CODIMES%))
    If Len(RFF$) >= 4 Then
      REIRE& = CVS(Left$(RFF$, 4))
    End If
    If REIRE& > ULTREG&("IRESVEN2") Then REIRE& = 0
    If REIRE& < 1 Then REIRE& = 1 + ULTREG&("IRESVEN2")
    Call GRAREG("IRESVEN2", TTT$, REIRE&)
    Call CIERRARCH("IRESVEN2")
    '
    Call CIERRARCH("*.*")
    Call BAJALDISCO
    '
...
1121 PRIRE& = 999999: ULTRE& = 0
     ...
     For I& = 1 To FIN&
       ...
       'TOMA COMO FECHA DEL MOVIMIENTO EL DE SU FACTURACION Y NO EL DE REAL ENTREGA
       COMPROB$ = TRIM$(Mid$(XS$, 33, 10))
       NUCL% = CVI(Mid$(XS$, 84, 2))
       CONDI$ = "CODICOM_COR LIKE '" & COMPROB$ & "%' AND NUCLIEN=" & NUCL%
       FF1% = CVINT(VALOBADA("CAJABANC", "FECHCONTA", CONDI$))
       If FF% < FF1% Then FF% = FF1%
...
Sub GENERAFACTUR(DES%, HAS%)

    Call COPYSTRU("FACTUR", "FACTUR2")
    Call SETULTREG("FACTUR2", 0)
    
    DESDE = FETEXCOR1$(DES%)
    HASTA = FETEXCOR1$(HAS%)
    REG& = 0
    
    CUERECH$ = CONTROL("SVD451S", "CUEVE-8")
    CUERECHA% = CUEINT%(CUERECH$)
    CUEDIFCA$ = CONTROL("SVD451S", "CUEVE-9")
    CUEDIFCAM% = CUEINT%(CUEDIFCA$)
    CUEGASBA$ = CONTROL("SVD451S", "CUEVE-16")
    CUEGASBAN% = CUEINT%(CUEGASBA$)
        
    SQLX$ = "SELECT * FROM CAJABANC WITH(NOLOCK) WHERE (FECHCONTA >= '" & DESDE & "' AND FECHCONTA <='" & HASTA & "')"
    SQLX$ = SQLX$ & " AND (CODICOM_LAR NOT LIKE 'AJ-%' AND TIPOMOVIM='VENTA')"
    SQLX$ = SQLX$ & " AND CUECONTAI<>" & CUERECHA% & " AND CUECONTAI<>" & CUEDIFCAM% & " AND CUECONTAI<>" & CUEGASBAN%
    ...
    With RST
        Do While Not .EOF
              ...
              NRO1& = XVALO(!NUMECOMP)
              ifac1# = Abs(XVALO(!IDEBECOMP) - XVALO(!IHABECOMP))
              
              XX$ = REGBLAN$("FACTUR")
              Call REPLA(XX$, MKI$(NC1%), 1, 2)
              Call REPLA(XX$, MKS$(NRO1&), 3, 4)
              Call REPLA(XX$, MKI$(FF1%), 7, 2)
              Call REPLA(XX$, Chr$(TI1%), 13, 1)
              Call REPLA(XX$, MKD$(ifac1#), 15, 8)
              
              REG& = REG& + 1
              Call GRAREG&("FACTUR2", XX$, REG&)
    ...
    On Error GoTo 0
    '
    Call SETULTREG("FACTUR2", REG&)
    Call CIERRARCH("FACTUR2")
    '
End Sub

Sub GENERAMOVISTO(DES%, HAS%)

    Call COPYSTRU("MOVISTO", "MOVISTO2")
    Call SETULTREG("MOVISTO2", 0)
    ...
    SQLX$ = SQLX$ & " AND (CODICOM_LAR NOT LIKE 'AJ-%' AND TIPOMOVIM='VENTA')"
    SQLX$ = SQLX$ & " AND CUECONTAI<>" & CUERECHA% & " AND CUECONTAI<>" & CUEDIFCAM% & " AND CUECONTAI<>" & CUEGASBAN%
    Set RSTCAJABANC = READSET(SQLX$)
    With RSTCAJABANC
        Do While Not .EOF
            DOCU$ = SAFETEXT$(!Codicom_lar)
            CONDI$ = CONDI$ & "DOSECLAR='" & DOCU$ & "' OR "
            .MoveNext
        Loop
    End With
    RSTCAJABANC.Close
    Set RSTCAJABANC = Nothing
    CONDI1$ = Mid$(CONDI$, 1, Len(CONDI$) - 4)
    A$ = ""
    SQLX$ = "SELECT * FROM MOVISTO WITH(NOLOCK) WHERE "
    SQLX$ = SQLX$ & " CODIMOVI='RT'"
    SQLX$ = SQLX$ & " AND (" & CONDI1$ & ")"
    Dim RST As ADODB.Recordset
    Set RST = READSET(SQLX$)
    '
    With RST
        Do While Not .EOF
            FF1% = CVINT(!FECHMOV)
            CIXI% = XVALO(!Cod_Inte)
            doc1$ = SAFETEXT$(!DoSecCor)
            NC1% = XVALO(!Nume_Clie)
            QSAL# = XVALO(!CANTSALID)
            QENT# = XVALO(!CANTINGRE)
            VAUN1# = XVALO(!VALOUNIT)
            '
            XX$ = REGBLAN$("MOVISTO")
            Call REPLA(XX$, MKI$(FF1%), 1, 2)
            Call REPLA(XX$, MKI$(CIXI%), 3, 2)
            Call REPLA(XX$, MKI$(NC1%), 5, 2)
            Call REPLA(XX$, MKI$(CIXI%), 7, 2)
            Call REPLA(XX$, doc1$, 33, 10)
            Call REPLA(XX$, MKI$(NC1%), 84, 2)
            Call REPLA(XX$, MKD$(QENT#), 88, 8)
            Call REPLA(XX$, MKD$(QSAL#), 96, 8)
            Call REPLA(XX$, MKD$(VAUN1#), 73, 8)
            '
            REG& = REG& + 1
            Call GRAREG("MOVISTO2", XX$, REG&)
            '
20          .MoveNext
        Loop
    End With
    ...
    On Error GoTo 0
    '
    Call SETULTREG("MOVISTO2", REG&)
    Call CIERRARCH("MOVISTO2")
    '
End Sub
Sub GENERESU()
    '
    ...
    J1& = 0: J2& = 0: J3& = 0
    '
    Call COPYSTRU("CVECLART", "CVECLAR2")
    Call COPYSTRU("RVECLART", "RVECLAR2")
    Call COPYSTRU("PVECLART", "PVECLAR2")
    '
    FIN& = ULTREG&("!DVECLAR2")
    For U& = 1 To FIN&
      Call TRACE(20, U&, FIN&)
      Z$ = REGLEIDO$("!DVECLAR2", U&)
      '
    ...
    Next U&
    '
    Call SETULTREG("!RVECLAR2", J1&)
    Call CIERRARCH("!RVECLAR2")
    '
    Call SETULTREG("!CVECLAR2", J2&)
    Call CIERRARCH("!CVECLAR2")
    '
    Call SETULTREG("!PVECLAR2", J3&)
    Call CIERRARCH("!PVECLAR2")
    '
Exit Sub
'
100 For JJ& = 1 To J1&
      If ABUS$(JJ&, 1) = ABUSQUE$ Then
        Z1$ = REGLEIDO$("!RVECLAR2", JJ&)
...
Sub GUARDARESU()
   '
   AGUARDA$ = "DVECLAR2": GoSub 10
   AGUARDA$ = "RVECLAR2": GoSub 10
   AGUARDA$ = "CVECLAR2": GoSub 10
   AGUARDA$ = "PVECLAR2": GoSub 10
   '
Exit Sub
'
'
...
Sub RECURESMEN()
   ...
   End If
   '
   RFF$ = RECFILT$("DVECLAR2", 1, MKI$(CODIMES%))
   JJ& = 0
   For U& = 1 To Len(RFF$) Step 4
     REG& = CVS(Mid$(RFF$, U&, 4))
     Z$ = REGLEIDO$("DVECLAR2", REG&)
     JJ& = JJ& + 1
     Call GRAREG("!DVECLAR2", Z$, JJ&)
   Next U&
   Call SETULTREG("!DVECLAR2", JJ&)
   Call CIERRARCH("!DVECLAR2")
   '
   RFF$ = RECFILT$("RVECLAR2", 1, MKI$(CODIMES%))
   JJ& = 0
   For U& = 1 To Len(RFF$) Step 4
     REG& = CVS(Mid$(RFF$, U&, 4))
     Z$ = REGLEIDO$("RVECLAR2", REG&)
     JJ& = JJ& + 1
     Call GRAREG("!RVECLAR2", Z$, JJ&)
   Next U&
   Call SETULTREG("!RVECLAR2", JJ&)
   Call CIERRARCH("!RVECLAR2")
   '
   Call CIERRARCH("*.*")
   Screen.MousePointer = 1
...
10  FEX = HOY(HO$)
    ...
    For U& = 1 To FIN&
        ...
        TONEVEN# = Val(Mid$(List3.TEXT, 28, 12))
        COSUNPI# = Val(Mid$(List3.TEXT, 40, 12))
        ...
        Call REPLA(Z$, MKS$(PORPAR), 13, 4)
        Call REPLA(Z$, MKS$(PORACU), 17, 4)
        Call REPLA(Z$, MKD$(COSUNPI#), 21, 8)
        Call REPLA(Z$, MKD$(TOTACOS#), 29, 8)
        Call GRAREG("RANKPIF", Z$, J&)
        '
    Next U&
    '
    Call SETULTREG("RANKPIF", J&)
    Call CIERRARCH("RANKPIF")
    Call HEADERS("RANKPIF", "")
    MONEC$ = "Pesos": If Option10.Value = True Then MONEC$ = "Dolares"
...
Sub CARCOSINFO()
   ...
   '
   On Error Resume Next
   Kill LUDAT$ & "PVECLAR2.DAT"
   FileCopy LUDAT$ & "PVECLART.DAT", LUDAT$ & "PVECLAR2.DAT"
   On Error GoTo 0
   
   RFF$ = RECFILT$("PVECLAR2", 1, MKI$(CODIMES%))
   For U& = 1 To Len(RFF$) Step 4
     REG& = CVS(Mid$(RFF$, U&, 4))
     RELE$ = REGLEIDO$("PVECLAR2", REG&)
     CII% = CVI(Mid$(RELE$, 7, 2))
     If CII% > 0 Then
       If CII% <= NUMAS% Then
   ...
   If CDMX% <> CODIMES% Then
     RFF$ = RECFILT$("PVECLAR2", 1, MKI$(CDMX%))
     For U& = 1 To Len(RFF$) Step 4
       REG& = CVS(Mid$(RFF$, U&, 4))
       RELE$ = REGLEIDO$("PVECLAR2", REG&)
       CII% = CVI(Mid$(RELE$, 7, 2))
