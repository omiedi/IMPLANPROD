...
'    SQLX$ = "SELECT cod_inte, SUM(canapro) AS CantidadFabricada FROM ORDEFABR INNER JOIN MASTER WITH(NOLOCK)"
'    SQLX$ = SQLX$ & " ON ORDEFABR.cod_inte = MASTER.codint"
'    SQLX$ = SQLX$ & " WHERE MASTER.timate=5 AND ORDEFABR.canapro > 0"
'    SQLX$ = SQLX$ & " AND ORDEFABR.fechgen >= '" & desde & "' AND ORDEFABR.fechgen <= '" & Hasta & "'"
'    SQLX$ = SQLX$ + " GROUP BY ORDEFABR.cod_inte"
    ...
    With MaterialFabricado
        Do While Not .EOF
            ...
            CostoArticulosFabricados(CI%) = XVALO(VALOBADA("HISTOCOSTOS", "costostandard", "periodo='" & PERIO$ & "' AND cod_inte=" & CStr(CI%)))
    ...
    ' TITULO
    ' ******
    GRID.TextMatrix(0, 0) = "COSTO POR UNIDAD DE UNA CARROCERIA"
    Call DisenioDeCelda(GRID, 0, 0, "", 11, True, False, True, False, 0, 0)
    ...
    Call DisenioDeCelda(GRID, RangoFila$, "0:1", "", 0, False, False, False, False, 0, &HEEEEDD)
    GRID.Rows = GRID.Rows + 1
    GRID.TextMatrix(GRID.Rows - 1, 0) = "Gasto Mensual Estimado de " & PERIO$
    ...
    GRID.Rows = GRID.Rows + 2
    TotalGastosPorUnidad = TotalDeGastos / CantidadTotalFabricada
    
    For ix& = 1 To NUMAS
        If ArticulosFabricados(ix&) > 0.05 Then
            GRID.Rows = GRID.Rows + 1
            CostoUnidad = (TotalGastosPorUnidad + CostoArticulosFabricados(ix&)) * ArticulosFabricados(ix&)
...
Private Sub Command2_Click()
    ...
    If TRIM$(NombreDelArchivo) <> "" Then
        ...
        End If
        Call GeneraExcelDeGrilla(NombreDelArchivo, GRID, "")
    End If
