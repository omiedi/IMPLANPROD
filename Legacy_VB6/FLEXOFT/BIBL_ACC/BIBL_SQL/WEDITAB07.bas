                 ...
                 If Left$(LEYEN$, 1) <> ":" Then
                    ...
                    If ARCHISAL$ = "" Then
                       ...
                       ElseIf ARCHISAL$ = "SCREEN" Then
                         ...
                         PREVIEW07.List4.AddItem Space$(OFS%) + Left$(SUBRA$ + Space$(ATOT%), ATOT%)
                       Else
                    ...
                    If Len(LEYF$) > 0 Then
                       If ARCHISAL$ = "" Then
                           ...
                           If INTERLIX > 0 Then CURRY = CURRY + INTERLIX: Printer.CurrentY = CURRY
                         ElseIf ARCHISAL$ = "SCREEN" Then
                           PREVIEW07.List4.AddItem Space$(OFS%) + Left$(Space$(1 + Len(SUBRA$)) + LEYF$, ATOT%)
                         Else
...
IMPREFORM:
   ...
   For II% = 1 To NFT%
      If WEDTAB07.FILABEL(II% - 1).Visible = True Then
         V2 = 4 * HREN + WEDTAB07.FIENTRA(II% - 1).Top + HREN * 1.3
   ...
   For II% = 1 To NFT%
      ...
      If WEDTAB07.FILABEL(II% - 1).Visible = True Then
         ...
         H1 = COEF * WEDTAB07.FILABEL(II% - 1).Left + MARGENI
         V1 = 4 * HREN + WEDTAB07.FILABEL(II% - 1).Top
         Printer.CurrentX = H1
         Printer.CurrentY = V1
         Printer.Print TTXX$
         '
         H1 = COEF * WEDTAB07.FIENTRA(II% - 1).Left + MARGENI
         H2 = H1 + COEF * WEDTAB07.FIENTRA(II% - 1).Width
         If H2 > MARGENI + ATEXTO - 150 Then H2 = MARGENI + ATEXTO - 150
         ANCHOCUA = H2 - H1 - 100
         V1 = 4 * HREN + WEDTAB07.FIENTRA(II% - 1).Top
         V2 = V1 + HREN * 1.3
         If V2 > VFILMAX Then VFILMAX = V2
         Printer.Line (H1, V1)-(H2, V2), QBColor(0), B
         '
...
34       If Printer.TextWidth(TTXX$) > ANCHOCUA Then
         ...
         If WEDTAB07.FIECO(II% - 1).Visible = True Then
            H1 = COEF * WEDTAB07.FIECO(II% - 1).Left + MARGENI
            H2 = H1 + COEF * WEDTAB07.FIECO(II% - 1).Width
            If H2 > MARGENI + ATEXTO - 150 Then H2 = MARGENI + ATEXTO - 150
            ANCHOCUA = H2 - H1 - 100
            V1 = 4 * HREN + WEDTAB07.FIECO(II% - 1).Top
            V2 = V1 + HREN * 1.3
            If V2 > VFILMAX Then VFILMAX = V2
            Printer.Line (H1, V1)-(H2, V2), QBColor(0), B
            '
