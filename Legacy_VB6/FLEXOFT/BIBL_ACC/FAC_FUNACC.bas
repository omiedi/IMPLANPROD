...
Sub GRAFAC98(LU$, ARCHIFAC$, OPCI%, RRR%, FLIM%, Tipo%, VA%, CAFOR%, NRO, NC%, CLI$, iva$, FF%, FV%, LPRE%, VENDE%, TOTAFAC#())
      ...
      'SUCUVEN% = XVALO(FORMFAC.Label12.Caption)
      SUCUVEN% = XVALO(FORFACNEU.Label12.Caption)
      SUCUVEN% = XVALO(FORFACLI.Label12.Caption)
      On Error GoTo 0
        '
        ...
        For I& = 1 To FIN&
             ...
             If CANTAA > 0 Then
               ...
               'If CI% > 0 Then 'DESHABILITADO PARA RESERVAR NUMERO
                 ...
                 COD$ = CODEXT$(CI%)
                 TICAX = 1: If MONEMI% = 2 Then TICAX = TICACH(MONEMI%)
                 ...
                 End If
                 '
                 Call MOVISTOK(FF%, CI%, LOTE$, CMOVQ$, DOPRILA$, DOPRIQ$, DOSELA$, DOSEC$, REFEQ$, 0, 0, 1, NC%, DEPOSALI%, (-1) * SIG% * CANTAA)
...
End Sub
'
Sub GRACLI(LU$, Archi$, NCL, TOTFAC#, DATCLI$(), OPCI%)
        '
        Exit Sub
        '
        If OPCI% <> 2 Then
...
10  XDC% = InStr(DESCX$, "+")
    KKK% = KKK% + 1
    DESC1(KKK%) = XVALO(Left$(DESCX$, XDC% - 1))
...
19  Next U&
    '
    If XVALO(FORECISTA.DESCUEN(0)) >= 0.005 Then
        Z$ = REGBLAN$("!PRINRECI")
        Call REPLA(Z$, "N/Cr", 1, 4)
        Call REPLA(Z$, "", 5, 12)   ' NUMERO DE N/CREDITO
        Call REPLA(Z$, "Desc.Caja", 17, 16)
        Call REPLA(Z$, MKD$(XVALO(FORECISTA.DESCUEN(0))), 33, 8)
        Call REPLA(Z$, MKI$(0), 51, 2)
        FEX = HOY(HOS$)
        Call REPLA(Z$, FECHATEX$(FEX), 41, 8)
        Call REGAPP("!PRINRECI", Z$)
    ...
    V0& = ULTREG&("!PRINRECI")
    '
    TOTAVAL# = CDbl(XVALO(FORECISTA.Label14(0).Caption))
    TOTACAN# = CDbl(XVALO(FORECISTA.Label18(0).Caption)) + CDbl(XVALO(FORECISTA.Label20(0).Caption))
    ITEXTO$ = NUMTEX$(TOTAVAL#)
    LOTEX% = XVALO(ATRIFORM$(FORIPRE$, "LOIMPLET")): If LOTEX% < 20 Then LOTEX% = 20
    Call FRATEX(ITEXTO$, NRX1$, NRX2$, NRX3$, LOTEX%)
    IMPLET1$ = TRIM$(NRX1$) + " " + "***************************************************************************************************************"
    IMPLET2$ = TRIM$(NRX2$) + " " + TRIM$(NRX3$) + " ctvs. *****************************************************************************************"
    '
    ANTI# = CDbl(XVALO(FORECISTA.Label20(0).Caption))
    '
...
312 Next U&
    ...
    For U& = ULTREG&("APLICOB") To 1 Step -1
        ...
        If Mid$(Z$, 5, 2) = "RB" Then
          If XVALO(Mid$(Z$, 15, 8)) = NROLL& Then
            FFA% = CVI(Left$(Z$, 2))
            NCA% = CVI(Mid$(Z$, 3, 2))
            ICA# = CVD(Mid$(Z$, 23, 8))
            dca$ = Mid$(Z$, 55, 18)
