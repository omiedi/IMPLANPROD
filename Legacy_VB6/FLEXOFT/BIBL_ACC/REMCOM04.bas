...
Sub CALRECOM()
     '
     REMECOMP04.Frame8.Visible = True
     REMECOMP04.Frame8.Refresh
     For XI% = 0 To 7: REMECOMP04.Check1(XI%).Value = 0: Next XI%
     '
     CCCXXX$ = CUECONTA$(-1)   ' para forzar que refresque el vector
     '
10   REMECOMP04.Check1(0).Value = 2
     Call VALICUEIVACOMP
     Screen.MousePointer = 11
     If YACUE% <> 1 Then
        REMECOMP04.Frame8.Visible = False
        On Error Resume Next
        REMECOMP04.Text2(0).SetFocus
        On Error GoTo 0
     ...
     '
     For IXX% = 1 To 8
       REMECOMP04.TASIVA(IXX%).Caption = TETASA(IXX%)
       REMECOMP04.IMPOGRA(IXX%).Caption = "0.00"
       REMECOMP04.IMPOIVA(IXX%).Caption = "0.00"
       REMECOMP04.TAREAL(IXX%).Caption = "0.00"
     Next IXX%
     REMECOMP04.TASIVA(0).Caption = "Total"
     REMECOMP04.PERIVA.Caption = "0.00"
     REMECOMP04.PERGAN.Caption = "0.00"
     REMECOMP04.PERIBRU.Caption = "0.00"
     '
     REMECOMP04.Check1(0).Value = 1
     '
20   Call ARMAMESCOMP
     '
     ...
     End If
     '
30   REMECOMP04.Check1(1).Value = 2
     If YACUE% <> 1 Then
        REMECOMP04.Frame8.Visible = False
        REMECOMP04.Text2(0).SetFocus
        Screen.MousePointer = 1
     ...
     For U& = 1 To FIN&
        ...
        NC% = CVI(Left$(PP1$, 2))
        If NC% > 0 Then
           TICUE%(NC%) = TICUEPRO%((-1) * NC%)
           If TICUE%(NC%) < 0 Or TICUE%(NC%) > 1 Then
              Call MENSERR(24, "Tipo de Cuenta no Válido\para Cuenta " + TRIM$(Str$(NC%)) + " " + TRIM$(RASOCLI$((-1) * NC%)) + ".")
              TICUE%(NC%) = 0
           End If
        End If
     Next U&
     ...
     '
     REMECOMP04.Check1(1).Value = 1
     '
     REMECOMP04.Check1(2).Value = 2
     On Error Resume Next
     For KI% = 0 To 31
         REMECOMP04.Text1(KI%).TEXT = ""
         REMECOMP04.Text1(KI%).Refresh
         SUMAFAC#(KI%) = 0
     Next KI%
     On Error GoTo 0
     '
...
40        VDEF$ = SS4$ + MKI$(FEMI%) + MKD$(Abs(IAJU#)) + MKI$(0)
  ...
  '
        ...
        If FEMI% < DES% Then
           ...
           REMECOMP04.Text1(0).TEXT = CSTRING(MKD$(SUMAFAC#(0)), 4, 11, 2, 2)
           REMECOMP04.Text1(0).Refresh
           REMECOMP04.Text1(7).TEXT = CSTRING(MKD$(SUMAFAC#(7)), 4, 11, 2, 2)
           REMECOMP04.Text1(7).Refresh
        ...
        If FEMI% >= DES% Then
           If FEMI% <= DES% + 31 Then
              If PRIREG& = 0 Then
                 PRIREG& = IX&
                 REMECOMP04.Check1(2).Value = 1
                 REMECOMP04.Check1(3).Value = 2
              End If
              If IX& > ULTIRE& Then ULTIRE& = IX&
              NUFAC& = IX&
              '
              ...
              '
              For ICAMPO% = 0 To 7
                 REMECOMP04.Text1(ICAMPO%).TEXT = CSTRING(MKD$(SUMAFAC#(ICAMPO%)), 4, 11, 2, 2)
                 REMECOMP04.Text1(ICAMPO%).Refresh
              Next ICAMPO%
              '
              If TI$ <> "OP" Then
                If TI$ <> "FF" Then
...
124                       Next KKI%
              ...
              End If
              '
              If Abs(Abs(SUMA3#) - SUMA4#) >= 0.005 Then
                 MXX$ = "3"
                 GoTo 130
              End If
              '
              ...
              GoTo 140
              '
130           TTXX$ = "Se ha Detectado una Inconsistencia(" + MXX$ + ")\en el Comprobante Número " + TRIM$(NF$) + "\de " + TRIM$(CLI$) + "."
              TTXX$ = TTXX$ + "\  \Este Comprobante Debe Anularse y Registrarse Nuevamente."
              Call MENSERR(24, TTXX$)
              '
140           If TI$ = "AS" Then GoTo 1007
              For KKI% = 1 To 8
                ...
                VALOR# = INEGRAVA#(KKI%) + Val(REMECOMP04.IMPOGRA(KKI%).Caption)
                REMECOMP04.IMPOGRA(KKI%).Caption = TRIM$(CSTRING$(MKD$(VALOR#), 4, 12, 2, 2))
              Next KKI%
              '
              For KKI% = 1 To 8
                IGRAIVA#(KKI%) = SIG% * Abs(CVD(Mid$(TT7$, 24 * KKI% - 15, 8)))
                VALOR# = IGRAIVA#(KKI%) + Val(REMECOMP04.IMPOIVA(KKI%).Caption)
                REMECOMP04.IMPOIVA(KKI%).Caption = TRIM$(CSTRING$(MKD$(VALOR#), 4, 12, 2, 2))
              Next KKI%
              '
              PERCEPIVA# = SIG% * Abs(CVD(Mid$(SS7$, 41, 8)))
              VALOR# = PERCEPIVA# + Val(REMECOMP04.PERIVA.Caption)
              REMECOMP04.PERIVA.Caption = TRIM$(CSTRING$(MKD$(VALOR#), 4, 12, 2, 2))
              '
              PERCEPGAN# = SIG% * Abs(CVD(Mid$(SS7$, 49, 8)))
              VALOR# = PERCEPGAN# + Val(REMECOMP04.PERGAN.Caption)
              REMECOMP04.PERGAN.Caption = TRIM$(CSTRING$(MKD$(VALOR#), 4, 12, 2, 2))
              '
              PERCEPIBRU# = SIG% * Abs(CVD(Mid$(SS7$, 57, 8)))
              VALOR# = PERCEPIBRU# + Val(REMECOMP04.PERIBRU.Caption)
              REMECOMP04.PERIBRU.Caption = TRIM$(CSTRING$(MKD$(VALOR#), 4, 12, 2, 2))
              '
...
1007          RESO$ = Space$(16)
              ...
              REMECOMP04.LISORT.AddItem RESO$
              '
           End If
        End If
        '
1009 Next IX&
     ...
     '
     For UI1% = 1 To REMECOMP04.LISORT.ListCount
       RESO$ = REMECOMP04.LISORT.List(UI1% - 1)
     ...
     '
     REMECOMP04.Check1(3).Value = 1
     '
     REMECOMP04.Check1(4) = 1
     '
     REMECOMP04.Check1(5) = 2
     ...
     REMECOMP04.Check1(5) = 1
     '
     REMECOMP04.Check1(6) = 2
     '
     ...
     If PRIREG& > 0 Then
        ...
        For IX& = PRIREG& To ULTIRE&
           ...
           REMECOMP04.List1.AddItem Z$
        Next IX&
        '
        FIN& = REMECOMP04.List1.ListCount
        For IZ& = 1 To FIN&
           Call TRACE(20, IZ&, FIN&)
           Z$ = REMECOMP04.List1.List(IZ& - 1)
           IX& = Val(Mid$(Z$, 9, 8))
155        SS$ = REGLEIDO$("CUECORR", IX&)
     ...
     End If
     '
     REMECOMP04.Check1(6) = 1
     '
     REMECOMP04.Check1(7) = 2
     ...
     REMECOMP04.Check1(7) = 1
     '
     '
     CORRESP$ = AJUSTI$(REMECOMP04.MESAN.TEXT, 12)
     ...
     On Error Resume Next
     For KI% = 0 To 7
         Call REPLA(REBLA$, MKD$(Val(REMECOMP04.Text1(KI%).TEXT)), 45 + 8 * KI%, 8)
         Call REPLA(REBLA$, MKD$(Val(REMECOMP04.IMPOGRA(KI%).Caption)), 45 + 64 + 8 * KI%, 8)
         Call REPLA(REBLA$, MKD$(Val(REMECOMP04.IMPOIVA(KI%).Caption)), 45 + 128 + 8 * KI%, 8)
     Next KI%
     Call REPLA(REBLA$, MKD$(Val(REMECOMP04.PERIVA.Caption)), 45 + 192 + 8 * 0, 8)
     Call REPLA(REBLA$, MKD$(Val(REMECOMP04.PERGAN.Caption)), 45 + 192 + 8 * 1, 8)
     Call REPLA(REBLA$, MKD$(Val(REMECOMP04.PERIBRU.Caption)), 45 + 192 + 8 * 2, 8)
     Call REPLA(REBLA$, MKD$(Val(REMECOMP04.IMPOGRA(8).Caption)), 45 + 192 + 8 * 3, 8)
     On Error GoTo 0
     '
...
159  Close #NX%
     ...
     REMECOMP04.Text4(0).TEXT = FECHATEX$(FEX)
     If FEX < 1 Then REMEVENT.Text4(0).TEXT = "00/00/00"
     REMECOMP04.Text4(0).Refresh
     HOX% = CVI(Mid$(REBLA$, 15, 2))
     REMECOMP04.Text4(1) = HORATEX$(HOX%)
     REMECOMP04.Text4(1).Refresh
     '
...
Sub CARGARESCOM()
        ...
        FEX = CVI(Mid$(REBLA$, 13, 2))
        REMECOMP04.Text4(0).TEXT = FECHATEX$(FEX)
        If FEX < 1 Then REMECOMP04.Text4(0).TEXT = "00/00/00"
        REMECOMP04.Text4(0).Refresh
        HOX% = CVI(Mid$(REBLA$, 15, 2))
        REMECOMP04.Text4(1) = HORATEX$(HOX%)
        REMECOMP04.Text4(1).Refresh
        ...
        For KI% = 0 To 7
            ...
            On Error Resume Next
            REMECOMP04.Text1(KI%).TEXT = CSTRING$(MKD$(SUMAFAC#(KI%)), 4, 11, 2, 2)
            REMECOMP04.Text1(KI%).Refresh
            REMECOMP04.IMPOGRA(KI%).Caption = TRIM$(CSTRING$(MKD$(IGRA1#), 4, 11, 2, 2))
            REMECOMP04.IMPOGRA(KI%).Refresh
            REMECOMP04.IMPOIVA(KI%).Caption = TRIM$(CSTRING$(MKD$(IIVA1#), 4, 11, 2, 2))
            REMECOMP04.IMPOIVA(KI%).Refresh
            On Error GoTo 0
        Next KI%
        PERIV1# = CVD(Mid$(REBLA$, 45 + 192 + 8 * 0, 8))
        REMECOMP04.PERIVA.Caption = TRIM$(CSTRING$(MKD$(PERIV1#), 4, 11, 2, 2))
        REMECOMP04.PERIVA.Refresh
        PERGA1# = CVD(Mid$(REBLA$, 45 + 192 + 8 * 1, 8))
        REMECOMP04.PERGAN.Caption = TRIM$(CSTRING$(MKD$(PERGA1#), 4, 11, 2, 2))
        REMECOMP04.PERGAN.Refresh
        PERIB1# = CVD(Mid$(REBLA$, 45 + 192 + 8 * 2, 8))
        REMECOMP04.PERIBRU.Caption = TRIM$(CSTRING$(MKD$(PERIB1#), 4, 11, 2, 2))
        REMECOMP04.PERIBRU.Refresh
        IINT1# = CVD(Mid$(REBLA$, 45 + 192 + 8 * 3, 8))
        REMECOMP04.IMPOGRA(8).Caption = TRIM$(CSTRING$(MKD$(IINT1#), 4, 11, 2, 2))
        REMECOMP04.IMPOGRA(8).Refresh
        '
        Close #NX%
        Call FRESHALL
        '
...
Sub VALICUEIVACOMP()
     ...
     Next I%
     '
     CUEIVA$ = AJUSTI$(REMECOMP04.Text2(0).TEXT, 12)
     CUEADU$ = AJUSTI$(REMECOMP04.Text2(1).TEXT, 12)
     CUEACRE$ = AJUSTI$(REMECOMP04.Text2(2).TEXT, 12)
     '
...
Sub ARMAMESCOMP()
     '
     CORRESP$ = TRIM$(REMECOMP04.MESAN.TEXT)
...
Sub CONASIDIACOM()
    '
    MESA$ = TRIM$(REMECOMP04.MESAN.TEXT)
    If MESA$ = "" Then
        MESAN.SetFocus
        Exit Sub
    End If
...
50  For I% = 0 To CARESPA%
    ...
    If MODO% = 3 Then
        ...
        If Mid$(LUDA$, 2, 1) <> ":" Then
            LUDA$ = Left$(CurDir, 2) + LUDA$
        End If
        C1$ = TRIM$(REMECOMP04.MESAN.TEXT)
        C2$ = TRIM$(REMECOMP04.Text4(0).TEXT)
        C3$ = TRIM$(REMECOMP04.Text4(1).TEXT)
        '
        Call CIERRARCH("*.*")
