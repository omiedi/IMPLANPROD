...
'
Public YAESTABAS%
Sub ASOCIA_CUENTA_CENTRO_COSTOS()
   VTB% = VENTABU%("CTAYCOST")
   'posiciones 1, 12 cuenta contable
   'descripcion 13,0 eco descripcion
   'codigo c.costo 13.8
   'descripcion c.costo 21,8
...
5   OPX% = COMALTER%("Opciones de Trabajo\\Actualizar Tabla de Centro de Costos\Asociar C.Cbles a C.Costos")
    If OPX% < 1 Or OPX% > 2 Then Exit Sub
    '
    If OPX% = 2 Then
       Call ASOCIA_CCBLE_CCOSTO
       GoTo 5
    End If
    '
...
Sub RECUMOCAJ(NUMEFA$, FECONTAFA%, OKEYY%)
   ...
   SQLX$ = SQLX$ + "AND FechConta = '" & FECONTAD & "' "
...
Sub ASOCIA_CCBLE_CCOSTO()
    'TEXTO CTAYCOST
    '
    archivo$ = "CTAYCOST"
    If EXISTE%(LUDAT$ + archivo$ + ".RFS") < 1 Then
       Call COMUNI("Falta Controlador CTAYCOST\Solicite La Instalación del Mismo a su Soporte de Sistemas")
       Exit Sub
    End If
    '
    Call TRALOCAL("CTAYCOST", "")

10  VTB% = VENTABU("CTAYCOST")
    If VTB% >= 0 Then
      For U& = 1 To ULTREG&("!CTAYCOST")
        X$ = REGLEIDO$("!CTAYCOST", U&)
        CCBLE$ = TRIM$(UCase$(Mid$(X$, 1, 12)))
        If TRIM$(CCBLE$) = "" Then
          Call MENSERR(24, "Falta Cuenta Contable en Fila: " & U&)
          GoTo 10
        End If
        '
        CCOS$ = TRIM$(Mid$(X$, 13))
        If TRIM$(ECOARCH("TACENCOS", CCOS$)) = "" Then
           Call MENSERR(24, "Código de Centro Costos Inexistente o No Válido en Fila: " & U&)
          GoTo 10
        End If
        
        ' Valida que el CC no este repetido
        For U2& = 1 To U& - 1
          Y$ = REGLEIDO$("!CTAYCOST", U2&)
          If TRIM$(Mid$(X$, 1, 12)) = TRIM$(Mid$(Y$, 1, 12)) Then
            Call MENSERR(24, "Cuenta Contable Repetida (" & TRIM$(Mid$(Y$, 1, 12)) & ") En Fila: " & U&)
            GoTo 10
          End If
        Next U2&
        '
      Next U&
      Call TRACENTRAL("CTAYCOST", "")
    End If
    '
End Sub
...
Function CENTROCOSTOASOCIADO$(CuentaCo%)
   'TEXTO CTAYCOST
   CENTROCOSTOASOCIADO$ = ""
   archivo$ = "CTAYCOST"
   If EXISTE%(LUDAT$ + archivo$ + ".RFS") < 1 Then Exit Function
   '
   COCUE$ = CODCUE$(CuentaCo%)
   RECORD$ = FILTERGETRECORD$("CTAYCOST", 1, COCUE$)
   CENTROCOSTOASOCIADO$ = TRIM$(Mid$(RECORD$, 13))
End Function
Sub VincularComprobanteDelSistemaConDocumentosExternos(MODO As String, ID As Long)

    Call ABRECABAN

    Dim R As New ListaSeleccion

    Select Case UCase(MODO)

        Case "CLIENTE", "COBRANZA"

                If ID = 0 Then ID = ListaSeleccionClientes()

                If ID > 0 Then

                    R.TITULO = "Vincular Comprobantes con Documentos Externos"
                    R.EncabezadoDeColumnas = "Fecha/D8;Tipo/A18;Comprobante/A18;Importe/F12.2"
                    R.ConsultaSql = "select fechconta, " _
                    & " case when substring(codicom_lar,1,2)='FC' then 'Factura'" _
                    & " when substring(codicom_lar,1,2)='ND' then 'Nota de Debito'" _
                    & " when substring(codicom_lar,1,2)='NC' then 'Nota de Credito'" _
                    & " when substring(codicom_lar,1,2)='RB' then 'Recibo'" _
                    & " when substring(codicom_lar,1,2)='AJ' then 'Ajuste'" _
                    & " else 'Otros'" _
                    & " end tipo," _
                    & " codicom_lar, valabscomp from cajabanc"
                    If UCase(MODO) = "COBRANZA" Then
                        R.ConsultaSql = R.ConsultaSql & " where (tipomovim='rcob') and nuclien=" & CStr(ID)
                    Else
                        R.ConsultaSql = R.ConsultaSql & " where (tipomovim='fven' or tipomovim='rcob') and nuclien=" & CStr(ID)
                    End If
                    R.ConsultaSql = R.ConsultaSql & " order by fechconta, codicom_lar"
                    R.AnchoLista = 8000
10                  Call R.Mostrar

                    fechaCbte$ = TRIM$(R.FilaElegida(1))
                    If fechaCbte$ <> "" Then
                            NumeroCliente& = ID
                        razonSocial$ = TRIM$(rasocli(NumeroCliente&))
                        TipoCbte$ = TRIM$(R.FilaElegida(2))
                        Comprobante$ = TRIM$(R.FilaElegida(3))
                        ImporteCbte$ = TRIM$(R.FilaElegida(4))

                        Call CARGA_ENCABEZADO(VICODOCEX.mfgComprobante, "Cta/I0;R.Social/A20;Fecha/D8;Tipo/A18;Comprobante/A18;Importe/F12.2", FRMZELECHO)
                        VICODOCEX.mfgComprobante.Rows = 2
                        VICODOCEX.lblModo = "CLIENTE"
                        VICODOCEX.mfgComprobante.TextMatrix(1, 1) = CStr(NumeroCliente&)
                        VICODOCEX.mfgComprobante.TextMatrix(1, 2) = razonSocial$
                        VICODOCEX.mfgComprobante.TextMatrix(1, 3) = fechaCbte$
                        VICODOCEX.mfgComprobante.TextMatrix(1, 4) = TipoCbte$
                        VICODOCEX.mfgComprobante.TextMatrix(1, 5) = Comprobante$
                        VICODOCEX.mfgComprobante.TextMatrix(1, 6) = ImporteCbte$
                        VICODOCEX.txtComprobante = Comprobante$

                        VICODOCEX.BackColor = &HE2D3C0
                        VICODOCEX.frameComprobante.BackColor = &HE2D3C0
                        VICODOCEX.frameDocExternos.BackColor = &HE2D3C0
                        VICODOCEX.Show 1
        ...
        Case "PROVEEDOR", "PAGOS"

                If ID = 0 Then ID = ListaSeleccionProveedores()
                ...
                If ID > 0 Then
                    ...
                    & " when substring(codicom_lar,1,2)='ND' then 'Nota de Debito'" _
                    & " when substring(codicom_lar,1,2)='NC' then 'Nota de Credito'" _
                    & " when substring(codicom_lar,1,2)='TF' then 'Ticket Factura'" _
                    & " when substring(codicom_lar,1,2)='CO' then 'Comprobante'" _
                    & " when substring(codicom_lar,1,2)='RH' then 'R. Honorarios'" _
                    & " when substring(codicom_lar,1,2)='RC' then 'Recibo'" _
                    & " when substring(codicom_lar,1,2)='OP' then 'Orden de Pago'" _
                    & " when substring(codicom_lar,1,2)='AJ' then 'Ajuste'" _
                    & " else 'Otros'" _
                    & " end tipo," _
                    & " codicom_lar, valabscomp from cajabanc"
                    If UCase(MODO) = "PAGOS" Then
                        R.ConsultaSql = R.ConsultaSql & " where (tipomovim='opag') and nuprove=" & CStr(ID)
                    Else
                        R.ConsultaSql = R.ConsultaSql & " where (tipomovim='fcom' or tipomovim='opag') and nuprove=" & CStr(ID)
                    End If
                    R.ConsultaSql = R.ConsultaSql & " order by fechconta, codicom_lar"
...
20                  Call R.Mostrar
                    ...
                    If fechaCbte$ <> "" Then
                            NumeroProveedor& = ID
                        razonSocial$ = TRIM$(rasocli((-1) * NumeroProveedor&))
                        TipoCbte$ = TRIM$(R.FilaElegida(2))
                        Comprobante$ = TRIM$(R.FilaElegida(3))
                        ImporteCbte$ = TRIM$(R.FilaElegida(4))
                        ...
                        Call CARGA_ENCABEZADO(VICODOCEX.mfgComprobante, "Cta/I0;R.Social/A20;Fecha/D8;Tipo/A18;Comprobante/A18;Importe/F12.2", FRMZELECHO)
                        VICODOCEX.mfgComprobante.Rows = 2
                        VICODOCEX.lblModo = "PROVEEDOR"
                        VICODOCEX.mfgComprobante.TextMatrix(1, 1) = CStr(NumeroProveedor&)
                        VICODOCEX.mfgComprobante.TextMatrix(1, 2) = razonSocial$
                        VICODOCEX.mfgComprobante.TextMatrix(1, 3) = fechaCbte$
                        VICODOCEX.mfgComprobante.TextMatrix(1, 4) = TipoCbte$
                        VICODOCEX.mfgComprobante.TextMatrix(1, 5) = Comprobante$
                        ...
                        VICODOCEX.BackColor = &HC7D9CC
                        VICODOCEX.frameComprobante.BackColor = &HC7D9CC
                        VICODOCEX.frameDocExternos.BackColor = &HC7D9CC
                        VICODOCEX.Show 1
        ...
        Case "CUPON"
                ...
                R.TITULO = "Vincular Comprobantes con Documentos Externos"
                R.EncabezadoDeColumnas = "Fecha/D8;Tipo/A18;Comprobante/A18;Importe/F12.2"
                R.ConsultaSql = "select fechconta, substring(descrimov,14,64) tipo, codicom_lar, sum(valabscomp) importe from cajabanc" _
                & " where tipomovim='concitc'" _
                & " group by fechconta, codicom_lar, descrimov" _
...
30              Call R.Mostrar
                ...
                If fechaCbte$ <> "" Then
                    ...
                    VICODOCEX.mfgComprobante.Rows = 2
                    VICODOCEX.lblModo = "PROVEEDOR"
                    VICODOCEX.mfgComprobante.TextMatrix(1, 1) = ""
                    VICODOCEX.mfgComprobante.TextMatrix(1, 2) = ""
                    VICODOCEX.mfgComprobante.TextMatrix(1, 3) = fechaCbte$
                    VICODOCEX.mfgComprobante.TextMatrix(1, 4) = TipoCbte$
                    VICODOCEX.mfgComprobante.TextMatrix(1, 5) = Comprobante$
                    VICODOCEX.mfgComprobante.TextMatrix(1, 6) = ImporteCbte$
                    ...
                    VICODOCEX.BackColor = &H95C5E1
                    VICODOCEX.frameComprobante.BackColor = &H95C5E1
                    VICODOCEX.frameDocExternos.BackColor = &H95C5E1
                    VICODOCEX.Show 1
