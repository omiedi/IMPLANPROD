...
30 On Error Resume Next
   ...
   If SENDBYMAIL$ <> "" Then
     ...
     ' VERIFICA LA EXISTENCIA DEL APLICATIVO BULLZIP PARA GENERAR EL PDF DESDE ESTE
     ModoBullzip% = 0
...
'     For Each PRX1 In Printers
'       ImpresoraNombre = PRX1.DeviceName
'       If InStr(UCase$(ImpresoraNombre), "BULLZIP") > 0 Then
'         ' VERIFICA LA EXISTENCIA DEL BULLZIP.BAT QUE CONTIENE LA SECUENCIA DE COMANDOS PARA QUE GENERE EL PDF
'         ' SOBRE LUCOM$ CON EL NOMBRE CONTENIDO EN LA VARIABLE ARANEX$
'         If EXISTE%(LUDAT$ + "BULLZIP.BAT") = 1 Then
'            Printer.EndDoc
'            On Error Resume Next
'            Set Printer = PRX1
'            If Err Then
...
'
'            On Error GoTo 32
'            ' CREA EN LUCOM$, UN NUEVO BULLZIP.BAT CON LOS PARAMETROS REQUERIDOS
'            ArchivoSeteoOriginal% = FILEOPEN%(LUDAT$ + "BULLZIP.BAT", 1, 128) 'ABRE EL ARCHIVO DE LECTURA
'            ArchivoSeteo% = FILEOPEN%(LUCOM$ + "BULLZIP.BAT", 2, 128) 'ABRE EL ARCHIVO DE ESCRITURA
'
'            Do While Not EOF(ArchivoSeteoOriginal%) ' RECORRE
'               Line Input #ArchivoSeteoOriginal%, REGISTRO$ ' LEE CADA REGISTRO
'               NuevoRegistro$ = REGISTRO$
'               PosicionEncontrada% = InStr(1, REGISTRO$, "ARCHIVO.PDF")
'               If PosicionEncontrada% > 0 Then ' CAMBIA EL NOMBRE DEL ARCHIVO
'                    NuevoRegistro$ = REPLACAR$(REGISTRO$, "ARCHIVO.PDF", ARANEX$ & ".PDF")
'               End If
'               Print #ArchivoSeteo%, NuevoRegistro$ ' ESCRIBE EN EL ARCHIVO EL NUEVO TEXTO
'            Loop
'            '
'            Close ArchivoSeteoOriginal%
'            Close ArchivoSeteo%
'            EDITFORM.Label1 = ARANEX$
'
'            ' EJECUTA BULLZIP.BAT EN FORMA OCULTA
'            RESULTADO = Shell(LUCOM$ + "BULLZIP.BAT", vbHide)
'            ModoBullzip% = 1
'            On Error GoTo 0
'            GoTo 35
'        Else
'            ' SI NO ENCUENTRA EL ARCHIVO BAT PASA A LAS RESTANTES IMPRESORAS
'            ' Call MENSERR(24, "No Se Ha Encontrado el Archivo de Configuración PDF.")
'            GoTo 31
'        End If
'      End If
'     Next
'     '
'31   If ModoBullzip% = 0 Then
'         For Each PRX1 In Printers
'           ImpresoraNombre = PRX1.DeviceName
'           If InStr(UCase$(ImpresoraNombre), "PDFWRITER") > 0 Or InStr(UCase$(ImpresoraNombre), "PDF4U") > 0 Or InStr(UCase$(PRX1.DeviceName), "BULLZIP") > 0 Then
'             Printer.EndDoc
'             On Error Resume Next
'             Set Printer = PRX1
'             If Err Then
...
'        DoEvents
'44      TINI = Timer
'45      If EXISTE%(LUCOM$ + "" + TRIM$(EDITFORM.Label1) + ".PDF") < 1 Then
'            If Timer < TINI + 30 Then GoTo 45
'        Else
'            ArchivoEncontrado% = 1
'            GoTo 60
'        End If
...
'        OPX% = COMALTER%(TX1$ & " \\Continuar\Cancelar")
'        If OPX% = 2 Then Exit Sub  'SI CANCELA SALE
'        If EXISTE%(LUCOM$ + "" + TRIM$(EDITFORM.Label1) + ".PDF") < 1 Then GoTo 44
'        GoTo 60
     End If
     '
49   TINI = Timer
50   If EXISTE%(LUCOM$ + "" + TRIM$(EDITFORM.Label1) + ".PDF") < 1 Then
       If Timer < TINI + 20 Then
         GoTo 50
       End If
       '
       TX1$ = "Generando Archivo PDF "
...
Function PORCENT_COSTO_NACIONALIZACION(NUMEBORE&, Optional MONEDX%, Optional MONECOMP%)
    PORCENT_COSTO_NACIONALIZACION = 0: GASTOS_APLICADOS_DOLAR# = 0
...
Function SaldoPendFacturaAnticipo#(NCL, Factu$)
   ...
   ImpOriginal# = XVALO(VALOBADA("FACTUANTI", "IMPODEBE", CONDI$, "NOMESS"))
