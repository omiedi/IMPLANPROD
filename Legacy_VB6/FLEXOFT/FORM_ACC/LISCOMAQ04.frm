...
Sub OCOMANU()
    '
2   If COMALTER%("Realmente Desea\ANULAR Ordenes de Compra ?\\No, Abandonar\Si, Anular O/C's") <> 2 Then
      Exit Sub
    End If
    '
    HO% = HOY(HOS$)
...
10  YY$ = REGSEL$("INDIOCPE/Indice de Ordenes de Compra Pendientes para Anulación")
    ...
    With OcomEnca
      ...
      If .NoMatch = False Then
        .Edit
        'CONFIRMACION
        NOC1$ = "OC." + Str$(NUOCO&) + " - " + !Refe_Ocom
        If COMALTER%("Desea Realmente Anular la Orden de Compra\ '" & NOC1$ & "' \\Si, Anular\No, Cancelar") <> 1 Then GoTo 10
        '
...
2   Screen.MousePointer = 11
    ...
    With OCCUMTEMP
      ...
      .MoveFirst
      If Err Then
        Call MENSERR(24, "No hay Ordenes de Compra\Pendientes que Puedan\darse por Cumplidas.")
        Screen.MousePointer = 1
        On Error GoTo 0
        Exit Sub
      End If
