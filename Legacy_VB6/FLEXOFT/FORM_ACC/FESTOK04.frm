   ...
   Begin VB.Frame Frame6 
      ...
      Begin VB.PictureBox Picture8 
         ...
         Begin VB.Label Label11 
            BackStyle       =   0  'Transparent
            Caption         =   "Re-Calculo Estadístico de Consumo"
            Height          =   750
            Left            =   735
            TabIndex        =   32
            Top             =   105
...
Private Sub Command19_Click()
   ...
   For U& = 1 To FIN&
     ...
     RESTAT$ = XX$
     ANESTK04.MESAN = Left$(XX$, 15)
     TTXX$ = REGBLAN$("STMINREC")
     CIXI% = CODINT%(ANESTK04.MESAN)
     MINIREC = Val(ANESTK04.Text1)
     LREPREC = Val(ANESTK04.Text2)
     '
     Call REPLA(TTXX$, MKI$(CIXI%), 1, 2)
     Call REPLA(TTXX$, UNIMED$(CIXI%), 3, 2)
     Call REPLA(TTXX$, MKS$(Val(ANESTK04.Label13)), 5, 4)
     Call REPLA(TTXX$, MKS$(MINIREC), 9, 4)
     Call REPLA(TTXX$, MKS$(Val(ANESTK04.Label16)), 13, 4)
...
Private Sub Command2_Click()
   ...
   OPCALST04.Show 1
   DoEvents
   If Val(OPCALST04.Label1) < 1 Then GoTo 99
   '
   Call ARMAMES
   Call CALCURESU
   '
...
Sub CALCURESU()
   ...
   Dim ACUCO%(2)
   '
   DES1# = CDbl(CVDAT(DESDEFE%))
   HAS1# = CDbl(CVDAT(HASTAFE%))
   '
'   Call ABREPEDCLI
...
36 Call ABRESTOCKS
   ...
   SQLX$ = SQLX$ + " WHERE CantSalid > 0.01 "
   SQLX$ = SQLX$ + " AND CDBL(FechMov) >= " & DES1# & " "
   SQLX$ = SQLX$ + " AND CDBL(FechMov) <= " & HAS1# & " "
   Set MOVITMP = Stocks.OpenRecordset(SQLX$, 4)
   With MOVITMP
      '
      On Error Resume Next
      .MoveFirst
      ...
      '
      Do Until .EOF = True
        TMV$ = Mid$(!DOPRICOR, 1, 2)
        If TMV$ = "OF" Or TMV$ = "RT" Then
            CAPED = !CantSalid
...
37 REBLA$ = REGBLAN$("STATSTOK")
   ...
   Call FILESORT("STATSTOK", "", 1, 1, "ASC")
   '
   'Call COCOBREC
   '
   TTXX$ = Space$(40)
...
'
   ...
   SQLX$ = SQLX$ + " WHERE CDBL(Femi_Ocom) >= " & DES1# & " "
   SQLX$ = SQLX$ + " AND CDBL(Femi_Ocom) <= " & HAS1# & " "
   SQLX$ = SQLX$ + " AND Stat_Ocom <> 9 " 'QUE NO ESTEN ANU
   SQLX$ = SQLX$ + " AND Stat_Ocom >= 0 " 'QUE ESTEN APRO
   Set COMPRTMP = Compras.OpenRecordset(SQLX$, 4)
   With COMPRTMP
      '
      On Error Resume Next
      .MoveFirst
      ...
      '
      Do While .EOF = False
        FF% = CVINT(!Femi_Ocom) ' fecha de emision
        NP% = !Npro_Ocom ' proveedor
        NOC& = !Nume_Ocom 'NUMERO DE O/C
        CAPEN1# = !Cant_Ocom
        CI% = !COD_INTE
        'TENGO QUE BUSCAR EL REGISTRO DEL BORECEP QUE CUMPLA
        'CON PROVE=NP%,NUMERO O/C =NOC& Y CODIGO MATERIAL = CI%
        YYY$ = " NumOC = " & NOC& & " "
        YYY$ = YYY$ + " NumProv = " & NP% & " "
        YYY$ = YYY$ + " Cod_Inte = " & CI% & ""
        With Bolrecep
      ...
      ' COMPARO LA CANTIDAD PEDIDA EN OC Y LA RECIBIDA EN BR
              CANREC# = !CantNom 'CANTIDAD RECIBIDA
              FECREC1 = !FECHRECEP
              CAPEN1# = CAPEN1# - CANREC#
              'Y GRABAR EL ARCHIVO COPLAENT
            End If
        End With
        .MoveNext
      Loop
