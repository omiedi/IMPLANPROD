...
Private Sub Command13_Click()
   ...
   DoEvents
   '
   If CLEANPRACC.Check1(2).Value = Checked Then
     Call VEPRECOS
     GoTo 99
   End If
   '
   LICOBAJ% = 0: If CLEANPRACC.Check1(0).Value = Checked Then LICOBAJ% = 1
   LISINST% = 0: If CLEANPRACC.Check1(1).Value = Checked Then LISINST% = 1
   CLEANPRACC.Check1(0).Value = Unchecked
   CLEANPRACC.Check1(1).Value = Unchecked
   CLEANPRACC.Check1(2).Value = Unchecked
   DoEvents
   '
...
Private Sub Command3_Click()
   ...
   '
   Call COPYSTRU("DEUDOR1", "DESCLPRE")
     LPI% = LISTACTIVA&
   Call FILTERPUTRECORD("DESCLPRE", 1, MKI$(LPI%), MKI$(LPI%) + Text4.TEXT)
   '
   Call CARILISTAS
...
Private Sub Command6_Click()
   ...
   AA1 = OBJPLA$("DESHASFECHPRO", String$(6, 0))
   Call OPNOIN("")
                  ...
                  If Asc(Mid$(x$, 50, 1)) <= 2 Then
                    If TRIM$(Mid$(x$, 51, 12)) <> "" Then
                      If CVI(Mid$(x$, 63, 2)) > 0 Then
                        'Se crea el archivo de la Lista de Precio
                        ARCHIPRE$ = TRIM$(Str$(NULIS%))
                        While Len(ARCHIPRE$) < 3: ARCHIPRE$ = "0" + ARCHIPRE$: Wend
                        ARCHIPRE$ = "LIPRE" + ARCHIPRE$
...
Sub CARLISTA()
   ...
   Text4 = TRIM$(Mid$(TTXY$, 3))
   CANDECI% = XVALO(Text3)
   If CANDECI% < 2 Then CANDECI% = 2: Text3 = "2"
   If CANDECI% > 5 Then CANDECI% = 5: Text3 = "5"
   '
   Call FRESHALL
   List2.Clear
   Call FRESHALL
   ...
   For U& = 1 To FIN&
     ...
     End If
     Call REPLA(TTXX$, CSTRING$(MKD$(PRE#), 3, 12, CANDECI%, 2), 72, 12)
     List2.AddItem TTXX$
     '
   Next U&
   Screen.MousePointer = 1
