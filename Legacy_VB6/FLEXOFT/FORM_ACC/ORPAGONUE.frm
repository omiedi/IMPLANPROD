...
Begin VB.Form ORPAGONUE 
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VB.Label Label2 
         Alignment       =   2  'Center
         BackStyle       =   0  'Transparent
         Caption         =   "No Válido como Factura"
         BeginProperty Font 
            Name            =   "MS Sans Serif"
            Size            =   8.25
            Charset         =   0
   ...
   Begin VB.Menu Archiv 
      ...
      Begin VB.Menu php1 
         Caption         =   "-"
      End
      Begin VB.Menu grabaeimprimeordendepago 
         Caption         =   "Grabar e Imprimir Orden de Pago"
      End
      Begin VB.Menu ss 
         Caption         =   "-"
      End
   ...
   Begin VB.Menu mnuedi 
      ...
      Begin VB.Menu ordenar 
         Caption         =   "Ordenar Cartera de Valores"
         Begin VB.Menu porfecha 
            Caption         =   "Por Fecha"
         End
         Begin VB.Menu porimporte 
            Caption         =   "Por Importe"
         End
      End
   End
   Begin VB.Menu ConsuLis 
      ...
      End
      Begin VB.Menu verimputacioncreditosyanticipos 
...
Private Sub Command5_Click()
    ...
    '
    NPRX% = XVALO(Text1)
    If NPRX% < 1 Or ECOARCH$("PROVED1", MKI$(NPRX%)) = "" Then
...
Private Sub Form_Load()
    ...
    If CONTROL("NEWASPEC", "CAMBIOAS") = "SI" Then
      ...
      BOTNEXT.ZOrder 0: BOTNEXT.Top = 6100
      Command5.ZOrder 0
      MARCOBARRA.ZOrder 0
    End If
...
Private Sub Label10_Change()
   ...
   End If
   'Label117 = FORMATNUM((-1) * PROMVEN#, "F4.0")
   Label117 = FORMATNUM(PROMVEN#, "F4.0")
   '\\\OT-08-0203-OD-FIN///
End Sub
...
Private Sub Label21_Change()
   ...
   For U& = 1 To List1.ListCount
      Z$ = List1.List(U& - 1)
      FEVE = FECHANUM(Mid$(Z$, 49, 8))
      Call REPLA(Z$, CSTRING$(MKD$(ICHE#), 3, 12, 2, 2), 57, 12)
      ICHE# = XVALO(Mid$(Z$, 70, 14))
      PROVALO# = PROVALO# + (DIENTRE(HOY(HO$), FEVE) * ICHE#)
      TOTVALO# = TOTVALO# + ICHE#
290 Next U&
    '
    If Abs(TOTVALO#) > 0.001 Then
       Label119 = FORMATNUM(PROVALO# / TOTVALO#, "F4.0")
      Else
       Label119 = ""
    End If
    '
    'Label123 = FORMATNUM$(XVALO(Label119) - XVALO(Label117), "F4.0")
    Label123 = FORMATNUM$(XVALO(Label119) + XVALO(Label117), "F4.0")
    '\\\OT-08-0203-OD-FIN///
End Sub
...
Private Sub porfecha_Click()
   MODOSORVAL% = 1
   Call Command8_Click
End Sub

Private Sub porimporte_Click()
   MODOSORVAL% = 0
   Call Command8_Click
...
22  Call APERBASDAT("CABAN")
    SQLX$ = "SELECT TOP 1 NumeComp FROM CajaBanc "
    SQLX$ = SQLX$ + "WHERE FechEmisi >= " & FLIMI & " "
...
80  If NROPRO& > 0 Then
       ...
       SQLX$ = SQLX$ + "WHERE CDBL(FechEmisi) >= " & FLINUME & " "
       SQLX$ = SQLX$ + "AND TipoMovim = 'OPAG' "
       SQLX$ = SQLX$ + "AND CodiCom_Cor LIKE 'OP*' "
...
796      NURETGA& = XVALO(InputBox$("Numero de Comprobante Ganancias", "Retencion Ganancias", TRIM$(Str$(NURETGA&))))
       ...
       Call REPLA(Z1$, NIBRCLI$(NC1%), 65, 14)
       Call REPLA(Z1$, MKD$(XVALO(Label23)), 79, 8)
       Call REPLA(Z1$, MKD$(XVALO(Label30)), 87, 8)
       If MONEMI% > 1 Then
    ...
    If TICUEPRO%(NC1%) = 0 Then
       ...
       For U& = 1 To List2.ListCount
         ...
         If DOCU$ <> "Anticipo no Aplicado" Then
              ...
              If MONEMI% > 1 Then
                 CANCEDOL# = CANCE#
                 CANCE# = ROUND(CANCEDOL * TICAMONE(MONEMI%))
              End If
              If CANCE# > 0 Then
...
113         SQLX$ = "DescriMov = '" & IDEVA$ & "' "
            If MONEMI% > 1 Then
                 SQLX$ = SQLX$ + "AND ABS(ValAbsComp - " & IMCHEX$ & ") < 1 " ' PARA EVITAR PROBLEMAS DE REDONDEO
               Else
...
VERIFACPEN:
    '
    If TICUEPRO%(NC1%) = 0 Then      ' SOLO CUENTAS DE  CORRIENTES
       '
       XX% = 0
       For U& = 1 To List2.ListCount
         Z$ = List2.List(U& - 1)
...
GRABATORPAG:                              ' grabar orden de pago en CAJABANC
    ...
    If TICUEPRO%(NC1%) = 0 Then
         SuBalan# = SuBalan# + ImpoDeb#
         AtraCheque& = XVALO(Label117)
         AtraCance& = XVALO(Label119)
         Call GRACABAN
       Else
...
19  Next U&
    ...
    If XVALO(Label30) >= 0.005 Then
       ...
       'Call REPLA(Z$, Space$(12), 5, 12)'Se modifico esta linea por la
       'de abajo para que en la impresion salga el numero de orden de la
       'retencion de ganancias
    ...
    For U& = 1 To List2.ListCount
        ...
        X$ = List2.List(U& - 1)
        DOCU$ = Mid$(X$, 1, 20)
        If InStr(EPAC$, "GRAMY") > 0 And Left$(UCase$(DOCU$), 12) = "ANTICIPO NO " Then
           DOCU$ = "Pg Cta Saldo CCte"
        End If
...
Private Sub verimputacioncreditosyanticipos_Click()
    Call OPORPA04.Command11_Click
End Sub
...
Sub CALRETGANVIE()
   '
   Static CTCALCU%, ALIGAN(8), MINIMPO#(8), MEPAGX%(8)
   ...
   End If
   '
   ' determinar categoria de Ganancias
   NPROVEI% = XVALO(Text1)
            ...
            If !Status < 9 Then   ' para no condiderar anulados
               ...
               For KKI% = 1 To CADETANT%
                 If TRIM$(Left$(DETANT$(KKI%), 18)) = TRIM$(!CodiCom_Lar) Then
                   IBRUCOM# = XVALO(Mid$(DETANT$(KKI%), 21, 14)) + !ValAbsComp
               ...
               DETANT$(KKI%) = TTXYZ$
               '
' MsgBox "Base Imponible: " & TRIM$(FORMATNUM$(VALBASIMP#, "F12.2")) & "   de Comprobante " & !CodiCom_Lar
19           End If
          .MoveNext
          Loop
        End If
      ...
      With CABATEMP
        ...
        If Err < 1 Then
          ...
          Do While Not .EOF
            ...
            For KKI% = 1 To CADETANT%
              If TRIM$(Left$(DETANT$(KKI%), 18)) = TRIM$(!CodiCom_Lar) Then
                ACURE# = XVALO(Mid$(DETANT$(KKI%), 53, 12)) + !ValAbsComp
                TTXYZ$ = DETANT$(KKI%)
                Call REPLA(TTXYZ$, FORMATNUM$(ACURE#, "F12.2"), 53, 12)
                DETANT$(KKI%) = TTXYZ$
                GoTo 29
              End If
            Next KKI%
            ...
            DETANT$(KKI%) = TTXYZ$
            '
' MsgBox "Retención Anterior: " & TRIM$(FORMATNUM$(!VALABSCOMP, "F12.2")) & "   de Comprobante " & !CodiCom_Lar
29        .MoveNext
          Loop
        End If
      End With
   End If
   '
'MsgBox "Acumulado Base Anterior: " & TRIM$(FORMATNUM$(ACUBASANT#, "F12.2")) & "   Neto Cancelado: " & TRIM$(FORMATNUM$(NETOCANCE#, "F12.2"))
'MsgBox "Acumulado Retencion Anterior: " & TRIM$(FORMATNUM$(ACURETANT#, "F12.2"))
   BASEIMTO# = ACUBASANT# + NETOCANCE#
   MINOIM# = MINIMPO#(CATIB1%)
   If ACURETANT# >= 0.005 Then
       ' SIMPLIFICA. Si en el mes ya hubo retencion, va la alicuota directa
       RETGACAL# = NETOCANCE# * ALIGAN(CATIB1%) / 100
     Else
       RETGACAL# = (BASEIMTO# - MINOIM#) * ALIGAN(CATIB1%) / 100 - ACURETANT#
   End If
   '
   If BASEIMTO# > MINOIM# Then
     ...
     If Len(RFF$) > 0 Then
       ...
       For KU& = 1 To ULTREG&("!ESCALGAN")
         ...
         If BASEIMTO# - MINOIM# > CVD(Mid$(ESCX$, 3, 8)) Then
           If BASEIMTO# - MINOIM# <= CVD(Mid$(ESCX$, 11, 8)) Then
             RETGACAL# = CVD(Mid$(ESCX$, 19, 8)) + (BASEIMTO# - MINOIM# - CVD(Mid$(ESCX$, 3, 8))) * CVS(Mid$(ESCX$, 27, 4)) / 100
             RETGACAL# = RETGACAL# - ACURETANT#
             GoTo 98
           End If
         End If
       Next KU&
   ...
   End If
   '
98 If RETGACAL# < 0 Then RETGACAL# = 0
   Label30 = TRIM$(FORMATNUM$(RETGACAL#, "F12.2"))
   Label32 = Str$(MEPAGX%(CATIB1%))
99 Screen.MousePointer = 1
   '
End Sub
'
...
'   NPROVEI% = XVALO(Text1)LO ESTA LEYENDO ARRIBA
   ...
   If NPROVEI% <> NPROVEB% Then
     ...
     NPROVEB% = NPROVEI%
   End If
   SHOWRETGA.Label7 = TRIM$(Str$(CATIB1%))
...
19           End If
   ...
   SHOWRETGA.Label11 = FORMATNUM(NETOCANCE#, "F12.2")
...
98 SHOWRETGA.Label17 = FORMATNUM(RETGATO#, "F10.2")
   ...
   '
   If MONEMI% > 1 Then
     RETGACAL# = ROUND(RETGACAL# / TICAMONE(MONEMI%))
   End If
   '
   Label30 = TRIM$(FORMATNUM$(RETGACAL#, "F12.2"))
   Label32 = Str$(MEPAGX%(CATIB1%))
...
'HASTA ACA LLEGUE EL 04/02/05
   If XVALO(Label30) >= 0.005 Then
      ...
      If MONEMI% > 1 Then
        DOCPARAM$(1) = TRIM$(FORMATNUM(TICAMONE(MONEMI%) * XVALO(Label18), "F12.2"))
        DOCPARAM$(2) = TRIM$(FORMATNUM(TICAMONE(MONEMI%) * XVALO(Label30), "F12.2"))
      End If
      DOCPARAM$(3) = ""
      '
      For U& = 1 To List2.ListCount
        ...
        SALPE# = TICAMONE(MONEMI%) * CDbl(XVALO(Mid$(X$, 56, 13)))
        CANCE# = TICAMONE(MONEMI%) * CDbl(XVALO(Mid$(X$, 70, 14)))
        If Abs(CANCE#) >= 0.005 Then
          If DOCPARAM$(3) <> "" Then
...
'      XX$ = REGLEIDO$("DARGAPRO", CVS(JJ$)) ' LEE CONFIGURACION DE LA RETENCION
      ...
      CAITAB1% = 0
      '
      OPX% = COMALTER%("Se Ha Generado una Retención de Ganancias.\¿Desea Imprimir el Comprobante?\\Si, Imprimir\No, Continuar")
      If OPX% = 2 Then CANCELPRINT = 1
...
Sub PRICORETIB()
   ...
   If XVALO(Label14) >= 0.005 Then
      ...
      If MONEMI% > 1 Then
        DOCPARAM$(3) = TRIM$(FORMATNUM(TICAMONE(MONEMI%) * XVALO(Label14), "F12.2"))
      End If
      ...
      For U& = 1 To List2.ListCount
        ...
        If Abs(CANCE#) >= 0.005 Then
          If DOCPARAM$(7) <> "" Then
            DOCPARAM$(7) = DOCPARAM$(7) + ", "
          End If
              DOCPARAM$(7) = DOCPARAM$(7) + DOCU$ ' Comprobantes que originan la retencion
        End If
      ...
      CAITAB1% = 0
      '
      OPX% = COMALTER%("Se Ha Generado una Retención de IIBB.\¿Desea Imprimir el Comprobante?\\Si, Imprimir\No, Continuar")
      If OPX% = 2 Then CANCELPRINT = 1
        ...
        If CANCE# > 0 Then
          ...
          If DOCU$ = "Anticipo no Aplicado" Then
            ...
            Else
              ...
              INECAN# = 0
              On Error Resume Next
              INECAN# = CANCE# * INEFA# / XVALO(Mid$(Z$, 32, 13))
              On Error GoTo 0
