...
Begin VB.Form FORCOMP04 
   ...
   Begin VB.Menu Orcom 
      ...
      Begin VB.Menu plh6 
         Caption         =   "-"
      End
      Begin VB.Menu aproba 
         Caption         =   "Aprobar Ordenes de Compra"
         Begin VB.Menu vercomprapendientedeaprobacion 
            Caption         =   "Ver Ordenes de Compra Pendientes de Aprobación"
         End
         Begin VB.Menu aprobarordenesdecompra 
            Caption         =   "Aprobar Ordenes de Compra"
         End
      End
      Begin VB.Menu ss3 
      ...
      Begin VB.Menu Maconsi1 
         ...
         Begin VB.Menu DevoConsi 
            Caption         =   "Consumo Materiales Consignados"
            Enabled         =   0   'False
         End
         Begin VB.Menu ss1 
            Caption         =   "-"
         End
         Begin VB.Menu depuraconsignacion 
            Caption         =   "Depuración de Consignaciónes"
         End
      End
   End
   Begin VB.Menu ConLis 
      ...
      Begin VB.Menu CoUlCom 
         ...
         End
      End
      Begin VB.Menu detalleultimacompras 
         Caption         =   "Detalle de Últimas Compras"
      End
      Begin VB.Menu resumenestadisticos 
         Caption         =   "Resumen Estadísticos"
      End
      Begin VB.Menu s2 
         Caption         =   "-"
      End
      Begin VB.Menu listadocomprapendienteaprobar 
         Caption         =   "Listado de Ordenes de Compra Pendientes de Aprobación"
      End
      Begin VB.Menu ss4 
         Caption         =   "-"
      End
      Begin VB.Menu costoreposicion 
   ...
   Begin VB.Menu utiles 
      Caption         =   "Útiles"
      Begin VB.Menu ocomprasegundisponibilidadstocks 
         Caption         =   "Generacion de OC Segun Disponibilidad de Stocks"
      End
   End
   Begin VB.Menu Config 
      Caption         =   "Configuracion "
      Begin VB.Menu Setup 
         Caption         =   "Configuración General de Compras"
      End
      Begin VB.Menu plh1 
         Caption         =   "-"
      End
...
Private Sub aprobarordenesdecompra_Click()
    Call FOAPROC04.Command14_Click
End Sub

Private Sub Command1_Click()
   Call SELECHO("ACREDOR")
   NCLIE% = XVALO(TRIM$(VALACT1$("ACREDOR")))
   If NCLIE% > 0 Then
     If REGICLI&((-1) * NCLIE%) > 0 Then
       Call BLANFORMU
       Text1.TEXT = TRIM$(Str$(NCLIE%))
       Call CARDACLI
       If Text1.TEXT = "" Then GoTo 99
       Call CARCOPEN
       Call CARCOANT
...
Private Sub Command14_Click()
    ...
    If XVALO(Text11) > 899999 Then
      Call MENSERR(24, "Número de Orden de Compra \Supera el Valor Máximo Permitido")
...
20  Call GRACOMPRA(OKX%)
    If OKX% > 0 Then
     ...
     If Left$(CONTROL$("OCOMPRA", "MODOAPRO"), 6) <> "DIRECT" Then
        Call COMUNI("Orden de Compra Registrada\como 'Pendiente de Aprobación'")
     End If
     '
     Text1.TEXT = ""
...
Sub CARCOMAR()
   ...
   Call BLANARCH("PROVHAB")
   '
   If NECOMAR04.Label2.Caption <> "ACEPTO" Then
      NECOMAR04.LIMATSEL.Clear
      For KKI& = 1 To ULTREG&("PLACOMP") Step 4
          XX$ = REGLEIDO$("PLACOMP", KKI&)
          CI0% = CVI(Left$(XX$, 2))
          CANTU$ = FORMATNUM$(CVS(Mid$(XX$, 21, 4)), "F10.1")
          If XVALO(CANTU$) > 0.05 Then
            TTXX$ = AJUSTI$(CODEXT$(CI0%), 16) + AJUSTI$(DESCRIT0$(CI0%), 40) + CANTU$
            NECOMAR04.LIMATSEL.AddItem TTXX$
          End If
9     Next KKI&
      Screen.MousePointer = 1
      '
      NECOMAR04.Label1.Caption = ""
      NECOMAR04.Text2.TEXT = ""
      NECOMAR04.Text3.TEXT = ""
   End If
   Screen.MousePointer = 1
   '
10 NECOMAR04.Show 1
   '
   If NECOMAR04.Label1.Caption = "CANCELO" Then
      Call BLANFORMU
      Exit Sub
   End If
   '
   If TRIM$(NECOMAR04.Text3) <> "" Then
      Text1 = NECOMAR04.Text3
   Else
   ...
   '
   ELIGIOCODI% = 0
   For I& = 0 To NECOMAR04.LIMATSEL.ListCount - 1
        If NECOMAR04.LIMATSEL.Selected(I&) = True Then
            ELIGIOCODI% = 1
            Exit For
        End If
   Next I&
   If ELIGIOCODI% = 0 Then
        Call COMUNI("Debe Elegir Alguno de los Codigos con Necesidad")
        GoTo 10
   End If
   '
   Call CARDACLI
   ...
   For I& = 0 To NECOMAR04.LIMATSEL.ListCount - 1
        If NECOMAR04.LIMATSEL.Selected(I&) = True Then
            CODIGO$ = TRIM$(Mid$(NECOMAR04.LIMATSEL.List(I&), 1, 16))
            CANTI$ = TRIM$(Mid$(NECOMAR04.LIMATSEL.List(I&), 58, 68))
                CI0% = CODINT(CODIGO$)
            ...
            '
            REG& = REG& + 1
            Call GRAREG("!OCOMDETA", ZZ$, REG&)
        End If
   Next I&
   '
   Call SETULTREG("!OCOMDETA", REG&)
...
Private Sub Command8_Click()
    ...
    If EXISTE%("OCOABI04.EXE") > 0 Then
       TTOP$ = "Ordenes de Compra Abiertas"
       OPABIE% = 1
       OPMAX% = 1
    End If
    '
    If EXISTE%("CONPRE04.EXE") > 0 Then
       OPMAX% = OPMAX% + 1
       If OPMAX% > 1 Then TTOP$ = TTOP$ + "\"
       OPCONC% = OPMAX%
       TTOP$ = TTOP$ + "Sub-Sistema Concurso de Precios"
    End If
    '
    OPMAX% = OPMAX% + 1
    If OPMAX% > 1 Then TTOP$ = TTOP$ + "\"
    OPGENCOM% = OPMAX%
    TTOP$ = TTOP$ + "Generacion de OC Segun Disponibilidad de Stocks"
    '
    If TTOP$ <> "" Then
    ...
    '
    OPX% = ALTERNA%(TTOP$)
    If OPX% > OPMAX% Then GoTo 20
    '
    If OPX% = OPABIE% Then
        If DERACCE%("OCOABIE", "Ordenes de Compra Abiertas") = 2 Then
          Call CONECRUN("OCOABI04", "Ordenes de Compra Abiertas")
        End If
        GoTo 99
      ElseIf OPX% = OPCONC% Then
        If DERACCE%("CONCUPRE", "Sub-Sistema de Concurso de Precios") = 2 Then
          Call CONECRUN("CONPRE04", "Sub-Sistema de Concurso de Precios")
        End If
        GoTo 99
      ElseIf OPX% = OPGENCOM% Then
             Call CARCOMAR
             GoTo 99
    End If
    '
...
Private Sub costoreposicion_Click()
    Call OPCICOM04.Command3_Click
End Sub

Private Sub depuraconsignacion_Click()
   Call OPCICOM04.DEPUCONSIG
End Sub
...
Private Sub detalleultimacompras_Click()
   Call OPCICOM04.Command12_Click
End Sub

Private Sub DevoConsi_Click()
   OPCICOM04.Command15_Click
End Sub
...
10     Caption = Caption + "  -  " + EPAC$
    ...
    '
    MONUMERA$ = "AUTO"
    If CONTROL$("", "NUMEOCOM") = "MANUAL" Then MONUMERA$ = "MANU"
    '
    If EXISTE%("OCOABI04.EXE") < 1 Then
...
Private Sub listadocomprapendienteaprobar_Click()
   Call FOAPROC04.Command6_Click
End Sub
...
Private Sub Nuorig_Click()
   Call OPCICOM04.Command19_Click
End Sub
...
Private Sub ocomprasegundisponibilidadstocks_Click()
   Call Command8_Click
End Sub
...
Private Sub recepcion_Click()
   Call OPLISCO04.Command50_Click
End Sub

Private Sub resumenestadisticos_Click()
      Call OPCICOM04.Command20_Click
End Sub
...
Private Sub Text11_GotFocus()
   '\\\OT-08-00884-OD-26/09/08///
   If CONTROL$("OCOMPRA", "NUMERA") = "AUTOMA" Then
      Text10.SetFocus
      Exit Sub
    End If
    '\\\OT-08-00884-OD-FIN///
...
Private Sub Text11_LostFocus()
   ...
   If NUMEPRE$ <> TRIM$(Text11.TEXT) Then
     Text11.TEXT = NUMEPRE$
     Call MENSERR(24, "Número de O/Compra Ajustado")
   End If
   '
End Sub
...
Sub CARDACLI()
    ...
    '
      ...
      If LIMICRE(NC%) < 0 Then
        EMPRESA$ = Left$(TRIM(RASOCLI$(NC%)), 32)
      ...
      If STATCLI%(NC%) > 0 Then
        ...
        If CONTROL("OCOMPRA", "PRESENCO") = "" Then 'VALIDACION DE NO PRESENTAR CONTACTO
          Text6 = CONTACTCLI$(NC%)
        End If
        ...
        With OEncaTemp
          ...
          If Err < 1 Then
            ATEN$ = !AtSr_Ocom
            If TRIM$(ATEN$) <> "" And CONTROL("OCOMPRA", "PRESENCO") = "" Then Text6 = ATEN$
...
Function PRONUOCO&()
   ...
   SQLX$ = SQLX$ + " WHERE Nume_Ocom < 1000000 "
...
'  End If
   ...
   If XVALO(Text11) < 1 Then
      Call MENSERR(24, "Número de O/Compra no Válido")
      Exit Sub
   End If
   '
   If Int(XVALO(Text11)) <> XVALO(Text11) Then
      Call MENSERR(24, "Número de O/Compra no Válido")
      Exit Sub
   End If
   '
   OcomEnca.Close
   Call APERBASDAT("COMPRAS")
   OcomEnca.FindFirst "Nume_Ocom = " & TRIM$(Text11.TEXT)
...
123 If COPRECIO < 0.99 Then
    ...
    End If
    '
    ' repite control por seguridad
    ' NPP& = PRONUOCO&
    ' If NPP& <> XVALO(Text11.TEXT) Then GoTo 15
    OcomEnca.FindFirst "Nume_Ocom = " & TRIM$(Text11.TEXT)
    If OcomEnca.NoMatch = False Then GoTo 15
    '
    Call APERBASDAT("COMPRAS")
    With OcomEnca
       .AddNew
    ...
    'Call REPLA(ENCAO$, REFE$, 5, 44)
       ...
       !Iotr1_Ocom = TOTPER#
       !Total_Ocom = TOTATO#
       !Lentrega = Left$(Text10, 64) 'OT-06-0227-WAR-05/06/06///
       !Obse_Ocom = Text13
       On Error Resume Next
       !Anex_Ocom = RAI$
       On Error GoTo 0
...
Sub CALTOCOM()
   ...
   For J& = 1 To ULTREG&("!OCOMDETA")
       ...
       If CANTU < 0.05 And CANSTXX < 0.05 Then GoTo 30
       '
       ...
       CANSTXX = CVS(Mid$(X$, 51, 4))
       '
       COEFI = 1: If CANTU > 0.05 Then COEFI = CANSTXX / CANTU
       ...
       NUORSE& = !N_OrServ
         ...
         If CAPEN / COEFCAN >= 0.05 Then
           ...
           Call REPLA(LI2TEX$, ATRAX$, 74, 4)
             While NPPP& > 999999: NPPP& = NPPP& - 1000000: Wend
           Call REPLA(LI2TEX$, FORMATNUM$(NPPP&, "I6"), 79, 6)
...
Sub CARLISCO()
   ...
   For U& = 1 To FIN&
     ...
     'If FENTRESOL% < FENTREGEN% Then FENTRESOL% = FENTREGEN%   ' SUPRIMIDO A PEDIDO DE GUIDI
     If FENTRESOL% < 1 Then FENTRESOL% = FENTREGEN%   ' la reemplaza solo si queda en blanco
     ...
     Call REPLA(TTXX$, DESCRIT0$(CI%), 17, 15)
       If COLTELIB% > 0 Then
         Call REPLA(TTXX$, RTrim$(RSPACE$(REGLEIDO$(ARCHILIB$, U&))), 17, 15)
       End If
     Call REPLA(TTXX$, UNISTO$, 34, 4)
     Call REPLA(TTXX$, FORMATNUM$(CANSTO, "F9.1"), 38, 9)
...
Function ULTICOM$(NPRO%, CIIX%)
   ...
   '
   Static UCOANT$, NPROANT%, CIIXANT%
   '
   If NPRO% = NPROANT% Then
     If CIIX% = CIIXANT% Then
       UCO$ = UCOANT$
       GoTo 90
     End If
   End If
...
Sub PRICOMPRA()
    ...
    If TRIM$(FORIPRE$) <> "" Then
      ...
      '\\\OT-05-0676-WAR-FIN///
      CAPARDOC% = 24     ' para importe en letras
      '
      DOCPARAM$(1) = FORPAG$
      DOCPARAM$(2) = EMBALA$
      DOCPARAM$(3) = Total(0)
      ...
      CODPARAM$(22) = "IMP-LET1": DOCPARAM$(22) = RETEX$(1)
      CODPARAM$(23) = "IMP-LET2": DOCPARAM$(23) = RETEX$(2)
      CODPARAM$(24) = "IMP-LET3": DOCPARAM$(24) = RETEX$(3)
      '
      CODTABU1$(1) = "COD-MAT"
      CODTABU1$(2) = "DES-MAT"
      CODTABU1$(3) = "UNIMED"
      ...
      For J& = 1 To ULTREG&("!OCOMDETA")
         ...
         TETABU1$(16, CAITAB1%) = NUPLANO$(CI0%)
         '
   ...
   If CIIINSTRU% > 0 Then
     If CIIINSTRU% <= URN& Then
       ...
       For U& = 1 To Len(RNC$) Step 4
         ...
         If RENOTA& > 0 Then
           If RENOTA& <= URN& Then
             TTYY$ = RTrim$(RSPACE$(Mid$(REGLEIDO$("REGLACOM", RENOTA&), 3)))
             If TTYZ$ <> "" Then
...
Private Sub vercomprapendientedeaprobacion_Click()
   Call FOAPROC04.Command4_Click
End Sub
