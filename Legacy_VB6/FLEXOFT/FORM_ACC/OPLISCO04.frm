...
Sub MATPENDCONS()
   ...
   On Error Resume Next
     ...
     Do While (.EOF = False)
         ...
         '
         ' esto que sigue es para validar reservas contra O/C's pendientes
         OcomEnca.FindFirst "Nume_Ocom = " & NUMEOC&
         If OcomEnca.NoMatch = True Then
            .Edit
         ...
         If OcomEnca!Stat_Ocom = 9 Then
            ...
            GoTo 19
         End If
         OcomDeta.FindFirst "Nume_Ocom = " & NUMEOC& & " AND Nuord_Item = " & NORDI%
         If OcomDeta.NoMatch = True Then
            .Edit
            !Cant_Asig = !Cant_Entreg
            .Update
            HUBORECU% = 1
            GoTo 19
         End If
         If OcomDeta!Stat_Ocom = 3 Or OcomDeta!Stat_Ocom = 8 Then
            .Edit
            !Cant_Asig = !Cant_Entreg
            .Update
            HUBORECU% = 1
...
Sub Command3_Click()
    ...
    If Option1.Value = True Then 'PANTALLA
        Select Case OPXX%
          ...
          Case 2
            ORD% = 0
            Call CoCoPen(ORD%, OKEY%) ' ORDENES PENDIENTES
            '
            If OKEY% < 1 Then GoTo 99 'si cancela en cocopen sale
            If HAYMATERIALIB = 1 Then
                Call COMUNI("La Consulta por Pantalla\Permite Visualizar Solamente\Materiales CODIFICADOS.")
            End If
            '
...
Sub Command4_Click()
    ...
    ORD% = 1
    Call CoCoPen(ORD%, OKEY%)   ' SELECCIONAR SOLO PENDIENTES
    '
    If OKEY% < 1 Then GoTo 99 'si cancela en cocopen sale
    '
...
Sub Command9_Click()
   ...
   'ACA LISTADO DE MAT EN CONSIGNACION
       If Option1.Value = True Then
            ...
            If MODOEMI% < 1 Or MODOEMI% > 2 Then GoTo 99
            Call GENEMATCON
            If MODOEMI% = 1 Then
                 Call FINAL("*COMATCO")
          ...
          ElseIf Option2.Value = True Then
            ...
            If MODOEMI% < 1 Or MODOEMI% > 4 Then GoTo 99
            If MODOEMI% = 1 Then
...
Sub CoCoPen(ORDEN%, OKEY%)
   ...
   End If
   Screen.MousePointer = 11
   SQLX$ = "SELECT MIN(FEntre_Sol) AS FECHAMIN , MAX(FEntre_Sol) AS FECHAMAX"
   SQLX$ = SQLX$ + " FROM OCOMDETA "
   Set FECHMINMAXTMP = compras.OpenRecordset(SQLX$, dbOpenSnapshot)
   With FECHMINMAXTMP
   ...
   SQLX$ = SQLX$ + " AND CDBL(FEntre_Sol) >= " & DESDE & ""
   SQLX$ = SQLX$ + " AND CDBL(FEntre_Sol) <= " & HASTA & ""
   ...
   Call HEADERS("OPENDETA", "")
   Call HEADERS("OPENDETA", "Fecha de Entrega : " + PERIO$)
   '
   Call SETULTREG("OPENDETL", JJ&)
   Call CIERRARCH("OPENDETL")
   Call HEADERS("OPENDETL", "")
   Call HEADERS("OPENDETL", "Fecha de Entrega : " + PERIO$)
...
Sub COCOMP0()
    ...
    '
    '\\\OT-08-0286-FL-22/05/08///
    If Option2.Value = True Then 'Impresion
        Call DESHASFECHA(DES%, HAS%, PERIO$)
        If DES% = 0 And HAS% = 0 Then
          DES% = 33: HAS% = HOY(HO$)
          PERIO$ = FECHATEX(DES%) + " - " + FECHATEX(HAS%)
        End If
        ...
        NCP% = Val(VALACT1$("PROVED1"))
        PROVEED$ = VALACT2$("PROVED1")
    End If
    '\\\OT-08-0286-FL-FIN///
    '
    Screen.MousePointer = 11
    '
    Call APERBASDAT("COMPRAS")
    SQLX$ = "Select * from OCOMDETA where Stat_Ocom < 9 "
    '\\\OT-08-0286-FL-22/05/08///
    If Option2.Value = True Then 'Impresion
        SQLX$ = SQLX$ + "AND INT(CDBL(Femi_Ocom)) >= " & DESDAT & " "
        SQLX$ = SQLX$ + "AND INT(CDBL(Femi_Ocom)) <= " & HASDAT & " "
    End If
    If NCP% <> 0 Then
       SQLX$ = SQLX$ + "AND Npro_Ocom =" & NCP% & " "
    End If
    '\\\OT-08-0286-FL-FIN///
    '
    ...
    '
    Call HEADERS("OCOMDETL", "")
    Call HEADERS("OCOMDETL", "Periodo: " + PERIO$)
    If NCP% <> 0 Then
       Call HEADERS("OCOMDETL", "Proveedor: " + PROVEED$)
    End If
