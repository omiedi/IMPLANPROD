...
10  AMODO% = ALTERNA%("Cuenta Corriente Vendedores\Informar Pagos\Saldos Pendientes por Vendedor\Listado de Control Mensual\Envío de Comisiones por Mail")
    If AMODO% = 1 Then
    ...
    '\\\OT-007-0393-FL-FIN///
       ...
       '28/03/07 (FIN)
       ElseIf AMODO% = 5 Then
         Call CONECRUN("SNDCOM04", "Envío de Comisiones por Mail")
    End If
...
20            NUMVEN% = VENDCLI%(NC%)
              NRO& = !NumeComp
              'NETO# = !IHabeComp - !IDebeComp
              NETO# = (!IHabeComp - !IDebeComp) * SIGNO% 'En caso de haber notas de debito, resta el importe
              NUSUCUVE% = !CodiSucu
              FEDOC% = CVINT(!FechEmisi)
              '
              Z$ = REBLA$
              ...
              If CUECOMI% = 1 Then
                 ...
                 Call REPLA(YCM$, RASOCLI$(NC%), 9, 32)
                 Call REPLA(YCM$, MKD$(ICOMI#), 41, 8)
                 REFE$ = !CodiCom_Lar + " - " + FECHATEX$(FEDOC%)
...
Sub BORRACOMIS()
    '
    Opciones% = COMALTER%("Opciones para Borrar\\Por Fecha\Por Mes")
    ...
    If Opciones% = 1 Then
        ...
        FechaComision% = FECHANUM(VDEV$)
        If FechaComision% = 0 Then Exit Sub
