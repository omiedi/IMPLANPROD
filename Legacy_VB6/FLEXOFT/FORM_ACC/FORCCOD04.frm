...
123 If COPRECIO < 0.99 Then
    ...
    For J& = 1 To ULTREG&("!OCOMDETA")
      ...
      Next I%
      '
      ' ESTO ES PARA MARCAR EN LA LISTA DE MATERIALES LOS COMPRADOS
      Call ABREORTRA
      CODEX$ = CODEXT$(CI0%)
      CANAREP = CANTU
      RELISTA% = CVI(Mid$(ZZ$, 125, 2))
      If RELISTA% >= 0 Then
        If RELISTA% < ITECOM04.List1.ListCount Then
          TTXYZ$ = Mid$(ITECOM04.List1.List(RELISTA%), 91)
          For KUL% = 1 To Len(TTXYZ$) Step 48
            TTXYY$ = Mid$(TTXYZ$, KUL%, 48)
            NOTRAX$ = TRIM$(Mid$(TTXYY$, 2, 24))
            NOGRUX$ = TRIM$(Mid$(TTXYY$, 26, 16))
            NORIX% = Val(Mid$(TTXYY$, 42, 7))
            Call ABREORTRA
            With ListaMat
              .FindFirst "Nro_Otrab = '" & NOTRAX$ & "' and Nro_Grup = '" & NOGRUX$ & "' AND NuOrden = " & NORIX% & " "
...
'      If TRIM$(CODEX$) <> "" Then
'        SQLX$ = "CodExte = '" & CODEX$ & "' "
'        SQLX$ = SQLX$ + "AND (Nro_Ocom = '' OR ISNULL(Nro_Ocom) = TRUE OR ISEMPTY(Nro_Ocom) = TRUE) "
...
Sub PRICOMPRA()
    ...
    If TRIM$(FORIPRE$) <> "" Then
      ...
      For J& = 1 To ULTREG&("!OCOMDETA")
         ...
         If PODESI% <= POCODI% + 3 Then CAITAB1% = CAITAB1% + 1
