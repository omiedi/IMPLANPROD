VERSION 5.00
Begin VB.Form COTRAN04 
   AutoRedraw      =   -1  'True
   BackColor       =   &H00FFFFE0&
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Presupuestación - Cotización de Transporte"
   ClientHeight    =   7785
   ClientLeft      =   45
   ClientTop       =   300
   ClientWidth     =   11910
...
'    '
'    If DERACCE%("GRACOTCAM", "Grabar Cotización") >= 2 Then
...
10    NUPRESU$ = TRIM$(Label31)
      ...
      With DETPRETMP
       ...
       If Err < 1 Then
        ...
        Do Until .EOF = True
          ...
          IMPO1# = !Impo_Det
          Call REPLA(Y$, MKD$(IMPO1#), 65, 8)
          J& = J& + 1
          Call GRAREG("!DECOTRAN", Y$, J&)
        .MoveNext
        Loop
       End If
       On Error GoTo 0
      End With
      '
      Call SETULTREG("!DECOTRAN", J&)
      Call CIERRARCH("!DECOTRAN")
      '
      PRF$ = "DECOTRAN"
      ATRI$ = "/NC"
      ATRI$ = ATRI$ + "/TITUPAN=Detalle de Cotización - Transporte/"
      ...
      'BORRA LOS REGISTROS///
      With DetaPresu
20      .FindFirst "Nro_PRESU = '" & NUPRESU$ & "' AND RUBRO_DET = 12 "
      ...
      TOTPIS# = 0
      For KKIL& = 1 To J&
         XX$ = REGLEIDO$("!DECOTRAN", KKIL&)
         LEY1$ = (Mid$(XX$, 1, 64))
         'VALIDACION SI codigo VALE 0
         If LEY1$ = "" Then GoTo 7
          IMP1# = CVD(Mid$(XX$, 65, 8))
          IMP2$ = FORMATNUM(IMP1#, "F12.4")
          Y$ = Space$(256)
          Call REPLA(Y$, LEY1$, 1, 60)
          'Call REPLA(Y$, IMP2$, 61, 12)
      ...
      With DETPRETMP
       ...
       If Err < 1 Then
        ...
        Do Until .EOF = True
          LEY1$ = !DESCRI_DET
          IMPO1# = !Impo_Det
          IMPO2$ = FORMATNUM(IMPO1#, "F12.4")
          Y$ = Space$(256)
          Call REPLA(Y$, LEY1$, 1, 60)
          'Call REPLA(Y$, IMPO2$, 61, 12)
          'CALCULO EL TOTAL DEL ITEM DE MANO DE OBRA
          IMPOIT1# = IMPO1#
15        IMPOIT2$ = FORMATNUM(IMPOIT1#, "F12.2")
      ...
      End With
    End If
    'CALCULA EL TOTAL DE TRANSPORTE.
    TOTCAM# = TOTPIS#
...
Private Sub text38_change()
  '
  TOTCAM1# = Val(Text38)
  FORPRE04.Label63 = FORMATNUM(TOTCAM1#, "F11.2")
  '
End Sub
