...
'\\\OT-05-0424-MH-05/10/06///
   Command18.Enabled = False
   Call REMEVE04.Command11_Click
   Command18.Enabled = True
End Sub

Private Sub Command19_Click()
   Command19.Enabled = False
   Call REMEVE04.Command30_Click
...
15  List2.ListIndex = (-1)
    ...
    For U& = 1 To FIN&
      ...
      NCLIE% = CVI(Left$(X$, 2))
      If NCLIE% > 0 Then
        GRCC%(NCLIE%) = GRUCOCLI%(NCLIE%)    'CVI(Mid$(X$, 157, 2))
      End If
    Next U&
    ...
    For U& = 1 To FIN&
      ...
      CII% = U&   ' CVI(Left$(X$, 2))
      If CII% > 0 Then
        GRCV%(CII%) = GRUCOVEI%(CII%)   ' CVI(Mid$(X$, 125, 2))
        If InStr(UCase$(EMPREAC$), "DOSIVA") > 0 Then
...
99 GRABARRA.MSChart1.RowCount = CAGRU%
   ...
   For KKI% = 1 To CAGRU%
     ...
     GRABARRA.MSChart1.RowLabel = Left$(DEGRU$(KKI%), 8)
     GRABARRA.MSChart2.Row = KKI%
     GRABARRA.MSChart2.RowLabel = Left$(DEGRU$(KKI%), 8)
   Next KKI%
   '
...
15 GRATORTA.MSChart1.ColumnCount = CAGRU%
   ...
   For KKI% = 1 To CAGRU%
     ...
     GRATORTA.MSChart1.ColumnLabel = Left$(DEGRU$(KKI%), 8)
   Next KKI%
   '
   TOTAVEN = 0: TOTACOS = 0
   FIN& = ULTREG&("!DVECLART")
...
Sub GRAPARVENMERC(MODO%)
   ...
   For KKI% = 1 To CAGRU%
     ...
     GRATORTA.MSChart2.ColumnLabel = Left$(DEGRU$(KKI%), 8)
   Next KKI%
   '
   TOTAVEN = 0: TOTACUM = 0
   FIN& = ULTREG&("CVECLART")
   ...
   If GRUCLI% > 0 Then
     ...
     ElseIf GRUCLI% = 0 Then
       ...
       For KKI% = 1 To CAGRU%
         ...
         GRAFUNCI.MSChart1.ColumnLabel = Left$(DEGRU$(KKI%), 8)
       Next KKI%
   End If
   '
   If GRUCLI% = 0 Then
...
15 If GRUCLI% > 0 Then
     ...
     ElseIf GRUCLI% = 0 Then
       ...
       For KKI% = 1 To CAGRU%
         ...
         GRAFUNPR.MSChart1.ColumnLabel = Left$(DEGRU$(KKI%), 8)
       Next KKI%
   End If
   '
   If GRUCLI% = 0 Then
...
Sub CALCURESU()
    ...
    '
    If TRIM$(Label3) <> "" Then
      If COMALTER%("Puede Mantener los Costos Registrados\Correspondientes al Cálculo Anterior de Este Mes,\o Re-Frescarlos Según Costos Actuales\\Mantener Anteriores\Re-Calcular Costos") <> 2 Then
        Call CARCOSINFO
      End If
    End If
    VEGRUVE$ = ""
...
15  DES% = DESDEFE%
    ...
    For JII% = 1 To CAFAC%
      ...
      Do While Not MOVISTEMP.EOF
        ...
        If MOVISTEMP!MoneVal > 1 Then
          PREUN# = PREUN# * MOVISTEMP!TIPOCAM
        End If
        IDET#(JII%) = IDET#(JII%) + PREUN# * CANTU
      MOVISTEMP.MoveNext
      Loop
      '
      MOVISTEMP.MoveFirst
      Do While Not MOVISTEMP.EOF
         ...
         FF% = FFI%(JII%)
         'NRO1& = NRO&(JII%)
         CII% = MOVISTEMP.Cod_Inte
         CANTUD# = MOVISTEMP!Cantsalid - MOVISTEMP!CantIngre
         PREUN# = MOVISTEMP!ValoUnit '* COEF#
         If MOVISTEMP!MoneVal > 1 Then
           PREUN# = PREUN# * MOVISTEMP!TIPOCAM
         End If
         '
         ...
         If CII% < 1 Then
           CDD$ = "Mat.s/Código"
           If Left$(NUMEFA$, 2) = "ND" Then
               CII% = (-1) ' DEBITOS LIBRES
               CDD$ = "Déb.Libre"
             ElseIf Left$(NUMEFA$, 2) = "NC" Then
         ...
         Call REPLA(Z$, NUMEFA$, 25, 24)
         Call REPLA(Z$, MKI$(FF%), 49, 2)
         Call REPLA(Z$, MKD$(CANTUD#), 51, 8)
         Call REPLA(Z$, String$(70, 0), 59, 70)
         Call REPLA(Z$, MKD$(CDbl(PRELISTX)), 129, 8)
         Call REPLA(Z$, MKD$(CANTUD# * PRELISTX), 137, 8)
         Call REPLA(Z$, MKD$(CDbl(PRELI)), 145, 8)
         Call REPLA(Z$, MKD$(CANTUD# * PRELI), 153, 8)
         Call REPLA(Z$, MKD$(CDbl(VUNI)), 161, 8)
         Call REPLA(Z$, MKD$(CANTUD# * VUNI), 169, 8)
            ...
            If COREGIS#(CII%) > 0.00005 Then
               ...
               Else
                 If CODIMESACT% = CODIMES% Or MODOCALCOS% = 2 Then
                     ...
                     COU = COSUNI(CII%) * (1 + Val(MERMAPRO) / 100) ' EN PESOS
                   Else
                     'CODOLA = COSMATCAL(CII%, 2)   ' LO CALCULA EN DOLARES
                     CODOLA = COSUNI(CII%) / TICAMONE(2) ' LO CALCULA EN DOLARES
         ...
         Call REPLA(Z$, MKD$(CANTUD# * COU), 193, 8)
         RENTA# = CANTUD# * (VUNI - COU)
         Call REPLA(Z$, MKD$(RENTA#), 201, 8)
         POREN = 0: If Abs(CANTUD# * VUNI) > 0 Then POREN = 100 * RENTA# / (CANTUD# * VUNI)
         ...
         '
         VALOMO# = ROUND(CANTUD# * VUNI, 2) * SIFAC%(JII%)
         ...
         End If
         '
27    MOVISTEMP.MoveNext
      Loop
      '
275   If Abs(IFAC#(JII%)) >= 0.005 Then
         CLI% = NCI%(JII%)
         FF% = FFI%(JII%)
         NRO1& = NRO&(JII%)
         CII% = 0
         CDD$ = "Mat.s/Código"
           If (IFAC#(JII%) * SIFAC%(JII%)) > 0 Then
               CII% = (-1) ' DEBITOS LIBRES
               CDD$ = "Déb.Varios"
             Else
               CII% = (-2) ' CREDITOS LIBRES
               CDD$ = "Créd.Varios"
           End If
         CANTUD# = Sgn(IFAC#(JII%) * SIFAC%(JII%))
         VUNI = Abs(IFAC#(JII%))
         PRELI = VUNI
         PRELISTX = VUNI
         VALOUN = VUNI
         VATOT# = ROUND(CANTUD# * VALOUN, 2)
         '
         Z$ = REGBLAN$("DVECLART")
         Call REPLA(Z$, MKI$(CODIMES%), 1, 2)
         Call REPLA(Z$, MKI$(1), 3, 2)       ' productos
...
111 CAFAC% = 0
    ...
    With CABATEMP
      ...
      Do While Not .EOF
        ...
        If TIMO$ = "VENTA" Then
           If InStr(!DescriMov, "Gasto Embalajes") < 1 Then ' EXCLUYE EMBALAJES
              CAFAC% = CAFAC% + 1
              NCI%(CAFAC%) = NCLIE%
              FFI%(CAFAC%) = CVINT(FECHAD)
              NUFA$(CAFAC%) = NUMEFA$
              IFAC#(CAFAC%) = ROUND(ITOTA#, 2)
              SIFAC%(CAFAC%) = SIG%
