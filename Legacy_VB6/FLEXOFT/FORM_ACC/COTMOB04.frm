VERSION 5.00
Begin VB.Form COTMOB04 
   AutoRedraw      =   -1  'True
   BackColor       =   &H00D0D0FF&
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Presupuestación - Cotización de Mano de Obra"
   ClientHeight    =   7020
   ClientLeft      =   45
   ClientTop       =   300
   ClientWidth     =   11910
...
Private Sub Form_Load()
  ...
  'LEO LA LISTA DE PRECIOS DE PUERTAS
  TEXT21 = CONTROL("LIPREVEN", "PUERTA")
  '
End Sub
'
...
'          Y$ = List1.List(I)
'          LEY1$ = Mid$(Y$, 1, 60)
'          HORA1# = Val(Mid$(Y$, 61, 7))
'          OPE1# = Val(Mid$(Y$, 68, 4))
'          VALHOR1# = Val(Mid$(Y$, 74, 12))
'          IMPITE# = Val(Mid$(Y$, 86, 12))
'          Call ABREPRECAM
'          With DetaPresu
'            SQLX$ = " Nro_Presu = '" & NUPRESU1$ & "' "
'            SQLX$ = SQLX$ + " AND  Rubro_Det = 11 "
'            SQLX$ = SQLX$ + " AND TRIM(DESCRI_Det) = '" & TRIM(LEY1$) & "' "
'            .FindFirst SQLX$
...
10    NUPRESU$ = TRIM$(Label31)
      ...
      With DETPRETMP
       ...
       If Err < 1 Then
        ...
        Do Until .EOF = True
          ...
          '
          DESCRI1$ = !DESCRI_DET
          Call REPLA(Y$, DESCRI1$, 1, 64)
          HORA1# = !Hora_Det
          Call REPLA(Y$, MKS$(HORA1#), 65, 4)
          OPERA1# = !Opera_Det
          Call REPLA(Y$, MKS$(OPERA1#), 69, 4)
          VALOHO1# = !ValoHor_Det
          Call REPLA(Y$, MKD$(VALOHO1#), 73, 8)
          J& = J& + 1
          Call GRAREG("!DECOTMOB", Y$, J&)
        .MoveNext
        Loop
       End If
       On Error GoTo 0
      End With
      '
      Call SETULTREG("!DECOTMOB", J&)
      Call CIERRARCH("!DECOTMOB")
      '
      PRF$ = "DECOTMOB"
      ATRI$ = "/NC"
      ATRI$ = ATRI$ + "/TITUPAN=Detalle de Cotización - Mano de Obra/"
      ATRI$ = ATRI$ + "/BORDEIZQ=W" + TRIM$(Str$(Left + COTPUE04.Left + 100))
      ATRI$ = ATRI$ + "/BORDESUP=W" + TRIM$(Str$(Top + COTPUE04.Top + 2250))
      VTB% = VENTABU%(PRF$ + ATRI$)
      If VTB% >= 0 Then
      'BORRA LOS REGISTROS///
      With DetaPresu
20      .FindFirst "Nro_PRESU = '" & NUPRESU$ & "' AND RUBRO_DET = 11 "
      ...
      TOTPIS# = 0
      For KKIL& = 1 To J&
         XX$ = REGLEIDO$("!DECOTMOB", KKIL&)
         LEY1$ = (Mid$(XX$, 1, 64))
         'VALIDACION SI codigo VALE 0
         If LEY1$ = "" Then GoTo 7
          HORA1# = CVS(Mid$(XX$, 65, 4))
          OPE1# = CVS(Mid$(XX$, 69, 4))
          VALOHO1# = CVD(Mid$(XX$, 73, 8))
          OPE2$ = FORMATNUM(OPE1#, "F3")
          HORA2$ = FORMATNUM(HORA1#, "F7.1")
          VALOHO2$ = FORMATNUM(VALOHO1#, "F12.4")
          Y$ = Space$(256)
          Call REPLA(Y$, LEY1$, 1, 60)
          Call REPLA(Y$, HORA2$, 61, 7)
          Call REPLA(Y$, OPE2$, 69, 4)
          Call REPLA(Y$, VALOHO2$, 74, 12)
          'CALCULO EL TOTAL DEL ITEM DE MANO DE OBRA
          IMPOIT1# = VALOHO1# * HORA1#
         ...
         '
          With DetaPresu
            ...
            .AddNew
              ...
              !SUBRUB_DET = 0
              !CODINTE_DET = 0
              !CODEXTE_DET = CODEXT$(0)
              !Cantid_Det = 0
              !PreUni_Det = 0
              !MONE_DET = 1 'POR AHORA
              !DESCRI_DET = LEY1$
              !Impo_Det = VALOHO1# * HORA1#
              !Hora_Det = HORA1#
              !Opera_Det = OPE1#
              !ValoHor_Det = VALOHO1#
            .Update
...
End Sub
Sub CARGODETPRESU() 'CARGO LISTA DE DET. DE UN PRESU DE MDO
    If TRIM$(Label31) <> "" Then
      NUPRESU$ = TRIM$(Label31)
      Call ABREPRECAM
      SQLX$ = "SELECT * FROM DETAPRESU "
...
15        IMPOIT2$ = FORMATNUM(IMPOIT1#, "F12.2")
    ...
    TOTCAM# = TOTPIS#
    Text38 = FORMATNUM(TOTCAM#, "F12.2") 'TOTAL
    Text13 = FORMATNUM(TOTCAM#, "F12.2") 'SUBTOTAL
    '
End Sub
...
Private Sub text38_change()
  '
  TOTCAM1# = Val(Text38)
  FORPRE04.Label62 = FORMATNUM(TOTCAM1#, "F11.2")
  '
End Sub
       ...
       Do While .EOF = False
        ...
        With DetaPresu
          .AddNew
            ...
            !Med2_Det = MED2
            !Med3_Det = MED3
            !Impo_Det = ((HORADET * VALOHORDET#) * TC1# / TC2#)
