...
Begin VB.Form COTCAM04 
   AutoRedraw      =   -1  'True
   BackColor       =   &H00D0E0FF&
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Presupuestación - Cotización de Cámara"
   ClientHeight    =   7785
   ClientLeft      =   45
   ClientTop       =   300
   ClientWidth     =   11910
   Icon            =   "COTCAM04.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ...
   Begin TabDlg.SSTab SSTab1 
      ...
      TabHeight       =   520
      TabCaption(0)   =   "Muros"
      TabPicture(0)   =   "COTCAM04.frx":074C
      ...
      Tab(0).Control(7)=   "Frame12"
      Tab(0).Control(7).Enabled=   0   'False
      Tab(0).Control(8)=   "Text1"
      Tab(0).Control(8).Enabled=   0   'False
      Tab(0).ControlCount=   9
      TabCaption(1)   =   "Piso"
      TabPicture(1)   =   "COTCAM04.frx":0768
      ...
      Tab(1).Control(4)=   "Frame17"
      Tab(1).Control(4).Enabled=   0   'False
      Tab(1).Control(5)=   "Frame18"
      ...
      TabCaption(2)   =   "Techo"
      TabPicture(2)   =   "COTCAM04.frx":0784
      ...
      Begin VB.PictureBox Picture3 
         ...
         Begin VB.ListBox List2 
            ...
            EndProperty
            Height          =   4200
            ItemData        =   "COTCAM04.frx":0835
            Left            =   -30
            List            =   "COTCAM04.frx":0837
            TabIndex        =   48
            Top             =   360
            Width           =   10830
         End
      ...
      Begin VB.PictureBox Picture2 
         ...
         Begin VB.ListBox List1 
            ...
            EndProperty
            Height          =   4200
            ItemData        =   "COTCAM04.frx":08CE
            Left            =   -30
            List            =   "COTCAM04.frx":08D0
            TabIndex        =   44
            Top             =   360
            Width           =   10830
         End
      ...
      Begin VB.PictureBox Picture5 
         ...
         Begin VB.ListBox List3 
            ...
            EndProperty
            Height          =   4200
            ItemData        =   "COTCAM04.frx":08D2
            Left            =   -30
            List            =   "COTCAM04.frx":08D4
            TabIndex        =   30
            Top             =   360
            Width           =   10830
         End
...
9   Screen.MousePointer = 1
    ...
    SQLX$ = SQLX$ + " AND SUBRUB_DET = " & SURUB% & " "
    DetaPresu.FindFirst SQLX$
    If DetaPresu.NoMatch = False Then
      If COMALTER%("El Presupuesto de Cámara " & PRESU2$ & " \ya Contiene Detalle en Rubro y Subrubro\¿Desea Sobre-Escribirlo ?\\Si, Sobre-escribir.\\No, Cancelar.") = 2 Then
          On Error GoTo 0
...
Private Sub Form_Load()
    ...
    'LEO LA LISTA DE PRECIOS DE CAMARAS
    TEXT21 = CONTROL("LIPREVEN", "CAMARA")
    '
End Sub
'
...
10      NUPRESU$ = TRIM$(Label31)
        ...
        ATRI$ = ATRI$ + "/TITUPAN=Detalle de Cotización - Piso"
        ATRI$ = ATRI$ + "/BORDEIZQ=W" + TRIM$(Str$(Left + SSTab1.Left + Picture2.Left))
        ATRI$ = ATRI$ + "/BORDESUP=W" + TRIM$(Str$(Top + SSTab1.Top + Picture2.Top + 80))
        VTB% = VENTABU%(PRF$ + ATRI$)
        '
        If VTB% >= 0 Then
          'BORRA LOS REGISTROS///
          With DetaPresu
20          .FindFirst "Nro_PRESU = '" & NUPRESU$ & "' AND RUBRO_DET = 1 AND SUBRUB_DET = 2"
          ...
          For KKIL& = 1 To J&
            ...
            MECUA1# = CVS(Mid$(XX$, 15, 4))
            PREUNI2$ = FORMATNUM(PREUNI1#, "F12.4")
            CANTI2$ = FORMATNUM(CANTI1#, "F7.1")
            MECUA$ = FORMATNUM(MECUA1#, "F6.2")
            Y$ = Space$(256)
            Call REPLA(Y$, CODI1$, 1, 16)
            Call REPLA(Y$, DESCRI1$, 19, 37)
            Call REPLA(Y$, PREUNI2$, 57, 12)
            Call REPLA(Y$, CANTI2$, 70, 7)
            Call REPLA(Y$, MECUA$, 78, 6)
              'CALCULO EL TOTAL DEL MATERIAL
            IMPOIT1# = PREUNI1# * MECUA1#
            IMPOIT2$ = FORMATNUM(IMPOIT1#, "F12.2")
            Call REPLA(Y$, IMPOIT2$, 85, 12)
            List1.AddItem Y$
            '
            With DetaPresu
...
10      NUPRESU$ = TRIM$(Label31)
        ...
        End With
        '
        Call SETULTREG("!DECOTECH", J&)
        Call CIERRARCH("!DECOTECH")
        '
        PRF$ = "DECOTECH"
        ATRI$ = "/NC/ARCHECO(1)=" + ARCHIPRE$
        ATRI$ = ATRI$ + "/TITUPAN=Detalle de Cotización - Techo/"
        ATRI$ = ATRI$ + "/BORDEIZQ=W" + TRIM$(Str$(Left + SSTab1.Left + Picture3.Left))
        ATRI$ = ATRI$ + "/BORDESUP=W" + TRIM$(Str$(Top + SSTab1.Top + Picture3.Top + 80))
        VTB% = VENTABU%(PRF$ + ATRI$)
        'NUEVA VALIDACION
        '
        If VTB% >= 0 Then
          '/// Aca FindFirst de NRO_PRESU+RUBRO+SUBRUBRO
          'BORRA LOS REGISTROS///
          With DetaPresu
20          .FindFirst "Nro_PRESU = '" & NUPRESU$ & "' AND RUBRO_DET = 1 AND SUBRUB_DET = 3"
            If .NoMatch = False Then
              .Delete
              GoTo 20
            End If
          ...
          For KKIL& = 1 To J&
            ...
            With DetaPresu
              ...
              .Update
            End With
            TOTECH# = TOTECH# + IMPOIT1#
            Text3 = FORMATNUM(TOTECH#, "F12.2")
7         Next KKIL&
        End If
      End If
    End If
...
Private Sub List3_dblClick()
    'GENERO EL DAT
    If TRIM$(Label31) <> "" Then
      If DERACCE%("GRACOTCA", "Grabar Cotización de Cámara") >= 2 Then
10      NUPRESU$ = TRIM$(Label31)
        Call ABREPRECAM
        SQLX$ = "SELECT * FROM DETAPRESU "
        SQLX$ = SQLX$ + "WHERE TRIM(Nro_presu) = '" & NUPRESU$ & "' "
        ...
        With DETPRETMP
          ...
          If Err < 1 Then
            ...
            Do Until .EOF = True
              ...
              Call REPLA(Y$, MKS$(CANTI1#), 11, 4)
                LONGI1# = !MED1_DET
              Call REPLA(Y$, MKS$(LONGI1#), 15, 4)
                MECUA1# = 0
                On Error Resume Next
                MECUA1# = !Med4_Det
                On Error GoTo 0
              Call REPLA(Y$, MKS$(MECUA1#), 19, 4)
              J& = J& + 1
              Call GRAREG("!DECOTCAM", Y$, J&)
            .MoveNext
            Loop
          End If
          On Error GoTo 0
        End With
        '
        Call SETULTREG("!DECOTCAM", J&)
        Call CIERRARCH("!DECOTCAM")
        '
        PRF$ = "DECOTCAM"
        ATRI$ = "/NC/ARCHECO(1)=" + ARCHIPRE$
        ATRI$ = ATRI$ + "/TITUPAN=Detalle de Cotización - Muros/"
        ATRI$ = ATRI$ + "/BORDEIZQ=W" + TRIM$(Str$(Left + SSTab1.Left + Picture5.Left))
        ATRI$ = ATRI$ + "/BORDESUP=W" + TRIM$(Str$(Top + SSTab1.Top + Picture5.Top + 80))
        VTB% = VENTABU%(PRF$ + ATRI$)
        '
        If VTB% >= 0 Then
          '/// Aca FindFirst de NRO_PRESU+RUBRO+SUBRUBRO
          'BORRA LOS REGISTROS///
          With DetaPresu
20          .FindFirst "Nro_PRESU = '" & NUPRESU$ & "' AND RUBRO_DET = 1 AND SUBRUB_DET = 1"
            If .NoMatch = False Then
              .Delete
              GoTo 20
            End If
          ...
          For KKIL& = 1 To J&
            ...
            PREUNI1# = CVD(Mid$(XX$, 3, 8))
            CANTI1# = CVS(Mid$(XX$, 11, 4))
            LONGI1# = CVS(Mid$(XX$, 15, 4))
            MECUA1# = CVS(Mid$(XX$, 19, 4))
            PREUNI2$ = FORMATNUM(PREUNI1#, "F12.4")
            CANTI2$ = FORMATNUM(CANTI1#, "F7.1")
            LONGI2$ = FORMATNUM(LONGI1#, "F8.3")
            MECUA$ = FORMATNUM(MECUA1#, "F6.2")
            Y$ = Space$(256)
            Call REPLA(Y$, CODI1$, 1, 16)
            Call REPLA(Y$, DESCRI1$, 19, 29)
            Call REPLA(Y$, PREUNI2$, 49, 12)
            Call REPLA(Y$, CANTI2$, 61, 7)
            Call REPLA(Y$, MECUA$, 69, 6)
            Call REPLA(Y$, LONGI2$, 76, 8)
            'CALCULO EL TOTAL DEL MATERIAL
            IMPOIT1# = PREUNI1# * MECUA1#
            IMPOIT2$ = FORMATNUM(IMPOIT1#, "F12.2")
            Call REPLA(Y$, IMPOIT2$, 85, 12)
           ...
           '
            ...
            With DetaPresu
              ...
              .Update
            End With
            TOTLAT# = TOTLAT# + IMPOIT1#
            Text1 = FORMATNUM(TOTLAT#, "F11.2")
7         Next KKIL&
        End If
      End If
    End If
...
Private Sub SSTab1_Click(PreviousTab As Integer)
  ...
  Select Case SSTab1.Tab
    Case 0
      Label19 = "Subtotal "
      Label19 = Label19 + SSTab1.Caption & ":"
      Text13 = Text1
    Case 1
      Label19 = "Subtotal "
      Label19 = Label19 + SSTab1.Caption & ":"
...
Private Sub Text1_Change()
  '
'  For I = 0 To COTCAM04.Controls.Count - 1
'    MsgBox COTCAM04.Controls(I).Name
'    If UCase(COTCAM04.Controls(I).Name) = UCase("TEXT12") Then
'      MsgBox "aca"
'    End If
'  Next I
...
15        IMPOIT2$ = FORMATNUM(IMPOIT1#, "F12.2")
          ...
          If !SUBRUB_DET = 1 Then
            List3.AddItem Y$
            TOTLAT# = TOTLAT# + IMPOIT1#
            Text1 = FORMATNUM(TOTLAT#, "F12.2")
          End If
          If !SUBRUB_DET = 2 Then
            List1.AddItem Y$
            TOTPIS# = TOTPIS# + IMPOIT1#
            Text2 = FORMATNUM(TOTPIS#, "F12.2")
          End If
          If !SUBRUB_DET = 3 Then
            List2.AddItem Y$
            TOTECH# = TOTECH# + IMPOIT1#
            Text3 = FORMATNUM(TOTECH#, "F12.2")
          End If
      ...
      End With
    End If
    'CALCULA EL  TOTAL CÁMARA
    TOTCAM# = TOTPIS# + TOTLAT# + TOTECH#
    Text38 = FORMATNUM(TOTCAM#, "F12.2")
    '
End Sub
...
Private Sub text38_change()
  '
  TOTCAM1# = Val(Text38)
  FORPRE04.Label40 = FORMATNUM(TOTCAM1#, "F11.2")
  '
End Sub
...
Private Sub TOTACAM()  'CALCULA EL  TOTAL CÁMARA
  '
  TOTLAT# = Val(Text1)
  TOTPIS# = Val(Text2)
  TOTECH# = Val(Text3)
  TOTCAM# = TOTPIS# + TOTLAT# + TOTECH#
  Text38 = FORMATNUM(TOTCAM#, "F11.2")
  '
End Sub
