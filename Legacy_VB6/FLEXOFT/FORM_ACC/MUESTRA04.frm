...
Begin VB.Form MUESTRA04 
   ...
   Begin ACTIVESKINLibCtl.Skin Skin1 
      Left            =   2625
      OleObjectBlob   =   "MUESTRA04.frx":030A
      Top             =   0
   End
   Begin ACTIVESKINLibCtl.SkinLabel SkinLabel999 
      ...
      Index           =   0
      Left            =   120
      OleObjectBlob   =   "MUESTRA04.frx":053E
      TabIndex        =   1
      Top             =   240
      Visible         =   0   'False
      Width           =   1485
...
Private Sub Command1_Click()
   ...
   If TRIM$(AREXPO$) <> "" Then
     If MSF_2.Visible = True Then
      Call GENEXCEL(AREXPO$, MUESTRA04.MSF_2)
     Else
      Call GENEXCEL(AREXPO$, MUESTRA04.GRID)
     End If
   End If
   '
...
Private Sub Command3_Click()
        ...
        For j& = 1 To MSFACTIVO.Rows - 1
           If MSFACTIVO.TextMatrix(j&, 0) = "*" Then
              FE% = CVINT(MUESTRA04.GRID.TextMatrix(j&, 1))
              NC = XVALO(MUESTRA04.GRID.TextMatrix(j&, 2))
              RASYDOMI$ = TRIM$(MUESTRA04.GRID.TextMatrix(j&, 3))
              CI1% = XVALO(MUESTRA04.GRID.TextMatrix(j&, 4))
              DESCRITRABAJO$ = MUESTRA04.GRID.TextMatrix(j&, 5)
              OPERARIO% = XVALO(MUESTRA04.GRID.TextMatrix(j&, 6))
              NAMEOPERARIO$ = TRIM$(MUESTRA04.GRID.TextMatrix(j&, 7))
              CANCUOTAS = XVALO(MUESTRA04.GRID.TextMatrix(j&, 8))
              ID = XVALO(MUESTRA04.GRID.TextMatrix(j&, 9))
        ...
        Next j&
        '
        If Seleccionados% > 0 Then Call Agenda.Command10_Click 'boton filtrar para actualizar
...
'    FEMI% = HOY(HO$)
    ...
    '        ANOTA$ = ANOTA$ + String$(88, "=") + Chr$(10) + Chr$(13)
            ...
            '\\\OT-08-0912-OD-FIN///
            SQLX$ = "UPDATE PEDENCA SET AnotaPed=" & "'" & ANOTA$ & "' WHERE NroPed=" & NROPEDI&
            Ventas.Execute (SQLX$)
       '
