VERSION 5.00
Begin VB.Form REPRIFAC 
   AutoRedraw      =   -1  'True
   BackColor       =   &H00C0FFFF&
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Repite Impresión de Factura en Cuenta Corriente"
   ClientHeight    =   8355
   ClientLeft      =   45
   ClientTop       =   300
   ClientWidth     =   11805
...
Private Sub Command1_Click()
   Call MENSERR(24, "Imposible Seleccionar Cliente\en Re-Impresión de Factura")
End Sub
...
Private Sub Label12_DblClick()
   If Label12 = "0001" Then
     ...
     Else
       Label12 = "0001"
   End If
   Call GRACONTROL("FACTURA", "PREFIFA", Label12)
   '
End Sub
...
Private Sub Form_Load()
    ...
    UPREFI$ = TRIM$(CONTROL$("FACTURA", "PREFIFA"))
    ...
    '
'    If PREFACTU$ <> "REMITO" Then
'      If PREFACTU$ <> "PEDIDO" Then
'        '\\\OT-5-884-FG-15/12/05
'        'Call MENSERR(24, "Imposible Facturar por " + PREFACTU$)
'        Call MENSERR(24, "Modalidad por Orden de Despacho\No Disponible en la Presente Versión")
'        Call Command5_Click ' CONECTA CON EL SETUP DE FACTURACION
'        '\\\OT-5-884-FG-FIN
'        Call FINAL("")
'      End If
'    End If
     ...
     If Abs(CANTAA) > 0 Then
       ...
       If CI% >= 0 Then                 ' para incluir remito texto libre
         ...
         If CANTAA > 0 Then ' PARA EVITAR CON NOTAS DE CREDITO
           ...
           If NUPEDIORI& > 0 Then
             Call BLANPEDIPE(CI%, CANAX#, NUPEDIORI&, NUORITPE%, FF%, REMILAR$)
           End If
         End If
         '
       End If
...
Sub BLANPEDIPE(CI%, CAN#, NUPEDIORI&, NUORITPE%, FF%, NROREMIT$)
    ' blanquear pedidos pendientes
    Call APERBASDAT("PEDCLI")
    '
    SQLX$ = " SELECT * FROM PEDDETA "
    SQLX$ = SQLX$ + " WHERE NroPed = " & NUPEDIORI& & " "
    SQLX$ = SQLX$ + " AND NuOrItem = " & NUORITPE% & " "   ' NO ANULADOS NI CUMPLIDOS
    SQLX$ = SQLX$ + " AND CodiInt = " & CI% & " "   ' APROBADOS
    '
    Set PEDTEMP = Ventas.OpenRecordset(SQLX$, dbOpenSnapshot)
    '
    With PEDTEMP
    ...
    '
    With PedDeta
      SQLX$ = " NroPed = " & NUPEDIORI& & " "
      SQLX$ = SQLX$ + " AND NuOrItem = " & NUORITPE% & " "
      SQLX$ = SQLX$ + " AND CodiInt = " & CI% & " "
      .FindFirst SQLX$
      If .NoMatch = False Then
        .Edit
        !CantEnt = !CantEnt + CAN#
        !FeUltEnt = CVDAT(FF%)
        !NroRemi = NROREMIT$
        If !TipoPed <> "ABIERTO" Or IsNull(!TipoPed) = True Then
          !CanSaldo = !CanPed - !CantEnt
...
Sub CALPERIB()
   ...
   '
   TASAX = TASAA
   If VARIDOC% = 2 Then TASAX = TASAB
   If VARIDOC% = 3 Then TASAX = TASAEXP
   '
   Text15 = ""
   NCLIE% = XVALO(Text1)
   ...
   If CATIB1% > 0 Then         ' NO ESTA EXENTO
     If PORCONVE >= 10.1 Then  ' PARTICIPACION CONVENIO MULTILATERAL
       ...
       'Lo que sigue se suprimió porque según M.Pazos (confirmado con Cont.Abadie
       'también en facturas 'B' la percepción se calcula sobre el NETO, no sobre
       'el bruto - Realizado por EPB 5/1/2007
       'If VARIDOC% = 2 Then BIMPON# = BIMPON# + XVALO(Total(3)) + XVALO(Total(4)) + XVALO(Total(5))
...
Sub CARDETPEDIPEN()
   ...
   SQLX$ = SQLX$ + "PEDDETA.FeSolEnt, PEDDETA.PreUnid, PEDDETA.MoneEmis,PEDDETA.TipoPed,"
     ...
     Do While (.EOF = False)
       ...
       If CODIMAT% > 0 And CODIMAT% <= NUMAS% Then
         ...
         If AGRUPAENTREGA% > 0 Then
            ...
            If LOCAENPE$ <> "" Then If TRANENPE$ <> LENTREGA(2) Then GoTo 28
         End If
         '\\\OT-07-0371-FL-FIN///
         '
5        Z$ = REGBLAN$("!CARDEFAC")
...
29   .MoveNext
   ...
   If DESCARTAPED% = 1 Then
      Call MENSERR(24, "Se han Descartados Pedidos que no Corresponden\con la Sucursal del Pedido más Antiguo")
   End If
    '\\\OT-07-0371-FL-FIN///
'
   JJ& = 0
...
Sub SELPEDIP()
   ...
   Call APERBASDAT("PEDCLI")
     '\\\OT-07-0371-FL-01/10/07///
   SQLX$ = " SELECT PEDDETA.codiint, PEDDETA.CanPed, PEDDETA.CantEnt, PEDDETA.NroPed,PEDDETA.FechEmis,"
   SQLX$ = SQLX$ + "PEDDETA.FeSolEnt, PEDDETA.PreUnid, PEDDETA.MoneEmis,PEDDETA.TipoPed,PEDDETA.NuOrItem,"
   SQLX$ = SQLX$ + "PEDENCA.DomiEnt, PEDENCA.LocaEnt, PEDENCA.TranspEnt"
   SQLX$ = SQLX$ + " FROM PEDDETA "
   SQLX$ = SQLX$ + "INNER JOIN PEDENCA ON (PEDDETA.NroPed = PEDENCA.NroPed) "
   SQLX$ = SQLX$ + " WHERE PEDDETA.NroClie = " & NC% & " "
   ...
   '
     '\\\OT-07-0371-FL-01/10/07///
  'SQLX$ = SQLX$ + " ORDER BY NroPed, NuOrItem;" 'ESTA LINEA LA AGREGUE
   SQLX$ = SQLX$ + " ORDER BY PEDDETA.NroPed, PEDDETA.NuOrItem;" 'ESTA LINEA LA AGREGUE
     '\\\OT-07-0371-FL-01/10/07///
   '
   Set PEDTEMP = Ventas.OpenRecordset(SQLX$, dbOpenSnapshot)
