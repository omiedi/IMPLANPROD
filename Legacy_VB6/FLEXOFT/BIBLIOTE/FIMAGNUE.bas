...
If OPCI% = 1 Or OPCI% = 5 Or NUEVO% = 1 Or CRG&(IARCH%) < 1 Then
    ...
    If NUEVO% = 1 Then
       ...
       If CRG&(IARCH%) < CARETO& - 128 Then
           ...
           FINUZ% = FILEOPEN%(ARY2$, 4, LRG%(IARCH%)) ' AQUI
           FINUX%(IARCH%) = FINUZ%
           RGL&(IARCH%) = 0
           PRIMOD&(IARCH%) = 0
           '
...
940 '***********************************************  abre efectivamente
    ...
    'FINU% = FreeFile
    ARY1$ = AX$(IARCH%): GoSub 910
    FINU% = FILEOPEN%(ARY2$, 4, LRG%(IARCH%)) 'AQUI
...
952 If FileLen(ARY2$) > LOF(FINUX%(IARCH%)) Then
       ...
       '
       If VUERABRE% > 3 Then
          FINU% = FILEOPEN%(ARY2$, 4, LRG%(IARCH%)) 'RE-ABRE EL ARCHIVO
          FINUX%(IARCH%) = FINU%
...
On Error GoTo 198
'
   BU2 = Left$(REGX$ + String(LOREG%, 0), LOREG%)
   Put #FINU%, 1 + (RECNO& - 1) * LOREG%, BU2
'
On Error GoTo 0
Exit Sub
'
...
Function CONTROL$(PROCE$, CLAVE$)
    ...
    '
'NNXX% = FreeFile
'Open LUDAT$ + "FLEXCRL.DAT" For Random Access Read Write Shared As #NNXX% Len = 96
