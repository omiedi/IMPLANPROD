...
5804 BLP% = Asc(Mid$(COD$, 4))
     VIV% = Asc(Mid$(COD$, 5))
     APPI% = Asc(Mid$(COD$, 6))
        FORVENTB.BOTAGRE.Enabled = True
        If APPI% < 1 Then
            FORVENTB.BOTAGRE.Enabled = False
        End If
     IDL% = Asc(Mid$(COD$, 7))
        FORVENTB.BOTINSE.Enabled = True
        FORVENTB.BOTERASE.Enabled = True
        If IDL% < 1 Then
            FORVENTB.BOTINSE.Enabled = False
            FORVENTB.BOTERASE.Enabled = False
        End If
     FORVENTB.BOTEDIT.Enabled = False
     HDR& = Asc(Mid$(COD$, 8))
     RFT% = Asc(Mid$(COD$, 9))
...
For KCL% = 1 To NCL%
      '
AAA = LC%(KCL%)
FORVENTB.MSFlexGrid1.Col = KCL% - 1
BBB = FORVENTB.MSFlexGrid1.CellAlignment
.ColWidth (KCL% - 1)
...
'
ENCA$ = ENCA$ + "   "
FORVENTB.Top = Screen.Height / 25 * (RGI&)
FORVENTB.Height = Screen.Height / 25 * (RGF& - RGI& - 1)
FORVENTB.Left = Screen.Width / 80 * (OFS% - 1)
'
If BSUW& <> 0 Then FORVENTB.Top = BSUW&
If BIZW& <> 0 Then FORVENTB.Left = BIZW&
'
FORVENTB.TEPRUEBA.Caption = ENCA$
