...
Begin VB.Form ANAVENCLI 
   AutoRedraw      =   -1  'True
   BackColor       =   &H00C0FFFF&
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Análisis Estadistico - Ventas por Cliente"
   ClientHeight    =   8355
   ClientLeft      =   45
   ClientTop       =   300
   ClientWidth     =   11910
   ...
   Begin MSChartLib.MSChart MSChart1 
      Height          =   4005
      Left            =   105
      OleObjectBlob   =   "ANAVENCLI.frx":07B4
      TabIndex        =   0
      Top             =   2415
      Width           =   11700
   End
...
Sub GENGRAFIC()
   ...
   Next UKI%
   For U& = 1 To List1.ListCount
     ACCLI%(Val(Mid$(List1.List(U& - 1), 65))) = U&
   Next U&
   '
   CAMESA% = Val(Label1)
   HACODI% = CODIMES%
   ...
   For U& = 1 To FIN&
      ...
      If CODIPE% >= DECODI% Then
        If CODIPE% <= HACODI% Then
          ...
          If AQQ% > 0 Then
            CI% = CVI(Mid$(Z$, 7, 2))
            TICAM = 1 ' FACONVERT(CODIPE%)
            '
            If CI% = (-1) Then
                IVEN(1, 1 + CODIPE% - DECODI%) = IVEN(1, 1 + CODIPE% - DECODI%) + CVD(Mid$(Z$, 169, 8)) / TICAM
                If AQQ% <= 2048 Then TOVEN(1, AQQ%, 1 + CODIPE% - DECODI%) = TOVEN(1, AQQ%, 1 + CODIPE% - DECODI%) + CVD(Mid$(Z$, 169, 8)) / TICAM
                If ACUCO%(1) > 0 Then ACCLI%(CLI%) = (-1) * AQQ%
              ElseIf CI% = (-2) Then
                IVEN(2, 1 + CODIPE% - DECODI%) = IVEN(2, 1 + CODIPE% - DECODI%) + CVD(Mid$(Z$, 169, 8)) / TICAM
                If AQQ% <= 2048 Then TOVEN(2, AQQ%, 1 + CODIPE% - DECODI%) = TOVEN(2, AQQ%, 1 + CODIPE% - DECODI%) + CVD(Mid$(Z$, 169, 8)) / TICAM
                If ACUCO%(2) > 0 Then ACCLI%(CLI%) = (-1) * AQQ%
              Else
                IVEN(0, 1 + CODIPE% - DECODI%) = IVEN(0, 1 + CODIPE% - DECODI%) + CVD(Mid$(Z$, 169, 8)) / TICAM
                If AQQ% <= 2048 Then TOVEN(0, AQQ%, 1 + CODIPE% - DECODI%) = TOVEN(0, AQQ%, 1 + CODIPE% - DECODI%) + CVD(Mid$(Z$, 169, 8)) / TICAM
                If ACUCO%(0) > 0 Then ACCLI%(CLI%) = (-1) * AQQ%
            End If
          End If
        End If
      End If
19 Next U&
   ...
   For UJ% = 1 To HACODI% - DECODI% + 1
      MSChart1.Row = UJ%
      For UL% = 0 To COLMAX%
        MSChart1.Column = UL% + 1
        MSChart1.Data = ACUCO%(UL%) * IVEN(UL%, UJ%)
      Next UL%
   Next UJ%
   '
   For U& = 1 To List1.ListCount
     AQQ% = ACCLI%(Val(Mid$(List1.List(U& - 1), 65)))
     If AQQ% < 0 Then
       AQQ% = Abs(AQQ%)
       TTXX$ = AJUSTI$(List1.List(U& - 1), 48)
       TTYY$ = ""
       For KI% = 1 To CAMESA%
         If KI% > CAMESA% - 24 Then
           IPAR = ACUCO%(0) * TOVEN(0, AQQ%, KI%) + ACUCO%(1) * TOVEN(1, AQQ%, KI%) + ACUCO%(2) * TOVEN(2, AQQ%, KI%)
           TTYY$ = TTYY$ + AJUSTD$(Format$(IPAR, "######0.00"), 10)
         End If
       Next KI%
       List3.AddItem TTXX$ + TTYY$
     End If
   Next U&
   '
   For U& = 1 To List3.ListCount
     List4.AddItem Mid$(List3.List(U& - 1), 49)
   Next U&
   '
...
Sub ARMATITU()
   ...
   For U& = 1 To List1.ListCount
     ...
     TTXX$ = TTXX$ + TRIM$(Left$(List1.List(U& - 1), 30))
   Next U&
