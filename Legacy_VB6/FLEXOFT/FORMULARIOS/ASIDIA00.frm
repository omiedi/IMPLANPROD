...
10 For U& = 1 To List1.ListCount
   ...
   '
   Call GRASIDIAR(NROAS, FEAS%, REFE$, CONTA%, CTA%(), IMPA#(), DETA$())
   If TIASI% = 3 Then
      Call MARCASCAJA
   End If
   '
...
83    List1.Clear
      For K1% = 1 To CONTA%
         ...
         If Asc(Left$(E8$, 1)) > 0 Then POSEDI% = (-1)
...
Sub MARCASCAJA()
   ...
   NROAS = Val(Text2.TEXT)
   FEAS% = FECHANUM(Text1.TEXT)
   If PRIMO% < 1 Then PRIMO% = 1
   FIN& = MAXDIA%
   For I% = PRIMO% To MAXDIA%
      U& = I%
      Call TRACE(20, U&, FIN&)
      Call LEESUCAJA(I%)
      If CVI(D11$) > 0 Then
