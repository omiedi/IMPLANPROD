VERSION 5.00
Begin VB.Form ANACHEMI 
   BackColor       =   &H00C0E0FF&
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Análisis de Pagos por Cheque"
   ClientHeight    =   2820
   ClientLeft      =   45
   ClientTop       =   300
   ClientWidth     =   5265
   ...
   Begin VB.Image Image2 
      Height          =   555
      Left            =   4050
      Picture         =   "ANACHEMI.frx":0454
      Stretch         =   -1  'True
      Top             =   2130
      Width           =   4725
   End
End
Attribute VB_Name = "ANACHEMI"
...
30  FEY = DIPOST(FEX)
    ...
    '
    JJ& = 0
    REBLA$ = REGBLAN$("ANACHEMI")
    For U& = 1 To ULTREG&("CHEQUEMI")
      X$ = REGLEIDO$("CHEQUEMI", U&)
      FECOBRA% = CVI(Mid$(X$, 47, 2))
      If FECOBRA% >= DES% Then
        If FECOBRA% <= HAS% Then
          NURECI = CVS(Mid$(X$, 41, 4))
          NCLIE% = CVI(Mid$(X$, 1, 2))
          BANCHE$ = TRIM$(Mid$(X$, 15, 16))
          If BANCHE$ = "" Then BANCHE$ = TRIM$(ECOARCH$("MEPAGO", Left$(X$, 2)))
          NUCHEBA$ = BANCHE$
          If NUCHEBA$ <> "" And TRIM$(Mid$(X$, 3, 12)) <> "" Then NUCHEBA$ = NUCHEBA$ + " - "
          NUCHEBA$ = NUCHEBA$ + TRIM$(Mid$(X$, 3, 12))
          IMPOCHE# = CVD(Mid$(X$, 33, 8))
          FACRE% = CVI(Mid$(X$, 49, 2))
