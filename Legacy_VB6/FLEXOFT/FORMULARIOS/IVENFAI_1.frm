...
Begin VB.Form IVENFAI_1 
   ...
   Begin MSChartLib.MSChart MSChart1 
      ...
      Index           =   0
      Left            =   210
      OleObjectBlob   =   "IVENFAI_1.frx":0000
      TabIndex        =   20
      Top             =   1680
      Visible         =   0   'False
      Width           =   9465
   ...
   Begin MSChartLib.MSChart MSChart1 
      ...
      Index           =   1
      Left            =   210
      OleObjectBlob   =   "IVENFAI_1.frx":303B
      TabIndex        =   21
      Top             =   1680
      Width           =   9465
   End
End
Attribute VB_Name = "IVENFAI_1"
...
'VER OBSERVACIONES MAS ABAJO
    ...
    '
    'Line (0, 1680)-(10965, Height), ESTAVENTA.BackColor, BF
    '
...
If PRELI < VUNI Then PRELI = VUNI
         ...
         '
         Z$ = REGBLAN$("LIANAFAC")
         GRCC% = GRUCOVE%(CII%): If GRCC% < 1 Then GRCC% = 999
...
Sub COSTIVEN()
   '
   Screen.MousePointer = 11
   Dim TOVEN(32767), COSVEN(32767), DEGRU$(32767)
   '
   VEGRUVE$ = ""
   CAGRU% = 0
   For U& = 1 To ULTREG&("GRUCOVEN")
   ...
   Next U&
   '
   MSChart1(1).RowCount = CAGRU%
   'MSChart1.Legend = "VENTA/COSTO"
   MSChart1(1).Title = "VENTAS VS. COSTO MATERIA PRIMA POR PRODUCTO"
   For KKI% = 1 To CAGRU%
     MSChart1(1).Row = KKI%
     MSChart1(1).RowLabel = DEGRU$(KKI%)
   Next KKI%
   '
   TOTAVEN = 0: TOTACOS = 0
   ...
   For U& = 1 To FIN&
      ...
      If CLI% = NC% Or NC% = 0 Then
         ...
         COSVEN(GRUCO%) = COSVEN(GRUCO%) + COSUNI(CI%) * CVD(Mid$(Z$, 17, 8))
         TOTAVEN = TOTAVEN + CVD(Mid$(Z$, 33, 8))
         TOTACOS = TOTACOS + COSUNI(CI%) * CVD(Mid$(Z$, 17, 8))
      End If
   Next U&
   '
   If TOTAVEN < 1 Then
   ...
   KKI% = 0
   For UI% = 1 To ULTGRU%
     If Abs(TOVEN(UI%)) >= 0.005 Then
       KKI% = KKI% + 1
       MSChart1(1).Row = KKI%
       MSChart1(1).Column = 1
       MSChart1(1).Data = TOVEN(UI%)
       MSChart1(1).Column = 2
       MSChart1(1).Data = COSVEN(UI%)
     End If
   Next UI%
   '
Exit Sub
       ...
       If CLI% > 0 Then
          If CLI% <= 32767 Then
             ...
             '   REG& = CVS(Mid$(PUNMO$(CLI%), KKI%, 4))
                ...
                '
                Z$ = REGBLAN$("LIANAFAC")
                GRCC% = GRUCOVE%(CII%): If GRCC% < 1 Then GRCC% = 99
                Call REPLA(Z$, MKI$(1), 1, 2)    ' productos
