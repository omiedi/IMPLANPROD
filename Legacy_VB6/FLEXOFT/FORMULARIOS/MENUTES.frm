...
Begin VB.Form MENUTES 
   ...
   Begin VB.CommandButton Command2 
      ...
      Height          =   800
      Left            =   4515
      Picture         =   "MENUTES.frx":0000
      Style           =   1  'Graphical
      TabIndex        =   23
      ToolTipText     =   "Imprimir Asientos Resumen"
      Top             =   4095
      Width           =   855
   End
   ...
   Begin VB.Image Image2 
      Height          =   510
      Left            =   3990
      Picture         =   "MENUTES.frx":1310
      Top             =   5440
      Width           =   2010
   End
End
Attribute VB_Name = "MENUTES"
...
10 OPX% = COMALTER%("Elija el Tipo de Asiento Resumen a Listar:\\Ventas\Compras\Caja y Bancos")
   If OPX% > 0 Then
     If OPX% <= 3 Then
       Call SELMESA(DES%, HAS%, CORRESP$)
       If CORRESP$ <> "" Then
         JJ& = 0
         TTXXY$ = Chr$(1) + Chr$(OPX%) + MKI$(HAS%)
         FIN& = ULTREG&("ASIDIAR")
         '
         For IL& = 1 To FIN&
         ...
         '
         CORR1$ = "Corresponde a: Resumen Mensual de "
         If OPX% = 1 Then CORR1$ = CORR1$ + "VENTAS"
         If OPX% = 2 Then CORR1$ = CORR1$ + "COMPRAS"
         If OPX% = 3 Then CORR1$ = CORR1$ + "CAJA Y BANCOS"
         CORRESP$ = CORR1$ + " de " + CORRESP$
         Call HEADERS("ASIDIAFI", "")
         Call HEADERS("ASIDIAFI", CORRESP$)
         '
...
10 OBX$ = OBJPLA$("TRANFONDOS", VDEF$)
   ...
   Call REPLA(Z$, "De " + TRIM$(ECOARCH$("ECUECON", CUEHABE$)) + " - " + REFE$, 71, 40)
   ...
   Call REPLA(Z$, MKI$(0), 67, 2)
   Call REPLA(Z$, "TF", 69, 2)
   Call REPLA(Z$, "A " + TRIM$(ECOARCH$("ECUECON", CUEDEBE$)) + " - " + REFE$, 71, 40)
   Call REPLA(Z$, MKD$(0), 153, 8)
