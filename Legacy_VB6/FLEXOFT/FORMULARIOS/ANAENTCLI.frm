...
Begin VB.Form ANAENTCLI 
   AutoRedraw      =   -1  'True
   BackColor       =   &H00C0FFC0&
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Análisis Estadistico - Entregas por Cliente"
   ClientHeight    =   8355
   ClientLeft      =   45
   ClientTop       =   300
   ClientWidth     =   11910
   ...
   Begin MSChartLib.MSChart MSChart1 
      Height          =   4005
      Left            =   105
      OleObjectBlob   =   "ANAENTCLI.frx":07B4
      TabIndex        =   0
      Top             =   2415
      Width           =   11700
   End
...
5  Screen.MousePointer = 1
   ...
   '
   ' En la primera hoja los clientes seleccionados
   TTXX$ = "Cuenta"
   RENGL& = 1: COLUL& = 1
   ...
   For U& = 1 To List1.ListCount
     Call TRACE(20, U&, FIN&)
     RENGL& = RENGL& + 1
     TTXX$ = Mid$(List1.List(U& - 1), 65, 8)
     COLUL& = 1
     XLSHEET.Cells(RENGL&, COLUL&) = Val(TTXX$)
     TTXX$ = TRIM$(Left$(List1.List(U& - 1), 64))
     COLUL& = 2
     XLSHEET.Cells(RENGL&, COLUL&) = Chr$(39) + TTXX$
   Next U&
   '
   ' en la segunda hoja el detalle por ARTICULO
   Set XLSHEET = XLBOOK.Worksheets.Add
   TTXX$ = "Codigo"
   RENGL& = 1: COLUL& = 1
   XLSHEET.Cells(RENGL&, COLUL&) = Chr$(39) + Left$(TTXX$, 48)
   ...
   '
   TTYY$ = Label4
   For CLXX% = 1 To Len(TTYY$) Step 8
     TENU$ = TRIM$(Mid$(TTYY$, CLXX%, 8))
     COLUL& = COLUL& + 1
   ...
   For U& = 1 To List3.ListCount
     ...
     TTXX$ = List3.List(U& - 1)
     RENGL& = RENGL& + 1
     CIXI% = Val(Mid$(TTXX$, 121, 8))
     COLUL& = 1
     XLSHEET.Cells(RENGL&, COLUL&) = Chr$(39) + CODEXT$(CIXI%)
     COLUL& = 2
     XLSHEET.Cells(RENGL&, COLUL&) = Chr$(39) + DESCRIT0$(CIXI%)
     '
     TTYY$ = List4.List(U& - 1)
     For CLXX% = 1 To Len(TTYY$) Step 8
       TENU$ = TRIM$(Mid$(TTYY$, CLXX%, 8))
...
Sub GENGRAFIC()
   ...
   '
   Call PREMESES
   MSChart1.Plot.Axis(1).AxisTitle.TEXT = "Unidades"
   '
   TOTAVEN = 0: TOTACOS = 0: CACODACT% = 0
   FIN& = ULTREG&("RVECLART")
   For U& = 1 To FIN&
      Call TRACE(20, U&, FIN&)
        ...
        If CODIPE% <= HACODI% Then
          ...
          If AQQ% > 0 Then
            ...
            If CI% > 0 Then
              ICOD% = ACCOD%(CI%)
              If ICOD% < 1 Then
                If CACODACT% >= 2048 Then GoTo 19
                CACODACT% = CACODACT% + 1
                ICOD% = CACODACT%
                ACCOD%(CI%) = ICOD%
                TTXYZ$ = AJUSTI$(TRIM$(CODEXT$(CI%)) + " - " + DESCRIT0$(CI%), 128)
                Call REPLA(TTXYZ$, TRIM$(Str$(CI%)) + Space$(8), 121, 8)
                List3.AddItem TTXYZ$ + Str$(ICOD%)
              End If
              '
              TOVEN(ICOD%, 1 + CODIPE% - DECODI%) = TOVEN(ICOD%, 1 + CODIPE% - DECODI%) + CVD(Mid$(Z$, 51, 8))
            End If
          End If
        End If
      End If
19 Next U&
   '
   For U& = 1 To List3.ListCount
     ICOD% = Val(Mid$(List3.List(U& - 1), 129))
     TTYY$ = ""
     For KI% = 1 To CAMESA%
       If KI% > CAMESA% - 24 Then
         IPAR = TOVEN(ICOD%, KI%)
         TTYY$ = TTYY$ + AJUSTD$(Format$(IPAR, "#####0.0"), 8)
       End If
     Next KI%
     List4.AddItem TTYY$
   Next U&
...
Sub PREMESES()
   ...
   For KKI% = DECODI% To HACODI%
     ...
     If KKI% > HACODI% - 24 Then
       Label4 = Label4 + AJUSTD$(ABREMESAN$(KKI%, 0), 8)
     End If
   Next KKI%
