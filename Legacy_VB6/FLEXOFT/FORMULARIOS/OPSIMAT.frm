VERSION 5.00
Begin VB.Form OPSIMAT 
   BackColor       =   &H00C0FFFF&
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Solicitud Interna de Materiales - Consultas y Listados"
   ClientHeight    =   5595
   ClientLeft      =   45
   ClientTop       =   300
   ClientWidth     =   5280
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VB.PictureBox Picture1 
         ...
         Begin VB.CommandButton Command45 
            ...
            Height          =   600
            Left            =   0
            Picture         =   "OPSIMAT.frx":0000
            Style           =   1  'Graphical
            TabIndex        =   24
            ToolTipText     =   "Anulación de SIM's"
            Top             =   0
            Width           =   560
         End
         Begin VB.Label Label1 
            BackStyle       =   0  'Transparent
            Caption         =   "Anulación de Soli- citudes Emitidas"
            BeginProperty Font 
               Name            =   "MS Sans Serif"
               Size            =   8.25
               Charset         =   0
      ...
      Begin VB.PictureBox Picture14 
         ...
         Begin VB.Label Label15 
            BackStyle       =   0  'Transparent
            Caption         =   "Ver y Re-Imprimir Soli- citud Interna Material"
            BeginProperty Font 
               Name            =   "MS Sans Serif"
               Size            =   8.25
               Charset         =   0
   ...
   Begin VB.Frame Frame3 
      ...
      Begin VB.PictureBox Picture3 
         ...
         Begin VB.Label Label3 
            BackStyle       =   0  'Transparent
            Caption         =   "Por Código de Artículo / Material"
            BeginProperty Font 
               Name            =   "MS Sans Serif"
               Size            =   8.25
               Charset         =   0
      ...
      Begin VB.PictureBox Picture5 
         ...
         Begin VB.Label Label4 
            BackStyle       =   0  'Transparent
            Caption         =   "Por Número de Sol. Int. Mat"
            BeginProperty Font 
               Name            =   "MS Sans Serif"
               Size            =   8.25
               Charset         =   0
   ...
   Begin VB.Image Image2 
      Height          =   510
      Left            =   3705
      Picture         =   "OPSIMAT.frx":199A
      Top             =   4920
      Width           =   2010
   End
End
Attribute VB_Name = "OPSIMAT"
...
2110 X$ = REGSEL$("INDIPEP/Pedidos Pendientes de Entrega")
     ...
     '
     RFQ$ = RECFILT$("PEDDETA", 1, MKS$(NUOBRA&))
     ...
     Next UKKK&
     '
     RFQ$ = RECFILT$("DESPAPED", 1, MKS$(NUOBRA&))
     For UKKK& = 1 To Len(RFQ$) Step 4
       UU& = CVS(Mid$(RFQ$, UKKK&, 4))
       XX$ = REGLEIDO$("DESPAPED", UU&)
       If Asc(Mid$(XX$, 27, 1)) < 9 Then
         If CVS(Mid$(XX$, 95, 4)) > 0 Then
           Call MENSERR(24, "Imposible Modificar.\Ya se Despachó Material.")
           GoTo 2110
         End If
       End If
     Next UKKK&
...
2120 If REGENCA& < 1 Or REGENCA& > ULTREG&("PEDENCA") Then
     ...
     MODPEDCLI.Text11.TEXT = TRIM$(Str$(NUOBRA&))
...
2210 X$ = REGSEL$("INDIPEP/Pedidos Pendientes para Anulación")
     ...
     For UKKK& = 1 To Len(RFQ$) Step 4
       UU& = CVS(Mid$(RFQ$, UKKK&, 4))
       XX$ = REGLEIDO$("DESPAPED", UU&)
       If Asc(Mid$(XX$, 21, 1)) < 9 Then
         If CVS(Mid$(XX$, 95, 4)) > 0 Then
           Call MENSERR(24, "Imposible Anular.\Ya se Despachó Material.")
           GoTo 2210
         End If
       End If
     Next UKKK&
     ...
     If CVS(Left$(RPENCA$, 4)) <> NUOBRA& Then
        Call MENSERR(24, "Imposible Anular este Pedido")
        GoTo 2210
     End If
     '
     ...
     '
     RFQ$ = RECFILT$("PEDDETP", 1, MKS$(NUOBRA&))
     For UKKK& = 1 To Len(RFQ$) Step 4
        I& = CVS(Mid$(RFQ$, UKKK&, 4))
...
2210 X$ = REGSEL$("INDIPEP/Pedidos Pendientes para Anulación por Items")
     ...
     REGENCA& = CVS(Mid$(X$, 53, 4))
     '
     DPEDI$ = TRIM$(Str$(NUOBRA&)) + " del " + TRIM$(Mid$(X$, 5, 48))
...
1205 TINI = Timer
     ...
     '
     RFF$ = RECFILT$("PEDDETA", 1, MKS$(NUOBRA&)) ' SOLO PENDIENTES DE ENTREGA Y NO DESPACHADOS
     For UU& = 1 To Len(RFF$) Step 4
       UJ& = CVS(Mid$(RFF$, UU&, 4))
       REGPEDA& = UJ&
...
Sub Command4_Click()
    ...
    If Option1.Value = True Then
      ...
      Call FINAL(SALI$)
    Else
10    VDEF$ = MKI$(COMESACT%) + MKI$(HOY(HO$))
      OBX$ = OBJPLA$("DESHASFECHA", VDEF$)
      If OBX$ = "" Then GoTo 99
      '
      FLI1% = CVI(Left$(OBX$, 2))
      ...
      '
      JJ& = 0
      Call COPYSTRU("BDSIMAT", "BDSIMSEL")
      FIN& = ULTREG&("BDSIMAT")
      For U& = 1 To FIN&
        Call TRACE(20, U&, FIN&)
        ...
        If FEXI% >= FLI1% Then
          If FEXI% <= FLI2% Then
            JJ& = JJ& + 1
            Call GRAREG("BDSIMSEL", XX$, JJ&)
          End If
        End If
      Next U&
      Call SETULTREG("BDSIMSEL", JJ&)
      '
      Call FINAL("*LISIMNU")
    End If
    '
...
10 XX$ = REGSEL$("INDISIM")
   If Len(XX$) > 4 Then
     NUSIM& = CVS(Left$(XX$, 4))
     '
     If NUSIM& > 0 Then
       If InStr(1, Mid$(XX$, 5, 60), "anul") = 0 Then
         If COMALTER%("Confirme la Anulación de la S.I.M. Nro. " + TRIM$(Str$(NUSIM&)) + "\\No, Conservarla\Si, Anularla") = 2 Then
            Call BLOQARCH("BDSIMAT")
            JJ& = 0
            FIN& = ULTREG&("BDSIMAT")
            For U& = 1 To FIN&
              Call TRACE(20, U&, FIN&)
              YY$ = REGLEIDO$("BDSIMAT", U&)
              If CVS(Left$(YY$, 4)) <> NUSIM& Then
                JJ& = JJ& + 1
                Call GRAREG("BDSIMAT", YY$, JJ&)
              End If
            Next U&
            Call SETULTREG("BDSIMAT", JJ&)
            '
            XX$ = FILTERGETRECORD$("INDISIM", 1, MKS$(NUSIM&))
            TTXX$ = "(anul) " + Mid$(XX$, 5)
            Call REPLA(XX$, TTXX$, 5, 60)
            Call FILTERPUTRECORD("INDISIM", 1, MKS$(NUSIM&), XX$)
            Call CIERRARCH("INDISIM")
            Call FRESHECHO("INDISIM")
            '
            TTXX$ = Space$(76)
            HOII% = HOY(HOS$)
            Call REPLA(TTXX$, HOS$, 1, 8)
            Call REPLA(TTXX$, "Anulación de Solicitud de Materiales:", 11, 56)
            Call REPLA(TTXX$, USERNAME$, 69, 24)
            Call REGAPP("ANOTASIM", MKS$(NUSIM&) + TTXX$)
            Call REGAPP("ANOTASIM", MKS$(NUSIM&) + String$(88, "="))
            '
         End If
       Else
         Call COMUNI("No se Puede Anular una SIM ya Anulada")
       End If
...
Private Sub Command5_Click()
   ...
   Call SELECHO("INDISIM/Indice General de Solicitudes Internas de Materiales")
...
10    VDEF$ = MKI$(COMESACT%) + MKI$(HOY(HO$))
      ...
      FEX = FLI2%: PERIO$ = PERIO$ + " - " + FECHATEX$(FEX)
      '
      MODO% = COMALTER%("Elija Modo de Selección:\\Solo Pendientes\Todas las SIM's")
      ...
      For U& = 1 To FIN&
        ...
        If FEXI% >= FLI1% Then
          If FEXI% <= FLI2% Then
            If CVS(Mid$(XX$, 131, 4)) < 1 Or MODO% = 2 Then
              JJ& = JJ& + 1
              CC$ = Mid$(XX$, 65, 2)
              XXX$ = CC$ & XX$
              Call GRAREG("SIMCOD", XXX$, JJ&)
            End If
          End If
        End If
      Next U&
      Call SETULTREG("SIMCOD", JJ&)
      Call FILESORT("SIMCOD", "", 1, 1, "ASC")
      '
      If MODO% = 1 Then Call FINAL("*LISIMCP")
      If MODO% = 2 Then Call FINAL("*LISIMCO")
    End If
    '
      ...
      For U& = 1 To FIN&
        ...
        If FEXI% >= FLI1% Then
          If FEXI% <= FLI2% Then
            ...
            CC$ = Mid$(XX$, 7, 8)
            XXX$ = CC$ & XX$
            Call GRAREG("SIMOBRA", XXX$, JJ&)
          End If
        End If
      Next U&
      Call SETULTREG("SIMOBRA", JJ&)
      Call FILESORT("SIMOBRA", "", 1, 1, "ASC")
      '
      Call FINAL("*LISIMOB")
    End If
    '
