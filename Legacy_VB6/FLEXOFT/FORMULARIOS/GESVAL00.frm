...
Begin VB.Form GESVAL00 
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VB.OptionButton Option8 
         BackColor       =   &H00C0E0FF&
         Caption         =   "Resumen Saldos de Tesorería"
         BeginProperty Font 
            Name            =   "MS Sans Serif"
            Size            =   8.25
            Charset         =   0
   ...
   Begin VB.Image Image2 
      Height          =   510
      Left            =   5145
      Picture         =   "GESVAL00.frx":23F0
      Top             =   630
      Width           =   2010
   End
End
Attribute VB_Name = "GESVAL00"
...
30 NCHE$ = OBJPLA$("CARNUCHE", NCHE$)
   ...
   RFF$ = RECFILT$("CAJABANC", 9, "CH")
   '
   JJ& = 0
   FIN& = Len(RFF$)
   '
   For KI& = 1 To FIN& Step 4
      Call TRACE(20, KI&, FIN&)
      I& = CVS(Mid$(RFF$, KI&, 4))
      X0$ = REGLEIDO$("CAJABANC", I&)
      ICHX$ = Mid$(X0$, 145, 8)
      X$ = Mid$(X0$, 71, 40) + String$(8, 0) + ICHX$
      NCHX$ = Mid$(X$, 19, 12)
      '
      If NCHE$ = "" Then
         If TRIM$(NCHX$) = "" Then
...
39 Next KI&
   ...
   '
45 RECAJA& = CVS(Mid$(Y$, 45, 4))
   YZ$ = REGLEIDO$("CAJABANC", RECAJA&)
   If Asc(Mid$(YZ$, 121, 1)) = 0 Then
     Call REPLA(YZ$, Chr$(1), 121, 1)
     Call GRAREG("CAJABANC", YZ$, RECAJA&)
   End If
   '
50 X$ = OBJPLA$("MUESTRACHEGES", YZ$)
   If X$ <> "" Then
     ...
     '
     If RECAJA& > 0 Then
       If RECAJA& <= ULTREG&("BADACHE") Then
         Call GRAREG("CAJABANC", X$, RECAJA&)
       End If
     End If
   End If
   GoTo 30
...
10 OBX$ = OBJPLA$("DESHESFECHREC", VDEF$)
   ...
   For I& = 1 To FIN& Step 4
     ...
     End If
     '
     FEVENCHE% = CVI(Mid$(X$, 63, 2))
     Z$ = CSTRING$(MKI$(FEVENCHE%), 1, 6, 0, 2) + CSTRING$(MKS$(RECHE&), 4, 8, 0, 2)
...
19 Next I&
   ...
   Z0$ = REGBLAN$(ASALID$)
   For JI& = 1 To FIN&
      RECHE& = Val(Mid$(FORMALTER.LINOSORT.List(JI& - 1), 7, 8))
      X$ = REGLEIDO$("CAJABANC", RECHE&)
      '
...
Sub LICHEQEM()
   ...
   For UB& = 1 To ULTREG&("CUEBANC")
     ...
     If CUEXTE$ <> "" Then
       ...
       If CUINTE% > 0 Then
         ...
         '
         RFF$ = RECFILT$("CAJABANC", 5, MKI$(CUINTE%))
         For KU& = 1 To Len(RFF$) Step 4
           RECAJA& = CVS(Mid$(RFF$, KU&, 4))
           X$ = REGLEIDO$("CAJABANC", RECAJA&)
           If Asc(Mid$(X$, 121, 1)) < 9 Then ' no esta anulado
             FACRE% = CVI(Mid$(X$, 63, 2))
             If FACRE% >= DESEM% Then
               CODMO$ = UCase$(Mid$(X$, 69, 2))
               If CODMO$ = "CP" Then
                 IMPEX# = CVD(Mid$(X$, 161, 8))
                 If IMPEX# >= 0.005 Then
                   '
                   PROV% = Abs(CVI(Mid$(X$, 67, 2)))
                   NUMERO$ = TRIM$(Mid$(X$, 89, 14))
                   BANCO$ = TRIM$(Mid$(X$, 71, 18))
                   EMIS% = CVI(Mid$(X$, 111, 2))   ' EMISION=FECHA O/PAGO
                   FENTREG% = CVI(Mid$(X$, 61, 2))
                   If EMIS% < 1 Then
                     EMIS% = FENTREG%
                   End If
                   IMPO# = IMPEX#
   ...
   REGB& = ICUEB%
   Call SETULTREG("ICUEBAN", REGB&)
   Call CIERRARCH("ICUEBAN")
   '
