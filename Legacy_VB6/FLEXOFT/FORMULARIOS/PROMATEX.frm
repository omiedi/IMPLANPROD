...
Begin VB.Form PROMATEX 
   ...
   Begin VB.Image Image2 
      Height          =   510
      Left            =   10320
      Picture         =   "PROMATEX.frx":2E46
      Top             =   7680
      Width           =   2010
   End
End
Attribute VB_Name = "PROMATEX"
...
'Dim CODIPRO%(32767)
Dim CIPRG%(32767), FEPRG%(32767), CAPRG(32767), FIPRG&
Dim CACODIP%
...
10     FEX1 = DIPOST(FEX1)
     ...
     End If
ABC$ = FECHATEX$(FEX1)
     FEX = FEX1 'FECHANUM(INISEM(KKI%).Caption + "/" + Right$(Label2, 2))
     ...
     'If DELX < COMESACT Then Text3(KKI%).Enabled = False
     '
     SEMI%(KKI%) = SMX%
     If SMX% < SMX0% - 1 Then
...
10         If DIASEM%(FEX) <> 1 Then
           ...
           For JJ% = 0 To 52
             ...
             If CAPROG > 0.05 Then
               SMX1% = SMX0% + JJ%: If SMX1% > 53 Then SMX1% = SMX1% - 53
               While SEMANU%(FEX, DELX, ALX) <> SMX1%
                 FEX = DIPOST(FEX)
               Wend
               While DIASEM(FEX) <> 1
                 FEX = DIPOST(FEX)
               ...
               FENTRE% = FEX: If FENTRE% < COMESACT% Then FENTRE% = COMESACT%
...
Sub MUEPROGENT()
   ...
   For UI& = 1 To FIPRG&
     ...
     If CI% > 0 Then
       If CI% <= NMS% Then
         CODE$ = TRIM$(CODEXT$(CI%))
         For L1& = 1 To List1.ListCount
           If TRIM$(Left$(List1.List(L1& - 1), 16)) = CODE$ Then
             TTXX$ = List1.List(L1& - 1)
             List1.RemoveItem (L1& - 1)
             GoTo 10
           End If
         Next L1&
         TTXX$ = Space$(128)
...
10
         If PROGENT$(UI&) <> String$(212, 0) Then
           ...
           For JJ% = 0 To 52
             CAPROG = CVS(Mid$(PROGENT$(UI&), 1 + 4 * JJ%, 4))
             SEMANI% = JJ% + SMX0%
If SEMI%(0) < 53 And SEMANI% >= 53 Then SEMANI% = SEMANI% - 1
             SUMACOD = SUMACOD + CAPROG
             If SEMANI% < SEMI%(0) Then
                 CAPRO(-1) = CAPRO(-1) + CAPROG
               ElseIf SEMANI% > SEMI%(5) Then
                 CAPRO(6) = CAPRO(6) + CAPROG
               Else
                 CAPRO(SEMANI% - SEMI%(0)) = CAPRO(SEMANI% - SEMI%(0)) + CAPROG
             End If
           Next JJ%
           For KKI% = -1 To 6
...
Sub GRAPROCAR()
   ...
   KKI% = CACODIP%
   '
10 'If SEMI%(0) < 53 And SEMI%(0) >= 48 Then kkii% = 1
   For JJ% = 0 To 52
     SEMANI% = JJ% + SMX0%
     If SEMANI% >= SEMI%(0) Then
       If SEMANI% <= SEMI%(5) Then
         On Error Resume Next
         Mid$(PROGENT$(KKI%), 1 + 4 * (JJ% + KKII%), 4) = MKS$(Val(Text3(SEMANI% - SEMI%(0))))
         On Error GoTo 0
       End If
     End If
   Next JJ%
