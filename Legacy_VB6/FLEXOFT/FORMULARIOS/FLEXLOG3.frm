...
Begin VB.Form FLEXLOG3 
   ...
   Begin VB.PictureBox Picture2 
      ...
      Begin VB.CommandButton Command4 
         ...
         Left            =   120
         TabIndex        =   20
         ToolTipText     =   "Verifica La Correcta Instalación de Controladores (OCX) y Bibliotecas Dinamicas (DDL) en esta Estacion de Trabajo"
         Top             =   1440
         Width           =   615
      End
   ...
   Begin VB.Menu archivo 
      ...
      Begin VB.Menu ss 
         Caption         =   "-"
      End
      Begin VB.Menu confilicencia 
         Caption         =   "Administrar Licencias"
      End
      Begin VB.Menu s 
         Caption         =   "-"
      End
...
Sub ACTUALIZA_CONSULTA(ARCHISQR$, OK%)
   If Trim$(ARCHISQR$) <> "" Then
       TEXMO$ = LOADFILE$(ARCHISQR$)
       TTXX$ = TEXMO$
       '
       CODMO$ = ""
       DESMO$ = ""
       ...
       While Len(TTXX$) > 0
          ...
          If Left$(COMAI$, 10) = "CODQUERY =" Then
            ...
            ElseIf Left$(COMAI$, 10) = "TIPQUERY =" Then
              TIPMO% = Val(Mid$(COMAI$, 11))
          End If
19     Wend
       '
       OK% = 1
       Call GRABAR_CONSULTAENSQL(CODMO$, DESMO$, TIPMO%, TEXMO$, OK%)
       Call GRABAR_CONSULTAENMDB(CODMO$, DESMO$, TIPMO%, TEXMO$, OK%)
       '
20 End If

End Sub
Sub GRABAR_CONSULTAENSQL(CODMO$, DESMO$, TIPMO%, TEXMO$, OK%)
       OK% = 1
       If Trim$(CODMO$) = "" Then
          MsgBox "Imposible Registrar Consulta sin Código"
          OK% = -1
          Exit Sub
       End If
       '
       SQLX$ = "SELECT COUNT(*) FROM INFORMATION_SCHEMA.TABLES WITH(NOLOCK) "
       SQLX$ = SQLX$ + " WHERE TABLE_NAME = 'SQL_QUERIES'"
       Dim RS As New ADODB.Recordset
       Set RS = MiConn.Execute(SQLX$)
       If Err Then On Error GoTo 0: OK% = -1: Exit Sub
       ...
       If EXISTE_LA_TABLA% > 0 Then
            ...
            RS1.Open SQLX$, MiConn, adOpenKeyset, adLockPessimistic, adCmdText
            With RS1
              If .EOF Then
                .AddNew
                !COD_QUERY = CODMO$
...
Sub GRABAR_CONSULTAENMDB(CODMO$, DESMO$, TIPMO%, TEXMO$, OK%)
       OK% = 1
       If Trim$(CODMO$) = "" Then
          MsgBox "Imposible Registrar Consulta sin Código"
          OK% = -1
...
End Sub
Sub CREACAMPOMICONN(NBASE$, NTABLA$, NCAMPO$, TCAMPO, LOCAMPO, Optional CONINDICE%)
    '
    ...
    '
    If BASEACTIVA$ = "" Then
       TX1$ = "Error Fatal en CREACAMPOMICONN" + vbCrLf
       TX1$ = TX1$ + "Falta Abrir Conexion y Base Inicial."
       MsgBox TX1$
       Close: End
    End If
    '
10  ' Verifica si existe BASEACTIVA
    ...
    If Err Then
       MsgBox "Error No Recuperable en Rutina 'CREACAMPO'\Buscando B.Datos '" + BASEACTIVA$
       Close: End
    End If
    On Error GoTo 0
    If (MIRS.EOF And MIRS.BOF) Then
       MsgBox "Error Fatal en CREACAMPO:\No Existe Base Inicial '" + BASEACTIVA
       Close: End
...
15  If YACREADA% < 2 Then
        MiConn.Execute ("USE " + BASEACTIVA$)
        YACREADA% = 2
    End If
    '
    EXICA% = EXISTELCAMPOMICONN%(NBASE$, NTABLA$, NCAMPO$, TCAMPO, LOCAMPO, CONINDICE%)
    If EXICA% = 3 Then
...
'       End If
    ...
    Select Case TCAMPO
      ...
      Case Else
        MsgBox "Imposible Crear Campo '" + NCAMPO$ + "' con Tipo = " & TCAMPO
        Close: End
    End Select
...
22  SQLX$ = "Select NAME from sysobjects where name = '" + NTABLA$ + "'"
    Set MIRS = MiConn.Execute(SQLX$)
    If (MIRS.EOF And MIRS.BOF) Then
       If VUELTA% < 1 Then
           strsql = "CREATE TABLE " & NTABLA$ & " (" + TXCAMPO$ + ") ON [PRIMARY]"
           On Error Resume Next
           MiConn.Execute (strsql)      ' CREO LA TABLA Y EL PRIMER CAMPO
           MIRS.Close
           Set MIRS = Nothing
           On Error GoTo 0
           VUELTA% = VUELTA% + 1
         ...
         Else
           MsgBox "Imposible Crear la Tabla '" + NTABLA$ + "' en '" + BASEACTIVA$
       End If
    End If
    '
    On Error Resume Next
    MiConn.Execute ("ALTER TABLE " & NTABLA$ & " ADD " & TXCAMPO$)
    On Error GoTo 0
    '
...
'    STRSQL = STRSQL & "and syscolumns.name ='" & NCAMPO$ & "'"
    ...
    If Not (MIRS.EOF And MIRS.BOF) Then
        If MIRS!xtype1 <> TSQL Or ((TSQL = 173 Or TSQL = 231) And LOCAMPO > MIRS!LENG / 2) Then
           ...
           TECOMU$ = "FLEXOFT Debe Ahora Modificar el Formato del Campo" + vbCrLf
           TECOMU$ = TECOMU$ + " & NCAMPO$ " & " en la Tabla '" + NTABLA$ + "' de " & MIRS!xtype1 + vbCrLf
           If (TSQL = 173 Or TSQL = 231) Then TECOMU$ = TECOMU$ + "(" & MIRS!LENG / 2 & ")" + vbCrLf
           TECOMU$ = TECOMU$ & " a " & TSQL + vbCrLf
           If (TSQL = 173 Or TSQL = 231) Then TECOMU$ = TECOMU$ + "(" & LOCAMPO & ")" + vbCrLf
           TECOMU$ = TECOMU$ & "Esto Puede Demandar Varios Minutos" + vbCrLf
           TECOMU$ = TECOMU$ & "Pulse 'Aceptar' Para Continuar." + vbCrLf
           ...
           Set MIRS1 = MiConn.Execute(strsql)
           If Not (MIRS1.EOF And MIRS1.BOF) Then   ' EXISTE INDICE - LO BORRA
              On Error Resume Next
               MiConn.Execute ("DROP INDEX " + NTABLA$ + ".I" + NCAMPO$)
              On Error GoTo 0
           End If
           On Error Resume Next          ' por si no se puede modificar
            MiConn.Execute ("ALTER TABLE " & NTABLA$ & " ALTER COLUMN " & TXCAMPO$)
           Screen.MousePointer = 1
           If Err Then
              On Error GoTo 0
              TX1$ = "No Fue Posible Modificar Campo '" + NCAMPO$ + vbCrLf
              TX1$ = TX1$ + "en la Tabla '" + NTABLA$ + "' de " & MIRS!xtype1 & " a " & TSQL & vbCrLf
              TX1$ = TX1$ + "Resultados Impredecibles. Consulte."
              MsgBox TX1$
           End If
           On Error GoTo 0
        End If
...
Function EXISTELCAMPOMICONN%(NBASE$, NTABLA$, NCAMPO$, TCAMPO, LOCAMPO, Optional CONINDEX%)
    '
   ...
   If BASACTI$ <> MiConn.DefaultDatabase Then
      '
      BASACTI$ = MiConn.DefaultDatabase
      CANCAM% = 0: NUEDIM% = 64
...
Private Sub Command4_Click()
    ...
    Call Verif_Components(MSHFLEXGRID_FRM, "MSHFlexGrid (MSHFLXGD.OCX)")
...
'    On Error GoTo 0
    ...
    If FrmErrorControl.LISTADEOBJETOS.ListCount > 0 Then
        ...
        If FrmErrorControl.Label3 = "BAJAR" Then
           ...
           Close: End
        ElseIf FrmErrorControl.Label3 = "FXSETUP" Then
           Call ACCEDERCARPETASETUP
           MsgBox "Re-Inicie Ahora la Aplicación"
           Close: End
        End If
    End If
    '
'    MsgBox "Control Completado"
    '
    Command4.Enabled = True
    '
End Sub
Sub ACCEDERCARPETASETUP()
     Screen.MousePointer = 11
     CARPINS$ = UCase$(App.Path)
     PPXX% = InStr(CARPINS$, "\FLEXOFT")
     If PPXX% > 0 Then CARPINS$ = Left$(CARPINS$, PPXX% + 7)
     CARPINS$ = CARPINS$ + "\FX_SETUP_2017"
     '
     If EXISTE%(CARPINS$ + "\.") < 1 Then
        MsgBox ("Error No se Detectó la Carpeta FX_SETUP_2017")
        Exit Sub
     End If
     
     DISCO$ = Left$(App.Path, 2)
     If Mid$(DISCO$, 2, 1) <> ":" Then
        MsgBox "Error al Intentar Reconocer la Unidad de Red"
        Exit Sub
     End If
     '
     ARCHI2$ = DISCO$ + "\FLEXOFT\FX_SETUP_2017\fxsetup2017.EXE"
     '
     If EXISTE%(ARCHI2$) > 0 Then
...
Private Sub confilicencia_Click()
    Call Command6_Click
End Sub
...
Private Sub Form_Load()
    ...
    End If
    '
  ' conecta con FLOGBRE.EXE
'    If EXISTE%("FLOGBRE.EXE") Then
'      RETOR$ = Trim$(UCase$(Command$))
'      Shell "FLOGBRE.EXE " + RETOR$, vbNormalFocus
'      Close: End
'    End If
...
280 Label2 = 170: DoEvents
    ...
    Call VERIBAK
    '
    Call Command4_Click 'control ocx y dll
...
'   If USNU% = 1 Then Exit Sub
  ...
  End If
End Sub
Sub GRABARCONTROL(PROCE$, CLAVE$, VALCON$)
    '
   SQLX$ = "SELECT COUNT(*) FROM INFORMATION_SCHEMA.TABLES WITH(NOLOCK) "
   SQLX$ = SQLX$ + " WHERE TABLE_NAME = 'FLEXCRL'"
   Dim RS As New ADODB.Recordset
   ...
   If EXISTE_LA_TABLA% > 0 Then
        CONDI$ = "PROBUS = '" & PROCE$ & "' AND CLABUS = '" & CLAVE$ & "'"
        SQLX$ = "SELECT COUNT(*) AS CANTIREG FROM FLEXCRL WITH(NOLOCK) "
        SQLX$ = SQLX$ + " WHERE " & CONDI$
        Dim RS1 As New ADODB.Recordset
        Set RS1 = MiConn.Execute(SQLX$)
        
        Dim RS2 As New ADODB.Recordset
        If Val(RS1!CANTIREG) < 1 Then
           SQLX$ = "INSERT INTO FLEXCRL (PROBUS,CLABUS,VALCONTROL) VALUES ('" & PROCE$ & "','" & CLAVE$ & "','" & VALCON$ & "')"
           Set RS2 = MiConn.Execute(SQLX$)
        Else
           SQLX$ = "UPDATE FLEXCRL SET VALCONTROL = '" & VALCON$ & "' WHERE " & CONDI$
           Set RS2 = MiConn.Execute(SQLX$)
        End If
...
End Sub
'
Function MUESTRACONTROL$(PROCE$, CLAVE$)
   ...
   Dim RS As New ADODB.Recordset
   Set RS = MiConn.Execute(SQLX$)
   If Not RS.EOF Then EXISTE_LA_TABLA% = Val(RS.Fields(0))
   RS.Close
   Set RS = Nothing
   ...
   If EXISTE_LA_TABLA% > 0 Then
    '
     CONDI$ = "PROBUS = '" & PROCE$ & "' AND CLABUS = '" & CLAVE$ & "'"
     SQLX$ = "SELECT VALCONTROL FROM FLEXCRL WITH(NOLOCK) WHERE " & CONDI$
     Dim RS1 As New ADODB.Recordset
     Set RS1 = MiConn.Execute(SQLX$)
     If Not RS1.EOF Then MUESTRACONTROL$ = RS1!VALCONTROL
     RS1.Close
     Set RS1 = Nothing
   End If
...
Sub GRARECEL()
    If EXISTE%(App.Path + "\MSTDRV04.EXE") > 0 Then
      ...
      If FILOG# > FIREL# Then
         ...
         If SVLX$ = "B87F-40A5" Then Call GRARE("07/12/29;174.13;;;S")           'TANIT
         If SVLX$ = "C0DD-AAE2" Then Call GRARE("01/05/29;174.13;;;S")           'DAKOTA
         'If SVLX$ = "689B-ABB3" Then Call GRARE("01/12/11;521.76;;;S")           'ALEJPA
         'If SVLX$ = "C431-7A89" Then Call GRARE("07/06/12;521.76;;;S")           'ALEJPA
         If SVLX$ = "E0AC-D6C8" Then Call GRARE("07/11/29;521.76;;;S")           'ALEJPA
         'If SVLX$ = "9056-86A2" Then Call GRARE("07/12/12;1054.94;;;S")          'GABAL
         If SVLX$ = "0091-87DA" Then Call GRARE("01/05/29;1054.94;;;S")          'GABAL         'NO TIENE MAS RECIBO
         If SVLX$ = "78D0-3CE2" Then Call GRARE("01/05/29;529.99;;;S")           'JYM           'NO TIENE MAS RECIBO
         If SVLX$ = "A48E-A690" Then Call GRARE("01/05/11;550.36;;;S")           'SILCAR
         If SVLX$ = "D4C8-6B00" Then Call GRARE("07/12/29;505.91;;;S")           'TELEBIT
         If SVLX$ = "E436-B6E2" Then Call GRARE("01/12/29;505.91;;;S")           'CONOMETAL
         'If SVLX$ = "CC34-EAB0" Then Call GRARE("01/01/12;298.12;;;S")           'EYM
         If SVLX$ = "C02E-3B78" Then Call GRARE("01/05/29;298.12;;;S")           'EYM           'NO TIENE MAS RECIBO
         'If SVLX$ = "9045-813A" Then Call GRARE("01/12/11;793.28;;;S")           'ESCORIAL
         If SVLX$ = "C447-C83F" Then Call GRARE("07/11/29;793.28;;;S")           'ESCORIAL
         If SVLX$ = "D411-AAA8" Then Call GRARE("01/05/11;79.16;;;S")            'COMPREFACIL
         If SVLX$ = "7CF3-1700" Then Call GRARE("01/05/29;170.00;;;S")           'CIVAL         'NO TIENE MAS RECIBO
         If SVLX$ = "D4FB-6302" Then Call GRARE("05/01/11;1349.55;;;S")          'ALECAR
         'If SVLX$ = "FC4A-8319" Then Call GRARE("01/11/11;1778.70;;;S")          'GRAMY
         If SVLX$ = "46B1-4D4B" Then Call GRARE("07/11/29;1778.70;;;S")          'GRAMY
         If SVLX$ = "9C64-0A83" Then Call GRARE("07/11/29;1778.70;;;S")          'ASEMBLI
         ...
         If SVLX$ = "6E79-9E9C" Then Call GRARE("01/05/29;1;;;S")                'ARCON NEUQUEN
      End If
    End If
End Sub
...
20 On Error GoTo 0
   If EXISTE%("FLEXMUL.DRV") = 1 Then
     ...
     If SELUDAT2.List2.ListCount > 0 Then
       ...
       End If
       '
       'RASO$ - ESTA VARIABLE TIENE LA LEYENDA DE LAS EMPRESAS (MULTIEMPRESA) RECORDAR QUE SE QUEDA CON LA ULTIMA QUE LEE
       'EN EL CASO DE JYM ES MULTIEMPRESA PERO ENTRA DIRECTAMENTE CON LA EMPRESA 0
       'SI DESEA CAMBIAR DEBE PASAR LO REALIZA DESDE LA PANTALLA PRINCIPAL DONDE TIENE DERECHOS POR USUARIO
       'AQUI NO SE PUEDEN VALIDAR USUARIOS POR QUE NO ESTA ASIGNADO EL USUARIO NI LA EMPRESA EN ESTA POSICION DEL CODIGO.
       If InStr(UCase(RASO$), "J Y M") Then
          SELUDAT2.List2.ListIndex = 0
          SELUDAT2.List2_DblClick
          GoTo 25
       End If
       '
       SELUDAT2.Show 1
...
20 CONNSQL$ = TRAPASS1$(Trim$(LOADFILE$(LUDAT$ + "CONNSQLS.DRV")))
   ...
   If VERDEFLOG% < 1 Then
      ...
      If WEBSE$ <> "ENABLED" Then
         ...
         End If
         '
'         SE COMENTA PARA QUE NO PRESENTA MENSAJES AL INGRESAR
...
30    Call UPDATES    ' INSTALA LAS ACTUALIZACIONES
      Call VERIPAGOS
      'VALIDO SI EXISTE EL
      If EXISTE%(LUDAT$ + "CONNSQLS.DRV") > 0 Then
      ' VERIFICA LA CONEXION
        CONNSQL1$ = Trim$(LOADFILE$(LUDAT$ + "CONNSQLS.DRV"))
        If CONNSQL1$ <> "" Then
          Call CREATABLA_BDCONTROL_MICONN
...
'   End If
   ...
   '
   If VerificaInstalacionDelFramework("HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\NET Framework Setup\NDP\") = 1 Then
        If ExisteArchivo("FtpNovedades.exe") Then
            If Not EstaelProceso("FtpNovedades.exe") Then
                rsp = Shell(App.Path & "\FtpNovedades.exe", vbMinimizedFocus)
            End If
        End If
   End If
...
Sub CREATABLA_BDCONTROL_MICONN()
   '
   
   Call CREACAMPOMICONN("BCONTROL", "AUDITRA", "FeReTra", 8, 8)
   Call CREACAMPOMICONN("BCONTROL", "AUDITRA", "TipoTra", 10, 8)
   Call CREACAMPOMICONN("BCONTROL", "AUDITRA", "DescriTra", 10, 64)
   Call CREACAMPOMICONN("BCONTROL", "AUDITRA", "NuCompro", 10, 24)
   Call CREACAMPOMICONN("BCONTROL", "AUDITRA", "UserTra", 10, 16)
   Call CREACAMPOMICONN("BCONTROL", "AUDITRA", "IdTermi", 10, 24)
   '
   Call CREACAMPOMICONN("BCONTROL", "BloArchA", "IdenTabla", 10, 32)
   '
   Call CREACAMPOMICONN("BCONTROL", "BloArchB", "CtrlName", 10, 32)
   Call CREACAMPOMICONN("BCONTROL", "BloArchB", "ExecName", 10, 32)
   Call CREACAMPOMICONN("BCONTROL", "BloArchB", "TermName", 10, 32)
   Call CREACAMPOMICONN("BCONTROL", "BloArchB", "IdenTabla", 10, 32)
   Call CREACAMPOMICONN("BCONTROL", "BloArchB", "InitDate", 8, 0)
   Call CREACAMPOMICONN("BCONTROL", "BloArchB", "UserName", 10, 32)
   '
   Call CREACAMPOMICONN("BCONTROL", "MENUES", "CodPadre", 10, 32)
   Call CREACAMPOMICONN("BCONTROL", "MENUES", "Relacion", 4, 4)
   Call CREACAMPOMICONN("BCONTROL", "MENUES", "CodHijo", 10, 128)
   Call CREACAMPOMICONN("BCONTROL", "MENUES", "Descripcion", 10, 64)
   '
   Call CREACAMPOMICONN("BCONTROL", "SQL_LISELEC", "NomLiSele", 10, 32)
   Call CREACAMPOMICONN("BCONTROL", "SQL_LISELEC", "ACol_1", 3, 2)
   Call CREACAMPOMICONN("BCONTROL", "SQL_LISELEC", "ACol_2", 3, 2)
   Call CREACAMPOMICONN("BCONTROL", "SQL_LISELEC", "Tabla_Origen", 10, 256)
   Call CREACAMPOMICONN("BCONTROL", "SQL_LISELEC", "Lista_Campos", 12, 0)
   Call CREACAMPOMICONN("BCONTROL", "SQL_LISELEC", "Condi_Selec", 12, 0)
   Call CREACAMPOMICONN("BCONTROL", "SQL_LISELEC", "Fech_Actu", 8, 0)
   Call CREACAMPOMICONN("BCONTROL", "SQL_LISELEC", "LiSelec", 12, 0)
   '
   Call CREACAMPOMICONN("BCONTROL", "SQL_QUERIES", "Cod_Query", 10, 16)
   Call CREACAMPOMICONN("BCONTROL", "SQL_QUERIES", "Des_Query", 10, 256)
   Call CREACAMPOMICONN("BCONTROL", "SQL_QUERIES", "PAR_Query", 12, 0)
   Call CREACAMPOMICONN("BCONTROL", "SQL_QUERIES", "SQL_Query", 12, 0)
   Call CREACAMPOMICONN("BCONTROL", "SQL_QUERIES", "TIP_QUERY", 3, 2)
   Call CREACAMPOMICONN("BCONTROL", "SQL_QUERIES", "CodiEmpr", 3, 2, 1)
   '
   Call CREACAMPOMICONN("", "AUDITACT", "AuTabla", 10, 64)
   Call CREACAMPOMICONN("", "AUDITACT", "AuCampo", 10, 64)
   Call CREACAMPOMICONN("", "AUDITACT", "AuCondi", 12, 0)
   Call CREACAMPOMICONN("", "AUDITACT", "AuValAnt", 11, 0)
   Call CREACAMPOMICONN("", "AUDITACT", "AuNueVa", 11, 0)
   Call CREACAMPOMICONN("", "AUDITACT", "AuFeHor", 8, 0)
   Call CREACAMPOMICONN("", "AUDITACT", "AuUser", 3, 0, 1)
   Call CREACAMPOMICONN("", "AUDITACT", "AuUsuario", 10, 64)
   Call CREACAMPOMICONN("", "AUDITACT", "AuAplica", 10, 32)
   Call CREACAMPOMICONN("", "AUDITACT", "AuTerminal", 10, 64)
