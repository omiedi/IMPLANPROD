...
' luego reemplazar por NOVEDAD
   RUTAWEB = "http://www.flexoft.com.ar/UPDATES/NOVETEST/IUPDATES.TXT"
   'JJK1& = DeleteUrlCacheEntry(RUTAWEB)
   Call BORRACACHE
   'MsgBox JJK1&
   '
   'If JJK1& = 1 Then
   '    MsgBox "Refrescado"
   '  Else
   '    MsgBox "No se Pudo refrescar"
   'End If
   '
   PPXX% = InStr(UCase$(App.Path), "\FLEXOFT\WINDOWS")
   If PPXX% < 1 Then PPXX% = Len(App.Path) - 7
   PPXX% = PPXX% + 7
   CARPEDOWN$ = Left$(App.Path, PPXX%) + "\DOWNLOAD\"
   '
   On Error Resume Next
   MkDir DIREVIE$
   On Error GoTo 0
   '
   If EXISTE%(CARPEDOWN$ + ".") < 1 Then
        ' CREA LA CARPETA
   ...
   '
   On Error Resume Next
   Kill CARPEDOWN$ + "IUPDATES.TXT"
   On Error GoTo 0
21 JJ1& = URLDownloadToFile(0, RUTAWEB, CARPEDOWN$ + "IUPDATES.TXT", 0, 0)
   If JJ1& <> 0 Then
      Exit Sub
   End If
   '
   FULTACT$ = LOADFILE$("IUPDATES.TXT")   ' LO LEE DEL DIRECTORIO DE TRABAJO
...
Private Sub Image6_DblClick()
  ...
  If MsgBox("Realmente Desea Actualizar Módulo 'FLOGON.EXE' ?", 3) = 6 Then
    Call BAJARCHIVO("FLOGON.EXE", OKX%)
    If OKX% < 1 Then
         MsgBox "No Ha Sido Posible la Actualización del Módulo"
       ElseIf OKX% = 1 Then
         MsgBox "El Módulo Bajado de Internet Coincide con el Actualmente Instalado. No se Requiere Actualización Alguna. Pulse 'Aceptar Mensaje' para Continuar."
       ElseIf OKX% = 2 Then
         MsgBox "Módulo 'FLOGON.EXE' ha sido Actualizado. Re-Inicie FLEXOFT para Instalar la Actualización."
         Close: End
    End If
  End If
End Sub
...
51   PPX% = Int(Rnd * 90)
   ...
   End If
   '
20 CONNSQL$ = Trim$(LOADFILE$(LUDAT$ + "CONNSQLS.DRV"))
   If CONNSQL$ <> "" Then
   ...
   If VERDEFLOG% < 1 Then
      Call GENLISUSR
      Call VERIACWEB  ' VERIFICA SI HAY ACTUALIZACIONES EN LA WEB
      Call UPDATES    ' INSTALA LAS ACTUALIZACIONES
      Call VERIPAGOS
   End If
   '
...
29 Next KU&
   '
'   If Val(RECLAPAGO.Label7) > 1 Then
...
60 Fact = HOY(HOS$)
   ...
   If DMOR1 > DIGRA Then         ' DIAS DE GRACIA
     TTXYZ$ = "No se ha Encontrado el Recibo Electrónico Correspondiente a " + CONCE$ + "." + CRLF$ + CRLF$
     ...
     TTXYZ$ = TTXYZ$ + "Recuerde Despachar este Correo a fin de Recibir el Recibo Electrónico de su Pago."
     MsgBox TTXYZ$
     '
     GoSub AVISAFLEXOFT
   End If
...
AVISAFLEXOFT:
   ...
   EPLI$ = EMPRELI$
   '
   TTYY$ = "Empresa " + EPLI$ + " falta Recibo Electrónico Correspondiente a " + CONCE$
   If EXISTE%("FLXMAIL.EXE") > 0 Then
         MAI_DESTI$ = "FLEXOFT Soluciones"
         MAI_DIREL$ = "asistec@flexoft.com.ar"                 ' "ebauer@flexoft.com.ar"
         MAI_ASUNT$ = "Recibo Electronico " + CONCE$ + " - " + EPLI$
         MAI_TEXTO$ = TTYY$
         '
         Call GENERAMAIL
         '
       Else
         MsgBox "Falta Utilitario 'FLXMAIL.EXE'. Comuniquese con FLEXOFT al 54-11-4524-1284 ´o 5422-2048. Muchas Gracias."
    End If
    '
Return
...
29   Next KU&
   ...
   If DIRENEW$ = "F:\FLEXOFT\NOVEDAD" Then
     DIRENEW$ = PARENTFOLDER$(DIREPRO$) + "\NOVEDAD"
     If DIRENEW$ <> "F:\FLEXOFT\NOVEDAD" Then
       GoTo 10
     End If
...
Sub VERIACWEB()
   ...
   End If
   '
21 LKXC$ = LOADFILE$(LUCOM$ + "FFURLCGR.TXT")
   If UCase$(LKXC$) = "ERROR 87" Then
      GoTo 99
   End If
   '
   Call BORRACACHE
' luego reemplazar por NOVEDAD
   ...
   End If
   '
   ' LEER LA TABLA DE ACTUALIZACIONES PREVIAS
   CANREGACT& = Len(FULTACT$) / 128
   JJ& = 0
   ReDim FIRECVIE$(CANREGACT&)
   ...
   For U& = 1 To CANREGNUE&
      ...
      If Trim$(Left$(FIRECNU$, 64)) <> "" Then
         ...
         ' SI LA INSTANCIA PREVIA DEL MODULO COINCIDE, NO LA BAJA
         For U2& = 1 To JJ&
            If Left$(FIRECNU$, 74) = Left$(FIRECVIE$(U2&), 74) Then GoTo 29 ' COINCIDE HASTA FECHA Y HORA
         Next U2&
         ' AGREGA A LA LISTA DE ARCHIVOS A BAJAR
         FINAM1$ = UCase$(Trim$(Left$(FIRECNU$, 64)))
         If EXISTE%(App.Path + "\" + FINAM1$) > 0 Then
            J2& = J2& + 1: ARCHAC$(J2&) = FIRECNU$: GoTo 29
         End If
         If EXISTE%(LUDAT$ + FINAM1$) > 0 Then
            J2& = J2& + 1: ARCHAC$(J2&) = FIRECNU$: GoTo 29
         End If
      End If
29 Next U&
   '
   ' AVISA CUALES SON LAS ACTUALIZACIONES DISPONIBLES
   COMUDOWN.List3.Clear
   COMUDOWN.Label3 = ""
   For U& = 1 To J2&
      If ARCHAC$(U&) <> "" Then
         ARCHA$ = UCase$(Trim$(Left$(ARCHAC$(U&), 64)))
         TTZZ$ = AJUSTI$(ARCHA$, 32) + FECHATEX$(Val(Mid$(ARCHAC$(U&), 65, 5))) + " " + HORATEX$(Val(Mid$(ARCHAC$(U&), 71, 4)))
         COMUDOWN.List3.AddItem TTZZ$
      End If
   Next U&
   '
   If COMUDOWN.List3.ListCount < 1 Then
       ' NO HAY ACTUALIZACIONES DISPONIBLES
       Exit Sub
   End If
   COMUDOWN.Label1.Caption = "Están Disponibles para Ser Bajadas de la WEB Las Siguientes Actua- lizaciones de los Módulos FLEXOFT Instalados. Pulse 'Instalar Ahora' para Bajarlas o 'Continuar sin Actualizar' para Seguir."
   Screen.MousePointer = 1
   COMUDOWN.Show 1
   OPENFORMS = DoEvents
   If COMUDOWN.Label3 <> "BAJAR" Then
      Exit Sub     ' SE CANCELO LA BAJADA
   End If
   '
   ' BAJA EL ARCHIVO Y LO PONE EN DOWNLOADS
   Screen.MousePointer = 11
   HUBOERRO% = 0
   For U& = 1 To J2&
      ARCHA$ = UCase$(Trim$(Left$(ARCHAC$(U&), 64)))
' luego reemplazar por NOVEDAD
      JJ1& = URLDownloadToFile(0, "http://www.flexoft.com.ar/UPDATES/NOVETEST/" + ARCHA$, CARPEDOWN$ + ARCHA$, 0, 0)
      If JJ1& <> 0 Then
         HUBOERRO% = 1
         ARCHAC$(U&) = ""
         MsgBox "Ocurrió un Error (" & JJ1& & ") al Intentar Bajar " + ARCHA$ + "." + vbCrLf + "Este Archivo no Pudo Bajarse / Instalarse. Consulte.", 48
      End If
   Next U&
   '
   ' COPIA DE CARPETA DOWNLOADS A NOVEDAD
   TTXX$ = "": J3& = 0: COMUDOWN.List3.Clear
   For U& = 1 To J2&
      If ARCHAC$(U&) <> "" Then
         ARCHA$ = UCase$(Trim$(Left$(ARCHAC$(U&), 64)))
         On Error Resume Next
         FileCopy CARPEDOWN$ + ARCHA$, DIRENEW$ + "\" + ARCHA$
         If Err < 1 Then
             On Error GoTo 0
             If TTXX$ <> "" Then TTXX$ = TTXX$ + vbCrLf
             TTZZ$ = AJUSTI$(ARCHA$, 32) + FECHATEX$(Val(Mid$(ARCHAC$(U&), 65, 5))) + " " + HORATEX$(Val(Mid$(ARCHAC$(U&), 71, 4)))
             'COMUDOWN.List3.AddItem TTZZ$
             TTXX$ = TTXX$ + TTZZ$
             J3& = J3& + 1
   ...
   Next U&
   '
   ' ACTUALIZA EL ARCHIVO INDICE
   CANREGVIE& = Len(FULTACT$) / 128
   For U& = 1 To J2&
      If ARCHAC$(U&) <> "" Then
         ARCHA$ = UCase$(Trim$(Left$(ARCHAC$(U&), 64)))
         For U2& = 1 To CANREGVIE&
            FIRECNU$ = UCase$(Mid$(FULTACT$, 128 * U2& - 127, 128))
            If Trim$(Left$(FIRECNU$, 64)) = ARCHA$ Then
               Mid$(FULTACT$, 128 * U2& - 127, 128) = ARCHAC$(U&)
               GoTo 39
            End If
         Next U2&
         FULTACT$ = FULTACT$ + AJUSTI$(ARCHAC$(U&), 128)
      End If
39 Next U&
   '
   If HUBOERRO% < 1 Then
      ...
      Else
        Call SAVEFILE(App.Path + "\IUPDATES.TXT", FULTACT$)
   End If
   '
   If J3& > 0 Then
      'MsgBox "Las Siguientes Actualizaciones han sido Bajadas de la WEB y Colocadas en " + DIRENEW$ + " para su Instalación Automática: " + TTXX$ + ". Pulse 'Aceptar Mensaje' para Continuar.", 48
      'COMUDOWN.Label1.Caption = "Las Siguientes Actualizaciones han sido Bajadas de la WEB y Colo- cadas en " + DIRENEW$ + " para su Instalación Automática. Pulse 'Aceptar Mensaje' para Continuar."
      'COMUDOWN.Show 1
      GoSub AVISAFLEXOFT
   End If
   '
...
AVISAFLEXOFT:
   ...
   EPLI$ = EMPRELI$
   '
   TTYY$ = "Usuario " + USERNA$ + " de la Empresa " + EPLI$ + " ha Bajado para Instalar las Siguientes Actualizaciones: " + vbCrLf + vbCrLf + TTXX$
   If EXISTE%("FLXMAIL.EXE") > 0 Then
         MAI_DESTI$ = "FLEXOFT Soluciones"
         MAI_DIREL$ = "asistec@flexoft.com.ar"                 ' "ebauer@flexoft.com.ar"
         MAI_ASUNT$ = "Actualización de Novedades - " + EPLI$
         MAI_TEXTO$ = TTYY$
         '
         Call ATTACHLIUSER
         Call GENERAMAIL
...
Sub BAJARCHIVO(NARCHI$, OKX%)
    '
    Static DIRENEW$
    '
    ' DETERMINA LA CARPETA DE INSTALACION DE NOVEDADES
    If DIRENEW$ = "" Then
       DIREPRO$ = App.Path
       If EXISTE%("F:\FLEXOFT\NOVEDAD\.") > 0 Then
           DIRENEW$ = "F:\FLEXOFT\NOVEDAD\"
    ...
    End If
    '
    ' BAJA EL ARCHIVO 'NARCHI$' Y LO PONE EN LA CARPETA DOWNLOADS
    PPXX% = InStr(UCase$(App.Path), "\FLEXOFT\WINDOWS")
    If PPXX% < 1 Then PPXX% = Len(App.Path) - 7
    PPXX% = PPXX% + 7
    CARPEDOWN$ = Left$(App.Path, PPXX%) + "\DOWNLOAD\"
    ...
    On Error GoTo 0
    '
    ' COPIA EL ARCHIVO A CARPETA DE NOVEDADES.
    OKX% = 0
' luego reemplazar por NOVEDAD
    JJ1& = URLDownloadToFile(0, "http://www.flexoft.com.ar/UPDATES/NOVETEST/" + NARCHI$, CARPEDOWN$ + NARCHI$, 0, 0)
    If JJ1& = 0 Then
       OKX% = 1  ' BAJADA OK
       RECIENBAJADO$ = LOADFILE$(CARPEDOWN$ + NARCHI$)
       ' AQUI REALIZAR LA CONVERSIÓN O DESENCRIPTADO
       BAJOANTERIOR$ = LOADFILE$(DIRENEW$ + NARCHI$)
       If RECIENBAJADO$ <> BAJOANTERIOR$ Then
          Call SAVEFILE(DIRENEW$ + NARCHI$, RECIENBAJADO$)
          OKX% = 2  ' ACTUALIZADO OK
       End If
    End If
    '
End Sub
...
10  On Error Resume Next
    ...
    If Err.Number <> 453 Then
        If (hGroup = 0) And (Err.LastDllError <> 2) Then
            Screen.MousePointer = 1
            TTXX$ = "Se ha Producido Error " & Err.LastDllError & " al Enumerar los Grupos de Cache. No Podrá Obtener Actualizaciones Automáticas FLEXOFT en esta Estación de Trabajo."
            Call SAVEFILE(LUCOM$ + "FFURLCGR.TXT", "ERROR 87")
            MsgBox TTXX$
            Exit Sub
        End If
      Else
