VERSION 5.00
Begin VB.Form RESCUECAJA 
   AutoRedraw      =   -1  'True
   BackColor       =   &H00C0FFC0&
   BorderStyle     =   3  'Fixed Dialog
   Caption         =   "Resúmenes de Cuenta de Caja y Bancos"
   ClientHeight    =   4125
   ClientLeft      =   45
   ClientTop       =   300
   ClientWidth     =   9630
...
29 Next KKL&
   ...
   If JJ& < 1 Then
      ...
      Else
        Call CONECRUN("*RESUCON/" + CODENOCUE$, "Resumen de Cuenta de Caja y Bancos")
   End If
   GoTo 10
   '
...
Sub RESCAJAPRIN()
   ...
   For KK% = 1 To LOF(NX3%) / 16
      ...
      If COCUE$ >= DESCUE$ Then
        If COCUE$ <= HASCUE$ Then
          ...
          If CUE% > 0 Then
            If CUE% <= 16384 Then
              If TODAS% = 1 Or Abs(SUDEBIN#(CUE%)) + Abs(SUHABIN#(CUE%)) + Abs(SUDEBE#(CUE%)) + Abs(SUHABE#(CUE%)) >= 0.005 Then
                '
                List1.Clear
                SUTRA# = 0
                TTTT = Len(PUNTE$(CUE%))
                For KLL& = 1 To Len(PUNTE$(CUE%)) Step 4
                  KML& = CVS(Mid$(PUNTE$(CUE%), KLL&, 4))
                  If KML& > 0 Then
                      Call LEESUCAJA(KML&)
                      ZZ$ = "A" + CSTRING$(D15$, 1, 6, 0, 2) + Space$(9)
                      Call REPLA(ZZ$, "B", 8, 1)
                    ...
                    ElseIf KML& < 0 Then
                      ...
                      ZZ$ = "A" + CSTRING$(E15$, 1, 6, 0, 2) + Space$(9)
                      Call REPLA(ZZ$, "A", 8, 1)
                ...
                For KL% = 1 To List1.ListCount
                  ...
                  KLL& = KL%
                  Call TRACE(20, KLL&, FIN&)
                  If KML& > 0 Then
                       Call LEEPASCAJ(KML&, 2)
                       FECHASIEN% = CVI(D15$)
                       NROASIEN = CVS(D12$)
                       NROASDEF% = CVI(D13$)
                       MARCADIA$ = "  "
                     ...
                     ElseIf KML& < 0 Then
                       KM% = KML&
                       Call LEEPASDIA(Abs(KM%), 2)
                       FECHASIEN% = CVI(E15$)
                       NROASIEN = CVS(E12$)
                       NROASDEF% = CVI(E13$)
                       MARCADIA$ = "* "
                       REFE$ = TRIM$(DETAPAS$) + " - " + TRIM$(REFASIEN$)
                       IPAS# = CVD(E16$)
                       If IPAS# > 0 Then
                           IDEBE# = IPAS#: IHABE# = 0
                         Else
   ...
   '
   Close #NX3%: NX3% = 0
   Call SETULTREG("RESUCOL", JJ&)
   Call CIERRARCH("RESUCOL")
   '
   Call HEADERS("RESUCOL", "")
   Call HEADERS("RESUCOL", "Corresponde a: " + EJACT$)
   '
   Screen.MousePointer = 1
   Call CONECRUN("*RESUCOL", "Resumen de Cuenta de Caja y Bancos")
   '
   Exit Sub

90 XX$ = REGBLAN$("RESUCOL")
   FFAX$ = Space$(8)
   If FECHASIEN% > 0 Then
