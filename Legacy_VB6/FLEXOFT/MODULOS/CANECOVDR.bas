...
'End If
         ...
         PUNOCPE$(CI%) = ""          ' AGREGADO 12/03/11 PUNTERO A O/C'S PENDIENTES
...
'End If
              ...
              If HAYPLAFAB%(CICX%) < 1 Then
                ...
                PUNOCPE$(CICX%) = PUNOCPE$(CICX%) + "OCOM" + MKS$(IL&) ' AGREGADO 12/3/2011
              End If
            End If
          End If
        End If
     ...
     For IL& = 1 To FINL&
        ...
        If STATU% < 8 Then      ' no esta anulada ni cumplida
...
'End If
                ...
                If HAYPLAFAB%(CICX%) < 1 Then
                  DISPACT(CICX%) = DISPACT(CICX%) + CAPEN
                  PUNOCPE$(CICX%) = PUNOCPE$(CICX%) + "DETA" + MKS$(IL&) ' AGREGADO 12/3/2011
                End If
              End If
            End If
          End If
...
' ******** agrega ordenes de fabricacion lanzadas y en curso ******
     ...
     MODOCA% = 1
     EPAC$ = EMPREAC$
     If Left$(UCase$(EPAC$), 4) = "NOVO" Then
        MODOCA% = 2
     End If
     '
             ...
             If CAPEN > 0 Then
               If CI% > 0 Then
                 If CI% <= NUMAS% Then
                   ...
                   PUNOCPE$(CI%) = PUNOCPE$(CI%) + "FABR" + MKS$(IL&) ' AGREGADO 12/3/2011
                   '
                   CIBX% = CI%
                   COD$ = CODEXT$(CI%)
...
'End If
          DISPACT(CI1%) = DISPACT(CI1%) - CAN1
          PUNOCPE$(CI1%) = PUNOCPE$(CI1%) + "RESE" + MKS$(IL&) ' AGREGADO 12/3/2011
...
'End If
          DISPACT(CI1%) = DISPACT(CI1%) - CAN1
          PUNOCPE$(CI1%) = PUNOCPE$(CI1%) + "REPU" + MKS$(IL&) ' AGREGADO 12/3/2011
          FELANZ% = HOII%
...
3900 Call CIERRARCH("*.*")
     ...
     '
     'Call FINAL(SALI$)
     Call FILESORT("COBERFA", "", 1, 2)
     Call FILESORT("PLACOMP", "", 1, 3)
     Call FILESORT("PLAFABR", "", 1, 3)
     Call FILESORT("ORFAEMI", "", 4, 5)
     Call FILESORT("OCOMEMI", "", 4, 5)
     '
     CIAX% = 0
...
Sub COBERFA(FFI%, CI%, REFE$, TIPO%, CAN)
      ...
      End If
      '
      ' SUPRIME SI FACLTA CANTIDAD O CODIGO - EPB 25/02/11
      If CI% < 1 Then Exit Sub
      If Abs(CAN) < 0.05 Then Exit Sub
      '
      If SEMANAL% = 1 Then
         FFX = FFI%
         If FFX < COMIEMACT% Then FFX = COMIEMACT%
         SMN% = SEMANU%(FFX, A, B)
      ...
      MESANIOT% = FECHANUM("01" + Mid$(FF$, 3)) '1000 * ANIO% + 10 * MES%
      '
...
108  FORCANEC.Check2.Value = 1
     ...
     Next IL&
     '
     ' MODIFICADO EL 7 DE OCTUBRE DE 2008 PARA TENER EN CUENTA ORDENES DE COMPRA
     ' EN CURSO. TODAVIA NO TIENE EN CUENTA ORDENES DE FABRICACION.
     '
     ' ******** inicializa necesidades secundarias de compra y fabricacion ********
110  FORCANEC.Check3.Value = 2
     Screen.MousePointer = 11
...
111  N13% = FreeFile
     ...
     For i& = 1 To NMS&
         ...
         PDS$ = String$(208, 0)
         PDM$ = String$(72, 0)
         ' ESTO NO INTERESA PORQUE LO PONE EN CERO
         If LOREC% = 208 Then
              Put #N13%, i&, PDS$
            Else
              Put #N13%, i&, PDM$
...
140 ' Call MENSAJE(14, "GENERANDO NECESIDADES SECUNDARIAS SEGUN EXPLOSION")
    ...
    IAGRE& = 0
    '
    Dim BII As String * 48
    Dim BJJ As String * 48
    For II& = X1& To X2&
      Call TRACE(20, II& - X1&, X2& - X1&)
      For JJ& = II& + 1 To X2&
        Get #N9%, II&, BII$
        Get #N9%, JJ&, BJJ$
        If CVI(Mid$(BII$, 7, 2)) > CVI(Mid$(BJJ$, 7, 2)) Then
          Put #N9%, JJ&, BII$         ' ORDENA POR FECHA
          Put #N9%, II&, BJJ$
        End If
      Next JJ&
    Next II&
...
Sub DIAGNONEG()
   ...
   FIN& = ULTREG&("COBERFA")
   For KKI& = 1 To FIN&
      Call TRACE(20, KKI&, FIN&)
      CI% = KKI&
      RFF$ = RECFILT$("COBERFA", 2, MKI$(CI%))
      SUMA# = 0
      For J& = 1 To Len(RFF$) Step 4
         REG& = CVS(Mid$(RFF$, J&, 4))
         XX$ = REGLEIDO$("COBERFA", REG&)
         If J& = 1 Then FEPRIMO% = CVI(Left$(XX$, 2))
         SUMA# = SUMA# + CVS(Mid$(XX$, 53, 4)) - CVS(Mid$(XX$, 57, 4))
         If SUMA# < (-0.05) Then
            If CVI(Left$(XX$, 2)) > FEPRIMO% Then ' SUPRIME EL PRIMER RENGLON
               NEGAT%(CI%) = 1
               HAYNE% = 1
               GoTo 19
            End If
...
19 Next KKI&
   ...
   '
   If HAYNE% < 1 Then
        Call COMUNI("Control de Integridad Completo")
      Else
        Call COMUNI("Se Han Detectado Negativos Transitorios\por Diferimiento de Compras.")
        Call COPYSTRU("ECOMAST", "CODNEGA")
        Call SETULTREG("CODNEGA", 0)
        Call FILESORT("CODNEGA", "", 1, 1)
        For KKI& = 1 To NUMAS%
           If NEGAT%(KKI&) > 0 Then
              CI% = KKI&
              XX$ = AJUSTI$(CODEXT$(CI%), 16) + DESCRIT0$(CI%)
              Call REGAPP("CODNEGA", XX$)
           End If
        Next KKI&
        '
        'OPCION DE VISTA POR PANTALLA (SELECHO) O GENERADOR DE REPORTES
        OPX% = COMALTER%("Opciones de Salida.\\ Visualizar por Pantalla\Impresión")
        If OPX% < 2 Then
           Call SELECHO("CODNEGA")
        ElseIf OPX% = 2 Then
         Call FINAL("*CODNEGA")
        End If
...
Function DISPOFE(CIXXX%, FFXXX%)
   '
   DPXXX = DISPACT(CIXXX%)
   For KKII% = 1 To Len(PUNOCPE$(CIXXX%)) Step 8
      BLQXX$ = Mid$(PUNOCPE$(CIXXX%), KKII%, 8)
      TARX$ = Left$(BLQXX$, 4)
      PREG& = CVS(Mid$(BLQXX$, 5))
      If PREG& > 0 Then
         If TARX$ = "OCOM" Then
             F$ = REGLEIDO$("OCOMPRA", PREG&)
             FENTRE% = CVI(Mid$(F$, 9, 2))
             If FENTRE% > FFXXX% Then               ' ENTREGA PROGRAMADA POSTERIOR DISMINIYE DISPONIBILIDAD
                CATOT# = CVS(Mid$(F$, 13, 4))
                CAPRA# = CVS(Mid$(F$, 58, 4))
                CAPEN = CATOT# - CAPRA#
                DPXXX = DPXXX - CAPEN
             End If
             '
         ElseIf TARX$ = "DETA" Then
             F$ = REGLEIDO$("OCOMDETA", PREG&)
             FENTRE% = CVI(Mid$(F$, 95, 2))
             If FENTRE% > FFXXX% Then               ' ENTREGA PROGRAMADA POSTERIOR DISMINIYE DISPONIBILIDAD
                COEFCAN = CVS(Mid$(F$, 63, 4))
                If COEFCAN < 0.005 Then COEFCAN = 1
             ...
             End If
         Else
            Call MENSERR(24, "Disponibilidad Indefinida para '" + TARX$ + "'")
         End If
      End If
   Next KKII%
   '
