...
Sub SHOWIMAGEN(ADEMO$, LDEMO$, INSTRU$, PAUSI%, PAUSF%, MABRE%)
   ...
   End If
   '
10 ADEMX$ = Trim$(UCase$(ADEMO$))
   For KKJ% = 1 To CAIMAG%
     If IMANAME$(KKJ% - 1) = ADEMX$ Then GoTo 15
   Next KKJ%
   Exit Sub
   '
15 For KKI% = 0 To 9
   ...
   Next KKI%
   '
   FORDEMO.Image1.Picture = VECPANTA.Image1(KKJ% - 1).Picture
   ...
   FORDEMO.Left = (Screen.Width - FORDEMO.Width) / 2
   FORDEMO.Top = (Screen.Height - 300 - FORDEMO.Height) / 2
...
Sub CARGALEYEN(ALEYEN$)
   '
   For ILEYE% = 1 To CALEYEN%
     If Trim$(Left$(LEYENDX$(ILEYE%), 16)) = ALEYEN$ Then GoTo 5
   Next ILEYE%
   Exit Sub
   '
5  TEGRABA$ = Trim$(Mid$(LEYENDX$(ILEYE%), 17))
6  PPXY% = InStr(TEGRABA$, Chr$(125))
   If PPXY% > 0 Then
     TEGRABA$ = Left$(TEGRABA$, PPXY% - 1) + Chr$(13) + Chr$(10) + Mid$(TEGRABA$, PPXY% + 1)
     GoTo 6
   End If
   '
7  PPXY% = InStr(TEGRABA$, Chr$(126))
   If PPXY% > 0 Then
     TEGRABA$ = Left$(TEGRABA$, PPXY% - 1) + Chr$(34) + Mid$(TEGRABA$, PPXY% + 1)
     GoTo 7
   End If
   '
   PPXX% = InStr(TEGRABA$, "|")
